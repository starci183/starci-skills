...
|---|---|---|---|
| `@mcp-qdrant` | `mcp/qdrant/context.md` | context | provision or verify the local vector service and MCP runtime |
| `@mcp-embedding` | `mcp/embedding/context.md` | context | inspect hardware, install Ollama and choose one dimension-locked embedding profile |
| `@mcp-source-context` | `mcp/source-context/context.md` | context | index one or more verified workspace roles and generate client configuration |
| `@mcp-clients` | `mcp/clients/context.md` | context | install the verified remote endpoint into both Codex/OpenAI and Claude Code |

...
The corpus holds one accepted anatomy from the flashcard result page: reason *"a name read against one
persisted value, repeated as rows"*, citing `flashcard-result-fact-row`, resting count 4, drawn only
...
shared-reason: a name read against one stored value, repeated as rows on a shared baseline
state-delta: none
citation-check: flashcard-result-fact-row exists; it is already cited twice inside one page entry, for grades and for weak topics
reason: the region asks the reader to do the same comparison, so the accepted anatomy applies with only the entry name generalised
```
...
cites: flashcard-result-grade-rows/2026-08-12
shared-reason: the same comparison
state-delta: this region can be empty — a drill may score no criterion — and the precedent never drew an empty state
citation-check: same entry; the empty state cites the empty-notice composite, which exists
reason: an empty criteria list is a real outcome here, and a region that cannot say so reads as a loading state that never finished
```
...
cites: none
shared-reason: none — the criteria are treated as one grouped visual with a caption rather than a run of rows
state-delta: no repeat at all; one part holds the whole comparison
citation-check: cites profile-breakdown, which exists and is used in one file
reason: if the criteria are read as a shape rather than one by one, a row per criterion spends the region's height on a reading nobody does
```

Candidate C is the departure the batch owes. Without it, A and B are one anatomy read twice.

## Scope

This module decides how earlier accepted anatomies are cited when generating block candidates. It does
not decide what a block may contain, which is the block laws' business, and it does not store the
corpus: precedents belong beside the source they were accepted for, not inside this shared tree.

...
For a course catalogue whose audience wants quick comparison, one candidate may be `quiet-precision`:
balanced contrast, compact density, soft shape, flat depth and measured motion. It reuses the project's
ground, surface and type tokens, rejects decorative gradients and floating surfaces, and proposes no
new vocabulary. Another candidate must differ on a real axis set, not merely call the same choices
`editorial`.

## Scope

This stage decides visual intent and its semantic-token vocabulary. It does not place regions, design
block states, choose utility classes, assign raw token values, or write frontend source.

...
const requestJson = ({ extra = {} } = {}) => ({
  schemaVersion: 9, operatorId: 'backend.generate', step: 1, parallel: 1, sessionId: 's-test',
...
function responseJson({ status = 'done', stop, fallbacks = [], fields = null, commits = [COMMIT], next = ['quality.verify'] } = {}) {
  return {
    schemaVersion: 9, operatorId: 'backend.generate', step: 1, parallel: 1, status, ...(stop ? { stop } : {}), fallbacks,
    fields: fields ?? {
...
function writeBranch(files) {
  ...
  const state = { id: 's-test', project: 'starci-academy', startedAt: '2026-09-03T00:00:00Z', requestHashes: {}, chain: [['1/1']], steps: { '1/1': 'backend.generate' }, current: '1/1', status: 'running' };
  if (files.producer) {
...
{
  const files = migrationFiles(); files.producer['critique/request/request.json'].operatorId = 'backend.generate';
...
await expectValid({ ...baseline(), 'request/request.json': requestJson({ extra: { scope: 'fix' } }) }, 'the conforming branch as a fix inside the contract');

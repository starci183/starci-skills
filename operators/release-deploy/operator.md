...
Done when the `release-deployment` records whether the release was deployed or the previous release
restored, with the `probes` observed across the whole steady window showing either the immutable
digest active on every declared target with every declared probe passing or the exact rollback
release restored and never reported as delivery of the rejected one.

## One image, one target

This operator releases an image: the artifact identified by its digest, rolled out to one declared
target and proved steady. A schema that must move before that image can serve is released by
`migration.release` from its own frozen plan, before this operator runs; the `migrate` step here
applies only the migration the image's own manifest declares as part of its rollout, and a
migration that cannot be applied that way is `MIGRATION_BLOCKED` and returns to the backend owner.
...
under the same law `identity.provision` states for the credential it resolves: proving
`CREDENTIAL_UNAVAILABLE` does not apply reports that a handle failed to resolve, never what it
resolved to.
...
`response.json`, `data/probes.json` and `response.md`. It does not release a source migration plan;
does not log, persist, echo, or return a
...
| `migration-release` | `migration.release`; the schema the image depends on, already applied to the target, when the release needed one | no |
...
| When | Operator |
| --- | --- |
| the host, the artifact registry or the safe release needs the shared runtime served or stopped first | `runtime.serve` |
| the target's schema must move before this image can be rolled out | `migration.release` |

...
| 2 | Ràng release và biên dịch kế hoạch | `release`, `target`, `approval` | @remote/ghcr/<image> tại digest đã đóng băng, @remote/github-actions/<runId> cho trạng thái quan sát được, đầu vào `migration-release` cho schema mà image phụ thuộc khi đã release một cái, @tools/git, @tools/ci | — | `MANIFEST_INVALID`, `APPROVAL_REQUIRED` |

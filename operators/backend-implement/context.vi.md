...
| `@be` | `<checkout:input.project.id/be>  (the backend checkout of the project this invocation binds)` | fingerprint + sourceHead (git rev-parse HEAD of the checkout) | Bắt buộc: The backend checkout the boundary belongs to; the one place this operator writes product source. |

...
Mọi bản ghi seed mà lượt chạy tiêu thụ đều mang `is_uat=true` và namespace mà seed receipt bind —
namespace riêng của flow, như kế hoạch seed đã cấp — nên chúng tách bạch với dữ liệu sản phẩm; một
lượt chạy không bind seed receipt thì namespace bản ghi của nó theo `runId`. Verifier không mutate dữ liệu đó. Nó handoff rollback set chính xác của

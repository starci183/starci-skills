...
| `@worktrees/_templates` | the UAT flow template a new flow directory is created from, which the tree ships at `templates/uat/`, which is exactly the three things step 4 reads: `uat/flow.md` with the cases and their named assertions, `uat/account.json` with a username, a role, a credential name and the sealed file's path and no field that could hold a secret, and `uat/seed/` with the records a run namespaces; consumed, never modified | yes |

...
| --- | --- | --- | --- | --- |
| 1 | Kiểm tra input và resume | input (lease), `@worktrees/uat/<flow>/<case>` (kết quả lần trước), `@workspaces/be` (source head đã đóng băng) | — | `INVALID_INPUT`, `SOURCE_DRIFT`, `NO_PROGRESS` |
| 2 | Xác nhận giấy phép vào | input (blind visual PASS cuối, quality PASS cuối) | — | `ADMISSION_MISSING` |
| 3 | Chạy preflight ràng buộc | `@worktrees/sessions/central-runtime` (danh tính tài khoản và fixture, mọi kho vật lý) | — | `PROVISIONING_UNAVAILABLE` |

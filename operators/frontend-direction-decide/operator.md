...
is raised only over a tie the scores prove.

## Several survivors are ranked, not offered

A choice goes to a person only when the candidates are genuinely level; when one is dominant the
operator decides, prints the candidates as evidence and continues, because a confirmation whose answer
the receipt already shows is not a decision anybody needs to take. When more than one candidate is
...
instrument — and records every score under `## Scores`, beside the printed candidates. A candidate is
...
under either selection policy. Only when no candidate dominates — the top means are equal, which is
the rubric's own resolution, or the top scorer loses a failed criterion to another — is the choice
open: under `automatic` the fallback breaks the tie and records it; under `approval-required` the run
stops with `DIRECTION_CHOICE_REQUIRED`, and the scores in the receipt are what prove the tie was one.
The validator refuses that stop when its scores show a dominant candidate, and refuses a decision over
more than one rendered candidate that carries no scores.
...
| `selectionPolicy` | choice | automatic | `automatic`: the operator selects, a tie included, and records why; `approval-required`: the operator still selects a dominant candidate and a tie is the person's |
| `approval` | id | null | The approved candidate id; required only under `approval-required` when the scores proved a tie, supplied on resume after `DIRECTION_CHOICE_REQUIRED` |
...
under `approval-required` stops, over a tie its `## Scores` proves, with the receipt, and its
`## Printed` table is the choice: one served candidate per option, each with a capture per viewport,
and at least three of them, because a composition is chosen by eye — when fewer survived, the run
renders the third from its own rejected or element-level alternatives so the person compares renders
rather than sentences. The stop's `reason` names the sheet URL and asks one question and nothing
more: two options written out in prose are not a choice, they are advice, and the validator refuses a
`user` route whose table holds fewer rendered candidates than the choice has options, or none, and one
whose scores name a winner the operator should have taken. A choice the person takes from that sheet
...
person took on its own evidence. `GRAMMAR_REQUIRED` and the caller stops are operational — a person
publishes, corrects or supplies something — so their `reason` says so and carries no candidate.
...
defaults produces no page and rests on step 10. A dominant candidate is decided under either policy;
over a tie, `automatic` takes the fallback and records it, and `approval-required` stops with

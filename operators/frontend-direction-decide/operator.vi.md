...
lên trên một thế hoà mà bảng điểm chứng minh được.

## Nhiều phương án sống sót thì được xếp hạng, không đem ra mời chọn

Một lựa chọn chỉ về tay người khi các phương án thật sự ngang nhau; khi một phương án trội, operator
tự quyết, in các phương án ra làm bằng chứng rồi đi tiếp, vì một lời xác nhận mà biên nhận đã cho
thấy câu trả lời thì không ai cần phải đưa ra. Khi có nhiều hơn một phương án được render, bước 11
...
dẫn đầu thua một phương án khác trên một tiêu chí đã rớt — lựa chọn mới mở: dưới `automatic`, fallback
phá thế hoà và ghi lại; dưới `approval-required`, lần chạy dừng với `DIRECTION_CHOICE_REQUIRED`, và
bảng điểm trong biên nhận chính là thứ chứng minh đó đúng là một thế hoà. Validator từ chối lần dừng
ấy khi bảng điểm cho thấy một phương án trội, và từ chối một quyết định trên nhiều hơn một phương án đã
render mà không mang điểm.
...
| `selectionPolicy` | choice | automatic | `automatic`: operator chọn, kể cả khi hoà, và ghi lý do; `approval-required`: operator vẫn chọn phương án trội, còn thế hoà là của người |
| `approval` | id | null | Id phương án được duyệt; chỉ bắt buộc dưới `approval-required` khi bảng điểm đã chứng minh thế hoà, nhập khi chạy lại sau `DIRECTION_CHOICE_REQUIRED` |
...
dưới `approval-required` dừng, trên một thế hoà mà `## Scores` của nó chứng minh, kèm biên nhận, và bảng `## Printed` của nó chính là lựa chọn: mỗi phương
...
đã render hơn số lựa chọn, hoặc không có phương án nào, và cả một route mà bảng điểm đã gọi tên người
thắng lẽ ra operator phải tự lấy. Một lựa chọn người lấy từ bảng ấy đóng lại những gì bảng đã cho
...
của mình. `GRAMMAR_REQUIRED` và các mã dừng về phía
...
dưới mặc định không sinh trang nào và dựa vào bước 10. Phương án trội được quyết dưới cả hai chính
sách; khi hoà, `automatic` lấy fallback và ghi lại, còn `approval-required` dừng với

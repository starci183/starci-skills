...
Xong khi `frontend-direction-decision` gọi tên một hướng được chọn đã qua mọi đòn phản chứng, khai
lớp bề mặt, delta trình bày và các chuẩn tham chiếu theo lớp của nó, và mang điểm cùng giới hạn đã
khai của mọi phương án đã render, `ui-coverage` đóng tập trạng thái dưới cùng lớp bề mặt ấy, và mọi
phương án buộc phải render tồn tại dưới dạng `candidates` được phục vụ qua `host` và được in kèm một
ảnh chụp mỗi viewport trước khi quyết định được viết.
...
hay một lần sửa hành vi tới được `frontend.source.apply` mà không phải bịa một dòng trình bày để thoả
contract; hướng khai `none` rồi lại đổi class thì bị lượt resolve từ chối, chứ không được tha.

...
**Analysis:** verify each hash and extract only evidence required to resolve exact backend files for the approved scope.
...
**Decision criteria:** each selected file has role, hash, relevance reason, and partition provenance.
**Analysis:** resolve exact files from routed partition evidence without broad repository loading. Record evidence, criteria, and conclusions only; never record chain-of-thought.
...
| `@exact-source` | `payload.loads.source.targetFiles` | exact-source | inspect only declared files |

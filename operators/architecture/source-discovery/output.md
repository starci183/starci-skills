# `architecture/source-discovery` output
...
| `payload.produced` | Session artifact `sourceReceiptRef` plus explicitly approved durable effects only. |
...
| --- | --- | --- | --- |
| `ready` | `completed` | `architecture.pattern / ready` | backend-source-ready |

`sourceReceiptRef`, evidence, receipts, observations, and output use `session://`. Only a product/worktree effect explicitly declared by `operator.json` may survive the skill.

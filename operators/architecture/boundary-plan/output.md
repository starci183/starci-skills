# `architecture/boundary-plan` output
...
| `payload.produced` | Session artifact `boundaryPlanRef` plus explicitly approved durable effects only. |
...
| --- | --- | --- | --- |
| `ready` | `completed` | `architecture.boundary.challenge / ready` | backend-boundary-ready |

`boundaryPlanRef`, evidence, receipts, observations, and output use `session://`. Only a product/worktree effect explicitly declared by `operator.json` may survive the skill.

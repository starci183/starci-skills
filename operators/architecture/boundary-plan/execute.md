...
This operator compiles one canonical backend implementation boundary. Input, output, context bindings, analysis evidence, worker observations, and receipts remain in task-session memory and are purged at every parent-skill terminal.
...
**Session write:** accepted artifact at `payload.session.scratchPrefix/boundaryPlanRef` and evidence under `scratchPrefix/evidence`.

# `architecture/decision-frame` input

This operator frames one architecture decision. The input is an ephemeral object owned by the current task session. It is never written to the repository, a worktree, a log, an index, or a receipt file. The runtime purges it at every parent-skill terminal state.
...
- `objectiveRef`: exact `session://` reference; this operator cannot replace or broaden it.
- `constraintEvidenceRefs`: exact `session://` reference; this operator cannot replace or broaden it.

# `architecture/decision-frame` output
...
| `payload.produced` | Session artifact `decisionFrameRef` plus explicitly approved durable effects only. |
...
| --- | --- | --- | --- |
| `ready` | `completed` | `architecture.decision.current / ready` | architecture-frame-ready |

`decisionFrameRef`, evidence, receipts, observations, and output use `session://`. Only a product/worktree effect explicitly declared by `operator.json` may survive the skill.

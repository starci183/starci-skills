# Execute `architecture/decision-frame`

This operator frames one architecture decision. Input, output, context bindings, analysis evidence, worker observations, and receipts remain in task-session memory and are purged at every parent-skill terminal.
...
**Decision criteria:** objective, scope, invariants, non-goals, deadline, and proof criteria are explicit.
**Analysis:** normalize business intent and constraints into one bounded question. Record evidence, criteria, and conclusions only; never record chain-of-thought.
**Session write:** candidate `decisionFrameRef` at `payload.session.scratchPrefix/candidate`.
...
**Session write:** accepted artifact at `payload.session.scratchPrefix/decisionFrameRef` and evidence under `scratchPrefix/evidence`.

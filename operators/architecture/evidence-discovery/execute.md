...
**Context:** load exact session refs plus `source.provenance`; source, configuration, deployment and runtime evidence are observations, not authority.  
...
**Stop:** Every claim carries kind, claim state, revision and evidence location; unknowns remain explicit.

## Step 4 — Validate and emit

**Read:** candidate, evidence refs and lineage.  
**Context:** no new load. Validate against `migration/v6.1/architecture-backend/schemas/evidence-claim-set.schema.json`.  

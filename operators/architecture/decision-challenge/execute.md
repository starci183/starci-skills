...
**Decision criteria:** coupling, cost, migration risk, reversibility, and constraints are resolved.
**Analysis:** test each option against the explicit frame. Record evidence, criteria, and conclusions only; never record chain-of-thought.
...
**Session write:** accepted artifact at `payload.session.scratchPrefix/challengeReceiptRef` and evidence under `scratchPrefix/evidence`.

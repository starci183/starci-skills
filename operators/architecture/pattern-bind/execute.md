...
This operator binds backend patterns to discovered responsibilities. Input, output, context bindings, analysis evidence, worker observations, and receipts remain in task-session memory and are purged at every parent-skill terminal.
...
**Session write:** accepted artifact at `payload.session.scratchPrefix/patternBindingRef` and evidence under `scratchPrefix/evidence`.

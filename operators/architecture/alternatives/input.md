# `architecture/alternatives` input

This operator derives materially distinct architecture options. The input is an ephemeral object owned by the current task session. It is never written to the repository, a worktree, a log, an index, or a receipt file. The runtime purges it at every parent-skill terminal state.
...
Acceptance requires that each option covers constraints, costs, failure modes, reversibility, and rejected assumptions. Validate the whole envelope before any load or side effect. Loaded content and intermediate analysis remain session-only.

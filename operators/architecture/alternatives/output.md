# `architecture/alternatives` output
...
| `payload.produced` | Session artifact `optionSetRef` plus explicitly approved durable effects only. |
...
| --- | --- | --- | --- |
| `ready` | `completed` | `architecture.decision.challenge / ready` | architecture-options-ready |

`optionSetRef`, evidence, receipts, observations, and output use `session://`. Only a product/worktree effect explicitly declared by `operator.json` may survive the skill.

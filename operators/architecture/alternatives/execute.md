...
This operator derives materially distinct architecture options. Input, output, context bindings, analysis evidence, worker observations, and receipts remain in task-session memory and are purged at every parent-skill terminal.
...
**Decision criteria:** each option covers constraints, costs, failure modes, reversibility, and rejected assumptions.
**Analysis:** build a bounded option set from the frame and current state. Record evidence, criteria, and conclusions only; never record chain-of-thought.
**Session write:** candidate `optionSetRef` at `payload.session.scratchPrefix/candidate`.
...
**Session write:** accepted artifact at `payload.session.scratchPrefix/optionSetRef` and evidence under `scratchPrefix/evidence`.

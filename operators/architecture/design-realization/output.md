# `architecture/design-realization` output

The output is an ephemeral typed artifact governed by `migration/v6.1/architecture-backend/schemas/design-realization.schema.json`.
...
Gate: No approved decision remains diagram-only; every write and deployment has an exact realization and proof path.

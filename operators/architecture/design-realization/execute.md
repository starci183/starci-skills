...
**Stop:** No approved decision remains diagram-only; every write and deployment has an exact realization and proof path.

## Step 4 — Validate and emit

**Read:** candidate, evidence refs and lineage.  
**Context:** no new load. Validate against `migration/v6.1/architecture-backend/schemas/design-realization.schema.json`.  

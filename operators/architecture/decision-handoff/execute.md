# Execute `architecture/decision-handoff`

This operator converts the selected option into planning context. Input, output, context bindings, analysis evidence, worker observations, and receipts remain in task-session memory and are purged at every parent-skill terminal.
...
**Decision criteria:** selection belongs to the challenged option set and retains consequences and proof duties.
**Analysis:** bind the option, frame, and challenge evidence. Record evidence, criteria, and conclusions only; never record chain-of-thought.
**Session write:** candidate `planningContextRef` at `payload.session.scratchPrefix/candidate`.
...
**Session write:** accepted artifact at `payload.session.scratchPrefix/planningContextRef` and evidence under `scratchPrefix/evidence`.
...
| `@knowledge-1` | `architecture.decision-analysis` | qdrant | retrieve only this operator law |

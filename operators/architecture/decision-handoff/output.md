# `architecture/decision-handoff` output
...
| `payload.produced` | Session artifact `planningContextRef` plus explicitly approved durable effects only. |
...
| --- | --- | --- | --- |
| `ready` | `completed` | `architecture.source / ready` | architecture-decision-ready |

`planningContextRef`, evidence, receipts, observations, and output use `session://`. Only a product/worktree effect explicitly declared by `operator.json` may survive the skill.

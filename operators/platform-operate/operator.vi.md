...
| `serve` | Merge phần việc của một phiên vào nhánh tích hợp, tự giải xung đột (nếu có) theo luật rồi chấm gate trên head đã gộp, và cho server chạy kết quả đó | đúng bộ của `start-role` cộng `gates-passed` |

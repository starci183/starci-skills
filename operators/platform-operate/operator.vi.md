...
| `serve` | Merge phần việc của một phiên vào nhánh tích hợp và cho server chạy kết quả đó | đúng bộ của `start-role` |
| `restart` | Khởi động lại đúng head đó, cùng nhánh, cùng cổng | đúng bộ của `start-role` |
| `reset` | Dừng, xoá cache build của framework cho worktree đó, khởi động lại | đúng bộ của `start-role` |
| `stop` | Dừng server: pid đã ghi bị kill và lease được nhả | `entry-declared`, `generation-advanced`, `server-pid-owned`, `lease-honoured` |
...
lệnh dev nào là `INVALID_INPUT` nêu tên trường nó thiếu; một lần merge xung đột là
`INTEGRATION_CONFLICT`, thuộc về con người và xảy ra ngay lúc tích hợp chứ không phải lúc publish, và
đó chính là lý do lần merge diễn ra ở đây.
...
thật sự gặp nhau, thay vì lúc publish nơi nó sẽ chặn một phần việc đã xong. Nhánh tích hợp cũng được
...
tự ứng biến. Một lần `stop` kill đúng pid mà entry đã ghi và không kill pid nào khác.
...
chỉ kill đúng pid mà chính entry đã ghi. Nó không hành động trong lúc phiên khác giữ lease; không

...
| --- | --- | --- | --- | --- |
| 1 | Validate input and resume | input, `@dynamic/workspace-route-binding.json`, `@workspaces/local/routes/<project>/<role>` (the frozen head), `@remote/git/<project>/<role>` (the remote observation) | — | `INVALID_INPUT`, `SOURCE_DRIFT`, `NO_PROGRESS` |
| 2 | Bind the route | `@dynamic/workspace-route-binding.json` (verified checkout, its head, its routed policy), `@workspaces/local/routes/<project>/<role>` | — | `ROUTE_UNVERIFIED` |
...
| 6 | Push non-force | `@workspaces/local/routes/<project>/<role>` (approved heads, declared ref), `@remote/git/<project>/<role>` (the observed remote head) | `@remote/git/<project>/<role>` | `NON_FAST_FORWARD` |
| 7 | Push the continuation tag | input (the requested tag), `@remote/git/<project>/<role>` (the heads this publication pushed) | `@remote/git/<project>/<role>` (the continuation tag) | — |
| 8 | Emit and stop | everything above | `@dynamic/git-publication.json` | — |

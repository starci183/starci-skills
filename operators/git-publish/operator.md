...
On `done`, print **Operator Result** as the published branch and commit link recorded in `response/response.md`. A rejected push, non-fast-forward conflict or authority stop shows the unchanged remote state and exact next owner instead of a publication link.

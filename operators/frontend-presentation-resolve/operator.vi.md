...
| 2 | Bind thẩm quyền | — | @knowledge/ui/presentation (mọi topic kèm fingerprint và kho luật), @grammar/core (các quan hệ đã sở hữu của gói đã publish), @workspaces/fe (head được route và cây đã đóng băng), đầu vào `frontend-direction-decision` | — | `KNOWLEDGE_UNBOUND`, `GRAMMAR_UNPUBLISHED` |

...
| --- | --- | --- | --- |
| alias có đúng provider account, role, membership và login product thật | tái dùng không mutation | ghi stable account ref và browser login proof mới | publish action `reuse` |
| alias thiếu | tạo với sealed credential chỉ resolve trong request body/form | xác minh provider, role, membership và login thật tại endpoint đã bind | publish action `create` |
| account sai hoặc login fail | chỉ sửa field thuộc provider bị sai | lặp login thật và check membership đổi | publish action `repair` hoặc exact `IDENTITY_UNPROVEN` delta |
| provider effect không chắc | không tạo account trùng | đọc lại bằng stable account ref và username | chỉ resume khi biết reuse hay bounded repair |
...
| 3 | Kiểm provider đã khai và từng flow alias, phân loại stable provider account, role, membership và prior login evidence là reusable, missing hoặc invalid | `routeKey`, `flow` | @worktrees/sessions/central-runtime cho provider và realm mà entry khai, @worktrees/uat/<flow> cho hồ sơ và tài liệu hiện có, @worktrees/_templates cho khuôn luồng | @worktrees/uat/<flow>, @tools/sourcewrite | `INVALID_INPUT` |
| 4 | Tái dùng account hợp lệ, tạo account thiếu, và chỉ sửa role, membership hoặc credential state thuộc provider bị sai; không tạo trùng account chưa chắc | `desiredState`, `identityRotation` | @worktrees/uat/<flow> cho các alias mà tài liệu luồng nêu, @workspaces/device-state cho thông tin xác thực theo tên, @tools/secrets, @tools/http | `response/data/delta.json` | `PROVISIONING_UNAVAILABLE` |
| 5 | Chứng minh provider reachability, credential custody, role và membership, rồi login thật qua product endpoint đã bind trong browser context mới; token-only proof không đủ | — | @worktrees/sessions/central-runtime cho provider, @tools/http, @tools/browsercontrol | `response/data/checks.json` | `IDENTITY_UNPROVEN` |
...
Với provisioning, in **Kết quả tốt nhất** là bảng reuse, create hoặc repair theo alias từ `response/data/delta.json` cùng kết quả đăng nhập an toàn trong `response/data/checks.json`; `response/data/account.json` có thể nêu tài khoản bằng tên làm evidence phụ. Với admin rotation, hiện credential cũ bị từ chối và credential thay thế được chấp nhận mà không phát account record; không bao giờ in, link hay sao chép giá trị credential, sealed file hoặc lệnh chứa secret, và login chưa chứng minh phải được hiện là chưa chứng minh.

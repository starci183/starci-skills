...
The collector always sends and records `x-apollo-operation-name: StarCiRuntimeObservation` so a
read-only GET can satisfy the server's explicit operation-header check. No caller-defined headers
are accepted, and the proof gate requires this exact non-secret request intent.
...
| 3 | Recheck the route entry and classify its process, generation, served head, configuration and health as reusable, missing, invalid or uncertain before any effect | `routeKey` | @worktrees/sessions/central-runtime for the entry re-observed, @tools/git for the served head | — | `INVENTORY_DRIFT` |

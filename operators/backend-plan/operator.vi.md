...
| --- | --- | --- | --- |
| partition hiện hành | tái dùng unit id và dependency | mỗi operation xuất hiện một lần, mỗi unit phục vụ branch goal | phát với source evidence mới |
| partition thiếu | tạo unit theo writer/store boundary | hợp đầy đủ, rời nhau và có thứ tự | phát plan và units khớp |
| partition sai | cập nhật unit bị ảnh hưởng, giữ phần khác | chạy lại completeness, ownership, dependency | writer chưa rõ handoff `architecture.decide` |
...
| 5 | Kiểm module trước, tái dùng module còn khớp operation, writer, store boundary, update module sai, tạo module thiếu, và chứng minh mỗi operation thuộc đúng một lần | — | các thao tác, source | — | `MODULE_UNDEFINED` |

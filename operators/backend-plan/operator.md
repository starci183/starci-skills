...
| --- | --- | --- | --- |
| current partition | reuse unit ids and dependencies | each operation occurs once and each unit serves its branch goal | emit with fresh source evidence |
| missing partition | create units by writer/store boundary | union is complete, disjoint and ordered | emit plan and matching units |
| invalid partition | update affected units, preserve others | rerun completeness, ownership and dependency checks | undefined writer hands to `architecture.decide` |
...
| 5 | Inspect prior modules, reuse those whose operation, writer and store boundaries still match, update invalid modules, create missing ones, and prove every operation belongs exactly once | — | the operations, the source | — | `MODULE_UNDEFINED` |

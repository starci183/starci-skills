...
| `frontend.surface.audit` | `sol-reviewer` | `@knowledge/grammars/<family>`, `@knowledge/ui/proof`, `@workspaces/fe`, `@worktrees/sessions/central-runtime` | `frontend-source-application`, `frontend-presentation-resolution`, `frontend-direction-decision`, `route`, `uat-account` | `frontend-surface-audit`, `capture`, `screenshot`, `verdicts`, `host`, `candidates` | 6 | `INVALID_INPUT`, `SOURCE_DRIFT`, `RUNTIME_UNAVAILABLE`, `IDENTITY_MISSING`, `EVIDENCE_MISSING`, `UNKNOWN_RULE`, `SURFACE_CLASS_MISSING`, `NO_PROGRESS` |
...
| `candidates` | `frontend.direction.decide`, `frontend.surface.audit` | — |

...
| --- | --- | --- | --- |
| `@receipt/fe-presentation-resolution/<invocationId>` | <@artifacts of the producing invocation>/<receipt file> | fingerprint + the sourceHead the receipt binds | Bắt buộc: The only source of values this operator may write. |
| `@source/starci-academy/fe` | <checkout:project/role> | fingerprint + sourceHead (git rev-parse HEAD of the checkout) | Bắt buộc: The checkout the declared write set lands on; the one place this operator writes product source. |
| `@receipt/fe-direction-decision/<invocationId>` | <@artifacts of the producing invocation>/<receipt file> | fingerprint + the sourceHead the receipt binds | Tuỳ chọn: Intent; never a source of values. |

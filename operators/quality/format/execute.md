...
This operator runs the formatting gate. Input, output, loaded context, command captures, diagnostics, observations, and receipts remain session-only and are purged at every parent-skill terminal.
...
**Decision criteria:** the pinned formatter reports no drift for the approved change boundary.
**Analysis:** execute the exact non-mutating format-check command and classify failures. Record applied criteria, structured diagnostics, and conclusions only; never record chain-of-thought.
**Session write:** candidate `formatReceiptRef` at `scratchPrefix/candidate`.

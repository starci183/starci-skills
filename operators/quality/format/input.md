...
This operator runs the formatting gate. Its input is an ephemeral task-session object. The runtime never writes the envelope, loaded values, command output, diagnostics, worker observations, or receipts outside the session and purges them at every parent-skill terminal.
...
- `formatCommandRef`: exact `session://` reference; the operator cannot replace or broaden it.
...
Acceptance requires that the pinned formatter reports no drift for the approved change boundary. Validate the complete envelope before any load, command, or mutation.

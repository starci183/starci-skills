# Execute `quality/unit-coverage`

This operator runs unit tests and coverage policy. Input, output, loaded context, command captures, diagnostics, observations, and receipts remain session-only and are purged at every parent-skill terminal.
...
**Decision criteria:** tests pass and measured coverage satisfies the pinned policy for the approved revision.
**Analysis:** evaluate the unit result and coverage, then classify failures. A bounded confirmation rerun may target only failed test IDs under an identical fingerprint. Contradictory outcomes emit `external-blocker` with an explicit flaky-test finding; a rerun never converts an unexplained failure into pass. Record applied criteria, structured diagnostics, and conclusions only; never record chain-of-thought.
**Session write:** candidate `unitCoverageReceiptRef` at `scratchPrefix/candidate`.
...
**Stop:** emit only one declared decision; stop on exhausted confirmation budget, inconsistent outcomes, or repair-boundary widening.

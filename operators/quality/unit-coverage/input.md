# `quality/unit-coverage` input

This operator runs unit tests and coverage policy. Its input is an ephemeral task-session object. The runtime never writes the envelope, loaded values, command output, diagnostics, worker observations, or receipts outside the session and purges them at every parent-skill terminal.
...
- `unitCommandRef`: exact `session://` reference; the operator cannot replace or broaden it.
- `coveragePolicyRef`: exact `session://` reference; the operator cannot replace or broaden it.
...
Acceptance requires that tests pass and measured coverage satisfies the pinned policy for the approved revision. Validate the complete envelope before any load, command, or mutation.

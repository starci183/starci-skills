...
This operator runs the declared production build gate. Its input is an ephemeral task-session object. The runtime never writes the envelope, loaded values, command output, diagnostics, worker observations, or receipts outside the session and purges them at every parent-skill terminal.
...
- `buildCommandRef`: exact `session://` reference; the operator cannot replace or broaden it.
...
Acceptance requires that the exact build command exits successfully and generated artifacts remain inside the declared checkout boundary. Validate the complete envelope before any load, command, or mutation.

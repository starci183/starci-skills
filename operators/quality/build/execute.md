# Execute `quality/build`

This operator runs the declared production build gate. Input, output, loaded context, command captures, diagnostics, observations, and receipts remain session-only and are purged at every parent-skill terminal.
...
**Decision criteria:** the exact build command exits successfully and generated artifacts remain inside the declared checkout boundary.
**Analysis:** execute the pinned build command and classify failures by ownership. Record applied criteria, structured diagnostics, and conclusions only; never record chain-of-thought.
**Session write:** candidate `buildReceiptRef` at `scratchPrefix/candidate`.

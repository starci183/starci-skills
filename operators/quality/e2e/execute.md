...
This operator runs the declared end-to-end gate. Input, output, loaded context, command captures, diagnostics, observations, and receipts remain session-only and are purged at every parent-skill terminal.
...
**Decision criteria:** the exact environment, seed, and test command prove user-observable outcomes.
**Analysis:** execute the pinned end-to-end command and classify failures by ownership. Record applied criteria, structured diagnostics, and conclusions only; never record chain-of-thought.
**Session write:** candidate `e2eReceiptRef` at `scratchPrefix/candidate`.

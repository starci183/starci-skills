...
This operator runs the lint gate. Its input is an ephemeral task-session object. The runtime never writes the envelope, loaded values, command output, diagnostics, worker observations, or receipts outside the session and purges them at every parent-skill terminal.
...
- `lintCommandRef`: exact `session://` reference; the operator cannot replace or broaden it.

# Execute `quality/lint`

This operator runs the lint gate. Input, output, loaded context, command captures, diagnostics, observations, and receipts remain session-only and are purged at every parent-skill terminal.
...
**Decision criteria:** the pinned lint machine reports no rule violation in the approved boundary.
**Analysis:** execute the exact lint command and retain only structured diagnostics. Record applied criteria, structured diagnostics, and conclusions only; never record chain-of-thought.
**Session write:** candidate `lintReceiptRef` at `scratchPrefix/candidate`.

...
| `landing-composition` | `landing.compose`; contract chỉ đọc về promise, mức áp dụng Grammar, storyboard, asset và motion cho một bề mặt landing | không |
...
| --- | --- | --- | --- | --- | --- |
| 1 | Kiểm gate và chạy lại, đếm số vòng audit, xác nhận phiên và head đóng băng, và chạy preflight của request trước lần ghi đầu ra ngoài thư mục phiên | `resume`, `approval`, `mode`, `maxRounds` | `request/request.json`, `state.json` của phiên và `step-N/parallel-M` của nhánh này, đầu vào `frontend-direction-decision` khi chạy lại và `frontend-surface-audit` khi đây là vòng lặp, @workspaces/fe ở head đóng băng | — | `INVALID_INPUT`, `SESSION_MISSING`, `SOURCE_DRIFT`, `NO_PROGRESS` |
...
| 3 | Quan sát context đang có và tham chiếu của người, rồi biên UI contract, coverage của nó và lớp bề mặt; với landing, bind composition contract mà không đổi promise, identity hay ownership handoff | `references` | @workspaces/fe (artifact trực tiếp của target, hoặc host được uỷ quyền và họ sản phẩm khi target chưa tồn tại), @worktrees/uat/<flow>/<case> nếu có, @knowledge/ui/composition (`COVERAGE-1` Case 7 publish bộ từ vựng lớp), các đầu vào `business-promise-authority` và `landing-composition` nếu có | `ui-coverage` | `EVIDENCE_MISSING`, `SCOPE_UNFROZEN` |

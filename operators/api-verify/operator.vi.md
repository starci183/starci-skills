...
Khi `done`, in **Operator Result** thành bảng verdict theo case và lane từ `response/data/verdicts.json`; đính kèm `response/data/cases.json` và `response/artifacts/api-output.txt` làm evidence phụ. Suite fail hoặc chưa đủ phải hiện đúng các dòng case fail và raw output, không bao giờ trình bày run đó như đã verify.

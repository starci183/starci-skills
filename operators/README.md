...
`materialize.mjs` deterministically generates the atomic operator shelf without creating a skill graph. Run `node materialize.mjs`, then `node validate-operators.mjs`. The validator proves path/manifest identity, exact eight-file shape, closed schemas, Qdrant `LOADS` parity, source-reference identity and fail-closed validators.

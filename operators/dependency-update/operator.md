...
Done when the `dependency-update` records exactly one session commit whose metadata delta touches
only the named dependency entries of the declared manifests and lockfile, the installed package
matches the digest-verified release file for file, and each `dependency-proof` with its raw
`dependency-log` shows the unchanged regression and every declared gate passing on that commit,
whose paths the `changes` names.

...
- `knowledge`: exact pinned `fe.e2e-testing` only. The approved test plan and seed receipt already project journey, state, and fixture authority; reloading their source knowledge would duplicate context.
...
- `cache`: one exact fingerprint and optional session receipt candidate; mutable services or data make the candidate non-reusable.

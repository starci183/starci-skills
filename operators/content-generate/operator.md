...
| 2 | Bind the served unit and the runtime | `unit` | @remote/minio/<contentId>/<locale> for curriculum and style as served, @worktrees/sessions/central-runtime for the runtime configuration | — | — |
| 3 | Write and freeze the brief | `naturalLanguages`, `implementationLanguages` | @remote/minio/<contentId>/<locale> for curriculum and source evidence | `response/brief.md` | `BRIEF_UNBOUND` |
...
| 5 | Generate the image to its claims, only when `stageModes` turns it on | `stageModes` | `response/brief.md` for the claims and the stated image intent | `response/artifacts/image.<name>`, `response/artifacts/prompt.<name>.txt` | `IMAGE_UNAVAILABLE` |
| 6 | Implement every declared track | `implementationLanguages`, `commands` | `response/brief.md`, @worktrees/sessions/central-runtime where each build command runs | `response/artifacts/track.<language>.<extension>` | `CODE_BUILD_FAILED` |
| 7 | Run the executable check, at most `maxE2eIterations` iterations | `maxE2eIterations` | @worktrees/sessions/central-runtime where each declared command runs | `response/data/e2e.json` | `E2E_FAILED`, `CONTRACT_WEAKENED` |

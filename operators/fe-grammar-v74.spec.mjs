...
const base = () => {
  const manifest = {
    ...
    semanticComposition: [
      { instanceRef: 'composition://intro', patternRef: 'pattern://ContextIntro', ownerRef: 'owner://intro', ownerLayer: 'grammar', authorityRef: 'grammar://context-intro', slots: [
      ...
      { instanceRef: 'composition://rail', patternRef: 'pattern://RightRail', ownerRef: 'owner://rail', ownerLayer: 'grammar', authorityRef: 'grammar://right-rail', slots: [{ slotRef: 'content', semanticRole: 'support', cardinality: 'many', order: 0 }] }],
    requiredDecisionRefs: ['decision://eyebrow', 'decision://heading', 'decision://description', 'decision://page-inset', 'decision://rail-space', 'decision://rail-sticky'],
    decisions: [
      ...
      { decisionRef: 'decision://page-inset', ownerRef: 'owner://page', kind: 'spacing', semanticRole: 'page-container.content-inset', tokenRefs: ['token://space/inline-3', 'token://space/block-6'], evidenceRefs: ['request://teacher'] },
      { decisionRef: 'decision://rail-space', ownerRef: 'owner://rail', kind: 'spacing', semanticRole: 'navigation.right-rail.content-inset', tokenRefs: ['token://space/inline-3', 'token://space/block-6'], evidenceRefs: ['request://teacher'] },
...
test('Grammar 7.4 rejects physical styling and incomplete sticky ownership', () => { const value = base(); value.decisions[4].tokenRefs = ['token://color/purple', 'token://space/px-3']; delete value.decisions[5].sticky; value.manifestFingerprint = fingerprintGrammarDecision(value); const errors = validateGrammarDecision(value).errors.join('\n'); assert.match(errors, /raw utility/); assert.match(errors, /sticky lifecycle/); });

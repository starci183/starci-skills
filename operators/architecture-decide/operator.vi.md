...
`data/current-state.json` với fingerprint riêng. Một đề xuất viết trước khi quan sát mô tả một hệ
thống đơn giản hơn hệ thống thật, và mọi so sánh sau đó thừa hưởng sự đơn giản hoá ấy. Quan sát ở
một head khác còn tệ hơn: trông chặt chẽ nhưng mô tả mã không còn tồn tại.
...
Context chỉ đọc. Operator chỉ ghi vào thư mục bước của chính nó: `response.md`,
`data/current-state.json`, `data/stack-model.json`, `critique.md`, trang so sánh phương án khi được
yêu cầu nhiều hơn một phương án, và `output.json`. Nó không sửa source được route, không publish
thẩm quyền nghiệp vụ, không khởi động hay cấu hình lại dịch vụ runtime, không nêu tên file
implementation trong handoff, và không tuyên bố implementation, cổng chất lượng hay UAT đã qua.
...
| `decisionId` | id | slug của `objective` | Tên mà các artifact mang |
...
| `resume` | token | null | Token của bước bị chặn khi vào lại sau một mã dừng |

## Các bước

| # | Bước | Tham số | Đọc | Ghi | Dừng với |
| --- | --- | --- | --- | --- | --- |
| 1 | Kiểm gate và chạy lại | `resume`, `approval` | `input.json`, `request.md`, đầu vào `architecture-decision` nếu có, @workspaces/be ở head đóng băng | — | `INVALID_INPUT`, `SOURCE_DRIFT`, `NO_PROGRESS` |
| 2 | Quan sát hiện trạng | — | @workspaces/be ở head đóng băng: manifest, cấu hình, file deploy | `data/current-state.json` | `CURRENT_STATE_UNOBSERVED` |
| 3 | Ràng inventory với lời hứa nghiệp vụ | — | `data/current-state.json`, @worktrees/businesses/<featureId> ở head đã publish | — | `BUSINESS_AUTHORITY_REQUIRED`, `EVIDENCE_MISSING` |
| 4 | Đóng khung quyết định | `objective`, `decisionId`, `constraints`, `tradeoffAxes` | `request.md` | — | `CONSTRAINT_CONTRADICTION` |
| 5 | Sinh các phương án | `alternatives` | `data/current-state.json`, @knowledge/patterns | `artifacts/<decisionId>-alternatives.html` chỉ khi được yêu cầu nhiều hơn một phương án | `NO_VIABLE_ALTERNATIVE` |
| 6 | Chọn | `selectionPolicy`, `tradeoffAxes`, `approval` | `artifacts/<decisionId>-alternatives.html` khi có | — | `CHOICE_REQUIRED` |
...
| 8 | Phản biện độc lập: một agent mới trên profile của operator này, không thừa hưởng lượt | — | `data/stack-model.json` và các khẳng định của nó, không bao giờ là lý lẽ của tác giả | `critique.md` | `CRITIQUE_UNRESOLVED` |
| 9 | Xác nhận hoặc trả lại lựa chọn | `selectionPolicy` | `critique.md`, `data/stack-model.json` | — | `CHOICE_REQUIRED`, `NO_VIABLE_ALTERNATIVE` |
| 10 | Viết handoff và phát | — | mọi thứ ở trên | `response.md`, `output.json` | — |
...
| --- | --- | --- | --- |
| `architecture-decision` | `response.md` | md | có |
| `current-state` | `data/current-state.json` | data | có |
| `stack-model` | `data/stack-model.json` | data | có |
| `alternatives` | `artifacts/<decisionId>-alternatives.html` | artifact | không |
| `independent-critique` | `critique.md` | md | có |

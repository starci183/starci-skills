...
| `@be` | `<checkout:input.project.id/be>  (the backend checkout of the project this invocation binds)` | fingerprint + sourceHead (git rev-parse HEAD of the checkout) | Bắt buộc: The routed checkout observed at the frozen head; the inventory comes from its manifests and deployment files. |
| `@knowledge/patterns` | `<Source>/.claude/knowledge/<group>/  (every canonical .md inside; a single file may be named as <group>/<topic>.md)` | fingerprint per file; the rule inventory is the set of `## PREFIX-n` headings across the folder's canonical files | Tuỳ chọn: Reusable patterns the scope may bind; a shape, not a selection. |

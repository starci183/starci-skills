...
export const GOVERNANCE_FIELDS = ['grammarBinding', 'iconographyManifest', 'mediaManifest', 'productFamilyEvidence'];
...
export function capturePreflightInputSemantic(value) {
  ...
  if (value.input.productFamilyEvidence.grammarBindingRef !== value.input.grammarBinding.bindingRef) errors.push('$.input.productFamilyEvidence: must bind the exact Grammar binding');
  for (const ref of [value.input.grammarBinding.auditPlanRef, value.input.iconographyManifest.manifestRef, value.input.mediaManifest.manifestRef, ...value.input.productFamilyEvidence.benchmarkRasterRefs]) {
    if (!value.context.evidenceRefs.includes(ref)) errors.push(`$.context.evidenceRefs: must include ${ref}`);
  }
...
export function capturePreflightOutputSemantic(value) {
  ...
  if (outcome === 'ready') {
    ...
    if (result.productFamilyEvidence.grammarBindingRef !== result.grammarBinding.bindingRef) errors.push('$.output.result.productFamilyEvidence: must bind the exact Grammar binding');
    for (const ref of [result.grammarBinding.auditPlanRef, result.iconographyManifest.manifestRef, result.mediaManifest.manifestRef, ...result.productFamilyEvidence.benchmarkRasterRefs]) if (!evidenceRefs.includes(ref)) errors.push(`$.output.evidenceRefs: must include ${ref}`);
  } else if (result !== null) {
...
export function renderCaptureOutputSemantic(value) {
  ...
  if (value.output.outcome === 'captured') {
    ...
    if (result.productFamilyEvidence.grammarBindingRef !== result.grammarBinding.bindingRef) errors.push('$.output.result.productFamilyEvidence: must bind the exact Grammar binding');
    const blindGrammarBinding = { bindingRef: result.grammarBinding.bindingRef, manifestRef: result.grammarBinding.manifestRef, decisionManifestFingerprint: result.grammarBinding.decisionManifestFingerprint, auditPlanRef: result.grammarBinding.auditPlanRef, auditPlanFingerprint: result.grammarBinding.auditPlanFingerprint };
    const blindIconographyManifest = { manifestRef: result.iconographyManifest.manifestRef, manifestFingerprint: result.iconographyManifest.manifestFingerprint, mode: result.iconographyManifest.mode, visualFamilyRef: result.iconographyManifest.visualFamilyRef };
    const blindMediaManifest = { manifestRef: result.mediaManifest.manifestRef, manifestFingerprint: result.mediaManifest.manifestFingerprint, mode: result.mediaManifest.mode, assetRef: result.mediaManifest.assetRef };
    if (JSON.stringify(result.blindReviewPacket?.grammarBinding) !== JSON.stringify(blindGrammarBinding)) errors.push('$.output.result.blindReviewPacket.grammarBinding: must equal the opaque Grammar projection');
    if (JSON.stringify(result.blindReviewPacket?.iconographyManifest) !== JSON.stringify(blindIconographyManifest)) errors.push('$.output.result.blindReviewPacket.iconographyManifest: must equal the opaque iconography projection');
    if (JSON.stringify(result.blindReviewPacket?.mediaManifest) !== JSON.stringify(blindMediaManifest)) errors.push('$.output.result.blindReviewPacket.mediaManifest: must equal the opaque media projection');
...
export function visualFidelityInputSemantic(value) {
  ...
  if (family?.grammarBindingRef !== packet.grammarBinding?.bindingRef) errors.push('$.input.blindReviewPacket.productFamilyEvidence: must bind the opaque Grammar identity');
  if (packet.iconographyManifest?.mode === 'none' && packet.iconographyManifest.visualFamilyRef !== null) errors.push('$.input.blindReviewPacket.iconographyManifest: none mode cannot carry a visual family');
  if (packet.iconographyManifest?.mode === 'resolved' && packet.iconographyManifest.visualFamilyRef === null) errors.push('$.input.blindReviewPacket.iconographyManifest: resolved mode requires a visual family');
  if (packet.mediaManifest?.mode === 'none' && packet.mediaManifest.assetRef !== null) errors.push('$.input.blindReviewPacket.mediaManifest: none mode cannot carry an asset');
  if (packet.mediaManifest?.mode !== 'none' && packet.mediaManifest?.assetRef === null) errors.push('$.input.blindReviewPacket.mediaManifest: reuse/generate mode requires an asset identity');
...
export function visualFidelityOutputSemantic(value) {
  const errors = [];
  const { outcome, result, gaps, evidenceRefs } = value.output;
  const passingOutcome = outcome === 'passed';
  ...
  if (result) {
    if (result.productFamilyEvidence.grammarBindingRef !== result.grammarBinding.bindingRef) errors.push('$.output.result.productFamilyEvidence: must bind the opaque Grammar identity');
    if (result.grammarAudit.grammarManifestRef !== result.grammarBinding.manifestRef || result.grammarAudit.grammarManifestFingerprint !== result.grammarBinding.decisionManifestFingerprint) errors.push('$.output.result.grammarAudit: Grammar decision identity differs from the blind packet binding');
    if (result.grammarAudit.iconographyManifestRef !== result.iconographyManifest.manifestRef || result.grammarAudit.iconographyManifestFingerprint !== result.iconographyManifest.manifestFingerprint) errors.push('$.output.result.grammarAudit: iconography identity differs from the blind packet binding');
    if (result.grammarAudit.mediaManifestRef !== result.mediaManifest.manifestRef || result.grammarAudit.mediaManifestFingerprint !== result.mediaManifest.manifestFingerprint) errors.push('$.output.result.grammarAudit: media identity differs from the blind packet binding');
    if (result.grammarAudit.proofMatrixFingerprint !== result.matrixFingerprint) errors.push('$.output.result.grammarAudit: proof matrix differs from the reviewed packet');
    if (!result.artifactRefs.includes(result.grammarAudit.auditRef) || !evidenceRefs.includes(result.grammarAudit.auditRef)) errors.push('$.output.result.grammarAudit: auditRef must be retained as artifact and evidence');
    for (const ref of result.grammarAudit.evidenceRefs) if (!evidenceRefs.includes(ref)) errors.push(`$.output.evidenceRefs: must include Grammar audit evidence ${ref}`);
    if (outcome === 'passed' && result.grammarAudit.status !== 'passed') errors.push('$.output.result.grammarAudit: visual PASS requires a passed Grammar audit');
    if (result.grammarAudit.status === 'failed' && outcome !== 'repair') errors.push('$.output.outcome: failed Grammar audit requires repair');
    if (visualRoundPurpose(result.visualRound?.number) !== result.visualRound?.purpose) errors.push('$.output.result.visualRound: number and purpose do not match');

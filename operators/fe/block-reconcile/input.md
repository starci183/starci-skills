...
- `priorStateRef`: the accepted upstream state that authorizes compare one approved Block contract with its observed surfaces and produce the minimal reconciliation plan.
- `businessHeadRef`: the selected business authority reference.
- `authorityRefs`: the exact surface-audit result, Block identity, and accepted maintenance boundary references.
...
- `upstream`: resolve only the declared session references for surface-audit result, Block identity, and accepted maintenance boundary.

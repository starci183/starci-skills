# Execute `fe/block-reconcile`

This operator's responsibility is to compare one approved Block contract with its observed surfaces and produce the minimal reconciliation plan. Its input, output, loaded bindings, worker observations, drafts, and evidence exist only inside the current task session.
...
**Decision evidence:** reference equality, revision hash, accepted business status, and the specific facts needed to compare one approved Block contract with its observed surfaces and produce the minimal reconciliation plan.
...
**Context:** resolve only surface-audit result, Block identity, and accepted maintenance boundary. Retrieve only the listed knowledge IDs at their pinned content hashes. Resolve `payload.loads.frontendSource` only: the hash-pinned plain-JSON contract snapshot, generator fingerprint, selected query IDs, and pinned index generation. Never load raw repository material or broaden the contract query.
...
**Durable write:** none.
**Stop:** stop when the Block owner, contract generation, or affected consumer set cannot be proven.
...
**Decision evidence:** Group observations by Block identity, variant, responsive state, and semantic owner. Distinguish contract drift from consumer misuse and specify the smallest correction without inventing business semantics.
...
**Session write:** a normalized draft at `scratchPrefix/block-reconcile-draft`.
**Durable write:** none at this step.
**Stop:** stop when the Block owner, contract generation, or affected consumer set cannot be proven.
**Orchestration:** Workers may inspect disjoint Block variants read-only. The coordinator joins by Block identity and refuses overlapping or contradictory proposals.
...
**Decision evidence:** Check that every proposed delta preserves closed Grammar invariants, names the affected contract axis, and identifies all consumers that must be aligned.

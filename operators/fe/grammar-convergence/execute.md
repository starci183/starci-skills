...
**Decision evidence:** reference equality, revision hash, accepted business status, and the specific facts needed to bind an approved layout to the locked Grammar packages and refuse business semantics inside Grammar.
...
**Context:** resolve only approved layout, neutral fact model, and Grammar lock. Retrieve only the listed knowledge IDs at their pinned content hashes. Resolve `payload.loads.frontendSource` only: the hash-pinned plain-JSON contract snapshot, generator fingerprint, selected query IDs, and pinned index generation. Never load raw repository material or broaden the contract query.
...
**Decision evidence:** Resolve each layout Block to common or selected Grammar ownership, verify required branches, composites, leaves, states, labels, and package dependencies, and keep all business meaning outside Grammar.
...
**Decision evidence:** Prove every rule is neutral, package-owned where declared, unambiguous, and compatible with the pinned frontend contract generation.
...
| `@fe-grammar-complex-cases` | `fe.grammar-complex-cases` | qdrant | retrieve only this operator law from the pinned generation |

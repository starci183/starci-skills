# Execute `fe/page-model`

This operator's responsibility is to turn one approved customer journey into an ordered page model with explicit page and global-progress ownership. Its input, output, loaded bindings, worker observations, drafts, and evidence exist only inside the current task session.
...
**Decision evidence:** reference equality, revision hash, accepted business status, and the specific facts needed to turn one approved customer journey into an ordered page model with explicit page and global-progress ownership.
...
**Context:** resolve only approved flow direction, approval identity, and business head. Retrieve only the listed knowledge IDs at their pinned content hashes. This operator has no frontend capability or repository-file binding.
...
**Durable write:** none.
**Stop:** stop on approval-hash mismatch, ownerless step, duplicate progress owner, or a page depending on an unevidenced fact.
...
**Decision evidence:** Map each journey step to exactly one page responsibility, entry condition, exit condition, primary action, supporting decisions, and cross-page continuity requirement.
...
**Session write:** a normalized draft at `scratchPrefix/page-model-draft`.
**Durable write:** none at this step.
**Stop:** stop on approval-hash mismatch, ownerless step, duplicate progress owner, or a page depending on an unevidenced fact.
**Orchestration:** Workers may model disjoint journey segments read-only. The coordinator joins ordered segments, resolves shared navigation, and assigns the single global progress owner.
...
**Decision evidence:** Prove every journey step has one page owner and every linear multi-page journey has exactly one global progress owner referenced by all pages.
...
| `@fe-page-model` | `fe.page-model` | qdrant | retrieve only this operator law from the pinned generation |

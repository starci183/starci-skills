# Execute `fe/render-capture`

Capture one deterministic rendered evidence set for one frozen contract after the latest source mutation and a validated `fe/capture-preflight=ready` result.

Bind the preflight reference, immutable matrix fingerprint, owner-partition fingerprint, visual round,
captured partition references, and dependency-proven reused partition references unchanged. Never
expand or reorder the matrix after preflight. Shared sentinels are always fresh captures.

Record the latest mutation fingerprint/time and the capture fingerprint/time. They must bind the same rendered source and capture must occur after the mutation. Freeze a raster-only blind-review packet as a retained artifact. The packet contains opaque image/probe cells only and excludes source, DOM, tests, measurements, authority text, producer rationale, earlier feedback, suspected defects, prior verdicts, and intended answers.

Freeze the exact source fingerprint and render-state matrix first. Capture actual full-viewport image
artifacts for every matrix cell: each in-scope entry, task, skeleton, loading, pending, empty,
recovery, result, exit, and declared handoff state at wide, intermediate, and compact. Skeleton and
loading must be captured as their own states whenever the surface owns them; they cannot inherit the
steady-state verdict. Capture the declared handoff state once more as an uncropped host-context image
from the exact browser surface and current content viewport the user will receive, with no viewport
override. The handoff capture uses no viewport override. Record that surface identity and content
width/height. A detached automation viewport or
resized emulation cannot stand in for a differently sized in-app panel. Capture focused rasters for every material surface or section with enough parent and adjacent-peer pixels to judge ownership, padding, density, and spacing. A single distant host screenshot is not a complete packet. A screenshot of an overlay
does not cover the obscured surface beneath it. CSS tests, DOM snapshots, geometry, accessibility
output, and source inspection are not image artifacts and cannot produce `captured`.

Also execute the frozen adversarial probe matrix from `knowledge/ui-render-review.md`. Try each attack
against the latest source, capture its resulting pixels, and return the observed outcome. Every probe
category must remain present. An unsupported interaction is `not-applicable` only with an exact
ownership reason; silently skipping it is incomplete capture. Continue through the bounded matrix
after finding a defect so the reviewer receives the full causal cluster.

The lifecycle probe set is plural, not one screenshot per category: state-transition probes cover
skeleton, loading, and steady; page-scroll covers start, middle, end, and restoration; bounded scroll
covers both limits and restoration; zoom covers zoom-in, zoom-out, and restoration. Preserve a fresh
raster after each applicable phase for AI visual-fidelity.

Use StarCi-native frontend authority from `knowledge/ui.md` and the routed project Grammar. Return only this atomic result; never inspect or route internally. Any source or render-state drift makes the capture stale and requires a new invocation.

# Execute `fe/request-emission`

This operator's responsibility is to emit stable extension or creation requests for approved frontend contract gaps without locally reconstructing Grammar. Its input, output, loaded bindings, worker observations, drafts, and evidence exist only inside the current task session.
...
**Decision evidence:** reference equality, revision hash, accepted business status, and the specific facts needed to emit stable extension or creation requests for approved frontend contract gaps without locally reconstructing Grammar.
...
**Context:** resolve only source-fit verdicts, Principle compilation, exact request boundary, and supporting evidence. Retrieve only the listed knowledge IDs at their pinned content hashes. Resolve `payload.loads.frontendSource` only: the hash-pinned plain-JSON contract snapshot, generator fingerprint, selected query IDs, and pinned index generation. Never load raw repository material or broaden the contract query. Resolve only `exactTargets` after validating repository-relative paths and SHA-256 values.
...
**Durable write:** none.
**Stop:** stop on invalid evidence, unstable path, identity collision, incomplete persistence, or source-boundary mismatch.
...
**Decision evidence:** Convert each create or extend verdict into one stable request containing owner, need, contract delta, negative boundary, evidence, and blocked local behavior.
...
**Session write:** a normalized draft at `scratchPrefix/request-emission-draft`.
**Durable write:** none at this step.
**Stop:** stop on invalid evidence, unstable path, identity collision, incomplete persistence, or source-boundary mismatch.
**Orchestration:** Workers may draft disjoint request records read-only. The coordinator joins by stable ID, rejects collisions, and alone writes exact approved request targets.
...
**Decision evidence:** Canonicalize and hash every request identity, detect conflicting content at the same target, and ensure Grammar gaps remain blocked rather than implemented locally.
...
| `@fe-request-lifecycle` | `fe.request-lifecycle` | qdrant | retrieve only this operator law from the pinned generation |

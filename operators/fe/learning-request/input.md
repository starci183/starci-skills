...
- `priorStateRef`: the accepted upstream state that authorizes convert one accepted source-first design correction into a stable, evidence-bound learning request.
- `businessHeadRef`: the selected business authority reference.
- `authorityRefs`: the exact accepted maintenance result, before/after evidence, and approved request boundary references.

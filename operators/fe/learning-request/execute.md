# Execute `fe/learning-request`

This operator's responsibility is to convert one accepted source-first design correction into a stable, evidence-bound learning request. Its input, output, loaded bindings, worker observations, drafts, and evidence exist only inside the current task session.
...
**Decision evidence:** reference equality, revision hash, accepted business status, and the specific facts needed to convert one accepted source-first design correction into a stable, evidence-bound learning request.
...
**Context:** resolve only accepted maintenance result, before/after evidence, and approved request boundary. Retrieve only the listed knowledge IDs at their pinned content hashes. Resolve `payload.loads.frontendSource` only: the hash-pinned plain-JSON contract snapshot, generator fingerprint, selected query IDs, and pinned index generation. Never load raw repository material or broaden the contract query. Resolve only `exactTargets` after validating repository-relative paths and SHA-256 values.
...
**Durable write:** none.
**Stop:** stop when evidence is incomplete, identity collides with different content, or the approved request path changed.
...
**Decision evidence:** Extract the situation, rejected behavior, accepted behavior, owner hypothesis, negative boundary, and proof references. Assign a stable request identity without copying source or business documents.
...
**Session write:** a normalized draft at `scratchPrefix/learning-request-draft`.
**Durable write:** none at this step.
**Stop:** stop when evidence is incomplete, identity collides with different content, or the approved request path changed.
**Orchestration:** Workers may normalize disjoint evidence bundles read-only. The coordinator deduplicates stable identities and alone writes the approved request target.
...
**Decision evidence:** Check that the request is deduplicated, evidence-bound, source-hash-bound, and does not prematurely promote a local correction into Grammar or Principle law.

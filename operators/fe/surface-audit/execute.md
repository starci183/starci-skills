...
This operator's responsibility is to audit declared frontend surfaces for visual, behavioral, contract, and ownership drift. Its input, output, loaded bindings, worker observations, drafts, and evidence exist only inside the current task session.
...
**Decision evidence:** reference equality, revision hash, accepted business status, and the specific facts needed to audit declared frontend surfaces for visual, behavioral, contract, and ownership drift.
...
**Context:** resolve only maintenance scope, accepted baseline, and declared surface inventory. Retrieve only the listed knowledge IDs at their pinned content hashes. Resolve `payload.loads.frontendSource` only: the hash-pinned plain-JSON contract snapshot, generator fingerprint, selected query IDs, and pinned index generation. Never load raw repository material or broaden the contract query.
...
**Durable write:** none.
**Stop:** stop when the baseline, surface identity, contract generation, or proof is missing or stale.
...
**Decision evidence:** Compare each declared surface against business intent, selected Grammar, Block contract, responsive states, and accepted proof. Classify local, consumer, Block, or authority drift.
...
**Session write:** a normalized draft at `scratchPrefix/surface-audit-draft`.
**Durable write:** none at this step.
**Stop:** stop when the baseline, surface identity, contract generation, or proof is missing or stale.
**Orchestration:** Workers may audit disjoint surface IDs read-only. The coordinator joins by shared Block and owner, deduplicates systemic findings, and keeps raw observations session-local.
...
**Decision evidence:** Attach a stable surface ID, observed fact, expected owner, source-capability match, severity, and proof reference to every finding.

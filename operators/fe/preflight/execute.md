...
This operator's responsibility is to freeze workspace, business, Grammar, frontend contract generation, and requested write boundaries before design work. Its input, output, loaded bindings, worker observations, drafts, and evidence exist only inside the current task session.
...
**Decision evidence:** reference equality, revision hash, accepted business status, and the specific facts needed to freeze workspace, business, Grammar, frontend contract generation, and requested write boundaries before design work.
...
**Context:** resolve only received request, verified workspace route, and selected business feature head. Retrieve only the listed knowledge IDs at their pinned content hashes. Resolve `payload.loads.frontendSource` only: the hash-pinned plain-JSON contract snapshot, generator fingerprint, selected query IDs, and pinned index generation. Never load raw repository material or broaden the contract query.
...
**Durable write:** none.
**Stop:** stop on stale workspace route, missing business evidence, invalid contract JSON, ambiguous Grammar, or an out-of-workspace write root.
...
**Decision evidence:** Verify the project route, business revision, selected Grammar package lock, generated frontend contract snapshot hash, Qdrant generation, requested write roots, and orchestration profile.
...
**Session write:** a normalized draft at `scratchPrefix/preflight-draft`.
**Durable write:** none at this step.
**Stop:** stop on stale workspace route, missing business evidence, invalid contract JSON, ambiguous Grammar, or an out-of-workspace write root.
**Orchestration:** The coordinator keeps route and authority checks sequential. Read-only workers may independently verify business, Grammar lock, and contract-generation fingerprints; the coordinator joins all three before emitting.
...
**Decision evidence:** Check the generator and schema fingerprints, reuse the pinned generation when hashes match, and require regeneration/upsert before routing when they do not.

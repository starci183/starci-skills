...
- `priorStateRef`: the accepted upstream state that authorizes resolve approved learning requests into the correct durable authority or an explicit local-only verdict.
- `businessHeadRef`: the selected business authority reference.
- `authorityRefs`: the exact learning-request batch, source acceptance evidence, and approved authority boundary references.
...
- `upstream`: resolve only the declared session references for learning-request batch, source acceptance evidence, and approved authority boundary.

...
- `priorStateRef`: the accepted upstream state that authorizes classify accepted design evidence against the owning Grammar, Principle, pattern, gate, or local source boundary and update only the approved authority target.
- `businessHeadRef`: the selected business authority reference.
- `authorityRefs`: the exact surface-audit result and approved authority boundary references.
...
- `upstream`: resolve only the declared session references for surface-audit result and approved authority boundary.

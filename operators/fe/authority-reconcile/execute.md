# Execute `fe/authority-reconcile`

This operator's responsibility is to classify accepted design evidence against the owning Grammar, Principle, pattern, gate, or local source boundary and update only the approved authority target. Its input, output, loaded bindings, worker observations, drafts, and evidence exist only inside the current task session.
...
**Decision evidence:** reference equality, revision hash, accepted business status, and the specific facts needed to classify accepted design evidence against the owning Grammar, Principle, pattern, gate, or local source boundary and update only the approved authority target.
...
**Context:** resolve only surface-audit result and approved authority boundary. Retrieve only the listed knowledge IDs at their pinned content hashes. Resolve `payload.loads.frontendSource` only: the hash-pinned plain-JSON contract snapshot, generator fingerprint, selected query IDs, and pinned index generation. Never load raw repository material or broaden the contract query. Resolve only `exactTargets` after validating repository-relative paths and SHA-256 values.
...
**Durable write:** none.
**Stop:** stop when ownership is ambiguous, evidence is stale, the target hash changed, or the correction would alter an unapproved authority.
...
**Decision evidence:** For each audited finding, compare the observed behavior, current owner, proposed owner, negative boundary, and proof. Classify it as an existing-authority correction, a new authority request, or a local source-only correction; reject mixed ownership.
...
**Session write:** a normalized draft at `scratchPrefix/authority-reconcile-draft`.
**Durable write:** none at this step.
**Stop:** stop when ownership is ambiguous, evidence is stale, the target hash changed, or the correction would alter an unapproved authority.
**Orchestration:** Read-only workers may classify disjoint finding IDs. Each receives one finding plus the same authority boundary; the coordinator joins by finding ID, resolves ownership collisions, and alone writes the exact target.
...
**Decision evidence:** Re-read the selected authority target, prove that every accepted finding maps to one owner and one stable identity, then apply only the approved authority delta.
...
| `@fe-design-maintenance` | `fe.design-maintenance` | qdrant | retrieve only this operator law from the pinned generation |
...
| `@exact-targets` | `payload.loads.exactTargets` | exact-source | open only approved files after hash verification |

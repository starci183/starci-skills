# Execute `fe/implementation`

This operator's responsibility is to turn the approved layout, Grammar convergence, source-fit verdicts, and Principle decisions into bounded frontend source changes. Its input, output, loaded bindings, worker observations, drafts, and evidence exist only inside the current task session.
...
**Decision evidence:** reference equality, revision hash, accepted business status, and the specific facts needed to turn the approved layout, Grammar convergence, source-fit verdicts, and Principle decisions into bounded frontend source changes.
...
**Context:** resolve only approved implementation boundary, layout hash, source-fit result, and Principle compilation. Retrieve only the listed knowledge IDs at their pinned content hashes. Resolve `payload.loads.frontendSource` only: the hash-pinned plain-JSON contract snapshot, generator fingerprint, selected query IDs, and pinned index generation. Never load raw repository material or broaden the contract query. Resolve only `exactTargets` after validating repository-relative paths and SHA-256 values.
...
**Durable write:** none.
**Stop:** stop on Grammar gap, stale layout or source fingerprint, undeclared target, closed invariant change, non-color structural global CSS change, or failed focused check.
...
**Decision evidence:** Map each approved Block to an existing, extended, or newly approved contract. Build a target-by-target patch plan that preserves Grammar invariants, tier ownership, responsive transformations, and declared business states.
...
**Session write:** a normalized draft at `scratchPrefix/implementation-draft`.
**Durable write:** none at this step.
**Stop:** stop on Grammar gap, stale layout or source fingerprint, undeclared target, closed invariant change, non-color structural global CSS change, or failed focused check.
**Orchestration:** Workers may inspect disjoint exact targets and prepare read-only patch proposals. The coordinator joins by path and shared dependency, resolves conflicts, and alone writes product source.
...
**Decision evidence:** Recheck exact target hashes, apply only allowed changes, and run the focused formatter, type, and static checks declared by the boundary.
...
| `@fe-implementation-boundaries` | `fe.implementation-boundaries` | qdrant | retrieve only this operator law from the pinned generation |

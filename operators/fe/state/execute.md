...
This operator's responsibility is to derive complete business and Block states plus owned transitions for every page in the approved model. Its input, output, loaded bindings, worker observations, drafts, and evidence exist only inside the current task session.
...
**Decision evidence:** reference equality, revision hash, accepted business status, and the specific facts needed to derive complete business and Block states plus owned transitions for every page in the approved model.
...
**Context:** resolve only page model, business state authority, and approved flow. Retrieve only the listed knowledge IDs at their pinned content hashes. This operator has no frontend capability or repository-file binding.
...
**Durable write:** none.
**Stop:** stop on stale page identity, ownerless transition, missing sensitive business fact, or a state without provenance.
...
**Decision evidence:** Enumerate initial, loading, empty, success, partial, validation, permission, error, recovery, and sensitive states only where business or Block provenance requires them; assign every transition to one Block.
...
**Session write:** a normalized draft at `scratchPrefix/state-draft`.
**Durable write:** none at this step.
**Stop:** stop on stale page identity, ownerless transition, missing sensitive business fact, or a state without provenance.
**Orchestration:** Workers may model disjoint pages read-only. The coordinator joins cross-page transitions, global progress state, and shared error or recovery ownership.
...
**Decision evidence:** Trace every page action through its resulting state, prove provenance, and identify sensitive facts that cannot be safely synthesized.
...
| `@fe-state-modeling` | `fe.state-modeling` | qdrant | retrieve only this operator law from the pinned generation |

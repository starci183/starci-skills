...
This operator reconciles declared domain and TLS state with the provider. All intermediate data is session-only and purged at every parent-skill terminal.
...
**Decision criteria:** hostname, zone ownership, target, proxy mode, certificate mode, and provider revision match the domain plan.
**Analysis:** compute the smallest DNS/TLS delta, apply only declared changes, and verify resulting records. Record value-safe evidence and conclusions, never chain-of-thought.
**Session write:** candidate `domainReceiptRef` at `scratchPrefix/candidate`.
...
**Durable write:** only for `ready`: reconcile declared domain and TLS provider state.

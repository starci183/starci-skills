...
Apply or classify the declared migration before rollout. All intermediate data and evidence are task-session-only.
...
**Action:** The coordinator verifies applicability and preconditions, then runs only the approved migration or initialization command, records the data revision, verifies postconditions, and selects rollback when safety cannot be proved. Workers never run commands or deployments and never mutate filesystem, runtime, provider, or data state; the coordinator owns effects and joins.
...
**Action:** select one manifest decision. Success requires the approved migration completes with its postcondition and rollback evidence, or exact evidence proves it is not applicable.

...
This operator publishes declared immutable artifacts to the approved registry. All intermediate data is session-only and purged at every parent-skill terminal.
...
**Read:** `payload.loads.commands`, `payload.loads.external`.
...
**Decision criteria:** local digest, registry destination, credential capability, remote revision, and immutability rule match the plan.
**Analysis:** verify digest identity, publish only missing immutable objects, and confirm the remote digest. Record value-safe evidence and conclusions, never chain-of-thought.
**Session write:** candidate `artifactPublishReceiptRef` at `scratchPrefix/candidate`.
...
**Durable write:** only for `ready`: publish declared immutable artifacts.

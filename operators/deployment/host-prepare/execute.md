# Execute `deployment/host-prepare`

This operator prepares the declared host idempotently. All intermediate data is session-only and purged at every parent-skill terminal.
...
**Read:** `payload.loads.source`, `payload.loads.commands`, `payload.loads.external`.
...
**Decision criteria:** host identity, OS facts, packages, users, directories, firewall intent, and setup source match the plan.
**Analysis:** verify exact setup source and apply only missing idempotent host steps. Record value-safe evidence and conclusions, never chain-of-thought.
**Session write:** candidate `hostReceiptRef` at `scratchPrefix/candidate`.
...
**Durable write:** only for `ready`: apply only declared idempotent host setup.
...
| `@exact-source` | `payload.loads.source.targetFiles` | exact-source | inspect only hash-pinned files |

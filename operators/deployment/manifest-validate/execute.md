# Execute `deployment/manifest-validate`

Validate one closed deployment manifest. All intermediate data and evidence are task-session-only.
...
**Action:** Check exact manifest fields, immutable identities, dependencies, health probes, migration declarations, rollback declarations, and secret-handle references without mutation. Workers never run commands or deployments and never mutate filesystem, runtime, provider, or data state; the coordinator owns effects and joins.
...
**Action:** select one manifest decision. Success requires the manifest is closed, internally consistent, revision-pinned, and every required proof and rollback field is present.

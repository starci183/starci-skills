# Execute `deployment/recover`

Repair the smallest declared deployment-owned failure boundary. All intermediate data and evidence are task-session-only.
...
**Action:** The coordinator verifies the failed condition and rollback availability, applies only approved reversible recovery actions, rechecks the condition, and routes to retry, rollback, approval wait, or blocker without changing immutable product source. Workers never run commands or deployments and never mutate filesystem, runtime, provider, or data state; the coordinator owns effects and joins.
...
**Action:** select one manifest decision. Success requires the failed condition clears within the approved recovery boundary and retry preconditions are restored, or exact evidence selects rollback or approval.

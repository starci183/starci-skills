...
| --- | --- | --- | --- |
| `retry` | `completed` | `deployment.retry / ready` | Adds `deployment-retry-ready`. |
| `rollback` | `completed` | `deployment.rollback / ready` | Adds `deployment-rollback-required`. |
| `approval-required` | `waiting` | `deployment.review / pending` | Adds `deployment-approval-required`. |
| `blocked` | `blocked` | `deployment.blocked / blocked` | Adds `deployment-recovery-blocked`. |

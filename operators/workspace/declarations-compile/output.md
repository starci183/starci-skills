...
| `payload.produced` | Session artifact `compiledDeclarationsRef` plus explicitly approved durable effects only. |
...
| --- | --- | --- | --- |
| `ready` | `completed` | `workspace.routes.hydrate / ready` | workspace-declarations-ready |

`compiledDeclarationsRef`, evidence, receipts, observations, and output use `session://`. Only a product/worktree effect explicitly declared by `operator.json` may survive the skill.

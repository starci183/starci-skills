# Execute `workspace/identity-verify`

This operator verifies machine identity and its encrypted credential roster. Input, output, context bindings, analysis evidence, worker observations, and receipts remain in task-session memory and are purged at every parent-skill terminal.
...
**Session write:** accepted artifact at `payload.session.scratchPrefix/identityReceiptRef` and evidence under `scratchPrefix/evidence`.
...
| `@knowledge-1` | `workspace.initialization` | qdrant | retrieve only this operator law |

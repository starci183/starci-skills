...
| --- | --- | --- | --- | --- |
| 1 | Kiểm tra input và resume | input, `@workspaces/be` (binding head đã đóng băng) | — | `INVALID_INPUT`, `SOURCE_DRIFT`, `NO_PROGRESS` |
| 2 | Chuẩn hoá bằng chứng | `@workspaces/be` (từng claim kèm vai trò, đường dẫn, khoảng dòng và head), `@dynamic/architecture-decision.json` (chỉ là bằng chứng kiến trúc) | — | `EVIDENCE_MISSING`, `CONTRADICTION_UNRESOLVED` |
| 3 | Kiểm tra head đã publish | `@worktrees/businesses/<featureId>` (head hiện tại và bằng chứng đã đóng băng), input (trạng thái đích được yêu cầu) | — | `LIFECYCLE_TRANSITION_INVALID`, `AUTHORITY_CONFLICT`, `APPROVAL_REQUIRED` |
| 4 | Mô hình hoá lời hứa | `@workspaces/be` (chỉ các claim loại fact) | — | — |
| 5 | Đóng băng ma trận coverage | input (các chiều), `@workspaces/be` (các consumer đã phát hiện, các claim đã chuẩn hoá) | `@dynamic/coverage-matrix.json` | `COVERAGE_INCOMPLETE`, `CONSUMER_UNPROVEN` |
| 6 | Định đoạt phần cùng tồn tại với di sản | `@dynamic/coverage-matrix.json` (các dòng tạo, đọc, tất toán di sản cùng bằng chứng của chúng) | — | — |
| 7 | Đối chiếu khi đích là implemented | `@workspaces/be` (source đã giao), `@dynamic/coverage-matrix.json` (ma trận đã đóng băng) | — | `RECONCILIATION_DISCREPANCY` |
| 8 | Publish đúng một head | `@dynamic/coverage-matrix.json`, `@worktrees/businesses/<featureId>` (head trước đó) | `@worktrees/businesses/<featureId>` (head `model.json` mới) | — |
| 9 | Phát ra và dừng | tất cả những gì ở trên | `@dynamic/business-promise-authority.json` | — |

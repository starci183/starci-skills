...
Generated by `scripts/generate-operators-index.mjs` from every `operator.json`, the Sequence table of every `execute.md`, and every `output.schema.json`; `--check` runs inside `npm test`, so this table cannot drift from the packages. Static refs exist before the session; dynamic refs (`@dynamic/<file>`) are produced by an earlier step of the same session and disappear with it. 14 operators.

## What each operator needs and produces

| Operator | Profile | Reads (static) | Reads (dynamic, this session) | Writes | Steps | Stop codes |
| --- | --- | --- | --- | --- | --- | --- |
...
## Hand-offs inside a session

Every file that only exists inside a session, who writes it, and who reads it next. A file with no consumer is a receipt kept for audit; a file with no producer is a defect the check reports.

| Dynamic file | Produced by | Consumed by |
| --- | --- | --- |

...
| `AUTHORITY_DRIFT` | `platform.operate` | `caller` | terminate | The approval — an approval id, or the environment declaration it references, whose content hash has moved — the plan hash, or the allowed effect set no longer matches the requested delta. | — | — | Bring a fresh approval for this exact plan: a new id, or the declaration's current reference. |

...
| `INTEGRATION_CONFLICT` | `platform.operate` | `product` | terminate | Merging the session branch into the integration branch conflicted, so the head a session asked to have served cannot be built from what is already there. | — | — | A person resolves the conflict in the session branch and asks for the head again; the merge is never rebased, forced or abandoned to make it apply. |

...
A delivery that composed nothing is not judged as though it had. When the direction this audit reads
declares a presentation delta of `none` — a copy, behaviour or binding change that publishes no
presentation value — the composition under the capture is an earlier direction's, so this round
carries that audit's taste lens rather than taking the scale again: `## Calibration` records one row
per lens whose anchor cell reads `inherited` and whose expected cell names the branch the lens was
scored in, the verdicts carry no anchor scores, and `## Ranked against` places nothing, because
nothing was placed here. Under that same delta the reference criterion stops applying, on the terms
`TASTE-12` Case 5 states, and the validator holds its marker to that criterion and that delta alone.
A `fix-first` carried this way is that earlier composition's finding: it stands where it was raised,
this branch opens no direction round for it, and `next` may name `quality.verify`. A `fix-first` on
composition this delivery did touch keeps the law above — the surface is composed again from a new
direction and the checkout's gates wait for a `ship`.
...
| `landing-composition` | `landing.compose`; the storytelling, motion, reduced-motion and performance evidence contract when the observed surface is a landing | no |
...
| `feature` | id | — | The feature key that addresses the unchecked ledger the deferred states and coverage gaps of this audit are written to, @worktrees/unchecked/<product>/<feature>.jsonl |
...
A deferred state is coverage left unchecked, and it is written down as such. Every entry of `deferredStates`
carries its reason in `deferrals`, the sentence the ledger under `@worktrees/unchecked` copies rather than
invents, so a reader of the ledger months later learns why the state was skipped from the audit that
skipped it. This branch measures one unit the mission's journey passes through — the fan-out never
sends a deferred unit here — and a journey unit is proved in every state a reader meets on the way
through it: the four the scope schema publishes as `journeyStates`. Naming one of them as deferred is
`UNCHECKED_UNLAWFUL` and goes back to the plan, because the repair for coverage the journey needs is a
different tiering, never a smaller audit.
...
presented without its finding and conformance/quality conclusion. The label identifies the attained
or failing region and verdict visible in the chosen decision-useful state and viewport; the primary
is the audit's own capture, never a prettier unrelated mock.
...
| a state of a journey unit was deferred rather than measured, so the plan that tiered the unit tiers it again before the surface is audited | `interface.plan` |

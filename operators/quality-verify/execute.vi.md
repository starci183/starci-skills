...
| --- | --- | --- | --- | --- |
| 1 | Kiểm tra input và resume | input, `@dynamic/<receiptType>.json`, `@workspaces/fe` hoặc `@workspaces/be` (head đã đóng băng của ranh giới đang kiểm) | — | `INVALID_INPUT`, `SOURCE_DRIFT`, `NO_PROGRESS` |
| 2 | Tiêu thụ tiền nhiệm nguyên trạng | `@dynamic/<receiptType>.json` (từng receipt thượng nguồn theo tham chiếu, loại, fingerprint và head) | — | `PREDECESSOR_MIXED`, `PREDECESSOR_STALE` |
| 3 | Xác minh head | `@workspaces/fe` hoặc `@workspaces/be` (head quan sát được của ranh giới đang kiểm, đối chiếu head đã đóng băng) | — | — |
| 4 | Chạy các gate theo thứ tự đã khai | `@workspaces/<project>/<role>/gates` (kế hoạch gate đã khai và các entrypoint của repo), `@workspaces/fe` hoặc `@workspaces/be` (đối tượng mà từng gate đo) | `@dynamic/gates/<gate>.json` | `GATE_UNAVAILABLE` |
| 5 | Áp chính sách độ phủ | `@dynamic/gates/<gate>.json` (kết quả độ phủ của cổng unit), `@workspaces/<project>/<role>/gates` (bốn ngưỡng) | `@dynamic/gates/unit-coverage.coverage.json` | — |
| 6 | Phân loại từng thất bại | `@dynamic/gates/<gate>.json` (chẩn đoán có cấu trúc của mọi gate đỏ) | — | — |
| 7 | Áp nợ đã duyệt | `@worktrees/debts` (các khoản nợ đã được chủ sở hữu duyệt cùng hạn của chúng) | — | `DEBT_UNAPPROVED` |
| 8 | Phát và dừng | tất cả những gì ở trên | `@dynamic/quality-verification.json` | — |

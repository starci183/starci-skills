...
| 4 | Chạy các gate theo thứ tự đã khai | `@workspaces/<project>/<role>/gates` (kế hoạch gate đã khai và các entrypoint của repo), `@workspaces/fe` hoặc `@workspaces/be` (đối tượng mà từng gate đo) | `@artifacts/gates/<gate>.json` | `GATE_UNAVAILABLE` |
| 5 | Áp chính sách độ phủ | `@artifacts/gates/<gate>.json` (kết quả độ phủ của cổng unit), `@workspaces/<project>/<role>/gates` (bốn ngưỡng) | `@artifacts/gates/unit-coverage.coverage.json` | — |

...
This operator queries redacted conversation provenance. Input, output, context bindings, analysis evidence, worker observations, and receipts remain in task-session memory and are purged at every parent-skill terminal.
...
**Session write:** accepted artifact at `payload.session.scratchPrefix/conversationQueryReceiptRef` and evidence under `scratchPrefix/evidence`.
...
| `@knowledge-1` | `source.provenance` | qdrant | retrieve only this operator law |

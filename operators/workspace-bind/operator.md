...
declaration and its hydrated projection alone, with the policy and cleanliness checks required by its write scope, at its observed source head with its derived mutation
...
A routed binding with no `declaredWriteRoots` observes repository identity, declared branch and head
without requesting mutation readiness. Its absent Git policy is recorded as null; its Policy rows
record an em dash, and existing user-owned working changes are left alone. A declared policy still
must agree between route halves. Session selection and any declared write root retain all policy,
cleanliness and installed-tree permission gates. `scripts/workspace-checkout.mjs` owns that predicate
and independently verifies routed read-only receipts as well as session receipts.
...
The businesses authority root is resolved by `scripts/business-head.mjs#businessesRootFor` from
`@worktrees/businesses`, the owning Workflow and its project partition; a present binding must match
that exact authority regardless of the checkout being read. It is null when no authority is bound.
The person never selects another root or project partition. Provenance and freshness are not a step
...
Evidence: [scoped handoff observations](../../tests/evidence/20260906-scoped-handoff-readiness.md).
...
| `role` | id | — | The declared project route role, using the identifier shape in `readiness/initialization/workspaces/portable-route.schema.json#/$defs/slug`; application route identities do not change source context roles |
...
| `gitPolicy` | object `{worktreeBranches, mutationBranch}`, the two fields of the declared route's `repository.gitPolicy` the binding is checked against | the declared policy, or null for a routed read-only binding; writes and session selection require it | The branch law this binding is verified against; `forbidden` keeps every write on the mutation branch |
| `declaredWriteRoots` | list | empty | The write ceiling; empty selects routed read-only observation. For writes or session selection, dirt outside these roots is `CHECKOUT_DIRTY`, and the canonical mutation checkout must be clean |
...
For a source-writing binding, under `worktreeBranches` set to forbidden, a route binds only on the mutation branch and
...
roots. For writes or session selection, the declared write roots exempt dirt only on a `session/<sessionId>` branch, where it is a
...
tree every other checkout is using; a write or session binding is refused with `INVALID_INPUT` unless the request
...
roots to produce the complete route receipt. Step 4 applies the selection predicate above, and a session selection also requires the canonical
...
session id to its containing coordinate, session state and frozen request hash. The first `attempt-gate open` validates the exact request while holding the owning session mutation, checks its bytes again, and atomically records that hash; this opening phase is unavailable outside the held mutation or for an existing attempt. Ordinary dispatch and receipt validation still require the stored hash.

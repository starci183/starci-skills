...
suy ra, và, khi runtime cần được tiêu thụ, fingerprint endpoint của phép chiếu đóng cùng mục sổ đăng
ký của route này mà head được phục vụ có chứa head source ấy.
...
## Runtime được ràng bằng quan hệ tổ tiên, không phải bằng phép bằng

Một máy phục vụ một nhánh tích hợp cho mỗi sản phẩm, và nhánh đó mang việc của mọi phiên đã xin, nên
head mà một lần bind ghim gần như không bao giờ là head đang phục vụ. Vì vậy lần bind không so hai cái
đó bằng phép bằng — làm thế sẽ hỏng vì số học ngay khi có phiên thứ hai — nó chứng minh head của chính
mình nằm bên trong head đang phục vụ: có mặt trong danh sách commit mà entry ghi là đã chứa, và là tổ
tiên của head đang phục vụ. Cả hai đều được ghi, vì người đọc không thấy head đã ghim nằm cạnh head
đang phục vụ thì không kiểm được lời khẳng định.

Một head đang phục vụ mà không chứa head của route này thì không phải runtime lần bind này được tiêu
thụ. Khi đó `RUNTIME_NOT_READY` nêu tên commit cần được phục vụ và thao tác sẽ phục vụ nó, để yêu cầu
phối hợp tiếp theo là một yêu cầu cụ thể chứ không phải một bản báo rằng có gì đó sai; còn trong lúc
phiên khác giữ lease và đang merge, câu trả lời là `RUNTIME_BUSY` kèm tên bên đang giữ và vị trí xếp
hàng, tức là một lần chờ chứ không phải một lần hỏng. Cả hai đều không phải giấy phép để tự phục vụ ở
đây: operator này vẫn không khởi động gì cả.
...
| `@worktrees/sessions/central-runtime` | registry chủ runtime: entry của route này, head nó đang phục vụ, những gì head đó chứa, lease, generation và bằng chứng sức khoẻ của nó, chỉ ràng khi người gọi tiêu thụ | không |
...
| 5 | Ràng runtime mà người gọi tiêu thụ từ entry của đúng project route này, chỉ khi `runtimeNeed` khác none | `runtimeNeed` | @worktrees/sessions/central-runtime, entry của `<project>/<role>` này cùng head đang phục vụ, những gì head đó chứa, lease, generation, bằng chứng health và khai báo danh tính của nó, @workspaces/ports/<project>, @tools/http | — | `ENDPOINT_AUTHORITY_STALE`, `RUNTIME_NOT_READY`, `RUNTIME_BUSY` |
...
dẫn checkout. JSON đó là quan sát checkout; operator vẫn phải ràng identity, gốc thẩm quyền và runtime
...
| route bind package của owner đã được giao sửa hành vi hiện có | `library.source.apply` |
| cần tiêu thụ bản phát hành package đã kiểm trong metadata dependency frontend chính xác | `dependency.update` |

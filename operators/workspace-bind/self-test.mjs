...
// whose field differs, is refused by a line that names the field, the shape and the declared value.
await expectError({ ...baseline(), 'request/request.json': requestJson({ extra: { gitPolicy: [{ worktreeBranches: 'forbidden', mutationBranch: 'mtp' }] } }) }, 'request.json: gitPolicy.worktreeBranches is absent because gitPolicy is a list, not the object {worktreeBranches, mutationBranch} the route declaration carries; it differs from the declared route (forbidden)', 'a gitPolicy sent as a list');
await expectError({ ...baseline(), 'request/request.json': requestJson({ extra: { gitPolicy: { worktreeBranches: 'session-only', mutationBranch: 'mtp' } } }) }, 'request.json: gitPolicy.worktreeBranches "session-only" differs from the declared route (forbidden)', 'a gitPolicy whose field differs from the route');

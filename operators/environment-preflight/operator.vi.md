...
| `runtimeRoles` | list | `[]` | Các vai có runtime đã được phục vụ mà chuỗi tiêu thụ nhưng không sở hữu `runtime.serve` — planner đặt sẵn mọi vai đã bind khi chuỗi quan sát hay đi thử một runtime có sẵn, nhưng không đặt vai nào khi chuỗi có `runtime.serve` vì đúng owner ấy kiểm kê, bootstrap và chứng thực runtime sau khi bind; họ runtime được kiểm cho các vai này và bỏ qua cho các vai còn lại |
...
In **Operator Result** là bảng readiness đầy đủ render từ `response/data/readiness-report.json`, kèm `response/response.md` làm phần tóm tắt. Wall hoặc check bị skip phải vẫn hiện cùng owner và evidence; chỉ trình bày môi trường sẵn sàng khi mọi dòng bắt buộc đều `ok`.

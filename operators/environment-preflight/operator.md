...
Applicability follows [the readiness-report contract](../../templates/kinds/readiness-report.schema.json)
and `scripts/mission-scope.mjs#documentationReadinessSkips`. Run that predicate against the owning frozen
mission and planned requirements before probing. The same predicate applies while executing a
document artifact at the implement stage, with every active operator still confined to the existing
handoff operator vocabulary. Every skipped row stays in the report with evidence
naming the frozen scope; it never grants an operation approval.
...
corrected silently is a route nobody declared. The applicable checkout checks then read the policy the
...
When applicable, the sealed admin credential of the environment's identity provider is checked through the tree's own
...
The environment declares the auxiliary services it runs beside its product routes, and applicable readiness
...
class in this environment. Each applicable approval check for a class the schema publishes states `declared` or
...
schema makes every applicable approval check a wall owned by `approval`.
...
| 3 | Apply the frozen applicability predicate and check each checkout: the route carries a git policy, the tree is clean and the branch is one the policy permits | — | @workspaces/local/routes/<project>/<role>, the resolved checkout, its branch and its working tree, @tools/git | — | — |
...
Steps 2 to 8 follow the contract applicability predicate above. Step 9 is the only step that stops on a wall: a wall found at steps 2 to 8 is recorded and the next
...
Print **Operator Result** as the complete readiness table rendered from `response/data/readiness-report.json`, with `response/response.md` as its summary. A wall or skipped check stays visible with its owner and evidence; readiness is shown only when every required row is `ok`.
...
| the coordinated portfolio changes no product environment and its frozen peer outcomes must be verified before conclusion | `workflow.verify` |

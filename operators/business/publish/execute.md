...
This operator publishes the routed business head. Input, output, context bindings, analysis evidence, worker observations, and receipts remain in task-session memory and are purged at every parent-skill terminal.
...
**Decision criteria:** the lifecycle transition is legal and the previous head still matches.
**Analysis:** commit the accepted model only to the routed business worktree. Record evidence, criteria, and conclusions only; never record chain-of-thought.
**Session write:** candidate `businessHeadRef` at `payload.session.scratchPrefix/candidate`.
...
**Session write:** accepted artifact at `payload.session.scratchPrefix/businessHeadRef` and evidence under `scratchPrefix/evidence`.

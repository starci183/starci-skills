# Execute `business/reconcile`

This operator reconciles delivery proof with business authority. Input, output, context bindings, analysis evidence, worker observations, and receipts remain in task-session memory and are purged at every parent-skill terminal.
...
**Context:** resolve exact task-session artifacts and retrieve only `business.authority-lifecycle` from its pinned Qdrant generation. Load only the declared business revision under `.worktrees/business/`.
...
**Decision criteria:** every accepted outcome is proved by final source heads or reported as a discrepancy.
**Analysis:** compare delivery evidence to the approved business model. Record evidence, criteria, and conclusions only; never record chain-of-thought.
**Session write:** candidate `reconciliationReceiptRef` at `payload.session.scratchPrefix/candidate`.
...
**Session write:** accepted artifact at `payload.session.scratchPrefix/reconciliationReceiptRef` and evidence under `scratchPrefix/evidence`.
...
**Action:** apply only this declared effect: write and commit only the routed business worktree.

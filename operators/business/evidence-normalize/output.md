# `business/evidence-normalize` output
...
| `payload.produced` | Session artifact `evidencePackRef` plus explicitly approved durable effects only. |
...
| --- | --- | --- | --- |
| `ready` | `completed` | `business.model / ready` | business-evidence-ready |

`evidencePackRef`, evidence, receipts, observations, and output use `session://`. Only a product/worktree effect explicitly declared by `operator.json` may survive the skill.

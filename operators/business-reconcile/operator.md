...
this operator takes that matrix as it stands and asks, for every row, whether the delivered source enforces what the row promised. sourceRole selects the required receipt and pinned checkout: be consumes `backend-source-application`, fe consumes `frontend-source-application`, because a reconciliation with no delivered source is an opinion about code nobody read,
...
Documented heads retain their authored sections and regenerate every projection through the [business head contract](../../templates/business-head/README.md) when `applyHeadPublication` publishes the reconciled model and evidence. Historical models without documentation remain readable; reconciliation cannot discard documentation that the previous head carried.
...
| `@workspaces/be` | the routed backend checkout read at the frozen head; every fact claim cites it by path, line range and head | when sourceRole=be |
| `@workspaces/fe` | delivered frontend checkout at the frozen head | when sourceRole=fe |
...
| Kind | From | Required |
| --- | --- | --- |
| `backend-source-application` | `backend.generate`; the delivered source the reconciliation reads | when sourceRole=be |
| `frontend-source-application` | `interface.generate`; delivered frontend source | when sourceRole=fe |
...
| `sourceRole` | enum:be,fe | be | Delivered source role; the respective source receipt and pinned checkout are required. |
...
| --- | --- | --- | --- | --- | --- |
| 1 | Validate the gate, the delivered-source input and the resume | `resume`, `sourceRole` | `request/request.json`, the sourceRole-selected input `backend-source-application` or `frontend-source-application`, @workspaces/be or @workspaces/fe at the frozen head | — | `INVALID_INPUT`, `SOURCE_DRIFT`, `NO_PROGRESS` |
...
| 3 | Normalize the delivered source into fact claims, each with its path, line range and head | — | the sourceRole-selected input `backend-source-application` or `frontend-source-application`, @workspaces/be or @workspaces/fe at the frozen head, inputs `quality-verification` and `uat-flow-verification` as evidence only, @tools/git | `response/data/claims.json` | `EVIDENCE_MISSING` |
...
On `done`, print **Operator Result** as the promise-to-delivery reconciliation in `response/response.md`, with `response/data/claims.json` and `response/data/model.json` as secondary tables. A mismatch leads with the unresolved claim rows and their evidence instead of claiming the promise is implemented.

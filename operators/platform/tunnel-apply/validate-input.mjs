...
function semantic(value) {
  const errors = []; if (!value.facts.includes('platform-tunnel-plan-ready')) errors.push('$.facts: missing platform-tunnel-plan-ready');
  const provided = Object.values(value.payload.provided).sort(), artifacts = value.payload.loads.artifacts.map((item) => item.ref).sort(); if (JSON.stringify(provided) !== JSON.stringify(artifacts)) errors.push('$.payload.loads.artifacts: exact provided bindings required');
  ...
  const paths = source.targetFiles.map((item) => item.path.replaceAll('\\', '/')); if (source.repositoryContext !== false || source.loadMode !== 'exact-files') errors.push('$.payload.loads.source: broad context forbidden'); if (new Set(paths).size !== paths.length) errors.push('$.payload.loads.source.targetFiles: duplicate normalized path');

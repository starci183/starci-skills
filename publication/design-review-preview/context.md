...
Schema 3 layout regions carry hashed `placement`, `width`, `height`, `align` and an impressionistic child brief.
The preview honors those bounds and renders labeled representative content in a visibly rough wireframe even when a child is missing; it never flattens regions

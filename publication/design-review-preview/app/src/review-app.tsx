    ...
    <div className="review-app" style={themeStyle(layout)}>
      ...
      <main className="review-stage">
        <div className={`viewport-frame ${route.blockId ? "desktop" : viewport}`}>
          {route.blockId ? (
            region ? <BlockRenderer block={region.block} content={layout.content} back={() => navigate(layoutRoute(layout.layoutId, candidate.hash))} /> : <main className="fatal"><h1>Block not declared</h1><p>{route.blockId} is not a child of this layout version.</p></main>

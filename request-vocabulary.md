...
| `redesign` for a page | Does not determine the UX/UI change level alone. If the request authorizes structural rebuilding of an existing page, bind `reconstruct`; if layout may be locked, ask. When the direction is not already approved, render and apply one dominant realistic UI direction through `visualize`; render three or four choices only when valid Grammar evidence leaves material ambiguity or the user explicitly requests comparison. |
| `redesign` for a feature or `nhánh` | Does not determine the UX/UI change level alone. After `reconstruct` is authorized, rework the closed journey and applicable surface family; otherwise ask before widening beyond element-level work. Use one rendered journey/surface direction by default; alternatives require valid-Grammar material ambiguity or an explicit comparison request. |

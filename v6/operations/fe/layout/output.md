...
The operation recommends one direction but does not approve it. It returns `layout.review/pending` and the exact commands `OK LAYOUT <direction-id>`. No implementation or third creative checkpoint begins until one command is received.

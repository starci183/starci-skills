...
export function validateInput(value) {
  return validateDefinition(value, "input", (document) => {
    if (document.stage === "flow.generate" && document.status !== "ready") return ["$.status: flow.generate requires ready"];
    if (document.stage === "flow.review" && document.status !== "rejected") return ["$.status: flow.review re-entry requires rejected"];
    return [];
  });
}

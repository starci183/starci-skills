...
title: changelog.md
slug: /fe/patterns/the-split/changelog
...
Sáu mã `SPLIT-<n>` là **cố định**. Mã đã phát hành thì giữ nguyên số và nguyên nghĩa, vì nó đang được
...
- **Từ một file phẳng thành module năm record.** `fe/canon/patterns/the-split.md` được diễn đạt lại
  thành `fe/patterns/the-split/` với `INDEX.md`, `vi.md`, `example.md`, `audit.md`, `changelog.md`.
...
- **Giữ nguyên sáu mã.** `SPLIT-1` … `SPLIT-6` giữ đúng số và đúng nghĩa của bản phẳng: nửa vẽ không
...
- **Thêm bảng `Tầng giữ`.** Mỗi mã nói rõ nó đang được giữ ở tầng nào: `SPLIT-1` bởi
...
- **Thêm bảng `Anchor`.** Mỗi mã chỉ vào code thật kiểm được: đường dẫn, và **tìm gì ở đó**. Hai neo
  ...
  `className` — và điều đó được nói rõ là neo âm.
...
- **Ghi ra hai chỗ luật rộng hơn rule.** `presentational-purity` gác theo tên file, nên một request
  ...
  có request". Cả hai nằm ở `audit.md` phần *Rủi ro còn mở*, không được sửa lặng lẽ ở đây.
...
`Forbidden` và bốn cặp ví dụ. Mọi quyết định của nó được giữ nguyên ở `2.00`; chỗ nào bản này không

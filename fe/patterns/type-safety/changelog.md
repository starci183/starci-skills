...
title: changelog.md
slug: /fe/patterns/type-safety/changelog
...
đã có người viết ra. Mã sai thì giữ nguyên và ghi vào "Rủi ro còn mở" của `audit.md`.
...
- **Từ một file phẳng thành một module năm record.** `fe/canon/patterns/type-safety.md` được diễn đạt
  lại thành `fe/patterns/type-safety/` với `INDEX.md`, `vi.md`, `example.md`, `audit.md`,
...
- **Thêm `## Tầng giữ`.** Mỗi mã nói rõ ai đang giữ nó: `unrepresentable`, `enforced` hay
...
- **Sửa một neo đã lệch.** Bản phẳng nêu `src/components/pages/ProfileSkillsPage/component.test.tsx`
  ...
  `src/modules/api/graphql/clients/links/bearer.test.ts`, đúng chỗ ví dụ trong bản phẳng vốn đang
...
- **Rút mọi ví dụ về TS/TSX thuần.** Tên thư viện thật được thay bằng `@vendor/*`; component riêng
  ...
  của một sản phẩm mới đọc được là ví dụ đứng sai chỗ.
...
- **Ghi một bất đồng thay vì sửa luật.** `TYPE-SAFETY-3` là mã duy nhất không nói về việc tắt kiểm,
  nên có thể bị cho là đứng sai module. Mã **giữ nguyên**; lập luận hai chiều nằm ở "Rủi ro còn mở".
...
ấy được giữ nguyên ở `2.00` và nay được nói rõ hậu quả trong `## Tầng giữ`.

...
**Cách phát hiện.** Đường dẫn được chuẩn hoá về dấu gạch chéo xuôi rồi kiểm bằng chuỗi con và biểu
...
**Vì sao luật này đáng có máy giữ.** Vì chiều vào không ai tự soi được. Một người đọc diff nhìn thấy
...
**Cách phát hiện.** Cổng tên file khoá đúng một đường dẫn kết thúc bằng `ModalShell/index.tsx`. Mỗi
...
**Vì sao luật này đáng có máy giữ.** Vì đệm hai lần **không nhìn ra bằng mắt trong một ảnh chụp**.
...
**Cách phát hiện.** Cổng tên file khoá đường dẫn kết thúc bằng `Field/index.tsx`. Import phải có
...
**Vì sao luật này đáng có máy giữ.** Vì biến thể mặc định vẽ ra một **bề mặt trường thứ hai** bên
...
**Cách phát hiện.** Cùng cổng tên file với luật trên. Import có đường dẫn chuẩn hoá kết thúc bằng
...
**Vì sao luật này đáng có máy giữ.** Vì cái biểu tượng đó bao giờ cũng trông đẹp ở một trường. Nó chỉ
...
**Cách phát hiện.** Đường dẫn file phải chứa thư mục lớp phủ. Chỉ một visitor `ImportDeclaration`:
đường dẫn nguồn chuẩn hoá phải khớp **và kết thúc bằng** `/components/branches/<một trong bốn tên>`.

**Vì sao luật này đáng có máy giữ.** Vì thẻ lồng trong thẻ là sai kiểu **cộng dồn**: mỗi lần thêm chỉ
...
**Cách phát hiện.** Cổng tên file kết thúc bằng `TextLink/index.tsx`. Sự hiện diện của import: bất kỳ
...
**Vì sao luật này đáng có máy giữ.** Vì một liên kết tự vẽ **trông giống hệt** một liên kết thật ở
...
**Cách phát hiện.** Ba biểu thức tên file chọn ra ba vai. Phép kiểm mảnh giải phẫu trong vỏ đọc
...
**Vì sao luật này đáng có máy giữ.** Vì đây là chỗ hai thứ **khác loại** hay bị gộp: cơ chế mở/đóng
...
**Cách phát hiện.** Hai biểu thức tên file: file thành phần của lớp phủ, và file chỉ mục hợp đồng.
...
**Vì sao luật này đáng có máy giữ.** Vì hai vật chứa lồng nhau cho ra một dải đệm dọc thừa mà chỉ đo
...
**Cách phát hiện.** Cổng tên file kết thúc bằng `Checkbox/index.tsx`. Ba biến luận **toàn file**,
...
**Vì sao luật này đáng có máy giữ.** Vì kiểu hỏng này **qua được cả hai loại kiểm tra rẻ tiền**. Đặt
...
**Cách phát hiện.** Phạm vi rộng nhất trên kệ này: mọi đường dẫn chứa `/src/` và không phải file test.
...
**Vì sao luật này đáng có máy giữ.** Vì một `href` nội bộ **hoạt động được**. Nó bấm ra đúng trang.

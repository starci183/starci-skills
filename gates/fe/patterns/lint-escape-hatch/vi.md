...
`sources/fe/lint-escape-hatch.mjs`. Nó duyệt mọi comment trong một file sản phẩm bằng visitor
...
là điều kiện đường dẫn duy nhất trong file đó. Twin test ở `sources/fe/lint-escape-hatch.test.mjs` mang
...
**Nó sinh ra gì trong source.** `linterOptions` trong `sources/fe/lint-escape-hatch.mjs`, được freeze
và export ngay cạnh `rules` để hai thứ không thể bị gắn tách rời do sơ ý, rồi được re-export từ plugin
tổng `sources/fe/index.mjs` để một config tiêu thụ lấy chúng từ đúng cái import đã lấy rules. Ca thứ
...
và là một script — `sources/fe/lint-adoption.mjs` và `scripts/audit-fe-lint-adoption.mjs`, đọc ra từ
...
`sources/fe/lint-escape-hatch.mjs`, đóng rule lại trước mọi option nên không allowlist nào cấu hình
*vào trong* rule được, và `recommended` publish đúng một entry ở đúng một mức, không có key đường dẫn —
không còn field nào để viết một suất miễn trừ vào. Trong `sources/fe/index.mjs`, `recommended` được gom
...
không viết ra được), `enforced` (một lint rule từ `sources/fe/lint-escape-hatch.mjs` bắt được, nêu đích
...
|---|---|---|
| `LINT-ESCAPE-1` | `sources/fe/lint-escape-hatch.mjs` | `INLINE_DIRECTIVE`, neo ở đầu thân comment; visitor `Program()` đi qua `getAllComments()` thay vì match text nguồn; và `isProductSource`, điều kiện đường dẫn duy nhất trong file |
| `LINT-ESCAPE-2` | `sources/fe/lint-escape-hatch.mjs` | `linterOptions`, được freeze và export ngay cạnh `rules` để hai thứ không thể bị gắn tách rời do sơ ý. **Neo một phần** — xem dưới |
| `LINT-ESCAPE-3` | `sources/fe/lint-escape-hatch.mjs` | `schema: []` trong meta của rule, và `recommended` publish đúng một entry ở đúng một mức, không có key đường dẫn — không còn field nào để viết một suất miễn trừ vào. **Neo một phần** — xem dưới |

Bằng chứng phụ, dùng khi neo chính đang bị sửa:

- `LINT-ESCAPE-1` — `sources/fe/lint-escape-hatch.test.mjs`: các ca hợp lệ giữ cho prose nói về một
...
- `LINT-ESCAPE-2` — `sources/fe/index.mjs`: options được re-export từ plugin tổng, để một config tiêu
  thụ lấy chúng từ đúng cái import đã lấy rules.
- `LINT-ESCAPE-2` — `sources/fe/lint-adoption.mjs` và `scripts/audit-fe-lint-adoption.mjs`:
  `refusesInlineConfig`, đọc ra từ config đã in. Nó là thứ duy nhất đo một repo thật, và nó thuộc về
  một module khác.
- `LINT-ESCAPE-3` — `sources/fe/index.mjs`: `recommended` được gom từ mọi module mà không module nào có

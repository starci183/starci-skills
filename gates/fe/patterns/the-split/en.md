---
title: The-split
description: Turning an accepted surface into two files — a connected half that settles the situation and a drawing half that renders it.
module: the-split
kind: pattern
...
**Common business situations.** A statistics row on a rail; an order summary card; an invoice list; a
search-results frame; a progress table; a course card; a notification line.
...
**Common business situations.** Waiting for data; empty because nothing exists yet; empty because a
filter matched nothing; broken with a retry; expired; not yet entitled; completed.
...
**Common business situations.** The lifecycle of a request; empty-because-nothing-yet versus
empty-because-filtered; access by plan; an expired session; a day not yet arrived versus a day with
nothing in it; search results.
...
**Common business situations.** The label on a statistics row; an error message; the words on a retry
button; a plural string keyed to a count; an order-status label; a currency name; an "N days left"
label.
...
**Common business situations.** A one-line statistics block; a thin card; a button that opens an
overlay and carries a count; a list row with a badge; a widget shown only when there is data.
...
**Common business situations.** A rail composing three blocks; a shell splitting columns; a tab
container holding the selected tab; a wrapper opening an overlay; a layout that only takes children.

## Layer held

Which tier actually holds each code. `enforced` names the rule from `sources/fe/the-split.mjs` that
...
| `SPLIT-5` | `src/components/**/index.tsx` and `sources/fe/the-split.mjs` | `import { _X } from "./component"` where `X` is the folder name, and `_X` is the only JSX identifier the file renders. The rule's `connectedBlock` matcher fixes `X` from the folder, so the twin name is not a convention a file can restate differently |
...
The accepted shape: a progress rail arranges three blocks side by side, and one of them is a
course-progress block that reads the learner's progress and shows it as loading, empty, failed with a
retry, or a completed count.

The rail resolves first, because the outermost surface is asked `SPLIT-6` before any child is
touched.
...
The fact that excludes the adjacent code: `SPLIT-6` would keep this one file, but this surface reads
the world itself, and once there is a request there is a data half — so the split is real and
`SPLIT-5` turns on with it. The four situations cross as one `state` name rather than as `isLoading`,
`hasError` and `isEmpty`, which is what keeps `SPLIT-3` from breaking; the retry button's words cross
...
This rule holds for any front-end code of this kind in this stack. It names no product, no feature,
no component library, no registry key and no repository. Every example is ordinary TSX with ordinary
props.

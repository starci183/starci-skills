...
The path contains `/src/` and does not end in one of the four suffixes, so the gate passes and the
visitor is installed. Two offending outer nodes, two blocks.
...
Gathering the value into a constant did not launder the second one: the offending node travelled into
the constant's initialiser and was visited exactly as before.

**Repaired.** The erasure is replaced by a widening in the open and a check the compiler can follow,
and the seed row is built by a typed builder:
...
But the same file, repaired to look exactly like that and with the check deleted, is silent too:

...
The path is read before any visitor exists, so the path rule decides first; the tree is then walked for
the other two.
...
Both open hatches survive the repair, and neither silence is compliance:
...
verdict: silent
hatch: destructured props — the declarator's id is an ObjectPattern and the guard requires Identifier, which is where handler names most often arrive
```
...
node: Program
verdict: silent
hatch: a route declared as a string — the rule reads a FILENAME, so the public address in a redirect map is in a second language with no file to point at
```

## Scope

This module documents only rules that exist in the source file that publishes them. A rule that ought to
exist and does not is not documented here — it is a finding for review, and review remains the
enforcement for everything the machines omit: the ARROW half of `NAMING-1`, the ban on a name that says
where a thing is used, and every romanised segment outside the list. A rule that cannot be pointed at is
a proposal, not a rule. This module judges names and paths only; it judges no file's structure, no
component's shape and no product decision.

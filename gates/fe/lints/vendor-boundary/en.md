...
`sources/fe/vendor-boundary.mjs`. It deliberately does not measure design by regex: the contract stays
...
`sources/fe/vendor-boundary.mjs` and is not published by this gate.
...
| implementation | The exact node predicates live in `sources/fe/vendor-boundary.mjs` and are not restated by this gate |
...
|---|---|
| filename | The path as the rule sees it, and which owner tier it resolves to |
| scope decision | Which path, named file or governed component matched, or that none did |
| import sources | The source string per import, and whether it is `@heroui/react` |
| import specifiers | The named specifiers, for the SurfaceCard and vendor checks |
| JSX tag names | The named tags read by the anatomy rules, and their nesting order |
| class literals | The class attribute as written, for `Modal.Body className="p-0"` |
| severity | The severity the repository actually adopted for each rule |
...
9. The twin test `node --test sources/fe/vendor-boundary.test.mjs` is the proof that the rules still
   behave as published.
...
**Input.** Two files this module rejects:
...
Repaired, the dropdown mechanics move to their named owner and the deleted tier is gone:
...
But an open hatch survives the repair. Re-export the vendor once and the block imports it again with
nothing reported:
...
This module documents enforcement, not law. The product law it serves is prose in
`../../patterns/vendor-boundary/INDEX.md` and is owned there; the shape of a surface is owned by the
contract, not by any check here. Rule names, import sources, path literals and class literals ship in
build output and are reproduced verbatim; everything written around them is ordinary markup and
ordinary imports.

...
**Input.** `/repo/src/checkout/total.tsx`:
...
The path contains `/src/`, so the gate opens and the `Program` handler reads every attached comment.
...
The trailing reason buys nothing: `\b` closes the match at `eslint-disable-next-line` and the text
after it is never consulted. The second comment is a severity comment, not a `disable`/`enable`
directive, so nothing is reported on it at all.

**Repaired.** The dependency array is corrected and the legacy import is replaced, so the first
comment is deleted. The second comment stays exactly as written, and the file is now silent:
...
This module documents enforcement of one law, in one rule set, shipped as one package. It does not
judge whether the underlying rule being disabled was right, nor whether the flat configuration was
wired — the first belongs to the module that owns that rule, the second to the repository's lint
adoption gate. The prose and every example name no product, no component library and no repository.
Rule identifiers are quoted verbatim, because the identifier is what a build log prints and a second
spelling would mean one rule with two names.

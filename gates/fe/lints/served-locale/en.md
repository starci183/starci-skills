...
**Repaired.** The chain attaches the locale link, and the header moves back to the one file that owns
it:
...
Both rules go green. An open hatch survives the repair — the header rule never asserts the string is
...
And a second one survives the same repair — the chain rule saw a name, not a chain:
...
that ship, and they are quoted verbatim because a renamed identifier is a different rule. Whether the
locale value is read from the address, whether a cookie carries it across an origin, and whether the
server default is a floor or a fallback are `LOCALE-2`, `LOCALE-3` and `LOCALE-4`, owned by the law and
by human review — no module here judges them.

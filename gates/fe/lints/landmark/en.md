...
**Input.** A dashboard layout that composes chrome and hands the routed page to a plain leaf, plus the
block that draws the reading column beside it.
...
`dashboard-main` is a reading column beside the rail, not the page. Repaired, the column stays a
neutral frame and the layout wraps the routed children in the landmark:
...
The same failure survives one move. Push the chrome into a shell and both rules go quiet:
...
This module documents the rules of one law and nothing else. It names no product, no component library
and no repository. Rule names, the element names they compare against and the package they ship in are
identifiers that appear in a build log, and they are written verbatim. Whether a document ends up with
exactly one landmark, whether each landmark element has one branch, whether a branch owns a class and
whether the neutral frame accepts an element-choosing prop are held by review, not by this module.

...
**Input.** A resolver class, one method, ordinary folder:
...
The method is a `MethodDefinition`. Its second parameter carries `CurrentUser`, which is in the closed
set, so the method is a candidate. No decorator on the method is named `UseGuards`, and the class node
reached through `node.parent.parent` carries only `Resolver`. It reports.
...
**Repaired.** The door carries the guard that establishes the identity it reads:
...
But the same failure survives one ordinary rewrite. Drop the guard again and take the identity out of
the context instead:
...
library. The rule name, the plugin namespace it ships under, and the decorator identifiers the rule
matches as literal strings are things that appear in build output and in the rule's own source, so they
are reproduced verbatim; that is the one exemption, and it does not extend to prose. What the law means
by ownership, leak-shaped refusal, entitlement and subject belongs to the law, and to the human who
reads `AUTHZ-1`, `AUTHZ-3`, `AUTHZ-4`, `AUTHZ-5` and `AUTHZ-6` because no machine here does.

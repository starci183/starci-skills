...
|---|---|
| `context.filename` | The path as the rule sees it, backslashes normalized to `/`, then suffix-matched against `/event/nats/nats-bridge.service.ts`. Falls back to `context.getFilename()` when falsy |
| gate decision | Whether the suffix matched, or that an empty visitor was returned |
| `sourceCode.getText()` | The complete file text, as one string, for all three offsets. Falls back to `context.getSourceCode()` |
| `originIndex` | The offset of the regex match, or absent |
| `digestIndex` | The offset of the `parsed.digest` substring, or absent |
| `emitIndex` | The offset of the FIRST `this.eventEmitter.emit`, or absent |
| provenance | Whether each matched token is a live statement, a comment, a string or dead code — and the fact that the rule cannot tell |
...
The path matches the gate, so the rule runs. Both tokens exist, and both sit after the emitter call.
...
**Repaired.** The guard drops the self-origin envelope and the digest is claimed before the fan-out:
...
The rule is now silent — and silence is not compliance. Add one retry path below and the module still
says nothing:
...
The same silence is reachable without a repair at all: turn the guard's body into `{ }`, or move the
comparison into a comment at the top of the file, and every offset stays in order while every
self-echo is emitted.

## Scope

This module documents one rule of one back-end law. It names no product, no company and no repository.
The rule name, its message identifiers, the plugin prefix in the `recommended` map and the identifiers
the regex matches are **identifiers that ship** and are reproduced verbatim; that exemption covers
nothing else. Whether the envelope carries producer identity and digest at all, whether `useLocal` and
`useNats` are declared per event, what a consumer asserts, and whether cross-instance behaviour was
proved with two real instances are owned by no rule here.

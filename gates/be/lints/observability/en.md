...
**Input.** One request-scoped service, outside every exempt folder:
...
**Repaired.** The framework logger is gone and the name is an enum member with its data beside it:
...
Both rules are silent. But an open hatch survives the repair, and the silence is not compliance:
...
company. The `console` exit is owned by the standard `no-console`, not by anything here. The path
exemption for standalone programs is owned by `standaloneProgramGlobs` in the consuming config, not by
a rule. `OBSERVABILITY-3`, `OBSERVABILITY-4`, `OBSERVABILITY-5`, `OBSERVABILITY-7` and
`OBSERVABILITY-8` are owned by human review. Rule names, the plugin namespace, the matched package
string and the matched receiver identifier are reproduced verbatim, because they are the exact strings
a build log prints and a reader must match against; that is the one exemption, and it does not extend
to prose.

...
**Repaired.** The enum drops `const`, the parameter shape becomes a named type in the module's types
folder, and the row is narrowed instead of overruled:
...
But two open hatches survive the same repair, and a silent run is not proof of either. Written this
way instead, nothing reports:
...
discriminated union, which nothing judges.

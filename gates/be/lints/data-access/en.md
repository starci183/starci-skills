...
**Input.** One service and one entity, both of which this module rejects:
...
One report for `items`, not two, although both branches match: the decorator name contains
`InjectRepository` and the type-reference identifier is `Repository`.
...
**Repaired.** The manager names its datasource, the repository is gone, the table is named:
...
All three verdicts now read `silent`. One of them is compliance. Two of them are not:
...
The repaired file is greener than it is correct. `DATA-1` still does not know which database this
service writes to, and `DATA-2` still has a repository in the code — the machine merely has no node to
put a cursor on. That is what these two blocks are for.

## Scope

This module documents only rules that exist in the source file that publishes them. A rule that ought
to exist and does not is not documented here — a rule that cannot be pointed at is a proposal, not a
rule. It judges neither `DATA-4` nor `DATA-5`; both remain with the person reading the change, which
is where the source deliberately left them. Whether a table name is the RIGHT table name, whether a
datasource exists, and whether a query asks for the right relations are all outside every rule here.
Rule names, regexes, decorator names and node types are identifiers that ship in build output and are
reproduced verbatim.

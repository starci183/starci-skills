...
|---|---|
| Source AST | The five declaration node types and their `id`; `MethodDefinition.key` |
| Export nodes | `ExportNamedDeclaration.declaration` and, through it, one `FunctionDeclaration` id or a `VariableDeclaration`'s declarator ids |
...
**Input.** One service file, `content-v2.service.ts`:
...
The file name itself, `content-v2.service.ts`, is not a finding: no rule reads it, and `NAME-1` has no
rule. Both reports are `warn`, so the build is green while both are on screen.

**Repaired.** The method says what the shape is instead of which generation it came from, and the
export says what it generates:
...
The third export was never repaired, and the file now reports nothing:
...
the source is not documented here; it is an open risk. Rule names are reproduced verbatim because the
name is the identity — it is what a build log prints, what a disable comment carries, and what every
conversation about the failure uses. Whether a name takes its meaning from a folder, a mechanism or its
first caller belongs to `NAME-1`, `NAME-3`, `NAME-4`, `NAME-6` and `NAME-7`, which are held by review
and by no machine. Prose and examples name no product.

...
Which tier actually holds each code. `unrepresentable` means a closed union or branded type makes the wrong value impossible to write; `enforced` means a lint rule published by `sources/be/type-safety.mjs` catches it; `documented` means nothing mechanical holds it and only a reader does.

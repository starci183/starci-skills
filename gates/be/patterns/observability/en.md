...
**Common business situations.** Payment handler · background job worker · data-cleanup cron · message-queue listener · interceptor · guard · third-party integration service.
...
**Common business situations.** Order created · order cancelled · subscription renewed · webhook received · retry attempt n · job finished · job died · sync skipped as duplicate.
...
**Common business situations.** `orderId` + `amount` + `currency` · `jobId` + `queueName` + `durationMs` · `tenantId` + `count` · `attempt` + `maxAttempts` · `outcome` + `reason`.
...
**Common business situations.** Granting a trial because the user never purchased · skipping record creation because it already exists · choosing the fallback provider because the primary is out of quota · rejecting a webhook on a bad signature · stopping retries at the threshold.
...
**Common business situations.** Job died · third-party API call failed · database constraint violation · timeout · quota exhausted · invalid payload.
...
**Common business situations.** Admin CLI · agent running in a sandbox · hand-run migration script · build tool · bootstrap before DI.
...
**Common business situations.** Adding distributed tracing · adding custom metrics · adding continuous profiling · adding a second dashboard set · raising retention.
...
**Common business situations.** Standing up a local collector · adding a TSDB · adding a dashboard service · a sidecar log shipper · a separate trace store.

## Layer held

Which tier actually holds each code today. `unrepresentable` means a closed union or branded type makes the wrong value impossible to write; `enforced` means a rule in `sources/be/observability.mjs` reports it, named below; `documented` means nothing mechanical holds it and only a reader does.
...
**The accepted shape:** a background worker runs one AI CV-generation job and reports how it ended.
...
**What the shape does not state, and therefore does not resolve.** It does not say which enum member names the failure, whether that member already exists in the closed set, or which typed message interface carries its fields. It does not say what the exception's `code` is, so the grouping key stays unresolved until that evidence is supplied. And it says nothing about the pipeline: whether this line crosses to the forwarding backend is a `OBSERVABILITY-7` decision, and no new telemetry process is proposed here, so `OBSERVABILITY-8` has nothing to weigh. At the anchor today `JobExecutedMessage.error?: string` is filled with `error.message` — the law being missed, held by a reader and by nothing else, because a parser cannot tell `error.code` from `error.message` once either sits in a field named `error`.

## Scope

This module states a rule true of any back end that logs. It names no product, no repository and no private module. Every example is ordinary TypeScript in an ordinary NestJS shape; the anchors are paths, and a repository that stores its house service elsewhere reads them as a description of what to look for, not as an import path.

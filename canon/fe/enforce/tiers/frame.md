...
**FRAME-5 · It composes classes — for arrangement, and for chrome it draws itself. Never appearance.**

The earlier wording of this rule said *"deciding appearance is exactly this tier's work"*, and that
was wrong. It contradicts FRAME-1 in the same document: a frame owns direction, seam, alignment and
its own chrome — appearance is the atom's word, exposed as `tone`, `size`, `variant`.

Measured on a tier that follows the rest of these rules: of nine frames, **eight touch no appearance
token at all**. Every class they compose is a lookup — `gap="related"` becomes a gap class,
`align="center"` becomes an alignment class — plus the caller's `classNames` merged in. That is not
deciding how something looks. It is turning a named scale into the only vocabulary CSS understands.

**Why the answer is still yes.** Flip it to "no" and the tier cannot exist: `gap="related"` has
nowhere to become `gap-2`, so the frame would have to be handed a finished class string — which is
the exact door FRAME-4 closes. A frame must compose. The rule is about **what** it composes.

**The one appearance a frame may own is chrome it drew itself.** The ninth frame is the exception
that proves it: `Stack`'s `nested` draws a rail — `border-l border-default pl-3` — a border and a
colour token, genuine appearance. It is legal because FRAME-1 grants a frame its own chrome, and
because no atom exists for a rail down the left of a group. By FRAME-8 that is a missing atom, and
the day it exists this class string leaves.

**What this forbids.** An ad-hoc conditional that is not a lookup from a named prop —
`cn(isCompact && "text-sm")` inside a frame is the frame deciding how its children read, which it
cannot know anything about (FRAME-2). If a shape needs it, the prop is missing or the tier is wrong.

> **Test:** could this class have been chosen without knowing what is inside the frame? Gap,
> direction, alignment, and the frame's own rail — yes. A text size, a colour, a radius on the
> content — no, and that is the line.

**A note on the word.** `p-*` and `gap-*` sit in the appearance list that ATOM-5 uses to judge call
sites, because a caller must not push them onto an atom — yet a frame composes exactly those. The
same token is arrangement when the frame that owns the seam writes it, and appearance when a caller
reaches past the scale to write it by hand. The list is about **who is writing it**, not about the
class.
...
**FRAME-12 · The app's copy is the same file with the inspection tooling removed, and a change lands in both.**

The same rule as [`atom.md`](atom.md) ATOM-11, one tier up, and it applies here for the same reason:
`.storybook/components/frames` is the blueprint, `src/components/frames` is the copy the app
imports, and nothing binds them but this.

The permitted differences are identical — the copy carries no `showAnatomy`/`anatPart`/
`data-anat-part`, its entry file is `index.tsx`, and its imports reach through the app's alias
rather than the blueprint's. Everything else matches, in both directions: no frame the blueprint
has that the copy lacks, and none the copy has that the blueprint never saw.

The stake is higher here than at the atom tier, because a frame is where a seam lives. Two copies of
`Stack` that have drifted put two different rhythms on two screens, and both look deliberate.

> **Test:** the same one — strip the overlay lines from the blueprint file and ask whether what
> remains is the app's file.

...
| lives in | two trees — the blueprint, and the app's copy of it minus the anatomy overlay |
...
**ATOM-3 · It may import: nothing from this system. The vendor, and only the vendor.**

An atom is the floor. There is nothing below it, so imports have nowhere to run — the only thing an
atom pulls in is the component library it wraps ([`atom.md`](atom.md) ATOM-9), which is not a tier,
it is the raw material the atom exists to constrain.

**Importing another house atom is the line, and it is not softened by the two being the same tier.**
A component that renders `Typography`, or `Button`, or `Avatar` is assembling the vocabulary rather
than being a word in it — that is a composite, whatever its folder says. The moment an atom imports
`X`, it can no longer be used anywhere `X` is absent, and it has stopped being the indivisible thing
the tier is defined as.

This was written the other way for one edit, allowing same-tier imports, and it was wrong: it turned
"one element" into "one element, plus whatever atoms it composes", which is the composite tier with a
smaller name. The five components that do it today — `PricePoint`, `UserCell`, `ImageDropzone`
rendering through `Typography`; `Alert` rendering a `Button`; `Toast` wrapping `Alert` — are **five
composites in the atom folder**, and they move.

**Rendering raw text is not importing an atom.** A `<span>{label}</span>` with a truncate class is
one element; `<Typography text={label} />` is one element built out of another. The first stays an
atom, the second is a composite. So a `Button` that needs its label to ellipsis applies the class
itself — it does not reach for `Typography` to get it.

> **Test:** delete every `import` from this file except the vendor. Does it still render? An atom
> does. A composite loses the pieces it was assembling.
...
**ATOM-11 · The app's copy is the same file with the inspection tooling removed, and a change lands in both.**

A system that keeps a blueprint the design work happens in and a copy the app imports has two
trees holding the same tier:

```
.storybook/components/atoms   the blueprint — carries the anatomy overlay
src/components/atoms          the copy the app imports — carries none
```

They are the same file. Same folders, same props, same states, same skeletons, same vendor line.
Exactly **three** differences are permitted, and all three are structural rather than editorial:

**The inspection tooling** — `showAnatomy`, `anatPart`, and the `data-anat-part` attribute — exists
to feed a panel that only Storybook has, so the copy carries none of it. Strip those lines from the
blueprint and what is left must be the copy, character for character.

**The entry file is named `index.tsx` in the copy.** The blueprint names a component's own file
after the component — `Button/Button.tsx` — because Storybook's sidebar and the anatomy panel are
read by a person scanning file names, and forty files called `index.tsx` are not scannable. The app
imports folders, so there the same file is `Button/index.tsx` and a call site reads
`from "@/components/atoms/buttons/Button"` with no name repeated in the path. Sibling files keep
their names on both sides: `ButtonBase.tsx` is `ButtonBase.tsx` in either tree, because it is not
the entry.

```
.storybook/components/atoms/buttons/Button/Button.tsx      ↔  src/…/Button/index.tsx
.storybook/components/atoms/buttons/Button/ButtonBase.tsx  ↔  src/…/Button/ButtonBase.tsx
.storybook/components/atoms/buttons/Button/button-tokens.ts ↔ src/…/Button/button-tokens.ts
```

**The import root differs, because the two trees sit under different aliases.** The blueprint reaches
its siblings through `@sb-components/…`; the copy reaches its own through `@/components/…`. That is
the same import naming the same file, written from two places — not a decision anybody made
differently. Everything after the root is identical, and a sibling reached by a relative path
(`./ButtonBase`) is identical outright.

This one is easy to mistake for drift and was, on the first run of the gate: it fires on every file
in the tier at once, which is the signature of a rule mis-stated rather than a tree gone wrong. When
a check fails everywhere, suspect the check.

Nothing else. A prop that exists on one side only, a state handled on one side only, a fix applied
on one side only — each is drift, and each is what this rule is for.

**The component set is identical, and it is identical in both directions.** Not "the copy has at
least what the blueprint has" — *the same*, with nothing extra and nothing missing. An atom the
blueprint does not have is an atom nobody designed, nobody has a story for, and no gate reading the
blueprint will ever see. An atom the copy does not have is a word the app cannot say, so the screen
that needed it hand-rolled something instead.

Both failures look identical from inside the tree that has the file — everything compiles, every
gate pointed at that side is green — which is why the count has to be taken across the pair and in
both directions. The four `*Group` files were caught exactly this way: present in the copy, gone
from the blueprint, and invisible to every check that only ever looked at one tree.

**Two copies with no rule binding them are not two copies for long.** On the first day these
existed, four `*Group` files the blueprint had already promoted to composites were still sitting at
atom tier in the app copy — the tier's clearest violations, fixed on one side and shipping on the
other. Nothing was wrong with either commit. What was missing was the rule saying they travel
together.

So a change to an atom lands in **both trees in the same change**. Editing the story side alone
leaves the app on the old shape while every gate reads green, because the gates were pointed at the
blueprint. Editing the app side alone leaves the blueprint describing something that no longer
ships, and the next person designs against a drawing of the past.

Neither tree is the source of truth. **The pair is** — and a change that touches one is unfinished
until it touches the other.

> **Test:** delete every line mentioning `showAnatomy`, `anatPart` or `data-anat-part` from the

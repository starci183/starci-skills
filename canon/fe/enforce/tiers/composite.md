...
**ActionBar — COMPOSITE-8 at N slots, in one line.** A row of mixed controls (a search input, then an
icon-only send button) is not `children`, and not an array of built elements. It is DATA describing
each slot's role and the COMPONENT that fills it:

```tsx
<ActionBar
    isSkeleton={isSkeleton}
    items={[
        { role: "input",  component: Input },
        { role: "action", component: Button, variant: "iconOnly" },
    ]}
/>
```

ActionBar renders each slot itself — `<slot.component {...rest} isSkeleton={isSkeleton} />` — so it
owns the arrangement (the row's gap, the stack-below width) and threads `isSkeleton` into every slot
from one place. Two screens that build an action bar this way land on the same rhythm, because neither
one drew the row; ActionBar did. The caller says WHICH components fill WHICH roles; the composite says
how they look and when they shimmer. The moment a slot's `component` is instead a built `<Button />`,
all of that is lost — the element is frozen (COMPOSITE-8), and the row's consistency leaks back out to
every call site.

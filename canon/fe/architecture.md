...
`design/storybook/references/tier-boundaries.md`, and every reason there came from something that
actually broke.
...
misplaced* column for each tier — is `design/storybook/data/tiers.csv`, and each tier is written out
in full in `design/storybook/architecture/elements/<tier>.md` with worked examples beside it in
`design/storybook/architecture/examples/<tier>.md`.
...
reason attached to each, is `design/storybook/data/import-rules.csv`.
...
`design/storybook/architecture/principles/position.md` holds the members. Everything else about how a
...
treatment is `design/storybook/architecture/split.md`.
...
is answered here. The component question is a lookup, and it lives in `design/component/` — the
matrix in `design/component/data/matrix.csv`, its fifteen sections and their deciding tests in
`design/component/data/sections.csv`, the reasoning in
`design/component/references/general-rules.md`, and the section-by-section traps in
`design/component/references/traps.md`.
...
`design/storybook/architecture/story.md`, and how a story file is spelled is
`canon/fe/authoring/storybook-stories.md`. Coverage is not left to discipline —
...
`canon/fe/testing.md`. The source-side gates live in `patterns/fe/gates/`, and

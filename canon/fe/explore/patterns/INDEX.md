...
| [`kitchen-sink-theme-check-not-component-source.md`](kitchen-sink-theme-check-not-component-source.md) | that the app's own components live only in the one storybook, and an in-app "kitchen sink" is a dev-only page that renders the raw vendor primitives to check the theme in the real runtime — never a second place components live |
...
| [`landing-and-app-split-across-domains.md`](landing-and-app-split-across-domains.md) | that the marketing landing lives at the apex domain and the product app at the `app.` subdomain (the code split being one monorepo, not two repos), with the exception that a product may instead keep one domain and let middleware route landing vs app by the visitor's situation |
| [`landing-generation-with-design-intelligence.md`](landing-generation-with-design-intelligence.md) | that a landing is generated by letting a design-intelligence database brief the plan while the real-data marketing rules and the build gates stay in force — the oracle writes the brief, this system builds and proves it |

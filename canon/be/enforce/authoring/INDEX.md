...
| File | Decides |
|---|---|
| [`naming-and-structure.md`](naming-and-structure.md) | that a top-level folder is a capability rather than a layer, the split between what is reusable and what wires it to a transport, that every import names the declaring file (a barrel is a bug; file paths are public API), that same-capability imports stay relative, that a module does not import another in-repo module (globals live at the app root), colocation by default with promotion on the second consumer, one operation per folder behind an entry-point method with a fixed name, the suffix that names a file's role, and the expand-and-contract rule for renaming something that has already spread |
...
| [`config-and-env.md`](config-and-env.md) | that `process.env` is read in exactly one place — the `parseEnv*` helpers in `src/modules/platform/env/utils/parse-env.ts` — so no service or provider touches the environment directly and loses its typing and defaults, and that secrets live behind that same boundary rather than being scattered across the codebase |
| [`imports-and-format.md`](imports-and-format.md) | that a `.ts` file under `src/**` is formatted the way `eslint.config.mjs` dictates — four-space indent, double quotes, no semicolons, one array element and one argument per line — that imports go through the `@modules` / `@features` aliases to the declaring file (never a folder barrel) in a fixed order, and that eslint, not the root `.prettierrc` (which governs only `apps/**` and `libs/**`), is the single authority `npm run lint` must pass clean |

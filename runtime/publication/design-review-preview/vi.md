...
|---|---|---|---|
| `@manifest-schema` | `runtime/publication/design-review-preview/schema.json` | file | kiểm tra review graph chỉ sống trong phiên |
| `@render-design-review` | `scripts/render-design-review.mjs` | script | ghi static HTML preview dưới session root, không build app |
| `@baseline-schema` | `knowledge/brainstorms/composition/schema.json` | file | bind reference bốn lock và owner tree |
| `@visual-proof-schema` | `runtime/publication/design-review-preview/visual-proof.schema.json` | file | prove same-viewport parity và delivery completion |

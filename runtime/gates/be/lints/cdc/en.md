...
The path ends in `projection.listener.ts` and is not the excluded base, so the rule exists here. One
class declaration, three checks, no early return — six findings in one pass.
...
The same block repeats for `topics` under `CDC-2`, and for `deriveTargets` and `recomputeTarget` under
`CDC-3` — one report per missing name, all placed at the class id.
...
**Repaired.** The class extends the shared base, declares the four members and owns no lifecycle hook:
...
Every check now passes, and the module is obliged to say what that silence is worth:
...
verdict: silent
report: none
hatch: the listener-contract rule reads only names; focused rules also refuse dynamic group ids and delta/non-upsert recompute shapes, while other semantic equivalents can still escape
```

The repaired file is compliant with everything the machine holds and still breaks `CDC-2` and `CDC-4`.
Silence here is the boundary of a name, not a certificate.

## Scope

This module documents enforcement, not product. It names no product, no repository and no component
library. Rule names, message ids, code tokens and the plugin namespace they ship under are identifiers
that appear in build output and are reproduced verbatim; that is the one exemption, and it does not
extend to prose. Whether a consumer group is stable, whether a topic list is complete, whether a
recompute rebuilds, whether a tombstone is skipped, whether a failure is isolated and whether delivery
was proved against a broker are `CDC-2`, `CDC-4`, `CDC-5`, `CDC-6` and `CDC-7` — owned by the law in
words and by no machine at all.

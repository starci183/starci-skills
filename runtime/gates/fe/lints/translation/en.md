...
The path contains `/src/components/leaves/`, so the gate opens and both rules install their visitors.
...
`aria-label="Close"` is never reported. It is a single word, so the whitespace half of the predicate
fails — a reader in another language still sees `Close`.

**Repaired.** The words are resolved by the connected half and arrive already decided:
...
But an open hatch survives the repair. A sibling leaf written in the house prop-bag shape reports
nothing at all:
...
This module documents enforcement only, not the law itself. It names no product, no component library
and no repository; the only proper nouns are published identifiers — rule names, the package they ship
in, and the strings the rules match on. A key crossing the line (`COPY-3`), a resolved string breaking
the data fence (`COPY-4`), the dictionary as source (`COPY-5`) and a matched-on value treated as copy
(`COPY-6`) belong to the pattern module that states the law; no machine here judges them.

...
The path contains `/src/`, so the gate opens and every rule runs.
...
That is the only block. The `p-1.5` on the next line is a fractional step in a named family and is
still not reported: it sits inside `cn(...)`, a `CallExpression` the reader has no case for, so the
whole string yields `null`. The verdict says the file has one finding; it does not say the file is
clean.

Repaired, the declarator moves onto the scale and the rule falls silent:

```tsx
const GLUE = "inline-flex gap-2"
```

The `cn(...)` call is unchanged and still unread. Clearing this shelf is not the same as clearing the
law — `TOKEN-3` is enforced here only where the text is static.

## Scope

This module documents enforcement only. It names no product and no repository; the rule identifiers
are shipped strings and are reproduced verbatim. What a token IS, and which value belongs on which
scale, is the tokens pattern's decision, not this file's.

...
|---|---|---|---|
| `@composition` | `knowledge/brainstorms/composition/context.md` | context | bind Scope, Owner, Invariant and Proof before visual decisions |
| `@directions` | `knowledge/brainstorms/directions/context.md` | context | preserve MASTER bypass and visual-direction limits |
| `@layouts` | `knowledge/brainstorms/layouts/context.md` | context | bind page synthesis, states, source ownership and staged approvals |
| `@blocks` | `knowledge/brainstorms/blocks/context.md` | context | bind block state-first anatomy and complete-parent proof |
| `@frontend-quality` | `knowledge/brainstorms/frontend-quality/context.md` | context | bind integrated frontend review and advisory-source boundary |
...
| `page-synthesis` | author page map, select one reusable archetype per page after isolated evidence and before Grammar, classify current layout as `conform`, `layout-drift` or `needs-evidence`, author journey direction, business obligations, merge bindings, UI direction, candidate anatomy and canonical schema-9 `pageContract`; own all ten lens decisions, character signature and eligibility; recommend the candidate | run isolated top-down/capability audits; collect bounded quality evidence and six detector-family results; only after coordinator freezes eligible JSON, generate complete wide/intermediate/compact HTML, interactions and captures | archetype receipts, current-layout conformance verdicts, direction receipt, quality review, synthesis matrix, pages artifact, preview HTML and maturity evidence | independent receipts pass before archetype selection and join; current implementation proves facts/capability rather than layout correctness; binding evidence outranks advisory evidence; HTML projects frozen eligible JSON; Sol/Opus presents `OK #1` |
...
| `parity` | decide parent fit and final parity | run runtime/gates/browser interactions and capture complete-page responsive proof | `visual-proof.json` and final state views | zero known mismatch; isolated block crops never substitute |
...
| `parity` | reproduce mismatch closure and issue final verdict | execute existing seeds, run runtime/gates/browser behavior and capture ensured states | `visual-proof.json` and at most five final complete-page views | zero known mismatch and no unrendered consumer left in impact cone |

...
def records_from_sources(files: Sequence[SourceFile], source_root: Path) -> list[dict[str, Any]]:
    ...
    for source in files:
        ...
        if len(parts) >= 4 and parts[1:3] == ("knowledge", "archetypes"):
            records.append(archetype_record(source))
        elif len(parts) >= 5 and parts[1:4] == ("knowledge", "compilers", "principles"):
            records.append(principle_record(source))
        elif len(parts) >= 4 and parts[1:3] == ("knowledge", "grammars"):
            grammar_id = parts[3]
...
def build_index(source_root: Path, index_path: Path, embedding_model: Path | None = None) -> dict[str, Any]:
    ...
    index = {
        ...
        "sourceAuthority": [
            ".claude/knowledge/archetypes/**/context.md",
            ".claude/knowledge/grammars/*/{context.md,grammar.json,facts.json,capsules.json,rulings.json,design-system.json,profiles/*.json,cases/*.json}",
            ".claude/knowledge/compilers/principles/*/context.md",
            ".claude/v6/knowledge/*.md",
        ],
        "embedding": provider,

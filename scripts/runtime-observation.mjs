...
async function probe(url){const startedAt=new Date().toISOString();const response=await fetch(url,{method:'GET',redirect:'manual',signal:AbortSignal.timeout(10000)});const body=Buffer.from(await response.arrayBuffer());return{url,method:'GET',status:response.status,bodySha256:sha(body),startedAt,endedAt:new Date().toISOString()};}

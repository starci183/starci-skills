...
test('Lite is opt-in, ships beside all full operators, and persists through update',()=>fixture(repo=>{
 const first=init({dir:repo,bootstrap:true,profile:'lite'},quiet);assert.equal(first.profile,'lite');assert.equal(first.bootstrapProfile,'lite');
...
test('explicit profile transitions replace only known generated shell and preserve custom CRLF suffix',()=>fixture(repo=>{
 const full=init({dir:repo,bootstrap:true},quiet);assert.equal(full.profile,'full');assert.match(text(repo,'AGENTS.md'),/For every user prompt, enter \[StarCi\]/);
 ...
 const stateFile=path.join(repo,'.worktrees/sessions/existing/state.json');mkdirSync(path.dirname(stateFile),{recursive:true});const state='{"runtimeRevision":3,"lifecycle":{"phase":"active"},"proof":"unchanged"}';writeFileSync(stateFile,state);
...
test('no-bootstrap records selected profile separately without claiming changed host routing',()=>fixture(repo=>{
 init({dir:repo,bootstrap:true},quiet);const before=text(repo,'AGENTS.md');const changed=update({dir:repo,profile:'lite',bootstrap:false},quiet);assert.equal(changed.profile,'lite');assert.equal(changed.bootstrapProfile,'full');assert.equal(text(repo,'AGENTS.md'),before);
}));
...
test('custom full-only suffix on a generated bootstrap refuses Lite for LF and CRLF hosts',()=>fixture(repo=>{
 init({dir:repo,bootstrap:true},quiet);const generated=text(repo,'AGENTS.md');

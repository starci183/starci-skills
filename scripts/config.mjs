...
/** `supervisor`, `validator` and `critique` share one shape: `{runtimes: [id, ...]}`, a nonempty list of runtime ids and nothing else. */
...
export function validateConfig(config) {
  if (!config || Array.isArray(config) || Object.keys(config).some(key => !['language','model','effort','supervisor','validator','critique'].includes(key)) || !runtimesOk(config.supervisor) || !runtimesOk(config.validator) || !runtimesOk(config.critique) || typeof config.language !== 'string' || !/^[a-z]{2,3}(?:-[A-Za-z0-9]+)*$/.test(config.language) || !(config.model === null || typeof config.model === 'string' && config.model.trim()) || !['none','minimal','low','medium','high','xhigh','max','ultra'].includes(config.effort)) throw Error('Invalid config.json: expected language, model (null or host model name), effort, and optionally supervisor.runtimes / validator.runtimes / critique.runtimes (nonempty lists of runtime ids).');
  return config;
}

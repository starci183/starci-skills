...
const DISPATCH_MODES = new Set(Object.keys(orchestrator.dispatchModes ?? {}));
if (!DISPATCH_MODES.has('inline') || !DISPATCH_MODES.has('fresh')) errors.push('orchestrator.json: dispatchModes must declare inline and fresh');
...
for (const [id, resources] of operators) {
  ...
  if (!DISPATCH_MODES.has(resources.dispatch)) errors.push(`${id}: resources.dispatch must be one of ${[...DISPATCH_MODES].join(', ')} (resources/orchestrator.json#dispatchModes)`);
...
// resources/INDEX.md repeats the bindings: | `operator` | profile | grammar | tools | dispatch | why |
const index = await readFile(path.join(root, 'resources', 'INDEX.md'), 'utf8');
const GRAMMAR_WORD = { true: 'yes', false: 'no' };
const rows = new Map();
for (const line of index.split(/\r?\n/)) {
  const m = /^\| `([a-z.]+)` \| ([a-z-]+) \| (yes|no) \| ([^|]*) \| (inline|fresh) \|/.exec(line);
  if (m) rows.set(m[1], { profile: m[2], grammar: m[3], tools: m[4].trim(), dispatch: m[5] });
}
for (const [id, resources] of operators) {
  ...
  const expect = { profile: resources.profile, grammar: GRAMMAR_WORD[resources.grammarBound], tools: expectTools, dispatch: resources.dispatch };
...
if (interaction.schemaVersion !== 2 || !Array.isArray(interaction.questionKinds) || !interaction.questionKinds.length || interaction.questionKinds.some((kind) => typeof kind !== 'string' || !kind.trim())) errors.push('interaction.json: schemaVersion and questionKinds must declare a communication contract');

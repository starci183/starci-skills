...
const protectedPath=value=>/(^|\/)\.git(\/|$)/.test(value);
...
function repositoryState(id,value){
  ...
  const dirty=git(root,['status','--porcelain=v1','--untracked-files=normal','--',':(exclude).starciwork/**']).stdout.length>0;
...
function scanIdentity({node,identity,repositories,selected,bindingId,repairPaths}){
  ...
  for(const declared of declaredRepositories){
    ...
    const repo=repositories.get(declared.repository),stack=array(declared.coverage?.paths).some(p=>p==='.starcistacks'||p.startsWith('.starcistacks/'));
...
function scanSourceRefs({node,refs,repositories,selected,repairPaths}){
  ...
  for(const [index,ref] of (refs??[]).entries()){
    ...
    const repo=repositories.get(ref.repository),stack=ref.path==='.starcistacks'||ref.path.startsWith('.starcistacks/'),route=nodeRoute(node,{stack}),bindingId=`work:${node.id}:sourceRef:${index}:${ref.repository}:${ref.path}`;
...
function scanCodeRefs({node,refs,repositories,selected,repairPaths}){
  const findings=[];for(const [index,ref] of refs.entries()){
    const route=nodeRoute(node),bindingId=`work:${node.id}:codeRef:${index}`;
    if(!object(ref)||!text(ref.repository)||!SHA.test(ref.commit??'')){findings.push(makeFinding({bindingId,nodeId:node.id,category:'input-unavailable',status:'unverifiable',layer:'source',code:'MALFORMED_CODE_REFERENCE',detail:'A codeRefs entry is malformed.',expected:{repository:true,commit:'full-git-sha'},observed:null,route,operator:route,repairPaths},selected));continue;}
    const repo=repositories.get(ref.repository);if(!repo){findings.push(makeFinding({bindingId,nodeId:node.id,category:'input-unavailable',status:'unverifiable',layer:'source',code:'REPOSITORY_MAPPING_MISSING',detail:'The codeRefs entry names a repository that was not explicitly mapped.',expected:{repository:ref.repository,revision:ref.commit},observed:null,route,operator:route,repairPaths},selected));continue;}
    if(!refExists(repo,ref.commit)){findings.push(makeFinding({bindingId,nodeId:node.id,category:'input-unavailable',status:'unverifiable',layer:'source',code:'SOURCE_REVISION_UNAVAILABLE',detail:'The codeRefs entry commit cannot be resolved.',expected:{repository:ref.repository,revision:ref.commit},observed:{head:repo.head},route,operator:route,repairPaths},selected));continue;}
    findings.push(makeFinding({bindingId,nodeId:node.id,category:'input-unavailable',status:'unverifiable',layer:'source',code:'CODE_REFS_COVERAGE_UNKNOWN',detail:'codeRefs bind a commit but declare no source coverage, so this scanner cannot prove which current paths remain conforming.',expected:{repository:ref.repository,revision:ref.commit},observed:{head:repo.head,dirty:repo.dirty},route,operator:route,repairPaths},selected));
  }return findings;
}
...
export function scanCanonicalWork({workRoot,repositories={},targets=[]}={}, {afterMeasurements=()=>{}}={}){
  ...
  for(const node of selected){
    ...
    else if(Array.isArray(metadata.completion?.codeRefs))findings.push(...scanCodeRefs({node,refs:metadata.completion.codeRefs,repositories:repoStates,selected,repairPaths}));
  }
  afterMeasurements();

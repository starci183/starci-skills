...
export const SDS_MAP_CONTRACT=parseYaml(fs.readFileSync(path.join(skillRoot,'modules','schemas','spec','sds-map.schema.yaml'),'utf8'));
...
const hasImplementationProvenance=value=>{
 ...
 if(Object.keys(value).some(key=>['sourceRefs','codeRefs','revision','commit','existence','implementationStatus','repositoryRole','path','symbols','symbol','signature','codeUnitRef','codeUnitRefs','callerCodeUnitRef','receiverCodeUnitRef','ownerCodeUnitRef'].includes(key)))return true;
...
function local(payload,type){
 ...
 if(type==='flow'){
  if(!Array.isArray(payload.businessRefs)||!payload.businessRefs.length||!object(payload.entryPoint)||!text(payload.entryPoint.channel)||!text(payload.entryPoint.trigger)||!text(payload.entryPoint.componentRef)||!texts(payload.preconditions,{empty:true})||!Array.isArray(payload.inputs)||!texts(payload.participants))fail('SDS flow needs Business refs, an exact logical entry point, preconditions, inputs and participants.');
 ...
 if(type==='component'){
  if(!['service','worker','store','gateway','client','external-system','ui-surface'].includes(payload.kind)||!text(payload.responsibility))fail('Component needs a logical kind and responsibility.');
  for(const key of ['interfaceRefs','dataRefs','qualityRefs','verificationRefs'])if(!texts(payload[key],{empty:true}))fail(`${key} must be a unique text array.`);
 }
 if(type==='contract'){
  if(!['api','event','job','function','webhook','stream'].includes(payload.kind)||!text(payload.callerComponentRef)||!text(payload.receiverComponentRef)||!['sync','async','in-process'].includes(payload.mode)||!text(payload.transport)||!text(payload.operation)||!Array.isArray(payload.request)||!Array.isArray(payload.response)||!texts(payload.errors)||!text(payload.authorization)||!text(payload.timeout)||!text(payload.retry)||!text(payload.idempotency)||!text(payload.compatibility))fail('Contract needs logical endpoints, interaction shape, errors, authorization, timing, retry, idempotency and compatibility.');
 }
 if(type==='data-model'){
  if(!text(payload.ownerComponentRef)||!text(payload.store)||typeof payload.authoritative!=='boolean'||!text(payload.classification)||!Array.isArray(payload.fields)||!payload.fields.length||!texts(payload.states)||!Array.isArray(payload.transactions)||!payload.transactions.length||!text(payload.consistency)||!text(payload.retention)||!object(payload.recovery))fail('Data model needs a logical owner, store, fields, states, transactions, consistency, retention and recovery.');
 }
 if(type==='quality'){
...
export function validateSDSMap(entries,srsEntries=[],businessEntries=[]){
 ...
 for(const entry of entries.filter(item=>!item.classification.aggregate)){const payload=entry.payload,type=entry.classification.type;if(!object(payload))continue;
  if(type==='flow'){
   for(const ref of payload.businessRefs??[])businessRef(entry,ref);resolve(entry,payload.participants,'component','Participant');resolve(entry,[payload.entryPoint?.componentRef].filter(Boolean),'component','Entry point');resolve(entry,payload.verificationRefs,'verification','Verification');resolve(entry,payload.topologyRefs,'deployment','Topology');
   for(const sequence of [payload.mainSequence,...(payload.alternativeSequences??[]),...(payload.exceptionSequences??[])])for(const step of sequence?.steps??[]){resolve(entry,[step.componentRef],'component','Component');resolve(entry,step.contractRefs,'contract','Contract',{empty:true});resolve(entry,step.dataRefs,'data-model','Data',{empty:true});}
  }
  if(type==='component'){resolve(entry,payload.interfaceRefs,'contract','Interface',{empty:true});resolve(entry,payload.dataRefs,'data-model','Data',{empty:true});resolve(entry,payload.qualityRefs,'quality','Quality',{empty:true});resolve(entry,payload.verificationRefs,'verification','Verification',{empty:true});}
  if(type==='contract'){resolve(entry,[payload.callerComponentRef],'component','Contract caller');resolve(entry,[payload.receiverComponentRef],'component','Contract receiver');}
  if(type==='data-model')resolve(entry,[payload.ownerComponentRef],'component','Data owner');
  ...
  if(type==='deployment')for(const placement of payload.placements??[])resolve(entry,[placement.componentRef],'component','Deployment placement');
 ...
 const flowEntries=entries.filter(entry=>entry.classification.type==='flow');if(flowEntries.length&&!entries.some(entry=>entry.classification.type==='component'))add(flowEntries[0],'SDS_COMPONENT_MAP','SDS flows require owned logical components.');
 return issues;
}

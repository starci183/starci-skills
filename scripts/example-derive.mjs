...
function collectIdsByTrail(value, trail, out) {
  if (typeof value === 'string') {
    const s = value.trim();
    if (ID_RE.test(s)) out.push({trail, id: s});
    return;
  }
...
function buildUsedBy(records) {
  ...
  for (const record of records.values()) {
    ...
    for (const {trail, id: targetId} of hits) {
      ...
      if (!kind) { unclassified.push({from: record.id, field: trail, target: targetId}); continue; }
      if (!usedBy.has(targetId)) usedBy.set(targetId, new Map());
      const byKind = usedBy.get(targetId);
...
function unmetBlockers(record, records) {
  ...
  return edges.filter(e => isPlainObject(e) && typeof e.record === 'string')
    .filter(e => records.get(e.record)?.state !== 'done');
}
 ...
 */
function resolveBlockers(record, records) {
  ...
  function walkEdge(targetId, because, visited) {
...
export function computeDerived(workRoot) {
  ...
  for (const record of records.values()) {
    ...
    derivedRecords.set(record.id, {
      ...
      blockers: blockedByEdges.length ? resolveBlockers(record, records) : [],
    });
  }
  ...
  const frontier = [...derivedRecords.values()]
    .filter(r => r.state === 'todo' && unmetBlockers(records.get(r.id), records).length === 0)

 ...
 * of freshness gate (scripts/checks/check-example-derived.mjs, extended below).
 *
...
function buildProvesUsedBy(rawRecords) {
  ...
  for (const rec of rawRecords.values()) {
    ...
    for (const t of targets) {
      ...
      // Compact format: `P#frag` / a collapsed `ac.*` id resolve to the record carrying the criterion.
      const canonical = resolveRecordRef(rawRecords, t, inline) ?? t;
      if (!provesUsedBy.has(canonical)) provesUsedBy.set(canonical, new Set());
      provesUsedBy.get(canonical).add(rec.id);
    }
  }
  return provesUsedBy;
}
...
export function runCritique(workRoot, {write} = {}) {
  ...
  if (write) {
    ...
    fs.writeFileSync(yamlPath, '# GENERATED by scripts/example/example-critique.mjs - do not edit by hand.\n' +
      '# Regenerate with: node scripts/example/example-critique.mjs --work <path-to-.starciwork> --write\n' + stringifyYaml(yamlDoc), 'utf8');

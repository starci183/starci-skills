...
// session names where it stopped. The per-branch gates stay in validate-request and validate-response;
// this file reads only the ledger they leave behind.
...
import { effectiveBudget } from './validate-request.mjs';
...
export async function validateSession(root, session) {
  ...
  if (state.brief?.report) {
    const shapes = Object.keys((await loadInteractionPolicy(root)).reportShapes ?? {});

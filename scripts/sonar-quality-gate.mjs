...
export async function sonarRequest({ baseUrl, path, method = "GET", token, fetchImpl = fetch, body }) {
    ...
    if (!response.ok) throw new Error(`Sonar API request failed: ${response.status}`)
    return response.json()
}

export const CONDITIONS = Object.freeze({
    new_bugs: { error: "0", op: "GT" }, new_vulnerabilities: { error: "0", op: "GT" }, new_code_smells: { error: "0", op: "GT" },
    reliability_rating: { error: "1", op: "GT" }, security_rating: { error: "1", op: "GT" }, sqale_rating: { error: "1", op: "GT" },
    security_hotspots_reviewed: { error: "100", op: "LT" }, duplicated_lines_density: { error: "3", op: "GT" }, new_duplicated_lines_density: { error: "3", op: "GT" },
    coverage: { error: "80", op: "LT" }, new_coverage: { error: "90", op: "LT" },
})

async function reconcileGate({ baseUrl, projectKey, token, fetchImpl }) {
    ...
    const gateId = gate.id ?? gate.idValue
    ...
    for (const metric of desiredKeys) await sonarRequest({ baseUrl, path: "/api/qualitygates/create_condition", method: "POST", token, fetchImpl, body: { gateId, metric, ...CONDITIONS[metric] } })
...
export async function reconcileQualityGate({ baseUrl, projectKey, analysisSha, execute = false, env = process.env, fetchImpl = fetch, gateResponse }) {
    ...
    const reconciliation = await reconcileGate({ baseUrl, projectKey, token: adminToken, fetchImpl })
    ...
    const metricKeys = Object.keys({ bugs: 1, vulnerabilities: 1, code_smells: 1, new_bugs: 1, new_vulnerabilities: 1, new_code_smells: 1, reliability_rating: 1, security_rating: 1, sqale_rating: 1, new_reliability_rating: 1, new_security_rating: 1, new_sqale_rating: 1, security_hotspots_reviewed: 1, duplicated_lines_density: 1, new_duplicated_lines_density: 1, coverage: 1, new_coverage: 1 })

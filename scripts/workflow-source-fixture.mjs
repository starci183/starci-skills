...
export async function acceptArchitecture(f, { beforeCritique, startStep = 1, sourceStep = startStep + 3, writerRef = 'src/modules/fixture/worker.mjs', operationId = 'fixture-worker', coordination = null, goal = { prerequisite: `${sourceStep}/1` } } = {}) {
  ...
  put(path.join(critiqueDir,'request/request.json'),critiqueRequest);
  const {collectArchitectureSource}=await moduleAt(f,'scripts/architecture-source-review.mjs');
  const sourceSnapshot=await collectArchitectureSource(f.root,critiqueDir);
  const sourceBytes=JSON.stringify(sourceSnapshot,null,2)+'\n';
  put(path.join(critiqueDir,'request/source-review.json'),sourceBytes);
  critiqueRequest.frozenInputs=[{ref:'request/source-review.json',sha256:sha(sourceBytes)}];
  critiqueRequest.contexts=[{alias:sourceSnapshot.alias,head:sourceSnapshot.head}];critiqueRequest.environment.reads=[sourceSnapshot.alias];
  put(path.join(critiqueDir,'request/request.json'),critiqueRequest);
  await beforeCritique?.(f,{parent:request,request:critiqueRequest,branch:critiqueDir,snapshot:sourceSnapshot});
  await open(f, critiqueRequest, critiqueDir);
  // A separate process reads the measured frozen source and selected model, without author rationale.
  const reviewProgram = "import assert from 'node:assert/strict'; import {readFileSync} from 'node:fs'; const snapshot=JSON.parse(readFileSync(process.argv[2])); assert.equal(JSON.parse(snapshot.files.find(f=>f.path==='package.json').chunks.join('')).type,'module'); assert.ok(snapshot.files.find(f=>f.path==='README.md').chunks.join('').trim()); const model=JSON.parse(readFileSync(process.argv[1])); const op=model.operations[0]; assert.equal(model.operations.length,1); assert.equal(op.transport,'worker'); assert.equal(op.transactionBoundary,'read-only'); assert.equal(op.idempotencyKind,'none'); assert.deepEqual(op.storeRefs,[]); assert.deepEqual(op.migrationRefs,[]); assert.deepEqual(model.stores,[]); assert.equal(model.boundaries[0].ownsData,false); process.stdout.write(JSON.stringify({pid:process.pid,paths:['partial-failure','retry-idempotency','concurrency','stale-state','deletion','recovery','dependency-outage','rollback']}));";
  const review = JSON.parse(execFileSync(process.execPath, ['--input-type=module', '-e', reviewProgram, path.join(dir, 'response/data/stack-model.json'),path.join(critiqueDir,'request/source-review.json')], { encoding: 'utf8', windowsHide: true }));
  put(path.join(critiqueDir, 'response/critique.md'), `# independent-critique — ${decision}\n` + table('Execution', ['Field', 'Value'], [['Reviewer execution', `process://${review.pid}`], ['Inherited turns', 'none'], ['Given', 'Selected model and frozen Git source, no author rationale'],['Source snapshot',sha(sourceBytes)],['Independent alternative','A persistent worker would add ownership and restore obligations to this stateless boundary.'],['Trade-offs','Cost and complexity increase with persistence; reversibility decreases when data exists.'],['Uncertainty','Source inspection does not establish deployment or product runtime behavior.']]) + table('Attacks', ['Adverse path', 'Attack', 'Resolution', 'Verdict'], review.paths.map(adverse => [adverse, `[source:package.json] [source:README.md] Can ${adverse} leave a partial persisted effect?`, 'The separately inspected contract has one read-only worker, no stores, migration or persistent effects.', 'holds'])) + table('Verdict', ['Field', 'Value'], [['Selection', 'keep']]));

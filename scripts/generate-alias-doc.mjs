...
const t = {
  ...
  vi: { title: '# Alias', intro: 'Mọi nơi một operator được đọc, theo alias, gom theo vùng. Sinh từ `alias/alias.json` (sổ cho máy) và mọi `operator.json` bởi `scripts/generate-alias-doc.mjs`; `--check` chạy trong `npm test`. Operator chỉ đọc những alias mà bảng Ref trong `context.md` của nó gọi tên; trang này là toàn bộ từ vựng mà các bảng đó lấy ra. Phân giải theo tiền tố đăng ký dài nhất, đuôi đường dẫn thu hẹp alias, đoạn trong ngoặc nhọn do lần gọi cung cấp.', zone: 'Vùng', head: '| Alias | Tham số | Trỏ tới | Bind | Ai ghi | Operator ràng | Mục đích |', seg: '## Đoạn thân thiện trong một checkout', segHead: '| Đoạn | Trỏ tới |', zones: { workspaces: 'Workspaces — vùng làm việc', grammar: 'Grammar — gói', knowledge: 'Knowledge — luật', worktrees: 'Worktrees — thẩm quyền và bằng chứng máy-cục-bộ', remote: 'Remote — internet', dynamic: 'Dynamic — sinh trong phiên' } },
};

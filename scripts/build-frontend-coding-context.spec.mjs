...
function fixture() {
  ...
  fs.writeFileSync(path.join(root, 'src/components/composites/cards/SurfaceCard/index.tsx'), `
    ...
    export const SURFACE_CARD_CONTRACT = { label: 'outside' } as const;
  `);
...
test('exports plain JSON component, props, descriptions, and contract signals without executing source', () => {
  const root = fixture();
  ...
  assert.ok(surface.contracts.includes('surface-in-surface'));
  assert.equal(snapshot.contractRegistries[0].name, 'SURFACE_CARD_CONTRACT');
});

...
function cmdDispatch(ledger, args, repo) {
  ...
  if (MANAGED_KINDS.includes(model.kind)) {
    return cmdDispatchManaged(ledger, args, { job, jobId, payload, op, model, packet, prompt, worktree, title });
  }
...
function cmdDispatchManaged(ledger, args, { job, jobId, payload, op, model, packet, prompt, worktree, title }) {
  const db = ledger.db;
  ...
  ledger.transaction(() => {
    db.prepare("UPDATE jobs SET status='running', worker_id=?, payload_json=?, updated_at=? WHERE job_id=?")
      .run(dispatchId, JSON.stringify(payload), Date.now(), jobId);
    ledger.appendEvent({
      workflowId: job.workflow_id, entityType: 'job', entityId: jobId,
      kind: 'op-dispatched',
      payload: { op, dispatch: dispatchId, model: model.target, worktree, managed: true, taskId, modelId },
    });
  });
  const out = {
...
function cmdSettle(ledger, args, repo) {
  ...
  const status = verdict === 'pass' ? 'succeeded' : 'failed';
  const out = { ok: true, jobId, verdict, status, report: reportAbs, leasesReleased: released, machineRefsReleased: machineReleased, terminalClosed, ...(managedWorker ? { managedWorker } : {}) };
  emit(out, `settled ${jobId} verdict=${verdict} status=${status} (leases released: ${released}${terminalClosed ? `, terminal ${terminalClosed.handle} closed=${terminalClosed.ok}` : ''}${managedWorker ? `, worker ${managedWorker.dispatchId} stop=${managedWorker.stop.ok} release=${managedWorker.release.ok}` : ''})`, args.json);
}

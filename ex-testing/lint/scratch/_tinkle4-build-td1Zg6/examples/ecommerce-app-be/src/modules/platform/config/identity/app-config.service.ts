...
function portsOf(parsed: unknown): PortsResult | null {
    ...
    if (typeof candidate.identityApi !== "number" || typeof candidate.orderApi !== "number"
        || typeof candidate.postgres !== "number" || typeof candidate.redis !== "number") {
        return null
    }
    return {
...
function readMetadata(): MetadataResult {
    ...
    if (!ports) {
        throw new MetadataPortsMissingException({
            message: `${file} does not carry the resolved ports this service reads (identityApi, orderApi, postgres, redis).` 
        })
    }
    return {
 ...
 * agrees only until somebody moves the offset. Every process env read lives behind this typed

...
const ENV_KEYS = [METADATA_FILE_ENV,
    PORT_ENV,
    DATABASE_URL_ENV,
    IDENTITY_API_URL_ENV]
...
describe("AppConfigService (order) - the metadata.json runtime projection",
    () => {
        ...
        it("reads every binding from the metadata file the env var names",
            async () => {
                ...
                try {
                    ...
                    expect(service.getPort()).toBe(6070)
                    expect(service.getDatabaseUrl()).toBe("postgres://postgres@localhost:5501/ecommerce")
                    expect(service.getIdentityApiBaseUrl()).toBe("http://localhost:5070")
                } finally {
                    await moduleRef.close()
                }
            })

        it("boots against a projection with no redis port - this service does not read one",
            async () => {
                fileFoundAt("/checkout/metadata.json",
                    {
                        project: "ecommerce-app",
        ...
        it("lets deployment env vars override the resolved projection",
            async () => {
                fileFoundAt("/checkout/metadata.json")
                process.env[METADATA_FILE_ENV] = "/checkout/metadata.json"
                process.env[PORT_ENV] = "9091"
                process.env[DATABASE_URL_ENV] = "postgres://prod-db:5432/order"
                process.env[IDENTITY_API_URL_ENV] = "https://identity.internal"

                const moduleRef = await boot()
                try {
                    const service = moduleRef.get(AppConfigService)
                    expect(service.getPort()).toBe(9091)
                    expect(service.getDatabaseUrl()).toBe("postgres://prod-db:5432/order")
                    expect(service.getIdentityApiBaseUrl()).toBe("https://identity.internal")
                } finally {
                    await moduleRef.close()
                }
            })

...
const repoRoot = findUp(assetsDir, path.join('scripts', 'checks', 'render.mjs'));
const { parseYaml } = await import(pathToFileURL(path.join(repoRoot, 'core', 'yaml.mjs')));
const { decodePng, checkPalette, checkEntityListInCard } = await import(pathToFileURL(path.join(repoRoot, 'scripts', 'checks', 'render.mjs')));

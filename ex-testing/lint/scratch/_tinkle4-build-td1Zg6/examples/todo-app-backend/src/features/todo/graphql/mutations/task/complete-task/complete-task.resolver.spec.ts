...
describe("CompleteTaskResolver",
    () => {
        ...
        it("fr.task.complete: resolves the caller, maps the id arg onto the command and returns the completion state",
            async () => {
                commandBus.execute.mockResolvedValue({
                    taskId: "task-1", complete: true 
                })

                const response = await resolver.completeTask(req,
                    "task-1")
        ...
        it("refuses before dispatching any command when the session cannot be resolved",
            async () => {
                ...
                await expect(resolver.completeTask(req,

...
export { AuditOperatorRoleNotAuthorizedException } from "@modules/shared/exceptions/errors/audit/audit-operator-role-not-authorized"
 ...
 */
@Injectable()
/** Guard consulted before the guarded audit operator proceeds; a refusal throws a house exception rather than returning false. */
export class AuditOperatorGuard {
   ...
   * resolved is an empty claim, and an empty claim is a denial, never a fallback grant. */
    canActivate(context: ExecutionContext): boolean {
        ...
        this.assertCanRead(request.actor ?? {
            personId: "unknown" 
        })
        return true
    }

...
/** Type alias naming the calendar rule shape set calendar.util switches on; a new member is added here once, not scattered as literals. */
export type RuleFrequency = "every-weekday" | "every-n-days" | "monthly-day";

/** The fields of a recur rule `datesForRule` walks - the interval/day inputs plus the anchor date. */
...
/** The decomposed fields of a `YYYY-MM-DD` date, with the month in `Date.UTC`'s 0-based convention. */
...
function parseIsoDate(date: string): IsoDateParts {
    const [y,
        m,
        d] = date.split("-").map(Number)
    return {
        year: y, monthIndex0: m - 1, day: d 
    }
}
...
function daysBetween(from: string, to: string): number {
    ...
    const aMs = Date.UTC(a.year,
        a.monthIndex0,
        a.day)
    const bMs = Date.UTC(b.year,
        b.monthIndex0,
        b.day)
...
function fires(rule: CalendarRuleShape, date: string): boolean {
    ...
    case "every-n-days": {
        const n = rule.n ?? 1
        const elapsed = daysBetween(rule.startDate,
            date)
        return elapsed >= 0 && elapsed % n === 0
    }
    case "monthly-day": {
        const dayOfMonth = rule.dayOfMonth ?? 1
        return isMonthlyDayMatch(date,
            dayOfMonth)
    }
    default:
        return false
 ...
 * before it began.
 */
export function datesForRule(rule: CalendarRuleShape, fromInclusive: string, toInclusive: string): Array<string> {
    const start = rule.startDate > fromInclusive ? rule.startDate : fromInclusive
    if (start > toInclusive) return []
    const dates: Array<string> = []
    let cursor = start
    let guard = 0
    const maxIterations = daysBetween(start,
        toInclusive) + 1
    while (cursor <= toInclusive && guard <= maxIterations) {
        if (fires(rule,
            cursor)) dates.push(cursor)
        cursor = addDays(cursor,
            1)
        guard += 1
    }
    return dates

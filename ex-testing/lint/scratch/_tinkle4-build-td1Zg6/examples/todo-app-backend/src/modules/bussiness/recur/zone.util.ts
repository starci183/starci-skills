...
/** A wall-clock local date and time inside an IANA zone - the input `resolveLocalTimeToUtc` anchors. */
/** Contract naming the zone resolution kind shape bussiness/recur code and its consumers share; a second site never retypes it inline. */
...
/** The clock half of a local time, parsed out of a rule's `HH:MM` field. */
/** Contract naming the zone resolution shape bussiness/recur code and its consumers share; a second site never retypes it inline. */
export interface LocalTimeParts {
  readonly hour: number;
  readonly minute: number;
}

/** The calendar half of a local time, parsed out of a rule's `YYYY-MM-DD` field. */
...
/** How a local time maps to UTC: `gap` and `fold` are the DST edges decision.recur.timezone.dst resolves. */
...
function offsetMinutesAt(utcMs: number, timeZone: string): number {
    const wall = zonedParts(utcMs,
        timeZone)
    const asUtc = Date.UTC(wall.year,
        wall.month - 1,
        wall.day,
        wall.hour,
        wall.minute,
...
function refinedOffsetMinutes(naiveGuessMs: number, timeZone: string): number {
    const first = offsetMinutesAt(naiveGuessMs,
        timeZone)
    return offsetMinutesAt(naiveGuessMs - first * 60000,
        timeZone)
}
...
export function resolveLocalTimeToUtc(timeZone: string, local: LocalDateTime): ZoneResolution {
    const dayStartGuess = Date.UTC(local.year,
        local.month - 1,
        local.day,
        0,
        0,
        0)
    const dayEndGuess = Date.UTC(local.year,
        local.month - 1,
        local.day,
        23,
        59,
        0)
    const offsetStart = refinedOffsetMinutes(dayStartGuess,
        timeZone)
    const offsetEnd = refinedOffsetMinutes(dayEndGuess,
        timeZone)

    const naiveNominal = Date.UTC(local.year,
        local.month - 1,
        local.day,
        local.hour,
        local.minute,
    ...
    if (offsetStart === offsetEnd) {
        return {
            instant: new Date(naiveNominal - offsetStart * 60000), kind: "normal" 
        }
    }

    const utcUnderStartOffset = naiveNominal - offsetStart * 60000
    const utcUnderEndOffset = naiveNominal - offsetEnd * 60000
    const matchesStart = sameWallClock(zonedParts(utcUnderStartOffset,
        timeZone),
    local)
    const matchesEnd = sameWallClock(zonedParts(utcUnderEndOffset,
        timeZone),
    local)

    if (matchesStart && matchesEnd) {
    // Fold: both real instants read this local time. Earliest of the two, always.
        const earliest = Math.min(utcUnderStartOffset,
            utcUnderEndOffset)
        return {
            instant: new Date(earliest), kind: "fold" 
        }
    }
    if (matchesStart) {
        return {
            instant: new Date(utcUnderStartOffset), kind: "normal" 
        }
    }
    if (matchesEnd) {
        return {
            instant: new Date(utcUnderEndOffset), kind: "normal" 
        }
    }
    // Gap: this local time never happens. Resolve with the pre-transition (day-start) offset directly.
    return {
        instant: new Date(utcUnderStartOffset), kind: "gap" 
    }
}

/** Parses "HH:MM" into {hour, minute}. */
export function parseLocalTime(time: string): LocalTimeParts {
    const [hourText,
        minuteText] = time.split(":")
    return {
...
export function resolveRuleInstant(timeZone: string, localDate: string, time: string): ZoneResolution {
    ...
    return resolveLocalTimeToUtc(timeZone,
        {

...
/** Contract naming the sepay create intent result shape integrations/sepay code and its consumers share; a second site never retypes it inline. */
...
/** Type alias naming the sepay transaction status set sepay.client switches on; a new member is added here once, not scattered as literals. */
export type SepayTransactionStatus = "pending" | "paid" | "failed";

/** Contract naming the sepay get transaction result shape integrations/sepay code and its consumers share; a second site never retypes it inline. */
...
const GATEWAY_STATUSES: ReadonlyArray<string> = ["pending",
    "paid",
    "failed"]
 ...
 */
@Injectable()
/** Outbound client for the sepay integration surface; transport failures surface as house exceptions, never raw HTTP noise. */
export class SepayClient {
    constructor(private readonly config: AppConfigService) {}

    async createIntent(params: SepayCreateIntentParams): Promise<SepayCreateIntentResult> {
        const { status, payload } = await this.request("create-intent",
            "/userapi/transactions/qr",
            {
                method: "POST",
                headers: {
        ...
        if (typeof gatewayIntentId !== "string" || !gatewayIntentId || typeof checkoutUrl !== "string" || !checkoutUrl) {
            throw new SepayRequestFailedException({
                reason: this.refusal("create-intent",
                    status,
                    "the gateway returned no transaction id and checkout url"),
            })
        }
        return {
            gatewayIntentId, checkoutUrl 
        }
    }

    async getTransaction(gatewayIntentId: string): Promise<SepayGetTransactionResult> {
        const { status, payload } = await this.request("get-transaction",
            `/userapi/transactions/details/${encodeURIComponent(gatewayIntentId)}`,
            {
                method: "GET",
            })
        ...
        if (typeof returnedStatus !== "string" || !GATEWAY_STATUSES.includes(returnedStatus)) {
            throw new SepayRequestFailedException({
                reason: this.refusal("get-transaction",
                    status,
                    `the gateway returned the unrecognised status ${JSON.stringify(returnedStatus)}`),
            })
        }
        ...
        if (returnedPeriodEnd != null && typeof returnedPeriodEnd !== "string" && typeof returnedPeriodEnd !== "number") {
            throw new SepayRequestFailedException({
                reason: this.refusal("get-transaction",
                    status,
                    "the gateway returned a periodEnd that is not a date"),
            })
        }
        ...
        if (periodEnd && Number.isNaN(periodEnd.getTime())) {
            throw new SepayRequestFailedException({
                reason: this.refusal("get-transaction",
                    status,
                    `the gateway returned an unreadable periodEnd (${JSON.stringify(returnedPeriodEnd)})`),
            })
        }
        return {
            status: returnedStatus as SepayTransactionStatus, periodEnd 
        }
    }
   ...
   * (integration.plan.sepay's credential), compared here rather than a computed body signature. */
    ...
    private async request(name: string, path: string, init: RequestInit): Promise<{ status: number; payload: Record<string, unknown> }> {
        ...
        try {
            ...
            throw new SepayRequestFailedException({
                reason: this.refusal(name,
                    undefined,
                    `the request did not complete (${describeFetchFailure(error)})`),
            })
        }
        ...
        try {
            ...
            throw new SepayRequestFailedException({
                reason: this.refusal(name,
                    response.status,
                    `the gateway returned a body that is not JSON (${preview(body)})`),
            })
        }
        ...
        return {
            status: response.status, payload: payload as Record<string, unknown> 
        }
    }
...
/** The fields a Node fetch failure carries (Node's own `cause` chain is untyped under this project's
 * ES2021 lib, so the shape is named once here rather than retyped at every read). */
interface FetchFailureShape {
    code?: string;
    message?: string;
    cause?: FetchFailureShape;
}

function describeFetchFailure(error: unknown): string {
    if (error instanceof Error && error.name === "TimeoutError") return `timed out after ${REQUEST_TIMEOUT_MS}ms`
    const failure = error as FetchFailureShape
...
function preview(body: string): string {
    const collapsed = body.replace(/\s+/g,
        " ").trim()
    return collapsed.length > 200 ? `${collapsed.slice(0,
        200)}…` : collapsed || "empty body"
}

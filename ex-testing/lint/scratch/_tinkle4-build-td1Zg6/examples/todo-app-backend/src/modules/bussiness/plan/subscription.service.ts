...
import {
    SubscriptionEntity 
} from "@modules/platform/databases/postgresql/primary/entities/subscription.entity"
import {
...
} from "./types/subscription-record"
import {
    FREE_PLAN, PAID_PLAN, PlanDefinition 
 ...
 */
@Injectable()
/** Injectable service owning the subscription logic the plan capability exposes; wired by the capability's own module. */
export class SubscriptionService {
   ...
   * since account creation is owned outside this feature. */
    async getOrCreate(personId: string): Promise<SubscriptionRecord> {
        const existing = await this.entityManager.findOneBy(SubscriptionEntity,
            {
                personId 
            })
   ...
   * be created by the caller (fr.plan.upgrade's mainFlow). */
    async tCheckoutStarted(personId: string): Promise<SubscriptionRecord> {
        ...
        row.status = "pending"
        return toRecord(await this.entityManager.save(SubscriptionEntity,
            row))
    }
   ...
   * them rather than an error, since "downgrade to free" is already true or on its way to true. */
    ...
    private async mustFindRow(subscriptionId: string): Promise<SubscriptionEntity> {
        const row = await this.entityManager.findOneBy(SubscriptionEntity,
            {
                id: subscriptionId 
            })
        if (!row) {
            throw new PlanSubscriptionNotFoundException({
                subscriptionId 
            })
        }
        return row
...
function toRecord(row: SubscriptionEntity): SubscriptionRecord {
    return new SubscriptionRecord(row.id,
        row.personId,
        row.plan,
row.status as SubscriptionStatus,
row.periodEnd,
row.gatewayCustomerId)
}

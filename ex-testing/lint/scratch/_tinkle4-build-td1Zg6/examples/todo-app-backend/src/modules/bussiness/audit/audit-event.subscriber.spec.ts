...
import {
    SignedInEvent, SignedOutEvent, TaskCompletedEvent, TaskCreatedEvent, TaskDeletedEvent 
 ...
 * real CQRS bus or database. */
class RecordingCommandBus {
    readonly dispatched: Array<AppendLogLineCommand> = []
...
describe("AuditEventSubscriber",
    () => {
        ...
        let commandBus: RecordingCommandBus
        ...
        beforeEach(async () => {
            ...
            moduleRef.get(AuditEventSubscriber).onModuleInit()
        })
        ...
        const eachEvent: Array<{ event: unknown; action: string; actorField: string; target: string | null }> = [
            {
                event: new TaskCreatedEvent("task-1",
                    "owner-1",
                    new Date(),
                    "src-1"), action: "task.created", actorField: "owner-1", target: "task-1" 
            },
            {
                event: new TaskCompletedEvent("task-1",
                    "owner-1",
                    new Date(),
                    "src-2"), action: "task.completed", actorField: "owner-1", target: "task-1" 
            },
            {
                event: new TaskDeletedEvent("task-1",
                    "owner-1",
                    new Date(),
                    "src-3"), action: "task.deleted", actorField: "owner-1", target: "task-1" 
            },
            {
                event: new SignedInEvent("person-1",
                    new Date(),
                    "src-4"), action: "login.signed-in", actorField: "person-1", target: null 
            },
            {
                event: new SignedOutEvent("person-1",
                    new Date(),
                    "src-5"), action: "login.signed-out", actorField: "person-1", target: null 
            },
        ]
        ...
        it.each(eachEvent)("fr.audit.log.append: $action is appended for its published event",
            async ({ event, action, actorField, target }) => {
                ...
                expect(commandBus.dispatched[0].params).toEqual({
                    actorId: actorField, action, target 
                })
            })

        it("unsubscribes on module destroy: a later publish dispatches nothing",
            async () => {
                moduleRef.get(AuditEventSubscriber).onModuleDestroy()

...
import {
    PlatformEventBus, PlatformEventHandler 
 ...
 */
@Injectable()
/** Subscriber wiring the audit capability's platform-event subscriptions; the spec beside it proves the wiring, not the timing. */
export class AuditEventSubscriber implements OnModuleInit, OnModuleDestroy {
    ...
    private async route(event: Parameters<PlatformEventHandler>[0]): Promise<void> {
        ...
        await this.commandBus.execute(new AppendLogLineCommand({

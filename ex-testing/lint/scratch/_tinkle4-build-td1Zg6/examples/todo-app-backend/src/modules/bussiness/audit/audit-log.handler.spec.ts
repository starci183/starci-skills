...
async function buildHandler(operatorSubjects: ReadonlyArray<string>) {
    ...
    await log.append("person-1",
        ...
        "task-1")
    return {
        moduleRef, handler: moduleRef.get(AuditLogHandler) 
    }
}
...
describe("AuditLogHandler (fr.audit.log.read)",
    () => {
        ...
        it("a person with no lines gets an empty list, not an error",
            async () => {
                const built = await buildHandler([])
                moduleRef = built.moduleRef

                const result = await built.handler.execute(new AuditLogQuery({
                    personId: "nobody" 
                }))
        ...
        it("refusal (fr.audit.log.read exceptionFlows): an empty actor is refused before any line is touched",
            async () => {
                const built = await buildHandler([])
                moduleRef = built.moduleRef

                await expect(built.handler.execute(new AuditLogQuery({
                    personId: "" 
        ...
        it("gap.audit.operator-role closed - fail-closed default: with no operator configured, a read returns only the caller's own lines and never the wider chain",
            async () => {
                const built = await buildHandler([])
                moduleRef = built.moduleRef
                ...
                expect(result.lines.map(line => line.action)).toEqual(["login.signed-in",
                    "task.created"])
                expect(result.lines).toHaveLength(2)
            })

        it("verifiable claim (decision.audit.operator-role): a subject outside the roster is no operator, so an action filter cannot escalate to the whole chain",
            async () => {
                const built = await buildHandler(["operator-1"])
                moduleRef = built.moduleRef
                ...
                const result = await built.handler.execute(new AuditLogQuery({
                    personId: "person-1", action: "task.created" 
                }))
        ...
        it("fr.audit.log.read mainFlow: a verified operator on the roster reads the whole chain, filtered by an action the contract emits",
            async () => {
                const built = await buildHandler(["operator-1"])
                moduleRef = built.moduleRef

                const result = await built.handler.execute(new AuditLogQuery({
                    personId: "operator-1", action: "task.created" 
                }))
                ...
                expect(result.lines.map(line => line.target).sort()).toEqual(["task-1",
                    "task-2"])
        ...
        it("fr.audit.log.read: an operator's unfiltered read returns every line in the chain, oldest first",
            async () => {
                const built = await buildHandler(["operator-1"])
                moduleRef = built.moduleRef
        ...
        it("fr.audit.log.read: an operator filtering by an action the contract never emits gets nothing",
            async () => {
                const built = await buildHandler(["operator-1"])
                moduleRef = built.moduleRef

                const result = await built.handler.execute(new AuditLogQuery({
                    personId: "operator-1", action: "task.updated" 
                }))

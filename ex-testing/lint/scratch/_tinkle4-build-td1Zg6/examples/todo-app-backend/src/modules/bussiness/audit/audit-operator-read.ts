...
/** Contract naming the operator filter shape bussiness/audit code and its consumers share; a second site never retypes it inline. */
...
export function toAuditLineSummary(record: ResolvedAuditLine): AuditLogLineSummaryResult {
    return {
        at: record.at, action: record.action, target: record.target 
    }
}

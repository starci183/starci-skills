        ...
        it("refuses every actor without an operator claim - which is every actor this example can resolve today",
            () => {
                expect(() => assertOperatorRead({
                    personId: "person-1" 
                })).toThrow(AuditOperatorRoleNotAuthorizedException)
                expect(() => assertOperatorRead({
                    personId: "person-1", role: "person" 
                })).toThrow(AuditOperatorRoleNotAuthorizedException)
                expect(() => assertOperatorRead({
                    personId: "person-1", role: "" 
                })).toThrow(AuditOperatorRoleNotAuthorizedException)
            })
...
describe("AuditOperatorGuard",
    () => {
        ...
        let guard: AuditOperatorGuard

        beforeEach(async () => {
            moduleRef = await Test.createTestingModule({
                providers: [AuditOperatorGuard] 
            }).compile()
            guard = moduleRef.get(AuditOperatorGuard)
        })
...
describe("operator filter/summary pair (fr.audit.log.read)",
    () => {
        ...
        const line = (at: Date, action: string, target: string | null) => ({
            at, action, target, actor: null, tombstoned: false 
        })

        it("filters by action and by target, and passes everything when unfiltered",
            () => {
                const rows = [line(new Date(),
                    "task.created",
                    "task-1"),
                line(new Date(),
                    "task.completed",
                ...
                line(new Date(),
                    "login.signed-in",
                    null)]
                expect(rows.filter(r => matchOperatorFilter(r,
                    {
                        action: "task.completed" 
                    })).map(r => r.target)).toEqual(["task-2"])
                expect(rows.filter(r => matchOperatorFilter(r,
                    {
                        target: "task-1" 
                    }))).toHaveLength(1)
        ...
        it("the summary carries exactly at/action/target - never actor, never keyId",
            () => {
                const summary = toAuditLineSummary({
                    ...line(new Date(),
                        "sign-in",
                        null), actor: "person-1", tombstoned: false 
                })
                expect(Object.keys(summary).sort()).toEqual(["action",
                    "at",
                    "target"])
            })

        it("readAllLines resolves a real tombstoned line into the same summary shape, unreadable but present",
            async () => {
                moduleRef = await Test.createTestingModule({
                    providers: [
                        AuditKeystoreService,
                ...
                const keystore = moduleRef.get(AuditKeystoreService)

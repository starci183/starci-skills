 ...
 */
@Injectable()
/** Injectable service owning the session logic the session capability exposes; wired by the capability's own module. */
export class SessionService {
    ...
    async tAccept(personId: string): Promise<SessionRecord> {
        ...
        const saved = await this.entityManager.save(SessionEntity,
            {
                token: randomUUID(), personId, issuedAt, expiresAt 
            })
    ...
    async tExpire(token: string): Promise<void> {
        await this.entityManager.delete(SessionEntity,
            token)
    }
    ...
    // mode unreachable from here regardless of what the ORM does with `undefined`.
        ...
        const row = await this.entityManager.findOneBy(SessionEntity,
            {
                token 
            })
        if (!row) {
...
function toRecord(row: SessionEntity): SessionRecord {
    return new SessionRecord(row.token,
        row.personId,
        row.issuedAt,
        row.expiresAt)
}

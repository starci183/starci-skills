 ...
 * `/` carries no locale, so something has to choose one and send the reader to the addressed form -
 * next-intl's middleware reads the cookie, falls back to the default, and redirects to the
 * prefixed path. Same shared routing as the shop, so `/vi` means the same thing on both origins.
 */

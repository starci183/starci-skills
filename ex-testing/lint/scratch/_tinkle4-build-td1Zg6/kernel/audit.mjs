...
import {CODE_PATTERN_REPORT,verifyCodePatternReportDigest} from '../scripts/checks/check-scoped-lint.mjs';
...
function commandProtocol(command){
  ...
  if(new RegExp(`^${starci}\\s+check-stales(?:\\s|$)`,'i').test(value)||
    /^node(?:\.exe)?\s+(?:"[^"]*scripts\/(?:checks\/)?check-stales\.mjs"|'[^']*scripts\/(?:checks\/)?check-stales\.mjs'|\S*scripts\/(?:checks\/)?check-stales\.mjs)(?:\s|$)/i.test(value))return 'stales';
  ...
  if(/^node(?:\.exe)?\s+(?:"[^"]*scripts\/(?:checks\/)?check-scoped-lint\.mjs"|'[^']*scripts\/(?:checks\/)?check-scoped-lint\.mjs'|\S*scripts\/(?:checks\/)?check-scoped-lint\.mjs)(?:\s|$)/i.test(value))return 'code-pattern';
  return 'shell';
}

param(
  [int]$BudgetSeconds = 5100,
  [int]$PollSeconds = 60,
  [string]$DoneDir = "ex-testing\lint\done",
  [string]$LogFile = "ex-testing\lint\scratch\v7-7-wait.log"
)

# v7-7 phase-2 gate: wait for v7-2.done, v7-3.done, v7-4.done, v7-5.done (my brief's marker set).
# One line per poll round so the wait is auditable; exits 0 when all four exist, 3 on budget expiry.
$markers = 2..5 | ForEach-Object { Join-Path $DoneDir "v7-$_.done" }

...
import {classifyChange} from '../checks/work-change.mjs';
 ...
 * disk holds exactly one side of it. checks/work-change.mjs already owns the semantics of the transition and
 ...
 * been committed. This script is that bridge - it hands checks/work-change.mjs the baseline it has been asking
 ...
 *
 * It reports and never repairs - the precedent check-stales.mjs and checks/work-change.mjs both set.
 */
 ...
 * changes either. checks/work-change.mjs's exported `normative()` is deliberately NOT used for this comparison
...
function checkRecord(record, out, {limit}) {
  ...
  for (let index = 0; index + 1 < versions.length; index++) {
    ...
    // classifyChange over empty criteria maps is checks/work-change.mjs's own verdict for a record read on its
    ...
    suspect('CHANGE_KIND_SUSPECT', `${older.at} -> ${newer.at} (rev ${revOlder ?? '(none)'} -> ${revNewer}): `
      + `declared kind: editorial while ${describeDelta(added, removed)} - checks/work-change.mjs reads the `

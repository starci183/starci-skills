...
| check | result |
|---|---|
| `node scripts/check-example-yaml.mjs` | 447 files, all accepted |
| `node scripts/check-example-work.mjs` | **4 refused**, 15 warned |
...
2. **Frontend implementation.** Stopped by owner order until the images land. It must capture each
   rendered state and compare it against the direction; `checks/render.mjs` and `checks/brand.mjs`
...
- **`starci validate` cannot run** in a fresh worktree — it needs a built `.dist`, and building the
  kernel is out of scope. The tree's real gates are the `scripts/check-example-*.mjs` family.

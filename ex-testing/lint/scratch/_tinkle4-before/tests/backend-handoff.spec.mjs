...
test('producer verification loads in different entry orders and a relocated runtime',t=>{
 ...
 for(const name of ['workflows','kernel','scripts','core','schemas','specifications','contracts','checks','.dist','SKILL.md'])fs.cpSync(new URL(name,sourceRoot),path.join(dir,name),{recursive:true});

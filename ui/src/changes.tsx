...
import { CommitHistory, EvidenceGallery } from './gallery';
...
export function CodeDiffTeaser({ data, open }: { data: AgentSnapshot | null; open: () => void }) {
  ...
  return <section className="space-y-4">
    <div className="flex flex-wrap items-end justify-between gap-3"><div><div className="mb-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-sky-400">Git & ảnh trực tiếp</div><h2 className="text-xl font-semibold tracking-tight">Code diff đang diễn ra</h2><p className="mt-1 text-sm text-zinc-500">Thay đổi trong phạm vi op đang chạy; thư viện bằng chứng có thêm ảnh AI, ảnh chụp và video UAT.</p></div><Button variant="outline" size="sm" onClick={open}>Xem diff & bằng chứng <ArrowRight className="size-4" /></Button></div>
...
export function CodeDiffPage({ data }: { data: AgentSnapshot | null }) {
  ...
  const [mode, setMode] = useState<'evidence' | 'history' | 'live'>('evidence');
  const selected = ordered.find((agent) => agent.id === chosenId) || ordered[0];
  const current = selected && changes[selected.id];
  return <div className="space-y-6"><div><div className="mb-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-sky-400">Visualize / diff</div><h2 className="text-2xl font-semibold">Code diff, ảnh và video</h2><p className="mt-2 text-sm text-zinc-500">Theo dõi thay đổi đang diễn ra và đối chiếu bằng chứng đã lưu trong runtime.</p></div><div className="flex flex-wrap gap-2"><Button variant={mode === 'evidence' ? 'secondary' : 'outline'} onClick={() => setMode('evidence')}>Ảnh & video</Button><Button variant={mode === 'history' ? 'secondary' : 'outline'} onClick={() => setMode('history')}>Diff đã commit</Button><Button variant={mode === 'live' ? 'secondary' : 'outline'} onClick={() => setMode('live')}>Diff op đang chạy</Button></div>
    {mode === 'evidence' ? <EvidenceGallery /> : mode === 'history' ? <CommitHistory /> : !data ? <div className="rounded-lg border border-zinc-800 p-8 text-sm text-zinc-500">Đang đọc danh sách agent...</div> : <><div className="flex flex-wrap items-end justify-between gap-3"><div><div className="mb-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-sky-400">Git & ảnh trực tiếp</div><h3 className="text-xl font-semibold">Code diff của op hiện tại</h3><p className="mt-2 max-w-3xl text-sm text-zinc-500">Chọn một op để xem diff chưa commit, tệp mới, commit trong ca và ảnh ở các đường dẫn được giao.</p></div><Button variant="outline" size="sm" onClick={refresh}><RefreshCw className="size-4" /> Làm mới</Button></div>

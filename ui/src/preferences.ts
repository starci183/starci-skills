...
const english: Record<string, string> = {
  ...
  'Diff & bằng chứng': 'Diff & evidence',
  'Code diff, ảnh AI, ảnh chụp và video UAT.': 'Code diffs, AI drawings, screenshots and UAT videos.',
  'Thay đổi trong phạm vi op đang chạy; thư viện bằng chứng có thêm ảnh AI, ảnh chụp và video UAT.': 'Changes within active operations; the evidence library also includes AI images, screenshots and UAT videos.',
  'Xem diff & bằng chứng': 'View diff & evidence',
  'Code diff, ảnh và video': 'Code diff, images and video',
  'Theo dõi thay đổi đang diễn ra và đối chiếu bằng chứng đã lưu trong runtime.': 'Track live changes and inspect saved runtime evidence.',
  'Ảnh & video': 'Images & video', 'Diff đã commit': 'Committed diffs', 'Diff op đang chạy': 'Live operation diff',
  'Bằng chứng từ runtime': 'Runtime evidence', 'Thư viện hình và video': 'Image and video library',
  'Ảnh AI của interface.draw, ảnh chụp màn hình triển khai, và video UAT từ các dự án. Mỗi tệp giữ đường dẫn nguồn để đối chiếu.': 'AI interface.draw images, implementation screenshots, and UAT videos across projects. Each file shows its source path.',
  'Ảnh AI vẽ': 'AI drawing', 'Ảnh chụp': 'Screenshot',
  'Ảnh chụp thật': 'Real screenshot', 'Video UAT': 'UAT video', 'Tìm tên tệp, tính năng, workflow...': 'Search files, features, workflows...',
  'Đang quét bằng chứng...': 'Scanning evidence...', 'Chưa có bằng chứng phù hợp bộ lọc.': 'No evidence matches this filter.',
  'Mở tệp gốc trong tab mới': 'Open source file in a new tab', 'Trước': 'Previous', 'Sau': 'Next',
  'Code diff đã commit': 'Committed code diff', 'Mở commit gần đây của BE hoặc FE; nội dung code được rút gọn và ẩn chuỗi nhạy cảm phổ biến.': 'Open recent BE or FE commits. Code is truncated and common secret patterns are redacted.',
  'Đang đọc Git...': 'Reading Git...', 'Chưa có commit.': 'No commits yet.', 'Chọn commit': 'Select a commit',
  'Commit này không có tệp code trong phạm vi hiển thị.': 'This commit has no displayable code files.', 'Đang đọc diff...': 'Reading diff...', 'Chưa chọn commit.': 'No commit selected.',
  'Code diff của op hiện tại': 'Current operation code diff',
  'System check · máy chủ': 'System check · host', 'Sức mạnh máy đang chạy': 'Host machine resources',
  'Đo toàn máy · làm mới mỗi 10 giây': 'Whole machine · refreshes every 10 seconds',
  'CPU toàn máy': 'Whole-machine CPU', 'RAM toàn máy': 'Whole-machine RAM', 'GPU toàn máy': 'Whole-machine GPU',
  'Bộ xử lý': 'Processor', 'Bộ nhớ hệ thống': 'System memory', 'Chưa phát hiện GPU': 'GPU not detected',
  'Chưa có số đo VRAM': 'VRAM metrics unavailable', 'Đang đo...': 'Measuring...',
  'GPU lấy từ NVIDIA khi có; CPU/RAM lấy từ Windows.': 'GPU metrics come from NVIDIA when available; CPU/RAM from Windows.',
...
const dynamic: Array<[RegExp, (...parts: string[]) => string]> = [
  [/^(\d+) tệp phù hợp$/, (count) => `${count} matching files`],
  [/^Trang (\d+) \/ (\d+)$/, (page, total) => `Page ${page} / ${total}`],
  [/^(\d+) nhân · (\d+) luồng$/, (cores, threads) => `${cores} cores · ${threads} threads`],
  [/^(\d+(?:\.\d+)?)% đang sử dụng$/, (percent) => `${percent}% in use`],
  ...
  [/^(\d+) tệp code · (\d+) ảnh$/, (files, images) => `${files} code ${Number(files) === 1 ? 'file' : 'files'} · ${images} ${Number(images) === 1 ? 'image' : 'images'}`],
  [/^(\d+) tệp code$/, (files) => `${files} code ${Number(files) === 1 ? 'file' : 'files'}`],
  [/^(\d+) tệp$/, (files) => `${files} ${Number(files) === 1 ? 'file' : 'files'}`],
  [/^(\d+) ảnh$/, (images) => `${images} ${Number(images) === 1 ? 'image' : 'images'}`],

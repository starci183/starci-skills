...
Orchestration controls how an operator executes; it never chooses the next Skill state, changes business authority, widens a write boundary, or grants approval. User authority waits and executable `RESUME` receipts belong to the owning Skill machine; orchestration has no alternate approval mode.

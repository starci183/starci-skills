...
|---|---|---|---|
| `@composition` | `brainstorms/composition/vi.md` | vi | bind Scope, Owner, Invariant và Proof trước visual decision |
| `@directions` | `brainstorms/directions/vi.md` | vi | giữ MASTER bypass và giới hạn visual direction |
| `@layouts` | `brainstorms/layouts/vi.md` | vi | bind page synthesis, state, source ownership và staged approval |
| `@blocks` | `brainstorms/blocks/vi.md` | vi | bind block state-first anatomy và complete-parent proof |
...
HTML trước decision contract đã freeze là design chưa được phép. Product code trước source-authorizing approval
...
| `page-synthesis` | author page map, journey direction, business obligation, merge binding, UI direction, candidate anatomy và schema-8 `pageContract`; recommend candidate | chạy isolated top-down fact audit và bottom-up capability audit; sau khi coordinator freeze candidate JSON, sinh complete desktop/narrow HTML, deterministic interaction và capture | direction receipt, synthesis matrix, pages artifact, preview HTML và maturity evidence | independent receipt pass trước join; HTML là projection của frozen JSON; Sol/Opus trình `OK #1` |
...
Hai origin của Layout giữ độc lập tới coordinator join. Worker được inventory journey fact và source capability,
nhưng chỉ coordinator author journey direction, UI direction và binding matrix.
...
| `direction` | liệt kê state set thật trước; chọn reuse/generalize/new part; author một UI direction mặc định hoặc 3–4 khi explicit brainstorm; freeze anatomy JSON | audit precedent/feasibility; sau anatomy freeze, sinh complete-parent desktop/narrow HTML và capture | schema-2 block anatomy, audit verdict và direction review | phủ mọi reachable state và luật BLOCK-1…14; Sol/Opus trình `OK #1` |
| `state-boundary` | chọn tối đa năm complete-page view family và freeze exact component/test boundary | verify state transition, source-owner chain và target coverage; render frozen state view trong complete parent | state review và exact source/test boundary | parent/journey không đổi; Sol/Opus trình `OK #2` |
| `implement` | giao một writer mỗi approved path, review parent preservation và integrate | implement mọi approved block state/transition và relevant test trong path tách rời | implementation receipt và integrated diff | chỉ sau `OK #2`; whole-page drift dừng |
...
| `direction` | author correcting UI direction chỉ từ fact/region đã render và journey direction freeze | sau direction freeze, sinh complete-context desktop/narrow HTML/capture; báo feasibility mismatch mà không redesign | direction review và cache artifact | grammar/maturity pass; Sol/Opus trình `OK #1` |
...
2. Cache HTML chỉ bắt đầu sau candidate/direction/anatomy JSON được coordinator freeze.

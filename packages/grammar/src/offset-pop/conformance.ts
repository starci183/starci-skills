...
export const OFFSET_POP_FAMILY_EVIDENCE = {
    ...
    "BOUNDARY-5": ["Objects carry a `--offset-pop-outline-width` (2px) solid ink outline in place of Common's 1px: nested surfaces, Button, Badge, StateMark, field and choice controls, overlay surfaces, DataTable, Calendar, Avatar, tags and pagination; grouped Buttons overlap by one outline width so a shared edge draws once."],
    ...
    "CORE-BOUNDARY-5": ["Unlike Core, the family adds width queries, and they change shadow geometry only: below 40rem the offset shadow and the surface radius shorten, below 30rem DataTable and Calendar take a smaller hard shadow; every owner hook stays. State paint changes opacity, shadow and pointer only."],
    ...
    "TONE-3": ["`Text tone=\"accent\"`, the SectionHeader eyebrow, the current tab label and a selected menu item read `--offset-pop-accent-text`, never the fill pink."],

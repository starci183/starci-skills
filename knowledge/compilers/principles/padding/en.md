...
**Common business situations.** *No boundary:* a stack between a heading and a form inside an
existing frame · a grid arranging cards · the page content column · a row wrapping two buttons · a
`min-w-0` wrapper that lets text truncate · a region that exists only to hold a `gap`. *Delegating
boundary:* a divided list · a data table frame · a stat strip split into columns · a card whose cover
image bleeds to the border · a command menu · a notification feed · a frame holding a horizontally
scrolling table.
...
**Common business situations.** A stat cell in a column-split strip · a day cell in a calendar · a
shortcut cell · a chart legend cell · a dense row holding a single label · a status-chip cell in a
table · a unit cell in a conversion table.
...
**Common business situations.** A grid cell with label + value + status · a list row with a main line
and a sub-line · a data-table cell · a transaction-history row · a member row with a role · a plan
comparison cell.
...
**Common business situations.** A course card · a profile frame · a billing summary block · a
quota-warning callout · a composed row inside a list · an empty-state card · an error card · a
bordered form block.
...
**Common business situations.** A lesson reading plane · a test-taking region · a documentation page
· a long form occupying the whole route · the content region of a dialog that holds a single task ·
the result page after a submission.
...
**Request.** "A billing card holding a heading and a bordered list of invoices; each invoice row
shows the plan name, the date it was paid, and the amount."

The request states four elements: the card, the stack that orders the heading above the list, the
list, and each invoice row. It states no page column, no filter bar and no dialog, so none of those
are resolved. It states nothing about the route existing for this card alone, so `PADDING-6` is not
reached; it states nothing about a cell holding one short datum, so `PADDING-2` is not reached.

```text
boundary owner: billing card
direct content role: ordinary-surface
situation: PADDING-4
className: p-4
nested boundaries: invoice list (PADDING-0, p-0 form); invoice row (PADDING-3)
reason: the card is a reusable surface with peer surfaces beside it and the route does not exist for it alone, which excludes PADDING-6
```
...
reason: the stack only orders the heading above the list and draws no background or border, so it owns no boundary to delegate, which excludes the p-0 form
```

```text
boundary owner: invoice list
direct content role: delegated
situation: PADDING-0
className: p-0
nested boundaries: invoice row (PADDING-3)
reason: the list is bordered and hands its whole inset to its rows so the dividers reach the edge, which excludes PADDING-4
```

```text
boundary owner: invoice row
direct content role: regular-cell
situation: PADDING-3
className: p-3
nested boundaries: none
reason: the row holds a group — plan name, paid date and amount — and still depends on its set, which excludes PADDING-2
```

If the request later says the card carries a cover image bleeding to its border, the card becomes
`PADDING-0` in the `p-0` form and the text region below the image takes `p-4`. If it later says a
quota callout with its own background sits inside the card, that callout is a second real boundary
and takes `PADDING-4` of its own, while the stack between them still takes no class.

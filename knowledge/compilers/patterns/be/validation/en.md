...
**Common business situations.** A GraphQL mutation input · a webhook body · a queue message · an
external payment response · an environment value read by an adapter.
...
**Common business situations.** Email casing · currency/unit conversion · pagination defaults · tag
lists · provider status strings mapped to one internal spelling.
...
**Common business situations.** End date after start date · voucher valid for this course and user ·
refund allowed only before fulfillment · mutually exclusive payment methods · a message valid only
...
**Common business situations.** A form with several invalid fields · batch item errors · a date-range
error attached to both fields · a provider response mapped to an internal field code.
...
These existing paths provide reference shapes for inspection.

| Code | Anchor | What to look for |
|---|---|---|
| `VALIDATE-1` | `src/features/api/core/graphql/mutations/courses/course-enroll/graphql-types/request.ts` | The external request shape before enrollment logic receives it |
| `VALIDATE-2` | `src/modules/api/rest/dtos/rest-transform.ts` | The boundary conversion and whether one canonical representation is produced |
| `VALIDATE-3` | `src/features/api/core/graphql/mutations/courses/course-enroll/course-pricing.service.ts` | Rules that need course, pricing and request facts together rather than field syntax alone |
| `VALIDATE-4` | `src/modules/platform/exceptions/filters/abstract-exception-http.filter.ts` | The transport projection of failure fields; keep validation code/path identity distinct from HTTP status |
...
Accepted shape: a course-enrollment request contains a learner, course, payment method and optional
voucher. `VALIDATE-1` checks the request shape at the GraphQL boundary. `VALIDATE-2` canonicalizes the
voucher and payment spelling once. `VALIDATE-3` evaluates voucher/course/learner eligibility where
those facts meet. `VALIDATE-4` returns stable field codes for the form. Resolver placement remains
`transport`; database handles remain `data-access`; domain invariant ownership remains
`domain-modeling`.

## Scope

This pattern governs runtime input trust, canonicalization ownership, cross-field/application rule
placement and stable validation failures. It does not govern static type narrowing, external routing,
database access, domain aggregate design, exception identity or maintainability mechanics.

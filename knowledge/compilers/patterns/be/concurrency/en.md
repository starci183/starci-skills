...
|---|---|---|
| `CONCURRENCY-1` | `src/modules/databases/postgresql/primary/lock/postgresql-advisory-lock.service.ts` | stable advisory-lock ownership and release around a database session |
| `CONCURRENCY-2` | `src/tests/e2e/refresh-token-concurrency.e2e-spec.ts` | two refresh attempts and one authoritative token outcome, with conflict/reuse visible |
| `CONCURRENCY-3` | `src/modules/databases/postgresql/primary/lock/postgresql-advisory-lock.service.ts` | lock acquisition on the database connection rather than a process-local mutex |
| `CONCURRENCY-4` | `src/tests/e2e/community-concurrency.e2e-spec.ts` | concurrent production requests and persisted invariant assertions; no direct handler invocation |
...
| contenders | The two or more platform/operations/actors that may run concurrently |
...
**Accepted shape.** Two sessions attempt to refresh the same token. The accepted outcome is one
valid refresh and one explicit reuse/conflict result.

```text
operation: mutation
invariant: one refresh-token family cannot be consumed twice
situation: CONCURRENCY-1
owner: database transaction keyed by the token family
conflict: refresh-token-reuse
proof: refresh-token-concurrency.e2e-spec.ts releases both requests from one barrier and reads the family row
reason: two distinct requests contend for one state transition, so idempotent replay is not the deciding concern
```

## Scope

This pattern applies to backend code that shares mutable state across requests, workers, jobs or
replicas. It names database-visible coordination and its proof; it does not prescribe a particular
ORM, queue or database vendor.

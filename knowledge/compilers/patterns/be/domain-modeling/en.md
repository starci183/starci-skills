...
**Common business situations.** Reserving inventory · adding a course lesson only when its order is
valid · spending credits without going below zero · closing a support thread exactly once.
...
**Common business situations.** A currency-bearing price · a normalized email · a bounded percentage
· a course code whose case is canonical · a date interval that cannot end before it starts.
...
**Common business situations.** Pending to paid · draft to published · active to suspended · review
to approved or rejected · a session that can expire only once.
...
**Common business situations.** Eligibility from learner and course state · pricing from a voucher
and a basket · allocating a seat against a cohort and its capacity · deciding a refund from payment
...
**Common business situations.** Rehydrating an order with a money value · storing a closed enum state
· migrating a legacy nullable column · keeping audit columns out of a domain aggregate.
...
The paths below are existing reference shapes. They are evidence to inspect, not permission to copy a
feature-specific name into a new domain model.

| Code | Anchor | What to look for |
|---|---|---|
| `DOMAIN-1` | `src/features/api/core/graphql/mutations/courses/course-enroll/course-enroll.handler.ts` | The enrollment decision and the aggregate facts it changes; identify which invariant is local and which is currently outside the model |
| `DOMAIN-2` | `src/features/api/core/graphql/mutations/courses/course-enroll/graphql-types/request.ts` | Request values whose business unit or format must become a domain value before the handler makes a decision |
| `DOMAIN-3` | `src/modules/databases/postgresql/primary/entities/enrollment.entity.ts` | Persisted enrollment state and whether a lifecycle is represented as one closed state or several writable flags |
| `DOMAIN-4` | `src/features/api/core/graphql/mutations/courses/courses-checkout/courses-checkout.handler.ts` | A decision that reads course, enrollment and payment facts; separate cross-aggregate policy from orchestration and persistence |
| `DOMAIN-5` | `src/modules/databases/postgresql/primary/entities/course.entity.ts` | ORM representation at the database edge; compare it with the domain shape rather than treating the entity as the domain object |
...
Accepted shape: enrolling a learner must check the learner, course and payment state; a course seat
cannot be consumed twice; payment persistence is transactional.

`DOMAIN-1` keeps the “seat can be consumed once” invariant on the course aggregate. `DOMAIN-4` puts
the eligibility decision that needs learner and course state in a stateless domain service.
`DOMAIN-5` maps the aggregate to enrollment and course rows. The transaction and manager belong to
`data-access`; the GraphQL resolver belongs to `transport`; none of those facts changes the domain
ownership decisions.

## Scope

This pattern governs domain ownership and representation in any TypeScript backend of this stack. It
does not govern static narrowing, external request placement, database handles, transactions, query
shape, exception identity or maintainability mechanics. Those concerns must be routed to their own
modules rather than smuggled into a domain rule.

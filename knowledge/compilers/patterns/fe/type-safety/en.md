...
**Common business situations.** A REST response body · the decoded payload of a token · a record read
from `localStorage` · a vendor type declared wrongly · a third-party library event · seed data pushed
into a function with a closed type.
...
**Common business situations.** A wrapper around an HTTP client · a handler taking a webhook payload ·
an adapter translating vendor data · a long-shared "util" function that lost its types · placeholder
props on a freshly built component.
...
**Common business situations.** A component's data type · a query result · a handler's rest parameter ·
a union containing nested arrays · a contract's readonly type.
...
**Common business situations.** Faking a transport link operation · building a response missing a
required field · forcing an `undefined` into a slot declared `Error` to see the defensive branch run ·
mocking a vendor module out of minimal fragments · building an invalid state to exercise a guard.
...
**Common business situations.** The implementation of an overloaded factory · public claims decoded
from a token · a vendor `.d.ts` missing a field the runtime always sends · a literal that must keep its
narrow type · a branded type constructed at exactly one checked place.
...
A law that cannot be pointed at in real code is a proposal. Paths are relative to the front-end source
tree.
...
**Accepted shape.** A session surface refreshes itself from a token the browser already holds, reads a
repository snapshot back from a sandbox service, exposes its props through an overloaded factory, and
is guarded by a test that proves the transport link refuses a malformed operation.

The shape states that these values arrive and where they are consumed. It does **not** state what the
token's claims contain, what the snapshot's fields are called, or whether anything validates them at
runtime — so it does not resolve those. Contract content belongs to the contract and props modules;
runtime validation belongs to nothing in this module. What the shape does resolve is the moment each
file is allowed to tell the compiler to stop looking.
...
This rule holds for any code of this kind in this stack: any front-end file that turns type checking
off inside one source tree. It names no single feature, no product and no library brand. Anchors cite
real paths because a pattern module owes a place to check; every worked example is ordinary TSX or TS
against placeholder modules.

It does not govern what a type should contain — that belongs to the contract and props modules — and it
does not govern runtime validation. It governs only the moment the compiler is told to stop looking.

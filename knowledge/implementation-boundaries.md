...
Implementation realizes the approved contracts without reopening product design or reconstructing package-owned lower tiers.
...
A repair is in-boundary only when it fixes code, accessibility, data wiring, or rendering while preserving approved structure and source ownership. A change to page boundaries, block responsibility, responsive strategy, persistent behavior, package anatomy, or extension policy is boundary drift and returns to the existing layout checkpoint.

...
```text
semantic tokens -> primitives -> semantic composition patterns -> layout patterns -> product blocks
```
...
| Pattern | Regions and behavior | Status |
| --- | --- | --- |
| `ExtendedNavbar` | brand + primary destinations + required compact projection + global utilities + optional subordinate section navigation | grammar-gap; candidate interface |
...
Promotion rule: a product block enters this catalog only after at least two consumers share its
semantic anatomy, or one consumer exposes a platform-level invariant such as landmarks, focus,
resize, sticky, scroll, responsive fallback, or restoration. Visual similarity alone never promotes
a block. Grammar owns anatomy and behavior; product code continues to own data, copy, permissions,
route destinations, business states, and analytics.

Newly published Core interfaces still require consumer adaptation proof before being called stable:

# Grammar Common object contracts

| Field | Value |
| --- | --- |
| Knowledge ID | `fe.grammar-common-contracts` |
| Package | `@starci/grammar/common` |
| Operators | `grammar-convergence` |
| Search tags | `object, contract, export, anatomy, variants, extension axes, invariants` |
| Dependencies | `fe.grammar-common-overview` |

Resolve every object from the exact installed public export before using it. Record package, version, integrity, export path, export name, layer, and contract hash. Never infer an export or prop from a screenshot or guide.

For each object, resolve:

| Contract part | Required evidence |
| --- | --- |
...
If a required contract part cannot be proven, classify the requirement as `grammar-gap` rather than completing it from intuition.

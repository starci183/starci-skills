...
The compiled request freezes the exact Grammar decision fingerprint, audit-plan fingerprint,
composition owner layers and authorities, iconography/media manifest fingerprints, and proof-matrix
fingerprint. Every audit check binds one frozen proof cell and owner; the audit fingerprint covers the
complete result. Emit that record through `runtime/contracts/grammar-audit.mjs`. `passed` is invalid when a

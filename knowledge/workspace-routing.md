...
| Search tags | `workspace, route, source, revision, write root, default-search partition` |
| Dependencies | `none` |

## Record

Resolve project and role to one verified checkout before reading an approved target file. Prove disk path, Git root, branch, origin, committed head, instructions, and allowed write roots. default search stores operator knowledge only; implementation opens exact approved checkout files after their paths and hashes are frozen.

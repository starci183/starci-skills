...
| Case 5 | Nửa thuần chọn giữa một block nối và Base của nó | Không bao giờ: `props.data === null ? <X /> : <XBase {...props.data} />` để nửa thuần tự quyết, theo từng lần render, xem một con có tự đi lấy dữ liệu hay không. Nửa thuần mount đúng một trong hai và luôn là cái đó; nơi cấp dữ liệu ở tầng page là `index.tsx`, nó giải dữ liệu rồi trao props cho `XBase`. Quan sát trên hai sản phẩm: bên theo luật có 0 trên 150 `component.tsx` mang lựa chọn ấy, và `index.tsx` của page render thẳng `<XBase …>` ở 46 trên 49 thư mục, re-export Base ở 1, trao cho một shell cục bộ ở 2; bên kia lặp đúng ternary ấy bốn lần trong một page |

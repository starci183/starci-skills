...
| Common renderer | Common contract | Core realization | Applicable UI rules |
| --- | --- | --- | --- |
| `GrammarRoot` | Neutral div/theme boundary | `CoreGrammarRoot` installs family scope | TONE-1, ACCESSIBILITY-4 |
...
| `Divider` | Separator with optional visible label | Core binds separator material | BOUNDARY-3 |
| `Heading` | Explicit HTML level and visual scale | Core binds typography without changing rank | FONT-1, FONT-4 |
| `Icon` | Role-based decorative or named glyph | Core binds geometry and tone | ICON-1..3 |
| `IconTile` | Stable identity/state icon plate | Core binds size/tone/radius | ACCENT-2, MEASURE-2 |
| `IconButton` | Native icon command with required label | Core binds target and focus | ACCESSIBILITY-2, ICON-5 |
| `Label` | Visible name for field/region | Core binds typography only | ACCESSIBILITY-1, FONT-3 |
| `Input` | Label, value, hint/error, validity and disabled state | Core binds field material | FIELD-1..4, FEEDBACK-1 |
| `Progress` | Named measurable value 0..100 | Core binds track/fill | ACCESSIBILITY-3, ACCENT-4 |
| `Text` | Semantic element, size/tone/weight/live/skeleton | Core binds type; missing standalone recipes remain Common gaps | FONT-1..5, RENDER-TRUTH-1 |
...
| `MediaFrame` | Aspect/fit/treatment/caption/frame | Core binds family material; loading/error prop is a Common gap | MEDIA-1..6 |
| `IncludedMark` | Decorative included-offering mark | Core binds 20px geometry; never claims completion | ICON-1, RENDER-TRUTH-1 |
| `RankArtwork` | Purpose-named decorative rank art | Core presents; feature resolves truth | MEDIA-1, RENDER-TRUTH-2 |
| `SurfaceCopyGroup` | Compact title/explanation rhythm | Core binds rhythm; exact 4px recipe is a Common gap | FONT-3, GAP-1 |
...
| `StateMark` | Compact presentation-state carrier | Core maps state treatment | STATE-1, TONE-4 |
| `LeadingNumber` | Ordered leading number anatomy | Core keeps it visually quiet | HIERARCHY-3 |
| `OtpInput` | Grouped one-time-code field | Core binds field geometry | FIELD-1..4 |
| `StaticStateRow` | Non-command row for one verified state | Core maps presentation state | RENDER-TRUTH-2, STATE-1 |
...
| `SurfaceCard` | Label/state/depth/frame/scroll/composition/whole action | Core paints labelled cards as one label-inside material box | SURFACE-1..5, BOUNDARY-1..2 |
| `SurfaceListCard` | Labelled repeated-peer collection | Core binds shared seams/material | SURFACE-3, GAP-3 |
| `FencedCodeBlock` | Semantic code boundary and overflow | Core binds code material | MEASURE-4, BOUNDARY-1 |
| `MarkdownArticle` | Semantic prose rhythm/reading owner | Core binds reading measure/type | FONT-1, MARGIN-1 |
| `MarkdownTableFrame` | Reachable table overflow frame | Core binds boundary/scroll | MEASURE-4, SURFACE-3 |
...
| `Tabs` | Controlled persistent peer-view selection | Core binds indicator/overflow/focus | STATE-2, CONTROL-STATE-3 |
| `Tooltip` | Supplementary description for an existing control | Core styles it; never replaces the accessible name | ACCESSIBILITY-2, ICON-5 |

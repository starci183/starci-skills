...
| `Divider` | Separator with optional visible label | Core binds separator material | CORE-BOUNDARY-3 |
...
| `IconButton` | Native icon command with required label | Core binds target and focus | A11Y-2, ICON-5 |
...
| `IncludedMark` | Decorative included-offering mark | Core binds 20px geometry; never claims completion | ICON-1, TRUTH-1 |
| `RankArtwork` | Purpose-named decorative rank art | Core presents; feature resolves truth | MEDIA-1, TRUTH-2 |
...
| `SurfaceCard` | Label/state/depth/frame/scroll/composition/whole action | Core paints labelled cards as one label-inside material box | CORE-SURFACE-1..5, CORE-BOUNDARY-1..2 |
| `SurfaceListCard` | Labelled repeated-peer collection | Core binds shared seams/material | CORE-SURFACE-3, GAP-3 |
| `FencedCodeBlock` | Semantic code boundary and overflow | Core binds code material | MEASURE-4, CORE-BOUNDARY-1 |
| `MarkdownArticle` | Semantic prose rhythm/reading owner | Core binds reading measure/type | FONT-1, MARGIN-1 |
| `MarkdownTableFrame` | Reachable table overflow frame | Core binds boundary/scroll | MEASURE-4, CORE-SURFACE-3 |
...
| `Tooltip` | Supplementary description for an existing control | Core styles it; never replaces the accessible name | A11Y-2, ICON-5 |

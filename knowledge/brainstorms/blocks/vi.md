...
|---|---|---|---|
| `@schema` | `knowledge/brainstorms/blocks/schema.json` | file | kiểm tra hình dạng JSON của bản ghi |
| `@validate-artifact` | `scripts/validate-artifact.mjs` | script | kiểm tra và băm artifact ứng viên |
| `@frontend-quality` | `knowledge/brainstorms/frontend-quality/vi.md` | vi | phản biện mỗi direction bằng review craft, UX, accessibility, engineering và detector dùng chung |

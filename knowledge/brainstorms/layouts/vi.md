...
|---|---|---|---|
| `@archetypes` | `knowledge/archetypes` | module | macro page topology dùng chung cùng full responsive contract trước product Grammar và Principles |
| `@contract-search` | `scripts/contract-search.mjs` | script | tìm contract entry theo nhu cầu đã nêu |
| `@schema` | `knowledge/brainstorms/layouts/schema.json` | file | kiểm tra hình dạng JSON của bản ghi |
| `@validate-artifact` | `scripts/validate-artifact.mjs` | script | kiểm tra và băm artifact ứng viên |
| `@frontend-quality` | `knowledge/brainstorms/frontend-quality/vi.md` | vi | phản biện mọi joined direction qua lens craft, UX, accessibility, engineering và detector tích hợp |

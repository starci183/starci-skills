...
| Field | Value |
| --- | --- |
| Knowledge ID | `fe.ui-routing-index` |
| Operators | `fe/request-compile, fe/direction-generate, fe/source-apply, fe/capture-preflight, fe/render-capture, fe/visual-fidelity` |
...
`knowledge/ui.md` remains the single `fe.ui` authority. Load this routing index after that authority,
then load every law record that can affect the frozen
...
| dominant versus alternative direction evidence and product-family benchmarks | [`direction-quality.md`](direction-quality.md) |
...
pass. `FAIL` is any direct contradiction. A finite choice is a machine-owned direction wait, never a
verdict. `BLOCKED` means required product authority or runtime evidence cannot be obtained.

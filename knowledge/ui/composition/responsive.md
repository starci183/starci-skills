...
| Case 2 | `ChatWorkspace` switches its rail between persistent and drawer | The composition owns the switch; the `.work` specification passes controlled state and callbacks only, and declares no listener of its own |
...
| Case 2 | Copy grows because it was translated or because a state added words | The `.work` specification names that growth inside the fit decision rather than as an exception to it |
...
| Case 5 | A state expands the surface, as an error, an expanded panel, or an overlay does | The expanded form is decided in the same `.work` specification entry as the resting one |
...
[CTA](cta.md). What the `.work` specification must enumerate about these branches is [Coverage](coverage.md).

...
playbook in `@knowledge/grammars/<family>`, and this rule only asserts that the receipt says enough
...
| Case 3 | The direction declares regions | `coverage.regions[]` is non-empty and covers every entry in `regionModel`, and each entry names one idiom in `@knowledge/grammars/<family>/playbook.md` and one published composition; no region resolves to a bare arrangement |

...
| Case 2 | The command participates in a form | The `.work` specification names the chosen `Button.type`, so submit behaviour is stated rather than inherited from the enclosing form |
| Case 3 | The direction wants the whole row or container to be clickable too | The `.work` specification names one owner for that activation; no clickable wrapper encloses a published command that owns the same effect |
...
| Case 4 | The direction is tempted to swap in `isDisabled` for the duration | The `.work` specification binds `isPending`, not `isDisabled`, for accepted work; disabled appears only where the work cannot start |
| Case 5 | The owner that needs pending publishes no pending prop | A `GRAMMAR_REQUIRED` gap naming that owner exists in the `.work` specification, and no application-level pending substitute appears in the tree |
...
after the action settles is [Feedback](feedback.md). What the `.work` specification must enumerate about these

...
| Case | When | Assert |
| --- | --- | --- |
| Case 1 | Peer features, rows, or sections need a compact anchor for scanning | `IconTile tone="accent"`, or `Icon` in its declared role, carries it, with a reviewed semantic glyph named in the `.work` specification |
...
| Case 2 | A final destructive CTA is required | A typed danger action treatment is required first, and while `ButtonVariant` publishes no danger value the `.work` specification records that gap |

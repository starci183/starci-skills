...
| Case 8 | The matrix judged leaves out a state the direction's coverage declares | The lens is `blocked`, not `ship` or `fix-first`, until the matrix covers every declared state; a mean computed over a narrower state set than a prior round's is not comparable to that round's mean, so a narrowed round neither closes the loop nor exhausts a round budget on that comparison |

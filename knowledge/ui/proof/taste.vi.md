...
| Case 8 | Matrix được phán quyết bỏ sót một state mà coverage của direction đã khai | Ống kính là `blocked`, không phải `ship` hay `fix-first`, cho tới khi matrix phủ hết mọi state đã khai; một điểm trung bình tính trên một tập state hẹp hơn vòng trước không thể so được với điểm trung bình của vòng ấy, nên một vòng bị thu hẹp không đóng được vòng lặp cũng không dùng hết ngân sách vòng trên phép so sánh đó |

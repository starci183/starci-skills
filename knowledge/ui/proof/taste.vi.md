...
| Case 8 | Phạm vi audit chọn bề mặt chính hoặc chủ động yêu cầu toàn bộ ma trận state | Đọc phạm vi đóng băng do `frontend.surface.audit` mang theo. Thiếu entry đã chọn vẫn bị chặn. State phụ đã khai ngoài phạm vi chính là `deferred`, không phải đạt: tiêu chí so state mà schema phạm vi cho phép mang `verdict: deferred`, score null và tên state hoãn, được loại khỏi điểm trung bình. Audit exhaustive thiếu state đã khai vẫn bị chặn. Hoàn thành phạm vi chỉ chứng minh bề mặt đã chọn; không so khác phạm vi để dùng ngân sách vòng |

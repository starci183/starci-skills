...
|---|---|---|---|
| `@config-schema` | `knowledge/contexts/workspaces/config.schema.json` | file | validate Source-wide defaults shared by every project and role |
| `@schema` | `knowledge/contexts/workspaces/schema.json` | file | validate the record's JSON shape |
...
7. **Resolve grammar explicitly.** When `context.grammar` is non-null, verify
   `knowledge/grammars/<grammar>/grammar.json` and the declared profile before design — `WORKSPACE-8`.

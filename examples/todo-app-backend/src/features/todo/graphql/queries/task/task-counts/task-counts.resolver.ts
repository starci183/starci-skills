...
import { TaskCountsQuery, TaskCountsQueryResult } from '@modules/bussiness/task';
...
import { TaskCountsResponse } from './graphql-types/response';
...
export class TaskCountsResolver {
  ...
  async taskCounts(@Context('req') req: GraphqlRequestLike): Promise<TaskCountsResponse> {
    ...
    const result = await this.queryBus.execute<TaskCountsQuery, TaskCountsQueryResult>(
      new TaskCountsQuery({ ownerId }),
    );
    return new TaskCountsResponse(result.open, result.complete);
  }
}

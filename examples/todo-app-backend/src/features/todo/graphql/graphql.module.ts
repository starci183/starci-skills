 ...
 * body - clients keyed on `TASK_FORBIDDEN`/`SESSION_EXPIRED`/etc. still see that code, just relocated.
 */
@Module({
  imports: [
    NestGraphQLModule.forRoot<ApolloDriverConfig>({
      ...
      playground: false,
      introspection: true,
      context: (ctx: GraphqlContextShape) => ({ req: ctx.req, res: ctx.res }),
      formatError: (formatted: GraphQLFormattedError, error: unknown): GraphQLFormattedError => {
        ...
        if (original instanceof AbstractException) {
          return { ...formatted, message: original.message, extensions: { ...formatted.extensions, code: original.code } };
        }
        return formatted;
      },
    }),

...
import { NotifyQueuePort } from './notify-queue.contracts';
import { encodeCommand, tryParse } from './resp';
...
export class NotifyQueueClient extends NotifyQueuePort {
  ...
  async enqueue(jobId: string, dueAtMs: number): Promise<void> {
    await this.run(['ZADD', QUEUE_KEY, String(dueAtMs), jobId]);
  }

  async dequeueDue(nowMs: number): Promise<string[]> {
    const result = await this.run(['EVAL', DEQUEUE_DUE_SCRIPT, '1', QUEUE_KEY, String(nowMs)]);
    return Array.isArray(result) ? (result as string[]) : [];
  }

  private async run(args: string[]): Promise<unknown> {
    const { hostname, port } = new URL(this.config.getRedisUrl());
    const socket = await connect(hostname || 'localhost', Number(port) || 6379);
    try {
      return await sendCommand(socket, args);
    } finally {
      socket.destroy();
    }
  }
...
function sendCommand(socket: Socket, args: string[]): Promise<unknown> {
  return new Promise((resolve, reject) => {
    let buffer = '';
    const timer = setTimeout(() => {
    ...
    const onData = (chunk: Buffer) => {
      ...
      const result = tryParse(buffer, 0);
    ...
    socket.write(encodeCommand(args), 'binary');
  });
}

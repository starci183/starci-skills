...
export function tryParse(buf: string, offset: number): ParseResult | null {
  ...
  const head = buf.slice(offset + 1, eol);
  ...
  switch (type) {
    ...
    case '-':
      return { value: new Error(head), next: afterHead };
    case ':':
      return { value: Number.parseInt(head, 10), next: afterHead };
    case '$': {
      const len = Number.parseInt(head, 10);
      if (len === -1) return { value: null, next: afterHead };
      if (buf.length < afterHead + len + 2) return null;
      return { value: buf.slice(afterHead, afterHead + len), next: afterHead + len + 2 };
    }
    case '*': {
      ...
      for (let i = 0; i < count; i += 1) {
        const result = tryParse(buf, cursor);

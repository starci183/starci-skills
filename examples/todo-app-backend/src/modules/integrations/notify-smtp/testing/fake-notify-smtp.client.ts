...
export class FakeNotifySmtpClient extends NotifySmtpPort {
  readonly sent: NotifySmtpMessage[] = [];
  ...
  async send(message: NotifySmtpMessage): Promise<void> {
    if (this.permanentFor.has(message.to)) {
      throw new NotifySmtpPermanentRejectionException({ reason: 'fake: permanently rejected' });
    }
    if (this.transientFor.has(message.to)) {
      throw new NotifySmtpTransientFailureException({ reason: 'fake: transient failure' });
    }

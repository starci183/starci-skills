...
import { InjectPrimaryEntityManager, AuditErasureRequestEntity } from '../../platform/databases/postgresql/primary';
import {
...
import { AuditErasureRequestRecord } from './types/audit-erasure-request-record';
...
export class AuditErasureService {
  ...
  async execute(requestId: string, callerId: string): Promise<AuditErasureRequestRecord> {
    ...
    if (row.state !== 'verified') {
      throw new ErasureRequestInvalidStateException({ requestId, state: row.state, expected: 'verified' });
    }
    ...
    if (stillReadable) {
      throw new ErasureNotConfirmedException({ requestId });
    }
  ...
  private async tRequest(personId: string): Promise<AuditErasureRequestRecord> {
    ...
    await this.logService.append(SYSTEM_ACTOR_ID, ACTION_ERASURE_REQUESTED, requestId);
  ...
  private async tVerify(requestId: string, callerId: string): Promise<AuditErasureRequestRecord> {
    ...
    if (row.state !== 'requested') {
      throw new ErasureRequestInvalidStateException({ requestId, state: row.state, expected: 'requested' });
    }
    ...
    row.state = 'verified';
    const saved = await this.entityManager.save(AuditErasureRequestEntity, row);
    return toRecord(saved);
  }
  ...
  private async tExecute(row: AuditErasureRequestEntity): Promise<void> {
    ...
    row.state = 'executing';
    await this.entityManager.save(AuditErasureRequestEntity, row);
  ...
  private async tComplete(row: AuditErasureRequestEntity): Promise<AuditErasureRequestRecord> {
    ...
    row.personId = null;
    const saved = await this.entityManager.save(AuditErasureRequestEntity, row);
    await this.logService.append(SYSTEM_ACTOR_ID, ACTION_ERASURE_COMPLETED, row.requestId);
    return toRecord(saved);
  }

  private async findRow(requestId: string): Promise<AuditErasureRequestEntity> {
    const row = await this.entityManager.findOneBy(AuditErasureRequestEntity, { requestId });
    if (!row) {
      throw new ErasureRequestNotFoundException({ requestId });
    }
    return row;
  }
}

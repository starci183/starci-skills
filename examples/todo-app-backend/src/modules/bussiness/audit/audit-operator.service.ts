...
import { ActorClaims, isOperatorRead } from './audit-operator-read';
...
export class AuditOperatorService {
  ...
  constructor(@Optional() subjects?: readonly string[]) {
  ...
  static subjectsFromEnv(): readonly string[] {
  ...
  claimFor(personId: string): ActorClaims {
    return { personId, role: this.subjects.has(personId) ? 'operator' : undefined };
  }

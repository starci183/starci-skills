...
describe('AuditOperatorService (decision.audit.operator-role)', () => {
  ...
  it('is fail-closed with no roster: nobody resolves to an operator', () => {
    const service = new AuditOperatorService();
  ...
  it('grants the operator claim only to a subject the roster names', () => {
    const service = new AuditOperatorService(['op-1', 'op-2']);
    expect(service.claimFor('op-1')).toEqual({ personId: 'op-1', role: 'operator' });
  ...
  it('reads the deployment roster from the environment, trimming and dropping blanks', () => {
    ...
    expect([...AuditOperatorService.subjectsFromEnv()]).toEqual(['op-1', 'op-2']);
    const service = new AuditOperatorService();
    expect(service.isOperator('op-2')).toBe(true);
    expect(service.isOperator('someone-else')).toBe(false);
  });

  it('resolves through Nest dependency injection, falling back to the environment roster (fail-closed when unset)', async () => {
    ...
    const moduleRef = await Test.createTestingModule({ providers: [AuditOperatorService] }).compile();
    try {

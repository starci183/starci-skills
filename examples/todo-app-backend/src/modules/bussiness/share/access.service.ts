...
export class AccessService {
  ...
  mayComplete(actorId: string, task: AccessSubjectTask): AccessDecision {
    ...
    if (this.cache.roleOf(task.id, actorId).role === 'editor') {
      return { allowed: true };
    }
    return { allowed: false, reason: 'not the owner and not an accepted editor collaborator' };
  }
  ...
  mayDelete(actorId: string, task: AccessSubjectTask): AccessDecision {
    ...
    return { allowed: false, reason: 'delete is owner-only; no role sharing grants ever widens it' };
  }
}

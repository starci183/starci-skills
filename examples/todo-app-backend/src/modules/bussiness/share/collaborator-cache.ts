...
import { ShareRole } from './types/invitation-record';
...
export class CollaboratorCache {
  ...
  set(taskId: string, personId: string, role: ShareRole): void {
    this.roles.set(this.key(taskId, personId), role);
  }

  delete(taskId: string, personId: string): void {
    this.roles.delete(this.key(taskId, personId));
  }

  roleOf(taskId: string, personId: string): CollaboratorLookup {
    return { role: this.roles.get(this.key(taskId, personId)) ?? null };
  }
}

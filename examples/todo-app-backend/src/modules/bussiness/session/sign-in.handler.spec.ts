...
describe('SignInHandler', () => {
  ...
  it('ac.login.password.sign-in.wrong-pair-is-refused: a known email with a wrong password is refused and no session is created', async () => {
    await expect(handler.execute(new SignInCommand({ email: 'person@example.com', password: 'wrong-password' }))).rejects.toMatchObject({
      code: 'INVALID_CREDENTIALS',
    });
  });

  it('br.login.password.sign-in: a sign-in succeeds only when Keycloak accepts the pair', async () => {
    const result = await handler.execute(new SignInCommand({ email: 'person@example.com', password: 'correct-horse' }));
  ...
  it('ac.login.password.sign-in.refusal-does-not-name-the-half: an unknown email and a wrong password carry the same refusal', async () => {
    ...
    try {
      await handler.execute(new SignInCommand({ email: 'nobody@example.com', password: 'anything' }));
    } catch (error) {
      unknownEmailError = error;
    }
    try {
      await handler.execute(new SignInCommand({ email: 'person@example.com', password: 'wrong-password' }));
    } catch (error) {
  ...
  it('event.login.signed-in: publishes on the PlatformEventBus after a successful sign-in', async () => {
    ...
    const keycloakClient = new FakeKeycloakClient({ 'person@example.com': 'correct-horse' });
    const withEvents = new SignInHandler(keycloakClient, sessionService, events);

    const result = await withEvents.execute(new SignInCommand({ email: 'person@example.com', password: 'correct-horse' }));
  ...
  it('a refused sign-in publishes nothing', async () => {
    ...
    await expect(withEvents.execute(new SignInCommand({ email: 'person@example.com', password: 'wrong' }))).rejects.toMatchObject({
      code: 'INVALID_CREDENTIALS',
    });

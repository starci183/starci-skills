...
import { TaskCreationPolicy, TaskService } from '@modules/bussiness/task';
import { PlanCapExceededException } from '@modules/shared/exceptions';
...
export class PlanCapGuardPolicy extends TaskCreationPolicy {
  ...
  async assertMayCreate(principal: CapGuardPrincipal): Promise<void> {
    ...
    if (activeCount >= plan.taskCap) {
      ...
      throw new PlanCapExceededException({ cap: plan.taskCap, upgradePath: UPGRADE_PATH });
    }

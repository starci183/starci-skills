...
import { FREE_PLAN, PAID_PLAN } from './types/plan-catalog';

describe('SubscriptionService', () => {
  let service: SubscriptionService;

  beforeEach(() => {
    service = new SubscriptionService(createFakeEntityManager<SubscriptionEntity>('id') as never);
  });

  it('data.plan.subscription: a person with no row yet is lazily given exactly one, starting free', async () => {
  ...
  it('sds.plan.subscription-lifecycle t-checkout-started: free -> pending', async () => {
    const record = await service.getOrCreate('person-1');
  ...
  it('sds.plan.subscription-lifecycle t-gateway-confirmed: pending -> active, periodEnd set, plan paid', async () => {
    const record = await service.getOrCreate('person-1');
    await service.tCheckoutStarted('person-1');
    const periodEnd = new Date('2027-01-01T00:00:00.000Z');
    const confirmed = await service.tGatewayConfirmed(record.id, periodEnd);
    expect(confirmed.status).toBe('active');
    expect(confirmed.plan).toBe('paid');
    expect(confirmed.periodEnd).toEqual(periodEnd);
  });

  it('sds.plan.subscription-lifecycle t-gateway-abandoned: pending -> free, periodEnd cleared', async () => {
    const record = await service.getOrCreate('person-1');
    await service.tCheckoutStarted('person-1');
  ...
  it('sds.plan.subscription-lifecycle t-renewal-due then t-lapse: active -> past-due -> lapsed', async () => {
    const record = await service.getOrCreate('person-1');
    await service.tCheckoutStarted('person-1');
    await service.tGatewayConfirmed(record.id, new Date());
  ...
  it('sds.plan.subscription-lifecycle t-renewal-confirmed: past-due -> active, periodEnd extended', async () => {
    const record = await service.getOrCreate('person-1');
    await service.tCheckoutStarted('person-1');
    await service.tGatewayConfirmed(record.id, new Date('2027-01-01T00:00:00.000Z'));
    await service.tRenewalDue(record.id);
    const renewed = await service.tRenewalConfirmed(record.id, new Date('2027-02-01T00:00:00.000Z'));
    expect(renewed.status).toBe('active');
    expect(renewed.periodEnd).toEqual(new Date('2027-02-01T00:00:00.000Z'));
  });

  it('br.plan.lapse.reverts-on-read / ac.plan.lapse.reverts-on-read.read-after-grace-is-free: a lapsed row reads as free without writing to it', async () => {
    const record = await service.getOrCreate('person-1');
    await service.tCheckoutStarted('person-1');
    await service.tGatewayConfirmed(record.id, new Date());
    await service.tRenewalDue(record.id);
  ...
  it('t-revert-on-read: active and past-due both read as the paid plan', async () => {
    const record = await service.getOrCreate('person-1');
    await service.tCheckoutStarted('person-1');
    await service.tGatewayConfirmed(record.id, new Date());
  ...
  it('br.plan.downgrade.freeze / decision.plan.downgrade.policy t-downgrade: active -> free immediately, never refused', async () => {
    const record = await service.getOrCreate('person-1');
    await service.tCheckoutStarted('person-1');
    await service.tGatewayConfirmed(record.id, new Date());
  ...
  it('t-downgrade from past-due also lands on free', async () => {
    const record = await service.getOrCreate('person-1');
    await service.tCheckoutStarted('person-1');
    await service.tGatewayConfirmed(record.id, new Date());
    await service.tRenewalDue(record.id);
  ...
  it('t-downgrade on an already-free person is a no-op, not an error', async () => {

...
import { SepayClient, SepayCreateIntentParams, SepayCreateIntentResult } from '../../integrations/sepay';
...
class FakeSepayClient extends SepayClient {
  public createIntentCalls: SepayCreateIntentParams[] = [];
  ...
  async createIntent(params: SepayCreateIntentParams): Promise<SepayCreateIntentResult> {
    ...
    return { gatewayIntentId: `fake-gw-${params.subscriptionId}`, checkoutUrl: `https://my.sepay.vn/qr/fake-${params.subscriptionId}` };
  }
}
...
describe('UpgradePlanHandler', () => {
  ...
  beforeEach(() => {
    ...
    handler = new UpgradePlanHandler(subscriptionService, paymentService, sepayClient, new AppConfigService());
  });

  it('fr.plan.upgrade mainFlow t-checkout-started: the subscription becomes pending and a payment intent is created', async () => {
    const result = await handler.execute(new UpgradePlanCommand({ ownerId: 'owner-1' }));
  ...
  it('data.plan.payment-intent: the created intent carries the catalog price and currency', async () => {
    const result = await handler.execute(new UpgradePlanCommand({ ownerId: 'owner-1' }));
    const paymentIntent = await paymentService.findById(result.paymentIntentId);
    expect(paymentIntent.amount).toBe(new AppConfigService().getPaidPlanPriceMinorUnits());

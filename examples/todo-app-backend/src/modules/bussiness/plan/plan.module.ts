...
import { TaskModule, TaskCreationPolicyRegistry } from '@modules/bussiness/task';
import { SepayModule } from '../../integrations/sepay';
import { ConfigurableModuleClass, OPTIONS_TYPE } from './plan.module-definition';
 ...
 * `PostgresqlPrimaryModule`/`ConfigModule`/`PlatformEventsModule` are not imported here: all three are
 * registered globally from `app.module.ts`.
 */
@Module({})
export class PlanModule extends ConfigurableModuleClass implements OnModuleInit {
  ...
  static register(options: typeof OPTIONS_TYPE = {}): DynamicModule {
    ...
    return {
      ...base,
      imports: [CqrsModule, TaskModule.register(), SepayModule.register()],
      providers: [
        ...(base.providers ?? []),
        SubscriptionService,
        PaymentService,

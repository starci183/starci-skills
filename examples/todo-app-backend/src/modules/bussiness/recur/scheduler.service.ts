...
export class SchedulerService implements OnModuleInit {
  ...
  onModuleInit(): void {
    ...
    this.schedulerRegistry.addCronJob('recur-generation-tick', job);
    job.start();
  }
  ...
  async onTick(): Promise<void> {
    ...
    if (summary.materialised.length > 0) {
      this.logger.log(`integration.recur.scheduler tick materialised ${summary.materialised.length} occurrence(s)`);
    }
  }
}

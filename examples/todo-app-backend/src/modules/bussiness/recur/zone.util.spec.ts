import { resolveRuleInstant, localDateInZone } from './zone.util';
...
describe('zone.util (sds.recur.generation-engine, br.recur.timezone.owner-local-time)', () => {
  ...
  it('exactly one instant is produced for the fold window - no second, later candidate is silently generated', () => {
    const first = resolveRuleInstant('America/New_York', '2024-11-03', '01:30');
    const second = resolveRuleInstant('America/New_York', '2024-11-03', '01:30');
  ...
  it('ac.recur.timezone.owner-local-time.distinct-zones-differ: Asia/Ho_Chi_Minh 09:00 keeps the same +07:00 offset across a DST-observing date change', () => {
    const january = resolveRuleInstant('Asia/Ho_Chi_Minh', '2026-01-15', '09:00');
    const july = resolveRuleInstant('Asia/Ho_Chi_Minh', '2026-07-15', '09:00');
  ...
  it('ac.recur.timezone.owner-local-time.distinct-zones-differ: Europe/Berlin 09:00 shifts offset between CET and CEST', () => {
    const january = resolveRuleInstant('Europe/Berlin', '2026-01-15', '09:00');
    const july = resolveRuleInstant('Europe/Berlin', '2026-07-15', '09:00');
  ...
  it('ac.recur.timezone.owner-local-time.distinct-zones-differ: the same nominal 09:00 differs between Ho_Chi_Minh and Berlin on both dates', () => {
    const hcmJan = resolveRuleInstant('Asia/Ho_Chi_Minh', '2026-01-15', '09:00').instant.toISOString();
    const berlinJan = resolveRuleInstant('Europe/Berlin', '2026-01-15', '09:00').instant.toISOString();
    const hcmJul = resolveRuleInstant('Asia/Ho_Chi_Minh', '2026-07-15', '09:00').instant.toISOString();
    const berlinJul = resolveRuleInstant('Europe/Berlin', '2026-07-15', '09:00').instant.toISOString();
  ...
  it('a nominal time safely before a gap-day transition resolves normally, using the pre-transition offset', () => {
    const resolution = resolveRuleInstant('Europe/Berlin', '2026-03-29', '01:00');
  ...
  it('a nominal time safely after a gap-day transition resolves normally, using the post-transition offset', () => {
    const resolution = resolveRuleInstant('Europe/Berlin', '2026-03-29', '10:00');
    expect(resolution.kind).toBe('normal');
  ...
  it('localDateInZone reads "today" from the rule\'s own zone, not the host clock', () => {
    ...
    expect(localDateInZone('Asia/Ho_Chi_Minh', new Date('2026-03-29T23:30:00.000Z'))).toBe('2026-03-30');
    expect(localDateInZone('Europe/Berlin', new Date('2026-03-29T23:30:00.000Z'))).toBe('2026-03-30');
    expect(localDateInZone('America/New_York', new Date('2026-03-29T23:30:00.000Z'))).toBe('2026-03-29');
  });
});

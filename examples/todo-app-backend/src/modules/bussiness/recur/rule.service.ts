...
import { InjectPrimaryEntityManager, RuleEntity } from '../../platform/databases/postgresql/primary';
import { RecurRuleForbiddenException, RecurRuleInvalidException, RecurRuleNotFoundException } from '@modules/shared/exceptions';
...
function assertValidFrequencyShape(frequency: RuleFrequency, n: number | null | undefined, dayOfMonth: number | null | undefined): void {
  if (frequency === 'every-n-days') {
    if (typeof n !== 'number' || !Number.isInteger(n) || n <= 0) {
      throw new RecurRuleInvalidException({ reason: 'n is required and must be a positive integer for every-n-days' });
    }
    if (dayOfMonth !== null && dayOfMonth !== undefined) {
      throw new RecurRuleInvalidException({ reason: 'dayOfMonth must be absent for every-n-days' });
    }
  } else if (frequency === 'monthly-day') {
    if (typeof dayOfMonth !== 'number' || !Number.isInteger(dayOfMonth) || dayOfMonth < 1 || dayOfMonth > 31) {
      throw new RecurRuleInvalidException({ reason: 'dayOfMonth is required and must be between 1 and 31 for monthly-day' });
    }
    if (n !== null && n !== undefined) {
      throw new RecurRuleInvalidException({ reason: 'n must be absent for monthly-day' });
    }
  } else if (frequency === 'every-weekday') {
    if (n !== null && n !== undefined) {
      throw new RecurRuleInvalidException({ reason: 'n must be absent for every-weekday' });
    }
    if (dayOfMonth !== null && dayOfMonth !== undefined) {
      throw new RecurRuleInvalidException({ reason: 'dayOfMonth must be absent for every-weekday' });
    }
  } else {
    throw new RecurRuleInvalidException({ reason: `unknown frequency: ${String(frequency)}` });
  }
}
...
export class RuleService {
  ...
  async create(input: CreateRuleInput): Promise<RuleRecord> {
    assertValidFrequencyShape(input.frequency, input.n ?? null, input.dayOfMonth ?? null);
  ...
  async findById(id: string): Promise<RuleRecord> {
    const row = await this.entityManager.findOneBy(RuleEntity, { id });
    if (!row) {
      throw new RecurRuleNotFoundException({ ruleId: id });
    }
    return toRecord(row);
  }

  async listOwnedBy(owner: string): Promise<RuleRecord[]> {
    const rows = await this.entityManager.findBy(RuleEntity, { owner });
    return rows.map(toRecord);
  }

  async listActive(): Promise<RuleRecord[]> {
  ...
  async edit(id: string, actorId: string, patch: EditRuleInput): Promise<RuleRecord> {
    const row = await this.findRowForOwner(id, actorId);
    ...
    assertValidFrequencyShape(nextFrequency, nextN, nextDayOfMonth);
    ...
    const saved = await this.entityManager.save(RuleEntity, row);
    return toRecord(saved);
  }
  ...
  private async findRowForOwner(id: string, actorId: string): Promise<RuleEntity> {
    ...
    if (row.owner !== actorId) {
      throw new RecurRuleForbiddenException({ ruleId: id, actorId });
    }
    return row;
  }
}

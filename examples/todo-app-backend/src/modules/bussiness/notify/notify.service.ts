...
import { NotifySmtpMessage } from '../../integrations/notify-smtp';
import { NotifyQueuePort } from '../../integrations/notify-queue';
import { DedupeService } from './dedupe.service';
import { DigestService, DEFAULT_DIGEST_WINDOW_MINUTES } from './digest.service';
import { PreferencesService } from './preferences.service';
import { DeliveryService } from './delivery.service';
import { DeliveryState } from './types/delivery-attempt-record';
...
export class NotifyService {
  ...
  async admit(input: AdmitEventInput, now: Date = new Date()): Promise<AdmitEventResult> {
    ...
    if (!isNew) {
      ...
      return { notificationId: record.id, isNew: false, deliveryState: attempt?.state ?? 'queued' };
    }

    const preference = await this.preferences.get(input.recipientId, input.channel);
    const attempt = await this.delivery.admit(record.id, now, preference.unsubscribed);

    if (!preference.unsubscribed) {
      const windowMinutes = preference.digestWindowMinutes ?? DEFAULT_DIGEST_WINDOW_MINUTES;
      const window = await this.digest.admit(input.recipientId, input.channel, now, windowMinutes);
      await this.dedupe.assignDigestGroup(record.id, window.windowId);
      if (window.opened) {
        await this.queue.enqueue(`${FLUSH_PREFIX}${window.windowId}`, window.closesAt.getTime());
      }
    }

    return { notificationId: record.id, isNew: true, deliveryState: attempt.state };
  }
  ...
  async runDueJobs(now: Date = new Date()): Promise<void> {
    ...
    for (const jobId of jobIds) {
      if (jobId.startsWith(FLUSH_PREFIX)) {
        await this.flushWindow(jobId.slice(FLUSH_PREFIX.length), now);
      } else if (jobId.startsWith(RETRY_PREFIX)) {
        await this.dispatchGroup(jobId.slice(RETRY_PREFIX.length), now);
      }
    }
  }

  private async flushWindow(windowId: string, now: Date): Promise<void> {
    const flushed = await this.digest.flush(windowId, now);
    if (!flushed) return; // already flushed, or not actually closed yet - nothing to dispatch
    await this.dispatchGroup(windowId, now);
  }
  ...
  private async dispatchGroup(groupId: string, now: Date): Promise<void> {
    ...
    const result = await this.delivery.dispatchBatch(notifications.map(notification => notification.id), now, message);
    if (result.retried.length > 0) {
      await this.queue.enqueue(`${RETRY_PREFIX}${groupId}`, now.getTime() + RETRY_BACKOFF_MS);
    }
  }
}
...
function renderMessage(notifications: NotificationRecord[]): NotifySmtpMessage {
  const to = notifications[0].recipientId;
  if (notifications.length === 1) {
    return { to, subject: subjectFor(notifications[0]), body: lineFor(notifications[0]) };
  }
  return {
    to,

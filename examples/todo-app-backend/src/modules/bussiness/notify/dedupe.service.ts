...
import { InjectPrimaryEntityManager, NotifyNotificationEntity } from '../../platform/databases/postgresql/primary';
...
export class DedupeService {
  ...
  async admit(params: AdmitNotificationParams): Promise<AdmitNotificationResult> {
    const dedupeKey = computeDedupeKey(params.kind, params.sourceEventId, params.recipientId);
    const existing = await this.entityManager.findOneBy(NotifyNotificationEntity, { id: dedupeKey });
    if (existing) {
      return { record: toRecord(existing), isNew: false };
    }
    ...
    return { record: toRecord(saved), isNew: true };
  }

  async findById(id: string): Promise<NotificationRecord | null> {
    const row = await this.entityManager.findOneBy(NotifyNotificationEntity, { id });
   ...
   * "in admission order"). */
  async findByDigestGroup(digestGroupId: string): Promise<NotificationRecord[]> {
    const rows = await this.entityManager.findBy(NotifyNotificationEntity, { digestGroupId });
  ...
  async assignDigestGroup(id: string, digestGroupId: string): Promise<void> {
    ...
    await this.entityManager.save(NotifyNotificationEntity, row);
  }
}
...
function toRecord(row: NotifyNotificationEntity): NotificationRecord {
  return new NotificationRecord(row.id, row.kind, row.recipientId, row.payload, row.digestGroupId, row.createdAt);
}

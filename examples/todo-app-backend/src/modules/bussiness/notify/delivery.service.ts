...
import { InjectPrimaryEntityManager, NotifyDeliveryAttemptEntity } from '../../platform/databases/postgresql/primary';
import { NotifySmtpMessage, NotifySmtpPort } from '../../integrations/notify-smtp';
import { NotifySmtpPermanentRejectionException, NotifySmtpTransientFailureException } from '@modules/shared/exceptions';
import { DeliveryAttemptRecord, DeliveryHistoryEntry } from './types/delivery-attempt-record';
...
export class DeliveryService {
  ...
  async admit(notificationId: string, now: Date, unsubscribed: boolean): Promise<DeliveryAttemptRecord> {
    const history: DeliveryHistoryEntry[] = [{ state: 'queued', at: now.toISOString() }];
    if (unsubscribed) {
      history.push({ state: 'suppressed', at: now.toISOString(), failureClass: 'unsubscribed' });
  ...
  async findById(notificationId: string): Promise<DeliveryAttemptRecord | null> {
    const row = await this.entityManager.findOneBy(NotifyDeliveryAttemptEntity, { notificationId });
   ...
   */
  async dispatchBatch(notificationIds: string[], now: Date, message: NotifySmtpMessage): Promise<DispatchBatchResult> {
    const rows: NotifyDeliveryAttemptEntity[] = [];
    for (const notificationId of notificationIds) {
      const row = await this.entityManager.findOneBy(NotifyDeliveryAttemptEntity, { notificationId });
      if (row && row.state === 'queued') rows.push(row);
    }
    if (rows.length === 0) {
      return { delivered: [], retried: [], bounced: [] };
    }
    ...
    for (const row of rows) {
      ...
      row.history = [...row.history, { state: 'sending', at: now.toISOString() }];
      await this.entityManager.save(NotifyDeliveryAttemptEntity, row);
    }
    ...
    try {
      ...
      for (const row of rows) {
        row.state = 'delivered';
        row.endedAt = now;
        row.history = [...row.history, { state: 'delivered', at: now.toISOString() }];
        await this.entityManager.save(NotifyDeliveryAttemptEntity, row);
      }
      return { delivered: rows.map(row => row.notificationId), retried: [], bounced: [] };
    } catch (error) {
      if (error instanceof NotifySmtpPermanentRejectionException) {
        for (const row of rows) {
          ...
          row.endedAt = now;
          row.history = [...row.history, { state: 'bounced', at: now.toISOString(), failureClass: 'permanent-bounce' }];
          await this.entityManager.save(NotifyDeliveryAttemptEntity, row);
        }
        return { delivered: [], retried: [], bounced: rows.map(row => row.notificationId) };
      }
      if (error instanceof NotifySmtpTransientFailureException) {
        ...
        for (const row of rows) {
          if (row.attempt < RETRY_BUDGET) {
            ...
            row.history = [...row.history, { state: 'queued', at: now.toISOString(), failureClass: 'transient' }];
            retried.push(row.notificationId);
          } else {
            row.state = 'bounced';
            row.failureClass = 'retries-exhausted';
            row.endedAt = now;
            row.history = [...row.history, { state: 'bounced', at: now.toISOString(), failureClass: 'retries-exhausted' }];
...
function toRecord(row: NotifyDeliveryAttemptEntity): DeliveryAttemptRecord {
  ...
  return new DeliveryAttemptRecord(
    ...
    row.startedAt,
    row.endedAt,
    row.history as DeliveryHistoryEntry[],
  );
}

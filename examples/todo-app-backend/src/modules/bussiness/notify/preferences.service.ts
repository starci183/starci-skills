...
import { InjectPrimaryEntityManager, NotifyPreferenceEntity } from '../../platform/databases/postgresql/primary';
import { PreferenceRecord } from './types/preference-record';
...
export class PreferencesService {
  ...
  async get(personId: string, channel: string): Promise<PreferenceRecord> {
    const row = await this.entityManager.findOneBy(NotifyPreferenceEntity, { personId, channel });
    if (!row) {
      return new PreferenceRecord(personId, channel, false, null);
    }
    return toRecord(row);
  }

  async isUnsubscribed(personId: string, channel: string): Promise<boolean> {
    return (await this.get(personId, channel)).unsubscribed;
  }

  async setUnsubscribed(personId: string, channel: string, unsubscribed: boolean): Promise<PreferenceRecord> {
    return this.update(personId, channel, { unsubscribed });
  }

  async setDigestWindowMinutes(personId: string, channel: string, digestWindowMinutes: number | null): Promise<PreferenceRecord> {
    return this.update(personId, channel, { digestWindowMinutes });
  }
  ...
  async update(
    personId: string,
    channel: string,
    patch: { unsubscribed?: boolean; digestWindowMinutes?: number | null },
  ): Promise<PreferenceRecord> {
    const existing = await this.get(personId, channel);
...
function toRecord(row: NotifyPreferenceEntity): PreferenceRecord {
  return new PreferenceRecord(row.personId, row.channel, row.unsubscribed, row.digestWindowMinutes);
}

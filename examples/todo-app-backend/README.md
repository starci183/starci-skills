...
`scripts/check-example-*.mjs` or `checks/**`, so the example's evidence comes from a declared GitHub-hosted
...
- **records** - the runtime's own YAML loader and Work-tree layout gate over both example repositories,
  plus the fixture suite behind `scripts/check-example-work.mjs`.

 ...
 * before removing local metadata" and "no unresolved owned resources for a pass"). A spec calls this
 * once per created/deleted/verified-absent resource so the reporter can write the true ledger instead
 * of a hardcoded empty pair that cannot back a real cleanup claim.
 */

...
/**
 * The harness lives inside the frontend repository (`examples/todo-app-frontend/uat`), which is the
 ...
 * backend). Every path below is resolved from this file's own location so it holds regardless of the
 * process's current working directory.
 */
const HERE = path.dirname(fileURLToPath(import.meta.url));

/** `examples/todo-app-frontend` - the repository this harness's own tests and config live in. */
export const FRONTEND_ROOT = path.resolve(HERE, '..', '..');

/** `examples/todo-app-backend` - the sibling repository that owns `.starciwork`. */
export const BACKEND_ROOT = path.resolve(FRONTEND_ROOT, '..', 'todo-app-backend');

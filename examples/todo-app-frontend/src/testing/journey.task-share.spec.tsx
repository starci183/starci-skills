...
import { TaskSharePage } from "@/components/pages/TaskSharePage"
...
describe("ui.share.invite journey: the served share screen over the real transport", () => {
    ...
    it("names the task the route addresses and reads its invitations with the reader's own session", async () => {
        ...
        // The title names the task in the breadcrumb, under the heading and on the "back" line.
        await waitFor(() => expect(screen.getAllByText(TASK_TITLE).length).toBeGreaterThan(0))
    ...
    it.each([
        ["SHARE_INVALID_EMAIL", "not-an-email", "Enter a valid email address."],
        ["SHARE_INVITATION_ALREADY_EXISTS", COLLABORATOR, "That person is already invited."],
    ])("br.share.invite: a %s refusal is drawn on the email field, which owns it", async (code, address, sentence) => {
        wire = shareWorld({ invite: { reason: sentence, code } })
        await openScreen()
        ...
        // The dictionary names the bad address; an address that already collaborates is reported in
        // the backend's own words, because no sentence was ever written for it - and both belong on
        // the field the reader has to correct.
        expect(screen.getByLabelText("Email address")).toHaveValue(address)
    })

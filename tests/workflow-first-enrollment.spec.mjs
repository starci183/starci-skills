...
test('public retry first enrolls an approved workflow without inventing a retired event generation',t=>{
  ...
  const pinFile=path.join(root,'pin.json'),journalFile=path.join(root,'runtime','journal.sqlite');
  fs.writeFileSync(pinFile,JSON.stringify(pin));
  const orca={invoke(){throw Error('Enrollment must not launch a worker');}};
  const result=kernelMain('workflow-retry',{id:state.id,'runtime-pin':pinFile,'journal-file':journalFile},{orca,cwd:root});
  assert.equal(result.ok,true);assert.equal(result.generation,1);
  ...
  assert.equal(retried.generation,2);assert.ok(fs.existsSync(path.join(store.dir,'events.g1.jsonl')));
  assert.ok(store.readEvents().some(event=>event.event==='owner-approved'));

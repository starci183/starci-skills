...
function setup({nodes=TREE,scope=[],reintake=[],ledgerMode='work',job='Design the remaining features',assessGoal,critiqueGoal,providers,runHeadless,budget,goalOptions={}}={}){
  ...
  // Runtime 1.0.4: the store holds a real ledger handle now; it must close before `tree.cleanup()`
  // removes the temp directory, or Windows refuses to delete the file still open underneath it.
  return {...tree,store,state,goal,error,cleanup:()=>{store.close();tree.cleanup();}};
}
...
test('a goal nobody assessed and nobody was asked to critique cannot be approved, and the repair is named on the page',()=>{
  ...
  try{
    ...
    assert.match(harness.store.goal().markdown,/do not\s+write the missing criteria or a verdict into this page by hand/);
  }finally{harness.cleanup();}
});
...
test('a plan-ledger goal whose model invented an operation kind is refused before the page is written',()=>{
  ...
  try{
    ...
    assert.equal(store.goal(),null,'no goal record is written for a plan the kernel cannot launch');
  }finally{store.close();tree.cleanup();}
});

// docs/ledger-db.md §4 `inputs` table: an owner-named external file is put into the ledger as bytes -
// never copied under `.starciwork/_local/inputs/<workflow>/`, which died with the worktree.
test('a file input outside the worktree is staged into the ledger inputs table and referred to by its ledger:// ref',()=>{
  const tree=workRepo([]);
  const outside=tmp();
  ...
  try{
    ...
    assert.match(state.inputs[0].ref,/^ledger:\/\/inputs\/20260915-inputs\/1-review\.md#sha256=[a-f0-9]{64}$/);
    const stored=store.inputs.get('1-review.md');
    assert.equal(Buffer.from(stored.bytes).toString('utf8'),'# review\n');
    assert.match(state.inputs[0].sha256,/^[a-f0-9]{64}$/);
    assert.equal(stored.sha256,state.inputs[0].sha256);
    ...
    assert.equal(fs.existsSync(path.join(tree.repo,'.starciwork','_local')),false,'no `_local` write ever happened');
...
test('on a plan ledger a decisive hidden decision is the owner\'s question on the page, never a decision.prepare op the Work gate refuses',()=>{
  ...
  try{
    ...
    assert.match(store.goal().markdown,/nivo-backup-store is removed as Nivo-owned/);

...
test('frontend catches route drawing, upward tiers, direct/deep data access, barrel bypass, world hooks, and raw fetch', t => {
  const root = fixture(t, 'frontend', {
    ...
    'src/components/blocks/home/Pure/component.tsx': '"use client"; import { useState } from "react"; import { query } from "../../../../bridge"; export const Pure=()=>{useState(0);fetch("/x");return <div>{query()}</div>}\n',
  ...
  const result = check(root);
  const rules = new Set(result.violations.map(item => item.ruleId));
  for (const expected of ['FE_ROUTE_ONE_PAGE', 'FE_TIER_IMPORTS_UPWARD', 'FE_COMPONENT_DEEP_HOOK_IMPORT', 'FE_PURE_CLIENT_BOUNDARY',

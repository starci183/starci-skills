...
import {ledgerFileFor,openLedger,compactSnapshots,SNAPSHOT_BODIES_KEPT} from '../kernel/ledger-db.mjs';
...
// A sibling directory, never nested inside a test's own `dir`: these one-liner tests never call
// admission.machine.close(), and on Windows an unclosed WAL handle blocks fs.rmSync of the directory
// that holds it. Machine dbs live here so a test's own cleanup never races its own open handle.
const temporaryMachineFile=()=>path.join(fs.mkdtempSync(path.join(os.tmpdir(),'starci-journal-machine-')),'machine.sqlite');
...
// 1.0.4 admission keys its machine side by meta.ledger_id (docs §5/§6), which only a real ledger has: every
// fixture below that drives admission/jobs/the job bridge - not kernel/journal.mjs's own behavior - opens a
// ledger, never a journal.
const withLedger=async(fn,{now=Date.now}={})=>{const dir=temporary(),journal=openLedger({file:path.join(dir,'runtime.sqlite'),now});try{return await fn(journal,dir);}finally{journal.close();fs.rmSync(dir,{recursive:true,force:true});}};
...
test('atomic multi-resource admission and stale fencing',()=>withLedger((journal,dir)=>{
  const admission=createAdmission({journal,now:()=>100,defaultAiCapacity:1,machineFile:temporaryMachineFile()});admission.setCapacity('db/test',1);admission.setBudget('tokens/day',5);
...
// Under 1.0.4 (docs §6) ai/* capacity lives on the machine DB and is TTL-swept on every reserve, independent
// of the ledger job's own status - unlike a repo-scoped resource the machine arbiter cannot see that job-1 is
// still unreconciled, only that its lease has expired. This ported from the pre-split journal, where a single
// db saw both, and reuse waited for reconciliation; see tests/admission.spec.mjs's side-by-side pair for the
// current, intentional contrast between a repo-scoped and an ai/* resource.
test('an expired lease becomes effect_unknown, and the machine arbiter reclaims ai/* capacity by TTL alone',()=>{
  let at=100;
  return withLedger((journal,dir)=>{
    const admission=createAdmission({journal,now:()=>at,defaultAiCapacity:1,machineFile:temporaryMachineFile()});journal.enqueueJob(identity(1));journal.enqueueJob(identity(2));
    assert.equal(admission.reserve({...identity(1),resources:[{key:GLOBAL_AI_RESOURCE,units:1}],ttlMs:5}).ok,true);at=106;
    assert.equal(admission.reserve({...identity(2),resources:[{key:GLOBAL_AI_RESOURCE,units:1}]}).ok,true);assert.equal(journal.getJob('job-1').status,'effect_unknown');
...
test('ten-slot cap is atomic across separate Node processes',async()=>{
  const dir=temporary(),file=path.join(dir,'runtime.sqlite'),machineFile=temporaryMachineFile(),journal=openLedger({file});for(let i=0;i<20;i+=1)journal.enqueueJob(identity(i));journal.close();
  const ledgerUrl=pathToFileURL(path.resolve('kernel/ledger-db.mjs')).href,admissionUrl=pathToFileURL(path.resolve('kernel/admission.mjs')).href;
  const code=`import {openLedger} from ${JSON.stringify(ledgerUrl)}; import {createAdmission} from ${JSON.stringify(admissionUrl)}; const [file,n,machineFile]=process.argv.slice(1); const j=openLedger({file}); const a=createAdmission({journal:j,machineFile}); const x={jobId:'job-'+n,workflowId:'workflow-'+n,opId:'op-'+n,attempt:1,generation:1}; const r=a.reserve({...x,resources:[{key:'ai/global',units:1}],ttlMs:60000}); process.stdout.write(r.ok?'1':'0'); j.close();`;
  const runs=Array.from({length:20},(_,i)=>new Promise((resolve,reject)=>{const child=spawn(process.execPath,['--input-type=module','-e',code,file,String(i),machineFile],{stdio:['ignore','pipe','pipe']});let out='',err='';child.stdout.on('data',b=>out+=b);child.stderr.on('data',b=>err+=b);child.on('exit',status=>status===0?resolve(out):reject(Error(err)));}));
  const results=await Promise.all(runs);assert.equal(results.filter(x=>x==='1').length,10);
  const check=openLedger({file});assert.equal(check.db.prepare("SELECT sum(units) AS n FROM leases WHERE resource_key='ai/global'").get().n,10);check.close();fs.rmSync(dir,{recursive:true,force:true});
});

test('async runner awaits a real promise and atomically emits one terminal event',async()=>withLedger(async(journal,dir)=>{
  const admission=createAdmission({journal,machineFile:temporaryMachineFile()});const jobs=createJobs({journal,admission});jobs.create(identity(1,'model'));
...
test('detached command bridge returns pending then replays the durable result',async()=>{
  const dir=temporary(),ledgerFile=path.join(dir,'runtime.sqlite'),bridge=createJobBridge({ledgerFile,machineFile:temporaryMachineFile()});const id={workflowId:'wf',opId:'op',attempt:1,generation:1};
...
test('command bridge preserves nonzero exit as a check result',async()=>{
  const dir=temporary(),bridge=createJobBridge({ledgerFile:path.join(dir,'runtime.sqlite'),machineFile:temporaryMachineFile()}),id={workflowId:'wf7',opId:'op7',attempt:1,generation:1};const first=replayCommandCheck(bridge,`${JSON.stringify(process.execPath)} -e "process.stderr.write('bad');process.exit(7)"`,id);
  let final;for(let i=0;i<100&&!final;i+=1){await new Promise(resolve=>setTimeout(resolve,10));const value=bridge.poll(first.identity);if(value&&!value.pending)final=value;}
...
test('durable worker retries terminal persistence without executing its child twice and records redacted lifecycle metadata',async()=>{
  const dir=temporary(),file=path.join(dir,'runtime.sqlite'),journal=openLedger({file}),admission=createAdmission({journal,machineFile:temporaryMachineFile()}),jobs=createJobs({journal,admission});
...
test('a completed child whose terminal transaction stays unavailable replays its exact staged result without rerunning',async()=>{
  const dir=temporary(),file=path.join(dir,'runtime.sqlite'),journal=openLedger({file}),admission=createAdmission({journal,machineFile:temporaryMachineFile()}),id={jobId:'completion-unknown',workflowId:'wf-unknown',opId:'op-unknown',attempt:1,generation:1,kind:'check'};
  ...
  const result=await runDurableJob({
    ledgerFile:file,jobId:id.jobId,leaseToken:lease.leaseToken,
  ...
  assert.equal(result.ok,false);assert.equal(childCalls,1);let check=openLedger({file}),job=check.getJob(id.jobId);assert.equal(job.status,'effect_unknown');assert.equal(job.lease_token,lease.leaseToken);assert.match(job.result.resultDigest,/^[a-f0-9]{64}$/);assert.ok(check.events({workflowId:id.workflowId}).some(event=>event.kind==='job-completion-uncommitted'&&event.payload.resultDigest===job.result.resultDigest));check.close();
  const replay=await runDurableJob({ledgerFile:file,jobId:id.jobId,leaseToken:lease.leaseToken,spawnChild:()=>{childCalls+=1;throw Error('provider must not rerun');},wait:()=>Promise.resolve()});assert.equal(replay.ok,true);assert.equal(childCalls,1);check=openLedger({file});job=check.getJob(id.jobId);assert.equal(job.status,'succeeded');assert.equal(job.result.stdout,'answer');assert.ok(check.events({workflowId:id.workflowId}).some(event=>event.kind==='job-result-replayed'));check.close();fs.rmSync(dir,{recursive:true,force:true});fs.rmSync(`${file}.workers`,{recursive:true,force:true});
});

test('staged completion rejects tampered result and wrong identity without invoking the provider',async()=>{
  for(const mutation of ['result','identity']){const dir=temporary(),file=path.join(dir,'runtime.sqlite'),journal=openLedger({file}),admission=createAdmission({journal,machineFile:temporaryMachineFile()}),id={jobId:`tamper-${mutation}`,workflowId:'wf-tamper',opId:'op-tamper',attempt:1,generation:1,kind:'check'};journal.enqueueJob({...id,payload:{handler:'command',command:process.execPath,args:['-e','process.stdout.write("original")'],timeoutMs:10000}});const lease=admission.reserve({...id,resources:[],ttlMs:60000});journal.close();let childCalls=0;await runDurableJob({ledgerFile:file,jobId:id.jobId,leaseToken:lease.leaseToken,spawnChild:(...args)=>{childCalls+=1;return spawn(...args);},wait:()=>Promise.resolve(),jobsFactory:options=>({...createJobs(options),complete:()=>{throw Error('busy');}})});const workerDir=`${file}.workers`,stage=path.join(workerDir,fs.readdirSync(workerDir).find(name=>name.endsWith('.result.json'))),value=JSON.parse(fs.readFileSync(stage,'utf8'));if(mutation==='result')value.result.stdout='tampered';else value.identity.generation+=1;fs.writeFileSync(stage,JSON.stringify(value));const replay=await runDurableJob({ledgerFile:file,jobId:id.jobId,leaseToken:lease.leaseToken,spawnChild:()=>{childCalls+=1;throw Error('provider must not rerun');},wait:()=>Promise.resolve()});assert.equal(replay.ok,false);assert.equal(replay.error.code,'STARCI_STAGED_RESULT_INVALID');assert.equal(childCalls,1);const check=openLedger({file});assert.equal(check.getJob(id.jobId).status,'effect_unknown');check.close();fs.rmSync(dir,{recursive:true,force:true});fs.rmSync(workerDir,{recursive:true,force:true});}
});

test('bridge relaunches only a valid staged completion with the original lease',async()=>{
  const dir=temporary(),file=path.join(dir,'runtime.sqlite'),machineFile=temporaryMachineFile(),journal=openLedger({file}),admission=createAdmission({journal,machineFile}),id={jobId:'bridge-completion-replay',workflowId:'wf-bridge-replay',opId:'op-bridge-replay',attempt:1,generation:1,kind:'check'},input={handler:'command',command:process.execPath,args:['-e','process.stdout.write("saved")'],timeoutMs:10000};journal.enqueueJob({...id,payload:input});const lease=admission.reserve({...id,resources:[],ttlMs:60000});journal.close();let providerCalls=0;await runDurableJob({ledgerFile:file,jobId:id.jobId,leaseToken:lease.leaseToken,spawnChild:(...args)=>{providerCalls+=1;return spawn(...args);},wait:()=>Promise.resolve(),jobsFactory:options=>({...createJobs(options),complete:()=>{throw Error('busy');}})});const launches=[],bridge=createJobBridge({ledgerFile:file,machineFile,spawnChild:(...args)=>{launches.push(args);return {pid:91,once(){},unref(){}};}}),replay=bridge.request({...id,input});assert.equal(replay.status,'running');assert.equal(launches.length,1);assert.equal(launches[0][1].at(-2),lease.leaseToken,'argv is [workerFile,journalFile,jobId,leaseToken,machinePath]');assert.equal(providerCalls,1);bridge.close();fs.rmSync(dir,{recursive:true,force:true});fs.rmSync(`${file}.workers`,{recursive:true,force:true});
});

test('staging write failure after provider result remains effect_unknown and cannot rerun the provider',async()=>{
  const dir=temporary(),file=path.join(dir,'runtime.sqlite'),journal=openLedger({file}),admission=createAdmission({journal,machineFile:temporaryMachineFile()}),id={jobId:'stage-write-failure',workflowId:'wf-stage-write',opId:'op-stage-write',attempt:1,generation:1,kind:'check'};journal.enqueueJob({...id,payload:{handler:'command',command:process.execPath,args:['-e','process.stdout.write("completed-result")'],timeoutMs:10000}});const lease=admission.reserve({...id,resources:[],ttlMs:60000});journal.close();fs.writeFileSync(`${file}.workers`,'blocks-private-stage-directory');let childCalls=0;const first=await runDurableJob({ledgerFile:file,jobId:id.jobId,leaseToken:lease.leaseToken,spawnChild:(...args)=>{childCalls+=1;return spawn(...args);},wait:()=>Promise.resolve()});assert.equal(first.ok,false);assert.equal(childCalls,1);let check=openLedger({file}),job=check.getJob(id.jobId);assert.equal(job.status,'effect_unknown');assert.match(job.result.resultDigest,/^[a-f0-9]{64}$/);check.close();const replay=await runDurableJob({ledgerFile:file,jobId:id.jobId,leaseToken:lease.leaseToken,spawnChild:()=>{childCalls+=1;throw Error('provider must not rerun');},wait:()=>Promise.resolve()});assert.equal(replay.ok,false);assert.equal(replay.error.code,'STARCI_INACTIVE_JOB');assert.equal(childCalls,1);check=openLedger({file});assert.equal(check.getJob(id.jobId).status,'effect_unknown');check.close();fs.rmSync(dir,{recursive:true,force:true});fs.rmSync(`${file}.workers`,{force:true});
});

test('duplicate wrapper after terminal completion is rejected before provider execution',async()=>{
  const dir=temporary(),file=path.join(dir,'runtime.sqlite'),journal=openLedger({file}),admission=createAdmission({journal,machineFile:temporaryMachineFile()}),id={jobId:'terminal-duplicate',workflowId:'wf-terminal',opId:'op-terminal',attempt:1,generation:1,kind:'check'};journal.enqueueJob({...id,payload:{handler:'command',command:process.execPath,args:['-e','process.stdout.write("once")'],timeoutMs:10000}});const lease=admission.reserve({...id,resources:[],ttlMs:60000});journal.close();let childCalls=0;const first=await runDurableJob({ledgerFile:file,jobId:id.jobId,leaseToken:lease.leaseToken,spawnChild:(...args)=>{childCalls+=1;return spawn(...args);}});assert.equal(first.ok,true);assert.equal(childCalls,1);const duplicate=await runDurableJob({ledgerFile:file,jobId:id.jobId,leaseToken:lease.leaseToken,spawnChild:()=>{childCalls+=1;throw Error('provider must not rerun');}});assert.equal(duplicate.ok,false);assert.equal(duplicate.error.code,'STARCI_INACTIVE_JOB');assert.equal(childCalls,1);const check=openLedger({file});assert.equal(check.getJob(id.jobId).status,'succeeded');assert.equal(check.getJob(id.jobId).result.stdout,'once');check.close();fs.rmSync(dir,{recursive:true,force:true});fs.rmSync(`${file}.workers`,{recursive:true,force:true});
});

test('pre-spawn admission rejection cancels a proven-no-effect reservation and never spawns',()=>{
  const dir=temporary();let launches=0;const bridge=createJobBridge({ledgerFile:path.join(dir,'runtime.sqlite'),machineFile:temporaryMachineFile(),eligibility:()=>({eligible:true}),beforeSpawn:()=>({ok:false,code:'probation-unavailable'}),spawnChild:()=>{launches+=1;return {once(){},unref(){}};}});const result=bridge.request({workflowId:'wfp',opId:'opp',attempt:1,generation:1,kind:'model',input:{handler:'model-function',functionName:'extractJson',args:'{}'}});assert.equal(result.pending,false);assert.equal(result.status,'cancelled');assert.equal(launches,0);assert.equal(bridge.journal.db.prepare('SELECT count(*) AS n FROM leases').get().n,0);bridge.close();fs.rmSync(dir,{recursive:true,force:true});
});
...
// §8: `paths.state` and every other file projection are removed - the ledger is the only record, so there
// is no separate JSON projection left to tear. Ported to the equivalent 1.0.4 property: a second store
// instance, with no in-process cache of its own, still reads the exact durably committed state.
test('a freshly opened store recovers committed state with no in-process projection to lose',async()=>{
  ...
  const reopened=createStore({repoRoot:repo,id:'wf'});assert.equal(reopened.loadState().counter,2);
  journal.close();store.close();reopened.close();fs.rmSync(repo,{recursive:true,force:true});
});
...
test('the snapshot ledger keeps one body and the transition checkpoints of the bound generation, and nothing of a retired one',async()=>{
  ...
  journal.close();store.close();reopened.close();
...
test('durable recovery rejects the same job text with changed approved inputs',async()=>{
  ...
  const reopened=createStore({repoRoot:repo,id:'wf'}),b={...a,inputs:['input-b']};assert.throws(()=>reopened.bindJournal(journal,1,{state:b}),/different approved goal/);journal.close();store.close();reopened.close();fs.rmSync(repo,{recursive:true,force:true});
});
...
test('dead read-only tool-disabled model is settled, while host-owned operation never enters PID reconciliation',()=>withLedger((journal,dir)=>{
  const admission=createAdmission({journal,defaultAiCapacity:2,machineFile:temporaryMachineFile()}),model={...identity('m','model'),payload:{execution:{schema:PURE_MODEL_EXECUTION,readOnly:true,toolsDisabled:true}}},operation=identity('o','operation');journal.enqueueJob(model);journal.enqueueJob(operation);const a=admission.reserve({...model,resources:[{key:GLOBAL_AI_RESOURCE,units:1}]}),b=admission.reserve({...operation,resources:[{key:GLOBAL_AI_RESOURCE,units:1}]});journal.appendEvent({eventId:'spawn-m',workflowId:model.workflowId,entityType:'job',entityId:model.jobId,generation:1,kind:'job-spawned',payload:{pid:101}});journal.appendEvent({eventId:'spawn-o',workflowId:operation.workflowId,entityType:'job',entityId:operation.jobId,generation:1,kind:'job-spawned',payload:{pid:102}});

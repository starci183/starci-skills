...
test('detached command bridge returns pending then replays the durable result',async()=>{
  const dir=temporary(),journalFile=path.join(dir,'journal.sqlite'),bridge=createJobBridge({ledgerFile:journalFile});const id={workflowId:'wf',opId:'op',attempt:1,generation:1};
...
test('command bridge preserves nonzero exit as a check result',async()=>{
  const dir=temporary(),bridge=createJobBridge({ledgerFile:path.join(dir,'journal.sqlite')}),id={workflowId:'wf7',opId:'op7',attempt:1,generation:1};const first=replayCommandCheck(bridge,`${JSON.stringify(process.execPath)} -e "process.stderr.write('bad');process.exit(7)"`,id);
  let final;for(let i=0;i<100&&!final;i+=1){await new Promise(resolve=>setTimeout(resolve,10));const value=bridge.poll(first.identity);if(value&&!value.pending)final=value;}

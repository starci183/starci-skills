...
test('durable validator excludes cooling choices before selecting its single provider and replays across skip changes',t=>{
  const f=fixture(t),bridge=createJobBridge({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,eligibility:()=>({eligible:true}),spawnChild:()=>({pid:7,once(){},unref(){}})}),runtime=createEngineRuntime({...f,bridge,eligibility:()=>({eligible:true})});
...
test('an in-process allocator cooldown routes a new model job to its healthy peer',t=>{
  const f=fixture(t),bridge=createJobBridge({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,eligibility:()=>({eligible:true}),spawnChild:()=>({pid:7,once(){},unref(){}})}),runtime=createEngineRuntime({...f,bridge,eligibility:()=>({eligible:true}),modelCooling:()=>[{runtime:'gpt-5.6-sol',until:Date.now()+60_000}]});
...
test('upgrade replays the exact legacy skip-bearing validator job after routing observations change',t=>{
  for(const status of ['running','succeeded','effect_unknown']){
    const f=fixture(t),bridge=createJobBridge({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,eligibility:()=>({eligible:true}),spawnChild:()=>({pid:7,once(){},unref(){}})}),bound={workflowId:'wf',opId:'legacy',attempt:1,generation:2};
...
test('mixed model cooldown and capacity blocks report availability rather than exhausted quota',t=>{
  ...
  const bridge=createJobBridge({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,eligibility:()=>({eligible:true}),spawnChild:()=>({pid:7,once(){},unref(){}})}),runtime=createEngineRuntime({...f,bridge,runtimeProfile:profile,eligibility:()=>({eligible:true}),modelCooling:()=>[{runtime:'gpt-5.6-sol',until:Date.now()+60_000}]});
...
test('ambiguous legacy validator jobs retain custody and cannot silently choose a completion',t=>{
  const f=fixture(t),bridge=createJobBridge({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,eligibility:()=>({eligible:true}),spawnChild:()=>({pid:7,once(){},unref(){}})}),bound={workflowId:'wf',opId:'legacy',attempt:1,generation:2},base={providers:['gpt-6-astra','claude-fable-5.1'],diff:{files:['a.js']}},saved={provider:'gpt-6-astra',runtime:'gpt-6-astra',mode:'qualified'};f.state.engine.modelSelections={};
...
test('prelaunch persistence failure retains one resumable reservation while launch intent remains effect-unknown',t=>{
  const f=fixture(t),bridge=createJobBridge({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,eligibility:()=>({eligible:true}),spawnChild:()=>{throw Error('native launch must not run');}}),runtime=createEngineRuntime({...f,eligibility:()=>({eligible:true}),bridge}),allocated={runtime:'gpt-5.6-sol',target:'gpt-5.6-sol',role:'implement'};
...
test('runtime exposes required validation and only sends evaluated providers to the detached model',t=>{
  const f=fixture(t),bridge=createJobBridge({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,eligibility:()=>({eligible:true}),spawnChild:()=>({pid:7,once(){},unref(){}})});
...
test('runtime refuses a model function before launch when every candidate is unqualified',t=>{
  const f=fixture(t),bridge=createJobBridge({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,eligibility:()=>({eligible:true}),spawnChild:()=>{throw Error('must not launch');}}),runtime=createEngineRuntime({...f,bridge,eligibility:()=>({eligible:false,reasons:['unqualified']})});
...
test('agent manager uses its semantic decision id for durable replay and the global model admission path',t=>{
  const f=fixture(t);f.state.engine.coordination='agent-v1';const bridge=createJobBridge({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,eligibility:()=>({eligible:true}),spawnChild:()=>({pid:7,once(){},unref(){}})}),runtime=createEngineRuntime({...f,bridge,eligibility:()=>({eligible:true})});
...
test('model replay hash excludes candidate payload and mutable operation runtime fields',t=>{
  const f=fixture(t),bridge=createJobBridge({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,eligibility:()=>({eligible:true}),spawnChild:()=>({pid:1,once(){},unref(){}})}),runtime=createEngineRuntime({...f,bridge,eligibility:()=>({eligible:true})}),op={id:'op',kind:'backend.implement',goal:'g',attempt:1,acceptance:['a'],allowlist:['a.js'],candidate:{huge:'x'.repeat(10000)},status:'running'};let first;try{runtime.model('validateOp',{providers:['gpt-5.6-sol'],op,diff:{files:['a.js'],text:'+x'}},op);}catch(error){first=error.job.identity.jobId;}op.candidate={different:'y'.repeat(10000)};op.status='blocked';let second;try{runtime.model('validateOp',{providers:['gpt-5.6-sol'],op,diff:{files:['a.js'],text:'+x'}},op);}catch(error){second=error.job.identity.jobId;}assert.equal(second,first);const payload=runtime.journal.getJob(first).payload;assert.equal(JSON.stringify(payload).includes('huge'),false);bridge.close();
});

test('worker-only settlement releases AI and retains writer until final acceptance settlement',t=>{
  const f=fixture(t),bridge=createJobBridge({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,eligibility:()=>({eligible:true}),spawnChild:()=>({pid:1,once(){},unref(){}})}),op={id:'op',kind:'backend.implement',attempt:1,status:'running',allowlist:['a.js']};f.state.ops=[op];const runtime=createEngineRuntime({...f,bridge,eligibility:()=>({eligible:true})}),lease=runtime.reserveOperation(op,{role:'implement',runtime:'gpt-5.6-sol',target:'gpt-5.6-sol'});assert.equal(lease.ok,true);assert.equal(runtime.settled(op,{workerOnly:true}).writerRetained,true);let rows=runtime.journal.db.prepare('SELECT resource_key FROM leases WHERE job_id=?').all(lease.jobId).map(x=>x.resource_key);assert.equal(rows.includes('ai/global'),false);assert.equal(rows.some(x=>x.startsWith('canonical-writer:')),true);assert.equal(runtime.settled(op).ok,true);rows=runtime.journal.db.prepare('SELECT resource_key FROM leases WHERE job_id=?').all(lease.jobId);assert.equal(rows.length,0);bridge.close();
});
...
test('cross-root admission acquires both writers atomically and retains both through acceptance',t=>{
  ...
  const bridge=createJobBridge({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,eligibility:()=>({eligible:true}),spawnChild:()=>({pid:1,once(){},unref(){}})}),
...
test('a stopped exact native Dispatch freezes and preserves unreported effects before releasing its writer',t=>{
  const f=fixture(t),bridge=createJobBridge({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,eligibility:()=>({eligible:true}),spawnChild:()=>({pid:1,once(){},unref(){}})}),op={id:'native',kind:'backend.implement',attempt:1,status:'running',allowlist:['a.js'],dispatch:'ctx-native'};f.state.ops=[op];
...
test('an authenticated answered decision releases an exact stopped no-change retry without creating another retry',t=>{
  const f=fixture(t),bridge=createJobBridge({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,eligibility:()=>({eligible:true}),spawnChild:()=>({pid:1,once(){},unref(){}})}),
...
test('a stopped native attempt with unsealable scope drift retains its writer and cannot launder paths into retry baseline',t=>{
  const f=fixture(t),bridge=createJobBridge({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,eligibility:()=>({eligible:true}),spawnChild:()=>({pid:1,once(){},unref(){}})}),op={id:'drift',kind:'backend.implement',attempt:1,status:'running',allowlist:['src/**'],dispatch:'ctx-drift'};f.state.ops=[op];
...
test('a stopped native attempt unpromotable only by head drift preserves in-scope effects and releases its writer',t=>{
  const f=fixture(t),bridge=createJobBridge({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,eligibility:()=>({eligible:true}),spawnChild:()=>({pid:1,once(){},unref(){}})}),op={id:'drifted',kind:'backend.implement',attempt:1,status:'running',allowlist:['src/**'],dispatch:'ctx-drifted'};f.state.ops=[op];
...
test('machine guard resources serialize native workers and detached checks while retaining only the writer',t=>{
  const f=fixture(t),spawned=[],bridge=createJobBridge({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,eligibility:()=>({eligible:true}),spawnChild:()=>{spawned.push(true);return {pid:1,once(){},unref(){}};}});
...
test('confirmed stopped old-generation lease can be settled before retry drops its state handle',t=>{
  const f=fixture(t),bridge=createJobBridge({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,eligibility:()=>({eligible:true}),spawnChild:()=>({pid:1,once(){},unref(){}})}),op={id:'op',kind:'backend.implement',attempt:1,status:'running',allowlist:['a.js']};f.state.ops=[op];const runtime=createEngineRuntime({...f,bridge,eligibility:()=>({eligible:true})});runtime.reserveOperation(op,{role:'implement',runtime:'gpt-5.6-sol',target:'gpt-5.6-sol'});runtime.settled(op,{workerOnly:true});const lease={...op.lease};bridge.close();assert.equal(settleGenerationLeases({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,leases:[lease]})[0].ok,true);const reopened=createJobBridge({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile});assert.equal(reopened.journal.db.prepare('SELECT count(*) AS n FROM leases WHERE job_id=?').get(lease.jobId).n,0);reopened.close();
});
...
test('retry admits only runtime reconciliation leases and fences unresolved generation jobs',t=>{
  ...
  assert.deepEqual(unsettledGenerationJobs({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,workflowId:'wf',generation:2}),[bridge.journal.listJobs()[0].job_id]);
  ...
  assert.deepEqual(unsettledGenerationJobs({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,workflowId:'wf',generation:1}),[]);bridge.close();
});
...
test('a non-operation model call combines fresh quota, admitted family load and bounded owner preference',t=>{
  ...
  const richBridge=createJobBridge({ledgerFile:rich.state.engine.ledgerFile,machineFile:rich.state.engine.machineFile,eligibility:()=>({eligible:true}),spawnChild:spawn});
  ...
  assert.deepEqual(chooser.journal.listJobs()[0].payload.args.providers,['claude-agent'],'a bounded preference does not override a much healthier family');
  richBridge.close();
  ...
  const closeBridge=createJobBridge({ledgerFile:close.state.engine.ledgerFile,machineFile:close.state.engine.machineFile,eligibility:()=>({eligible:true}),spawnChild:spawn});
  ...
  assert.deepEqual(closeChoice.journal.listJobs()[0].payload.args.providers,['codex-agent'],'the bounded preference decides a close feasible choice');closeBridge.close();
  ...
  const drainedBridge=createJobBridge({ledgerFile:drained.state.engine.ledgerFile,machineFile:drained.state.engine.machineFile,eligibility:()=>({eligible:true}),spawnChild:spawn});
  ...
  assert.deepEqual(overflow.journal.listJobs()[0].payload.args.providers,['claude-agent'],'a preference never starves: an exhausted window overflows to the other member');
  drainedBridge.close();
});
...
test('provider-family admission is atomic for concurrent model selections and unknown effects retain the slot',t=>{
  ...
  for(const runtime of Object.values(profile.runtimes))if(runtime.provider==='codex')runtime.maxParallel=runtime.target==='codex-agent'?1:0;
  const bridge=createJobBridge({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,eligibility:()=>({eligible:true}),spawnChild:()=>({pid:7,once(){},unref(){}})}),runtime=createEngineRuntime({...f,bridge,runtimeProfile:profile,eligibility:()=>({eligible:true})});
  ...
  assert.match(reviewed.blocked.find(item=>item.runtime==='codex-agent').reason,/authoritative admission/);
...
test('settled model service remains durable pressure and rotates an equal-headroom peer pool',t=>{
  ...
  const profile=withProviderPreference(loadRuntimes(),{mode:'adaptive',preferredProvider:null}),
    bridge=createJobBridge({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,now,eligibility:()=>({eligible:true}),spawnChild:()=>({pid:7,once(){},unref(){}})}),
...
test('launched cancellation and worker-only stop remain service pressure while never-launched cancellation does not',t=>{
  const f=fixture(t),profile=withProviderPreference(loadRuntimes(),{mode:'adaptive',preferredProvider:null}),
    bridge=createJobBridge({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,eligibility:()=>({eligible:true})}),runtime=createEngineRuntime({...f,bridge,runtimeProfile:profile,eligibility:()=>({eligible:true})}),
...
test('a pure model job whose worker never started settles instead of fencing the generation for good, while a command job keeps its fence',t=>{
  ...
  assert.deepEqual(unsettledGenerationJobs({ledgerFile:file,workflowId:'wf',generation:2}),[stranded.job_id],'and it fences the generation');
  ...
  const settled=settleNeverStartedModelJobs({ledgerFile:file,workflowId:'wf',generation:2,now:()=>9});
  ...
  assert.deepEqual(unsettledGenerationJobs({ledgerFile:file,workflowId:'wf',generation:2}),['ran-a-command']);
  assert.deepEqual(settleNeverStartedModelJobs({ledgerFile:file,workflowId:'wf',generation:2,now:()=>9}),[],'settling twice changes nothing');
  bridge.close();
});
test('missing model output is not proof of a never-started worker',t=>{
  ...
  assert.deepEqual(settleNeverStartedModelJobs({ledgerFile:file,workflowId:'wf',generation:2}),[]);
...
test('a cancelled never-launched reservation re-keys to a fresh durable job; a launched-cancelled job keeps its fence',t=>{
  const f=fixture(t),bridge=createJobBridge({ledgerFile:f.state.engine.ledgerFile,machineFile:f.state.engine.machineFile,eligibility:()=>({eligible:true}),spawnChild:()=>{throw Error('native launch must not run');}}),runtime=createEngineRuntime({...f,bridge,eligibility:()=>({eligible:true})}),allocated={runtime:'gpt-5.6-sol',target:'gpt-5.6-sol',role:'decide'};

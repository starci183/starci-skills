...
test('a direct business correction invalidates prior semantic proof',t=>{
  const f=fixture(t);f.node('module/business',{schema:'work/node@1'});f.done('module/business');
...
test('runtime custody files never become nodes or alter canonical hashes',t=>{const f=fixture(t);f.node('business');f.done('business');const before=validateWorkspace(f.root);f.put('kernel-strays/one/index.yaml','broken: [');f.put('kernel-strays/two/run.yaml','status: done\n');assert.deepEqual(validateWorkspace(f.root),before);});
test('mixed node formats and unsafe input asset ownership are rejected',t=>{const f=fixture(t);f.node('frontend',{assets:[{path:'../other.png'}]});assert.ok(validateWorkspace(f.root).errors.some(e=>e.code==='NODE_ASSET_PATH'));f.node('frontend');f.put('frontend/node.yaml','schema: work/node@1');assert.ok(validateWorkspace(f.root).errors.some(e=>e.code==='CANONICAL_FORMAT'));});
...
test('UI supports recursive scopes and many original-format design assets without evidence folders',t=>{
  ...
  f.put('product/chatbot/ui/index.yaml',{schema:'work/node@1',id:'chatbot.ui',kind:'ui',required:true,description:'Aggregate UI scope.'});
  for(const p of paths)f.put(`${dir}/${p}`,'Synthetic asset payload, not an actual render.');
  f.node(dir,{schema:'work/node@1',kind:'ui',ui:uiSpec(paths),assets:paths.map(p=>({path:p,description:'Synthetic resource role'}))});
...
test('asset payload folders do not create Work nodes or parse sample metadata',t=>{
  ...
  f.node(dir,{schema:'work/node@1',assets:files.map(p=>({path:p}))});
...
test('nested UI cannot bind duplicate or escaping assets, waits for the ones it has still to draw, and still requires state coverage',t=>{
  ...
  f.node(dir,{schema:'work/node@1',kind:'ui',ui:uiSpec(['assets/missing.png']),assets:[{path:'assets/missing.png'}]});
  ...
  f.node(dir,{schema:'work/node@1',kind:'ui',ui:uiSpec(['assets/design.png']),assets:[{path:'assets/design.png'},{path:'assets/design.png'}]});
  assert.ok(validateWorkspace(f.root).errors.some(e=>e.code==='NODE_ASSET_PATH'));
  f.node(dir,{schema:'work/node@1',kind:'ui',ui:uiSpec(['assets/../outside.png']),assets:[{path:'assets/../outside.png'}]});
  assert.ok(validateWorkspace(f.root).errors.some(e=>e.code==='NODE_ASSET_PATH'));
  f.node(dir,{schema:'work/node@1',kind:'ui',ui:{...uiSpec([]),states:[]}});
...
test('UI scope must remain under ui and shared assets bind consumers without duplicate files',t=>{
  const f=fixture(t);
  f.node('wrong/screen',{schema:'work/node@1',kind:'ui',ui:uiSpec([])});
  assert.ok(validateWorkspace(f.root).errors.some(e=>e.code==='UI_LAYOUT'));
  f.node('wrong/screen',{schema:'work/node@1'});
  f.put('shared/assets/logo.svg','Synthetic shared resource');f.node('shared',{assets:[{path:'assets/logo.svg'}]});
  f.node('module/ui/consumer',{schema:'work/node@1',kind:'ui',ui:uiSpec([]),refs:['shared']});
...
test('backend and frontend implementation scopes can grow recursively while their shared parent stays thin',t=>{
  ...
  f.put('module/implementation/index.yaml',{schema:'work/node@1',id:'implementation',kind:'implementation',required:true,description:'Shared implementation scope.'});
  for(const side of ['backend','frontend']){
    const dir=`module/implementation/${side}`;
    f.put(dir+'/index.yaml',{schema:'work/node@1',id:side,kind:'implementation',required:true,implementation});
    f.node(dir+'/documents/update',{schema:'work/node@1',kind:'implementation',implementation});
  }
  let result=validateWorkspace(f.root);assert.ok(result.ok,JSON.stringify(result.errors));assert.equal(result.nodes.length,5);
  f.node('module/wrong/update',{schema:'work/node@1',kind:'implementation',implementation});
...
test('implementation owns the mapping from logical SDS components to source code',t=>{
  ...
  f.put('module/implementation/index.yaml',{schema:'work/node@1',id:'implementation',kind:'implementation',required:true,description:'Implementation aggregate.'});
  f.put('module/implementation/backend/index.yaml',{schema:'work/node@1',id:'implementation.backend',kind:'implementation',required:true,state:'todo',implementation:{status:'proposed',changes:[],currentCodeMap:[mapping],gaps:['Synthetic mapping-only fixture.']}});
...
test('nested asset payload folders never follow junctions',t=>{
  const f=fixture(t);f.node('scope',{schema:'work/node@1'});f.put('kernel-strays/target/sample.txt','Not canonical');
  ...
  fs.symlinkSync(path.join(f.root,'kernel-strays/target'),link,process.platform==='win32'?'junction':'dir');

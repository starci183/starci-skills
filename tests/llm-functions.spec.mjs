...
test('validateOp carries the brand record into the prompt, with the rules that make it binding, and omits it when the tree has none',()=>{
  ...
  for(const rule of ['when a `brand` record is given it is binding',
    ...
    'an interface.draw result is image-first',
    'every generated image keeps its exact prompt and actual tool provenance',
    'a generated direction is proposed visual guidance, not exact Grammar component/render/API proof',

...
test('operation request carries the whole chain with its launch kinds and owns the canonical Task identity',()=>{
  ...
  assert.deepEqual(planned.candidates.map(candidate=>[candidate.selection.target,candidate.launch]),[['qwen-agent','command-terminal'],['claude-agent','managed-agent'],['codex-agent','managed-agent']]);
  ...
  // The reasoning chain carries its downgrade now: the Fable window, then the Codex window's reasoning model,
  // then the Claude window - all three managed agents.
  assert.deepEqual(buildOperationLaunch(reasonInput).candidates.map(candidate=>[candidate.selection.target,candidate.launch]),[['claude-fable','managed-agent'],['codex-agent','managed-agent'],['claude-agent','managed-agent']]);
...
test('a Qwen operation runs in one command terminal: create, wait for the prompt, return-preamble, send, submit-verify, attest assignee',()=>{
  ...
  assert.equal(result.selection.target,'qwen-agent');
...
test('a Qwen terminal that never renders its prompt is closed with no effects and the chain continues to Claude',()=>{
  ...
  const result=startOperation(input,{orca:fake.orca,wait:noWait});
  assert.equal(result.ok,true);
  assert.equal(result.selection.target,'claude-agent');
...
test('a managed candidate stall is classified as no-effect and the launcher falls through in the same invocation',()=>{
  ...
  assert.equal(result.ok,true);
  assert.equal(result.selection.target,'codex-agent');
  assert.equal(result.attempts.length,1);
  assert.equal(result.attempts[0].target,'claude-fable');
  assert.equal(result.attempts[0].effectState,'none');
...
test('an exhausted chain is a typed failure, never a thrown string',()=>{
  ...
  assert.deepEqual(result.attempts.map(attempt=>attempt.target),['qwen-agent','claude-agent','codex-agent']);
...
test('a worker whose tab the kernel already closed settles to none only on Orca\'s exited record with the tab gone',()=>{
  ...
  const list=(...perCall)=>(args,count)=>json(0,{ok:true,result:{terminals:(perCall[Math.min(count,perCall.length)-1]).map(handle=>({handle,title:'x'}))}});
...
assert.equal(proven.effectState,'none');assert.equal(proven.exitedWorker.proven,true);assert.equal(proven.exitedWorker.terminal,'term_gone');assert.deepEqual(proven.residualTerminal,{state:'release_unknown',reason:'The agent terminal was closed but its process could not be confirmed stopped',processAction:'closed_agent_terminal'});
  ...
  const stuck=fakeOrca({'worker-stop':stopFailed,'worker-release':releaseUnknown,'worker-show':showAt('failed','process_exited','2026-09-15T10:30:00Z'),'terminal-list':list(['term_gone'],['term_gone']),'terminal-close':closeOk,'terminal-read':screen(idleScreen)});
  const idleStale=settleDispatch(stuck.orca,'ctx_gone',{cwd:'.',wait:noWait,now:()=>T0});
  assert.equal(idleStale.effectState,'none','a pty Orca cannot stop, showing an idle prompt with a heartbeat half an hour old, is a finished worker');assert.equal(idleStale.exitedWorker.screen,'finished-unreported','a turn that ended at its prompt');assert.equal(idleStale.exitedWorker.residualTab.terminal,'term_gone');
  const idleFresh=settleDispatch(fakeOrca({'worker-stop':stopFailed,'worker-release':releaseUnknown,'worker-show':showAt('failed','process_exited','2026-09-15T10:58:00Z'),'terminal-list':list(['term_gone'],['term_gone']),'terminal-close':closeOk,'terminal-read':screen(idleScreen)}).orca,'ctx_gone',{cwd:'.',wait:noWait,now:()=>T0});
  assert.equal(idleFresh.effectState,'unknown');assert.match(idleFresh.exitedWorker.reason,/heartbeat is not stale/);
  const stillWorking=settleDispatch(fakeOrca({'worker-stop':stopFailed,'worker-release':releaseUnknown,'worker-show':showAt('failed','process_exited','2026-09-15T10:30:00Z'),'terminal-list':list(['term_gone'],['term_gone']),'terminal-close':closeOk,'terminal-read':screen(workingScreen)}).orca,'ctx_gone',{cwd:'.',wait:noWait,now:()=>T0});
  assert.equal(stillWorking.effectState,'unknown');assert.match(stillWorking.exitedWorker.reason,/screen reads working/);
  const closeRefused=settleDispatch(fakeOrca({'worker-stop':stopFailed,'worker-release':releaseUnknown,'worker-show':show('failed','process_exited'),'terminal-list':list(['term_gone']),'terminal-close':()=>json(1,{ok:false,error:{code:'terminal_busy',message:'cannot close'}})}).orca,'ctx_gone',{cwd:'.',wait:noWait});
  assert.equal(closeRefused.effectState,'unknown');assert.match(closeRefused.exitedWorker.reason,/could not be closed/);
...
test('--skip records a verified no-effect failure for a chain target and starts at the next candidate',()=>{
  ...
  assert.deepEqual(planned.candidates.map(candidate=>candidate.selection.target),['claude-agent','codex-agent']);
  assert.deepEqual(planned.skipped,[{target:'qwen-agent',reason:'unavailable',effectState:'none',source:'monitor-verified-skip'}]);
  assert.throws(()=>buildOperationLaunch({...input,skip:'qwen3.8-flash:permission-denied'}),/reason must be one of/);
  assert.throws(()=>buildOperationLaunch({...input,skip:'claude-fable-5.1:unavailable'}),/outside this operation chain/);
  ...
  assert.equal(result.ok,true);
  assert.equal(result.selection.target,'claude-agent');
  assert.deepEqual(result.attempts.map(attempt=>[attempt.target,attempt.reason,attempt.effectState]),[['qwen-agent','unavailable','none']]);
...
test('an explicit candidate list replaces chain resolution, so the allocator launches exactly one runtime',()=>{
  ...
  const opus=chain.find(candidate=>candidate.target==='claude-agent');
  const planned=buildOperationLaunch({...input,candidates:[opus]});
  assert.deepEqual(planned.candidates.map(candidate=>candidate.selection.target),['claude-agent']);
  ...
  assert.throws(()=>buildOperationLaunch({...input,candidates:[{target:'claude-agent'}]}),/resolved selection with a target and an Orca launch shape/);
  const fake=fakeOrca({
    'run-show':()=>json(0,runShow),
    'task-create':()=>json(0,taskCreated('task_operation_sales',opName)),
    'worker-start':args=>{assert.ok(has(args,'--agent','claude'));return json(0,started('ctx_claude','task_operation_sales'));},

...
assert.equal(proven.effectState,'none');assert.equal(proven.exitedWorker.proven,true);assert.equal(proven.exitedWorker.terminal,'term_gone');assert.deepEqual(proven.residualTerminal,{state:'release_unknown',reason:'The agent terminal was closed but its process could not be confirmed stopped',processAction:'closed_agent_terminal'});
  ...
  const tabStillThere=settleDispatch(fakeOrca({'worker-stop':stopFailed,'worker-release':releaseUnknown,'worker-show':show('failed','process_exited'),'terminal-list':list(['term_gone'],['term_gone']),'terminal-close':closeOk}).orca,'ctx_gone',{cwd:'.',wait:noWait});
  assert.equal(tabStillThere.effectState,'unknown');assert.match(tabStillThere.exitedWorker.reason,/still listed after its close/);

...
test('adaptive capacity is the default and the owner may prefer one declared provider without creating a chain',()=>{
  ...
  try{
    ...
    assert.deepEqual(configuredAllocationPolicy(base),{mode:'adaptive',preferredProvider:null,policy:'balanced',
      shares:{'claude-agent':25,'codex-agent':25,'devin-agent':25,'qwen-agent':25},windowHours:24,
      grants:{'devin-agent':{slots:10,roles:['implement','verify','write']}},source:'allocation'},'the shipped default balances four pools and grants Devin');

...
test('adaptive capacity is the default and the owner may prefer one declared provider without creating a chain',()=>{
  ...
  try{
    ...
    assert.deepEqual(configuredAllocationPolicy(base),{mode:'adaptive',preferredProvider:null,policy:'balanced',
      shares:{'qwen-agent':40,'claude-agent':20,'codex-agent':20,'devin-agent':20},windowHours:24,
      grants:{'devin-agent':{slots:10,roles:['implement','verify','write']}},source:'allocation'},'the shipped default balances four pools, the Qwen base pool with the largest share, and grants Devin');

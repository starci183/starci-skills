...
import {persistPrelaunchReservation,retryOwnedBaseline,retryableV6Operation} from '../kernel/kernel.mjs';
...
test('prelaunch persistence failure retains one resumable reservation while launch intent remains effect-unknown',t=>{
  const f=fixture(t),bridge=createJobBridge({journalFile:f.state.engine.journalFile,eligibility:()=>({eligible:true}),spawnChild:()=>{throw Error('native launch must not run');}}),runtime=createV6Runtime({...f,eligibility:()=>({eligible:true}),bridge}),allocated={runtime:'gpt-5.6-sol',target:'gpt-5.6-sol',role:'implement'};
...
test('restart adopts the exact prelaunch reservation once and incomplete or unknown bindings never launch',t=>{
  ...
  const firstBridge=createJobBridge({journalFile:f.state.engine.journalFile,eligibility:()=>({eligible:true}),spawnChild:()=>{throw Error('no native launch');}}),first=createV6Runtime({...f,bridge:firstBridge,eligibility:()=>({eligible:true,mode:'probation'}),modelPolicy:policy});const reserved=first.reserveOperation(op,{runtime:'gpt-5.6-sol',target:'gpt-5.6-sol',role:'implement'});assert.equal(consumed,1);delete op.v6Lease;firstBridge.close();
  const reloadedState={...f.state,ops:[{id:'reload',kind:'backend.implement',attempt:1,status:'ready',allowlist:['src/']}]},secondBridge=createJobBridge({journalFile:f.state.engine.journalFile,eligibility:()=>({eligible:true}),spawnChild:()=>{throw Error('no native launch');}}),second=createV6Runtime({...f,state:reloadedState,bridge:secondBridge,eligibility:()=>({eligible:false,reasons:['fresh admission changed']}),modelPolicy:policy});second.pulse();const restored=reloadedState.ops[0];assert.equal(restored.v6Lease.jobId,reserved.jobId);assert.deepEqual(second.reservationPhase(restored),{phase:'reserved',runtime:'gpt-5.6-sol',target:'gpt-5.6-sol',role:'implement'});assert.equal(second.reserveOperation(restored,{runtime:'gpt-5.6-sol',target:'gpt-5.6-sol',role:'implement'}).reattached,true,'already admitted reservation does not run fresh eligibility again');second.beginLaunchIntent(restored);assert.equal(consumed,1,'restart does not consume probation or create another job');assert.equal(second.journal.listJobs().filter(job=>job.op_id==='reload').length,1);
  second.journal.db.prepare("UPDATE jobs SET status='effect_unknown' WHERE job_id=?").run(restored.v6Lease.jobId);assert.throws(()=>second.beginLaunchIntent(restored),error=>error.effectState==='unknown');assert.equal(second.journal.getJob(restored.v6Lease.jobId).status,'effect_unknown');secondBridge.close();
});

test('reservation phase rejects a missing canonical writer resource',t=>{const f=fixture(t),bridge=createJobBridge({journalFile:f.state.engine.journalFile,eligibility:()=>({eligible:true}),spawnChild:()=>{}}),runtime=createV6Runtime({...f,bridge,eligibility:()=>({eligible:true})}),op={id:'writer',kind:'backend.implement',attempt:1,status:'ready',allowlist:['src/']};runtime.reserveOperation(op,{runtime:'gpt-5.6-sol',target:'gpt-5.6-sol',role:'implement'});runtime.journal.db.prepare("DELETE FROM leases WHERE job_id=? AND resource_key LIKE 'canonical-writer:%'").run(op.v6Lease.jobId);assert.equal(runtime.reservationPhase(op).phase,'unknown');assert.throws(()=>runtime.beginLaunchIntent(op),error=>error.effectState==='unknown');bridge.close();});

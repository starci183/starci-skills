...
function fixture(t) {
  ...
  function node(p,overrides={},body='Scope: synthetic unit fixture. Done when declared test assertions are observed.') {
    ...
    metas.set(p,m);write(`${p}/index.yaml`,stringifyYaml({...m,description:body}));return m;
  }
...
test('valid todo spec and synthetic done are different; validation is read-only',t=>{
  ...
  f.done('biz');const before=fs.readFileSync(path.join(f.root,'biz/index.yaml'));
  assert.equal(f.run().ok,true);assert.equal(f.run().nodes[0].effectiveState,'done');
  assert.deepEqual(fs.readFileSync(path.join(f.root,'biz/index.yaml')),before);
});
test('branch rollup ignores optional unmet children with explicit warning',t=>{
  ...
  delete branch.state;f.write('biz/index.yaml',stringifyYaml({...branch,description:"Synthetic branch scope."}));
...
test('branch cannot manually claim state; authored leaf state is restricted to uninvestigate, todo and done',t=>{
  const f=fixture(t);f.node('biz');f.node('biz/a',{state:'blocked'});f.node('biz/b',{state:'na'});
  const c=codes(f.run());for(const code of ['BRANCH_STATE','STATE','BLOCKER','NA_REASON'])assert.ok(c.includes(code));
});
test('ancestor semantic edit stales descendant while timestamps do not',t=>{
  ...
  f.write('biz/index.yaml',stringifyYaml({...b,description:"Approved scope A."}));f.done('biz/leaf');
  const initial=f.run().nodes.find(n=>n.id==='biz.leaf').inputDigest;
  f.write('biz/index.yaml',stringifyYaml({...{...b,updatedAt:'2099-01-01'},description:"Approved scope A."}));
  assert.equal(f.run().nodes.find(n=>n.id==='biz.leaf').inputDigest,initial);
  f.write('biz/index.yaml',stringifyYaml({...b,description:"Approved scope B."}));
  assert.equal(f.run().nodes.find(n=>n.id==='biz.leaf').effectiveState,'uninvestigate');
});
...
test('inherited prerequisite blocks child even when its own refs are empty',t=>{
  ...
  f.write('group/index.yaml',stringifyYaml({...{schema:'work/node@1',id:'group',kind:'group',required:true,dependsOn:['contract']},description:"Bounded implementation group."}));
...
test('local scratch is ignored and never promoted as canonical evidence',t=>{
  ...
  const f=fixture(t);f.node('biz');f.write('kernel-strays/cache/index.yaml','malformed scratch');assert.equal(f.run().ok,true);
  f.done('biz',{assets:[{path:'../../../kernel-strays/cache/index.yaml',sha256:sha256('malformed scratch')}]});assert.equal(f.run().ok,false);
});
test('one evidence bundle supports explicitly bound leaves without copying assets',t=>{
  ...
  const changed=f.run();assert.equal(changed.nodes.find(n=>n.id==='flow.b').effectiveState,'uninvestigate');assert.equal(changed.nodes.find(n=>n.id==='flow.a').effectiveState,'done');
});
...
test('arbitrary extension namespaces and resource details remain preserved and hashed',t=>{
  ...
  f.done('biz');const before=fs.readFileSync(path.join(f.root,'biz/index.yaml'),'utf8');const r=f.run();assert.equal(r.ok,true);assert.equal(fs.readFileSync(path.join(f.root,'biz/index.yaml'),'utf8'),before);
  f.node('biz',{...f.metas.get('biz'),extensions:{...extensions,anotherNamespace:{newValue:1}}});assert.equal(f.run().nodes[0].effectiveState,'uninvestigate');
});
...
test('artifact directories cannot smuggle completion nodes into the tree',t=>{
  for(const p of ['biz/evidence/run/hidden','biz/assets/hidden','_resources/custom/hidden','biz/EVIDENCE/run/hidden']) {
    const f=fixture(t);f.node('biz');f.node(p,{id:'smuggled'});
...
test('changing reasoned scope text invalidates dependent proof while bare state does not change digest',t=>{
  const f=fixture(t);f.node('applicability',{naReason:'No regulated records are processed.'});f.node('consumer',{refs:['applicability']});f.done('consumer');
  assert.equal(f.run().ok,true);const before=f.run().nodes.find(n=>n.id==='consumer').inputDigest;
  f.node('applicability',{...f.metas.get('applicability'),state:'uninvestigate'});
  const stateOnly=f.run();assert.equal(stateOnly.nodes.find(n=>n.id==='consumer').inputDigest,before);
  f.node('applicability',{...f.metas.get('applicability'),state:'todo',naReason:'Regulated records are handled by an external service.'});
  const changed=f.run();assert.notEqual(changed.nodes.find(n=>n.id==='consumer').inputDigest,before);assert.equal(changed.nodes.find(n=>n.id==='consumer').effectiveState,'uninvestigate');
});
test('goal refs its requirements subtree while sibling implementation depends on requirements without a false cycle',t=>{
  const f=fixture(t);
  f.write('goal/index.yaml',stringifyYaml({...{schema:'work/node@1',id:'goal',kind:'business',required:true,refs:['requirements']},description:"Deliver the bounded business goal."}));
  f.write('goal/business/index.yaml',stringifyYaml({...{schema:'work/node@1',id:'requirements',kind:'business',required:true},description:"Approved requirement set."}));
  ...
  f.node('goal/business/fr',{id:'functional-requirement'});f.node('goal/business/nfr',{id:'nonfunctional-requirement',required:false});
  ...
  const changed=f.run();assert.ok(!codes(changed).includes('CYCLE'));assert.equal(changed.nodes.find(n=>n.id==='goal.implementation').effectiveState,'uninvestigate');
});
test('explicit self references and ancestor dependency loops still fail cycle detection',t=>{
  ...
  const g=fixture(t);g.write('goal/index.yaml',stringifyYaml({...{schema:'work/node@1',id:'goal',kind:'business',required:true,dependsOn:['child']},description:"Invalid self-blocking prerequisite."}));g.node('goal/child',{id:'child'});assert.ok(codes(g.run()).includes('CYCLE'));
});
test('module account/chat diamond gates start, suspends on prerequisite regression and never rewrites evidence',t=>{
  ...
  const after=f.run();for(const id of ['account','permission','chat']){
    const n=after.nodes.find(n=>n.id===id);assert.equal(n.effectiveState,'uninvestigate');assert.equal(n.state,'done');assert.equal(n.eligible,false);assert.ok(n.suspensionReasons.some(r=>r.code==='PREREQUISITE_NOT_DONE'));
  }
...
test('an unfinished prerequisite never proves availability and suspended is not an authored leaf state',t=>{
  const f=fixture(t);f.node('module');f.node('account',{dependsOn:['module']});
  ...
  f.done('account');assert.equal(f.run().nodes.find(n=>n.id==='account').effectiveState,'uninvestigate');
...
test('semantic refs propagate business changes through architecture/code/UAT but do not impose execution gates',t=>{
  ...
  const r=f.run();for(const id of ['architecture','code','uat']){const n=r.nodes.find(n=>n.id===id);assert.equal(n.effectiveState,'uninvestigate');assert.equal(n.eligible,false);assert.deepEqual(n.blockedBy,[]);}
...
test('actual design source bytes suspend only linked UI consumers without a revision bump',t=>{
  ...
  f.write('feature/index.yaml',stringifyYaml({...{schema:'work/node@1',id:'feature',kind:'group',required:true},description:"Synthetic delivery scope."}));
  ...
  const after=f.run();assert.equal(after.nodes.find(n=>n.id==='feature.ui').effectiveState,'uninvestigate');assert.equal(after.nodes.find(n=>n.id==='feature.uat').effectiveState,'uninvestigate');assert.equal(after.nodes.find(n=>n.id==='feature.backend').effectiveState,'done');
...
test('impact distinguishes ancestor spec changes from child-input rollup and is read-only',t=>{
  const f=fixture(t);f.write('group/index.yaml',stringifyYaml({...{schema:'work/node@1',id:'group',kind:'group',required:true},description:"Synthetic shared scope."}));f.node('group/a');f.node('group/b');f.node('external',{refs:['group.a']});
  const before=fs.readFileSync(path.join(f.root,'group/index.yaml'));
  ...
  assert.deepEqual(fs.readFileSync(path.join(f.root,'group/index.yaml')),before);assert.equal(impactWorkspace(f.root,'absent').target,null);
});
test('source-imported uninvestigated scope needs no fabricated prior done or evidence and never satisfies prerequisites',t=>{
  const f=fixture(t);f.node('imported-business',{state:'uninvestigate'});f.node('implementation',{dependsOn:['imported-business']});
  const before=fs.readFileSync(path.join(f.root,'imported-business/index.yaml'));const r=f.run();assert.equal(r.ok,true);
  const imported=r.nodes.find(n=>n.id==='imported-business');assert.equal(imported.state,'uninvestigate');assert.equal(imported.effectiveState,'uninvestigate');assert.ok(imported.suspensionReasons.some(reason=>reason.code==='NOT_INVESTIGATED'));
  assert.equal(f.metas.get('imported-business').completion,undefined);assert.equal(fs.existsSync(path.join(f.root,'imported-business/evidence')),false);assert.deepEqual(fs.readFileSync(path.join(f.root,'imported-business/index.yaml')),before);
...
test('operational annotations never enter the digest and fake done still needs proof',t=>{
  const f=fixture(t);f.node('imported',{state:'uninvestigate'});const before=f.run().nodes[0].inputDigest;
  f.node('imported',{...f.metas.get('imported'),suspensionReason:'Source-derived inventory awaits requirement review.'});assert.equal(f.run().nodes[0].inputDigest,before);
...
const uiNode=(state='todo',trailer='')=>`schema: work/node@1
id: sales.ui
kind: ui
required: true
state: ${state}
...
${trailer}`;
const leaf=(id,kind)=>`schema: work/node@1
id: ${id}
kind: ${kind}
required: true
state: todo
...
function brandFixture(t,record=brandRecord()) {
  ...
  write('features/sales/index.yaml','schema: work/node@1\nid: sales\nkind: business\nrequired: true\ndescription: Sales feature.\n');
...
test('a declared asset that has not been drawn yet is pending on a todo node and missing on a done one',t=>{
  ...
  assert.deepEqual(pending.warnings.filter(w=>w.code==='NODE_ASSET_PENDING').map(w=>w.path),['ui/screen/index.yaml']);
  ...
  assert.deepEqual(gone.errors.filter(e=>e.code==='NODE_ASSET_UNREADABLE').map(e=>e.path),['ui/screen/index.yaml']);

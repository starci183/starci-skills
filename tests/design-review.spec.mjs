...
function fixture(t) {
  ...
  // this file needs its knowledge/business SRS tree at the plain work/workspace@1 root schema
  // validateWorkspace treats as canonical, which the current todo-app-backend/frontend examples do not
  // carry. tests/fixtures/nested-business/ reconstructs the deleted example's tree byte-for-byte.
  ...
  put('knowledge/architecture/index.yaml',{schema:'work/node@1',id:'example.architecture',kind:'architecture',required:true,description:'Synthetic architecture scope.',dependsOn:['example.business.srs']});
  put('knowledge/architecture/overview/index.yaml',{schema:'work/node@1',id:'example.architecture.overview',kind:'architecture',required:true,state:'todo',description:'Synthetic design overview, not a completed design.'});
  put(sds,{schema:'work/node@1',id:'example.sds',kind:'architecture',required:true,state:'todo',description:'Synthetic source-independent document design.',assertions:['scope-reviewed'],dependsOn:['example.business.srs'],extensions:{work3:{specification:spec}}});
...
test('inline review cannot accept draft requirements, incomplete upstream design or operational effects',t=>{
  ...
  for(const kind of ['implementation','ui','uat','operations','business']){
    const p=`other-${kind}/index.yaml`;f.put(p,{schema:'work/node@1',id:`other-${kind}`,kind,required:true,state:'todo',assertions:['scope-reviewed']});f.complete(p);
...
test('accepted structured overview completes against current SRS and topology then invalidates with its dependency',t=>{
  ...
  f.put(f.sds,{schema:'work/node@1',id:'example.sds',kind:'architecture',required:true,description:'SDS aggregate.',extensions:{work3:{sds:{schema:'starci/sds-aggregate@1'}}}});
  f.put('knowledge/architecture/sds/components/index.yaml',{schema:'work/node@1',id:'example.sds.components',kind:'architecture',required:true,description:'Component aggregate.',extensions:{work3:{sds:{schema:'starci/sds-aggregate@1'}}}});
  f.put('knowledge/architecture/sds/components/update/index.yaml',{schema:'work/node@1',id:'example.sds.update-component',kind:'architecture',required:true,state:'todo',description:'Logical update component.',extensions:{work3:{sds:{schema:'starci/sds-component@1',id:'COMP-UPDATE',title:'Update component',status:'accepted',kind:'service',responsibility:'Apply the accepted update contract.',interfaceRefs:[],dataRefs:[],qualityRefs:[],verificationRefs:[]}}}});
  f.put('knowledge/architecture/sds/deployment/index.yaml',{schema:'work/node@1',id:'example.sds.deployment',kind:'architecture',required:true,description:'Deployment aggregate.',extensions:{work3:{sds:{schema:'starci/sds-aggregate@1'}}}});
  f.put('knowledge/architecture/sds/deployment/local/index.yaml',{schema:'work/node@1',id:'example.sds.deployment.local',kind:'architecture',required:true,state:'todo',description:'Target topology.',extensions:{work3:{sds:{schema:'starci/sds-deployment@1',id:'DEPLOY-LOCAL',title:'Local topology',status:'accepted',topology:'One process.',placements:[{componentRef:'COMP-UPDATE',runtime:'application'}],connections:[],configuration:{owner:'Platform'},credentials:{owner:'Platform'},scaling:'One process.',failureDomains:['Process'],rollout:'Compatible release.',rollback:'Prior compatible release.',recovery:{owner:'Platform'}}}}});

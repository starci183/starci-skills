...
test('an allowlist folder written with a glob tail covers every file beneath it, and a file entry covers only itself',()=>{
  ...
  assert.deepEqual(validateReport(report,{allowlist:['.starciwork/brand/index.yaml']}).errors.filter(error=>/outside/.test(error)),['File outside the allowlist: .starciwork/brand/assets/logo/mark.svg']);
  assert.deepEqual(validateReport({...report,files:['src/x/y.ts']},{allowlist:['src/*']}).errors.filter(error=>/outside/.test(error)),[]);
});

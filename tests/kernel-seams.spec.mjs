...
test('the validator is handed the declaration of the kind it judges',()=>{
  ...
  try{
    ...
    assert.deepEqual(seen[0].io,{reads:[],writes:['design']},'the kind declares what it produces; today it declares no input');
  }finally{fs.rmSync(dir,{recursive:true,force:true});}
});
...
test('a profile that declares no writes declares no rule, and the same report is accepted',()=>{
  const dir=tmp();
  try{
    const store=stubStore(dir);
    const op=toOp({id:'build-1',kind:'backend.implement',goal:'Build the intake.',

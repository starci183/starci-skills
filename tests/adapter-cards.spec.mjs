...
const ADAPTERS=path.join(ROOT,'providers','orca','adapters');
// Lane m13: provider differences are data. Every adapter card must parse and
// carry the fields its kind needs — a card missing readiness/submission
// patterns or a fallback command is a dispatch stall waiting to happen.
//
...
assert.ok(cards.length>0,'providers/orca/adapters holds no cards');

const KINDS=['command-terminal-agent','native-managed-agent'];

for(const file of cards){
  test(`adapter card ${file} parses and carries the fields its kind needs`,()=>{
    ...
    assert.equal(card.schema,'starci/orca-agent-adapter@1',`${file} schema`);

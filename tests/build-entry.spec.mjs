...
function fixture(t){const dir=fs.mkdtempSync(path.join(os.tmpdir(),'starci-build-'));t.after(()=>{assert.equal(path.dirname(dir),fs.realpathSync(os.tmpdir()));assert.ok(path.basename(dir).startsWith('starci-build-'));fs.rmSync(dir,{recursive:true,force:true});});for(const p of ['config.example.yaml','cli','ops','workflows','model','kernel','hosts','models','checks','providers','approvals','execution','knowledge','contracts','specifications','examples','scripts','core','schemas'])fs.cpSync(path.join(root,p),path.join(dir,p),{recursive:true});return dir;}

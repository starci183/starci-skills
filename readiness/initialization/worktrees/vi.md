...
`<Source>/.worktrees/<project>/{registries,cache}` là write boundary của project. Registry là linked
worktree được lock trên branch của project; cache là local state dựng lại được đã ignore và draft chưa
xong nằm dưới `cache/drafts`.
...
toàn project root. Proof cho thấy registry đã lock, clean, đúng owner và branch; cache được ignore; và
không còn state ở legacy path bị từ chối hay trong trust tree.

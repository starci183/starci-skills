...
export async function main(argv = process.argv.slice(2), io = { out: value => process.stdout.write(value), err: value => process.stderr.write(value) }) {
  ...
  try {
    ...
    if(command==='code-patterns'){
      ...
      const {scopedLintMain}=await import('../scripts/checks/check-scoped-lint.mjs');
      return (await scopedLintMain(args.slice(1),{write:value=>io.out(value)})).exitCode;
    }
    if(command==='check-stales'){
      const {checkStalesMain}=await import('../scripts/checks/check-stales.mjs');
    ...
    if(command==='architecture'){
      ...
      const {checkArchitecture}=await import('../scripts/checks/architecture.mjs');
    ...
    if(command==='work-change'){
      ...
      const {checkWorkChange}=await import('../scripts/checks/work-change.mjs');
    ...
    if(command==='work-layout'){
      ...
      const {checkWorkLayout}=await import('../scripts/checks/work-layout.mjs');
    ...
    if(command==='brand'){
      ...
      exactArgs(rest,1);
      const {runBrandChecks,formatBrandChecks}=await import('../scripts/checks/brand.mjs');
    ...
    if(command==='render'){
      ...
      exactArgs(rest,1);
      const {runRenderChecks,formatRenderChecks}=await import('../scripts/checks/render.mjs');

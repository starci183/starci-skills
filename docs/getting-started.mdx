...
Inside such a chain it still runs only once all of this exists:
...
- the flow directory: `uat/<flow>/flow.md` with the cases and their named assertions,
  `uat/<flow>/account.json` with a username, a role and a credential *name*, and `uat/<flow>/seed/`
  with the records the run namespaces;
- the sealed shared password, resolved by name from the credential roster at login and read nowhere
  else — no field of this operator can hold a secret, and its validator rejects a credential-shaped
  string anywhere in what it writes;
...
Anything missing is a stop rather than a silence: no requester or no admission is `INVALID_INPUT` at
the request gate, no sealed credential is `PROVISIONING_UNAVAILABLE`, and nothing serving the product
is `RUNTIME_UNAVAILABLE`. A mission that never reaches UAT should be able to say which of those it
met.

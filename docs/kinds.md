...
| `uat.verify` | prove | verify | no | code | `uat.verify` | Walk one end-to-end scenario and leave it with its evidence. |
...
| `implementation/backend` | `implementation` + backend side, or no side named | `backend.implement` -> `review.verify` |
| `uat` | `uat` | `uat.verify` |

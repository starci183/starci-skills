...
page is written, `critiqueGoal` is called on the host's supervisor runtimes (`supervisor.runtimes`, fable then
astra) with the job text, the scope, the ledger items, the **decided** business and architecture records in scope

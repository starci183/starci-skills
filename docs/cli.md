...
| Command | Effect |
| --- | --- |
| `init --dir <host>` | Copy source payload, verify the local source runtime, then record install; managed host bootstraps and absent local config. |
| `update --dir <host>` | Update installer-owned runtime; verify the installed source tree before recording version; preserve local modifications by default. |

---
title: "Introduction"
description: "What StarCi Skills is: one entry, operators with explicit boundaries, a chain planned from the mission, and a session made of typed requests and responses."
---

# StarCi Skills

StarCi Skills is the runtime that completes one StarCi mission. It is a tree of files, not a
framework: `SKILL.md` is the entry, `operators/` holds the units of work, the chain between them is planned
from the mission, and `routing.json` is the closed map that decides what happens after each one.
...
that touches routed source, plans the chain from the confirmed mission, selects the first operator, and then sequences the
...
holds up goes to `interface.audit`, and so on. A request that names no owner, or two owners
...
No chain is stored. Once the person confirms the mission goal, `scripts/plan-chain.mjs` derives the
chain from the operators the "done when" lines name, walking back through the Inputs, Context and
Next tables of the operators; a feature with many pages or flows runs its `<domain>.plan` first and
fans out one blind agent per unit, at most three branches a step; `scripts/validate-chain.mjs`
accepts the chain before anything is dispatched and again every time a stop redraws it.

See [Chain](/concepts/chain).

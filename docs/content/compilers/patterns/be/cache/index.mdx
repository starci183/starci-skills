...
|---|---|---|
| `CACHE-1` | `src/modules/lib/native/redis/constants.ts` | stable Redis namespace/instance keys |
| `CACHE-2` | `src/modules/databases/postgresql/primary/cache/in-memory-query-result-cache.ts` | derived query-result cache separated from source persistence |
| `CACHE-3` | `src/modules/platform/winston/types/messages/cache.ts` | cache invalidation/read observability vocabulary |
| `CACHE-4` | `src/modules/ai/utils/openrouter-cache-headers.ts` | explicit cache-control/freshness headers and policy inputs |
...
This pattern governs disposable derived read data in backend adapters and services. Durable business
effects, broker/CDC delivery claims, retry budgets and test-lane obligations remain under their own
patterns.

...
**Common business situations.** A command handler that writes data; a job replaying a mutation for a
stuck order; a CLI backfill calling the same handler; a saga retrying a failed step; an e2e test
going through the real transport; a harness calling the handler directly to measure one branch.
...
**Common business situations.** A new mutation copied from an old one; a resolver split out of a
class that carried a class-level guard; a webhook controller that adds a user parameter "just for
logging"; a subscription resolver; a query reading the caller's own data.
...
**Common business situations.** Editing or deleting one's own comment; cancelling one's own order;
reading an invoice detail; changing a profile's settings; acting on a draft; unpinning an item.
...
**Common business situations.** A private draft; someone else's installment plan; a private message;
an unpublished share link; a locked profile; a record belonging to another tenant.
...
**Common business situations.** Paid content; capstone submission; certificate download; a
higher-plan feature; commenting rights after purchase; a quota still in effect; a seat in an
organisation.
...
**Common business situations.** An infrastructure status page; key rotation; replaying a job; a
partner webhook; a pod self-registering at boot; an internal operations tool; a system-wide data
export.
...
| `AUTHZ-2` | `@canon-be` → `IDENTITY_PARAM_DECORATORS`, `hasGuard`, `identityNeedsGuard` · `features/api/core/graphql/mutations/courses/update-course-review/update-course-review.resolver.ts` | The three parameter decorators the rule counts as identity readers, the climb from the method to its class, and a live door carrying `@UseGuards(...)` above the parameter that reads the user |
...
**Accepted shape.** A signed-in product viewer deletes a course review they wrote, through a GraphQL
mutation whose resolver dispatches to a command handler.
...
**What the shape does not state, and therefore does not resolve.** It does not name which guard
class establishes the identity, so `AUTHZ-6` — whose subject's guard this is — stays open until the
subject's guard family is given. It does not say whether the caller reached this review through a
listing they were entitled to see, so the `AUTHZ-4` choice above holds only under the disclosure
input recorded with it. It says nothing about a paid relationship, so `AUTHZ-5` does not arise here
at all.

## Scope

This rule holds for any back end that authenticates at a transport boundary and decides authorization
inside handlers. It names no single feature: examples are ordinary TypeScript in a NestJS-shaped
application, and they name no product, no repository and no course. The rule id and the decorator
names the rule matches are the only proper nouns, because they are the enforcement identity and a
renamed rule cannot be cited in a config.

...
|---|---|---|---|
| `@pattern-reference-catalog` | `knowledge/compilers/patterns/source-references/references.json` | file | gắn precedent FE/BE dùng chung vào immutable Git truth |
| `@pattern-reference-schema` | `platform/readiness/initialization/workspaces/pattern-references.schema.json` | file | validate generated offline reference route |
| `@install-pattern-references` | `platform/readiness/initialization/workspaces/install-pattern-references.mjs` | script | cài immutable ref dưới `.workspaces/local/references` |
| `@portable-route-schema` | `platform/readiness/initialization/workspaces/portable-route.schema.json` | file | từ chối absolute path, observed head và secret-bearing portable declaration |

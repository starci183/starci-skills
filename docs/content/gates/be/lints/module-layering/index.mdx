...
The file sits under `/src/modules/`, so `no-self-module-alias` finds a root and derives self keys
`["ai"]`. `must-deep-module-import` is in scope everywhere and never reads the path.
...
Those two are the same line, `@modules/ai`. They are independent tests that happen to agree.
...
That is `@modules/billing`. The `export * from "@modules/platform"` is visited as an
`ExportAllDeclaration`, and `platform` is a meta root standing alone:
...
`@nestjs/common` matches no alias prefix and is never examined.

**Repaired.** The self alias becomes relative, each cross-capability import names the declaring file,
and the re-export names a file under the category folder:
...
An open hatch survives the repair. Add one more line and the seam disappears without a finding:
...
The silence is not compliance. The same import written as `@modules/billing/billing.repository`
crosses the same boundary and reads the same way to a human; only the spelling changed.

## Scope

This module documents enforcement of two rules of one back-end law, not the law itself. It does not
judge whether a barrel may be *written* — `LAYERING-5`'s declaring half has no machine — nor whether
a capability should have been wired at the composition root instead of imported, which is
`LAYERING-3` and `LAYERING-4`, held by nothing and owned by a tree-walking gate this module does not
contain. It names no product, no company and no repository. Rule names, message identifiers, alias
prefixes and folder names the rules match are **identifiers that ship** and are reproduced verbatim;
that exemption covers nothing else.

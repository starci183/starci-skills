...
Repaired — the export opens with a block that names the role, the pictograph becomes an icon
component, the functional literal keeps its value under a pragma, and the identifier is renamed:
...
Two open hatches survive this repair, and the silence is not compliance. The same prose written
without its diacritics, and the same badge written as a keycap sequence:
...
And a third silence has no rule to be silent on: `export const StepBadge = () => …` still carries no
documentation block, `COMMENTS-5` and `COMMENTS-6` are unenforced, and nothing above would say so.

## Scope

This module documents enforcement, not law. It names three rules by their published names and the law
codes they enforce. Rule identifiers, the plugin prefix and the pragma token are quoted verbatim,
because those strings ship. Everything else — every example, every path, every explanation — is
ordinary source in an ordinary front end, and names no product, library or repository. Whether a
comment adds information over the line under it, and whether it names the decision it argues with,
belong to `COMMENTS-5` and `COMMENTS-6`, which no module in this tree enforces.

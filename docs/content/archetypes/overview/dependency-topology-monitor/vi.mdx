# Bộ giám sát topology phụ thuộc
...
| Nhóm | Overview |
| Tác vụ chi phối | Hiểu health, phụ thuộc upstream/downstream và blast radius để ưu tiên ứng phó. |
| Bí danh tìm kiếm | `service dependency graph`, `blast radius monitor`, `causal topology`, `dependency health` |
| Thẩm quyền | Topology macro dùng chung, trung lập sản phẩm. |

### Bất biến

- Region graph phải giữ nguyên `topology-monitor` → `environment-filter` → `dependency-graph` → `causal-impact-path` → `impact-ranked-entity-list` → `selected-node-evidence` → `response-owner-and-action`.
...
|---|---|---|
| `AR-DT-01` | Dominant task đúng là: Hiểu health, phụ thuộc upstream/downstream và blast radius để ưu tiên ứng phó. | Bằng chứng ứng viên. |
| `AR-DT-02` | Toàn bộ region graph bắt buộc phải cùng hiện diện về mặt semantics. | Bằng chứng bắt buộc. |
| `AR-DT-03` | Compact phải giữ selection, action, state và recovery của wide. | Bằng chứng bắt buộc. |
| `AR-DT-04` | Mỗi region có owner riêng và giữ association với selection hiện tại. | Giữ như bất biến. |
| `AR-DT-90` | Dominant task thuộc ranh giới loại trừ: knowledge discovery không có health. | Từ chối. |
| `AR-DT-91` | Dominant task thuộc ranh giới loại trừ: single-parent infrastructure tree. | Từ chối. |
| `AR-DT-92` | Dominant task thuộc ranh giới loại trừ: flat service KPI grid. | Từ chối. |
| `AR-DT-93` | Dominant task thuộc ranh giới loại trừ: free-form diagram editing. | Từ chối. |

### Quy tắc chọn

Chỉ chọn `dependency-topology-monitor` khi `AR-DT-01`, `AR-DT-02` và `AR-DT-03` có bằng chứng, đồng thời không mã nào từ `AR-DT-90` đến `AR-DT-93` mô tả dominant task. Trả `needs-evidence` khi thiếu một quan hệ bắt buộc. Trả `reject` khi có rejection evidence thay vì thích nghi topology theo vẻ ngoài.
...
|---|---|---|
| `topology-monitor` | Sở hữu dominant task cấp trang và trạng thái của toàn graph. | Là gốc của graph. |
| `environment-filter` | Sở hữu evidence hoặc action của environment filter mà không vay product semantics. | Theo sau `topology-monitor` trong semantic order và giữ cùng selection context. |
| `dependency-graph` | Sở hữu evidence hoặc action của dependency graph mà không vay product semantics. | Theo sau `environment-filter` trong semantic order và giữ cùng selection context. |
| `causal-impact-path` | Sở hữu evidence hoặc action của causal impact path mà không vay product semantics. | Theo sau `dependency-graph` trong semantic order và giữ cùng selection context. |
| `impact-ranked-entity-list` | Sở hữu evidence hoặc action của impact ranked entity list mà không vay product semantics. | Theo sau `causal-impact-path` trong semantic order và giữ cùng selection context. |
| `selected-node-evidence` | Sở hữu evidence hoặc action của selected node evidence mà không vay product semantics. | Theo sau `impact-ranked-entity-list` trong semantic order và giữ cùng selection context. |
| `response-owner-and-action` | Sở hữu evidence hoặc action của response owner and action mà không vay product semantics. | Theo sau `selected-node-evidence` trong semantic order và giữ cùng selection context. |
...
- **Đáp ứng topology:** Giữ đồng thời các region bắt buộc từ `dependency-graph` đến `response-owner-and-action` khi mỗi region còn đủ measure cho nhiệm vụ.
...
- **Chủ sở hữu overflow:** `dependency-graph` là region duy nhất có thể sở hữu bounded horizontal overflow.
...
- **Chủ sở hữu overflow:** `dependency-graph` giữ overflow axis duy nhất và có hướng dẫn keyboard.
...
- **Chủ sở hữu overflow:** `dependency-graph` là view tùy chọn; text hoặc list equivalent là primary.

### Reflow

- Semantic order và DOM order là `topology-monitor` → `environment-filter` → `dependency-graph` → `causal-impact-path` → `impact-ranked-entity-list` → `selected-node-evidence` → `response-owner-and-action`.
...
|---|---|---|
| Khởi tạo / loading | `environment-filter` | Nêu scope và region đang chờ; giữ trước semantic position. |
| Sẵn sàng | `dependency-graph` | Trình bày đầy đủ dominant task và các association bắt buộc. |
| Rỗng / không áp dụng | `causal-impact-path` | Phân biệt sự vắng mặt có nghĩa với evidence không khả dụng. |
| Lỗi / thử lại | `impact-ranked-entity-list` | Giữ context còn hợp lệ và cung cấp retry cục bộ mà không reset selection. |
| Quyền / không khả dụng | `response-owner-and-action` | Không suy diễn hidden evidence là absent; cung cấp safe exit hoặc alternate route. |
| Đang chờ | `response-owner-and-action` | Chặn action lặp, giữ exact target và announce tiến độ mà không chuyển focus. |
| Thành công | `response-owner-and-action` | Hiển thị outcome, giữ selected context và cung cấp next valid action. |
| Cũ / xung đột | `environment-filter` | Giữ last safe value, nêu version hoặc time conflict và yêu cầu recovery rõ. |
| Chuyển focus | `response-owner-and-action` | Chỉ đưa focus vào modal hoặc error summary bắt buộc rồi trả về exact trigger. |
| Trình bày responsive | `topology-monitor` | Giữ state, selection, query và recovery khi topology đổi. |
...
- Từ chối khi dominant task là knowledge discovery không có health; đây là evidence `AR-DT-90` và phải route sang adjacent archetype.
- Từ chối khi dominant task là single-parent infrastructure tree; đây là evidence `AR-DT-91` và phải route sang adjacent archetype.
- Từ chối khi dominant task là flat service KPI grid; đây là evidence `AR-DT-92` và phải route sang adjacent archetype.
- Từ chối khi dominant task là free-form diagram editing; đây là evidence `AR-DT-93` và phải route sang adjacent archetype.
...
|---|---|---|
| [OpenTelemetry — Service Graph Connector](https://explorer.opentelemetry.io/collector/components/contrib/servicegraphconnector) | Hỗ trợ quan hệ giữa services và semantics telemetry tạo service graph. | Không chọn archetype này, không định nghĩa product truth và không cho phép copy geometry. |
...
```json
{
  ...
  "dominantTask": "Hiểu health, phụ thuộc upstream/downstream và blast radius để ưu tiên ứng phó.",

# Tổng quan impact và likelihood rủi ro
...
| Nhóm | Overview |
| Tác vụ chi phối | Ưu tiên risks theo likelihood, impact, exposure và mitigation state rồi kiểm tra evidence của một risk. |
| Bí danh tìm kiếm | `risk matrix`, `impact likelihood register`, `risk prioritization`, `risk exposure overview` |
| Thẩm quyền | Topology macro dùng chung, trung lập sản phẩm. |

### Bất biến

- Region graph phải giữ nguyên `risk-overview` → `scope-horizon-category-filters` → `impact-by-likelihood-matrix` → `prioritized-risk-register` → `mitigation-summary` → `selected-risk-evidence`.
...
|---|---|---|
| `AR-RI-01` | Dominant task đúng là: Ưu tiên risks theo likelihood, impact, exposure và mitigation state rồi kiểm tra evidence của một risk. | Bằng chứng ứng viên. |
| `AR-RI-02` | Toàn bộ region graph bắt buộc phải cùng hiện diện về mặt semantics. | Bằng chứng bắt buộc. |
| `AR-RI-03` | Compact phải giữ selection, action, state và recovery của wide. | Bằng chứng bắt buộc. |
| `AR-RI-04` | Mỗi region có owner riêng và giữ association với selection hiện tại. | Giữ như bất biến. |
| `AR-RI-90` | Dominant task thuộc ranh giới loại trừ: portfolio health matrix. | Từ chối. |
| `AR-RI-91` | Dominant task thuộc ranh giới loại trừ: generic issue queue. | Từ chối. |
| `AR-RI-92` | Dominant task thuộc ranh giới loại trừ: scenario forecast. | Từ chối. |
| `AR-RI-93` | Dominant task thuộc ranh giới loại trừ: color-only heatmap không có risk identities. | Từ chối. |

### Quy tắc chọn

Chỉ chọn `risk-impact-likelihood-overview` khi `AR-RI-01`, `AR-RI-02` và `AR-RI-03` có bằng chứng, đồng thời không mã nào từ `AR-RI-90` đến `AR-RI-93` mô tả dominant task. Trả `needs-evidence` khi thiếu một quan hệ bắt buộc. Trả `reject` khi có rejection evidence thay vì thích nghi topology theo vẻ ngoài.
...
|---|---|---|
| `risk-overview` | Sở hữu dominant task cấp trang và trạng thái của toàn graph. | Là gốc của graph. |
| `scope-horizon-category-filters` | Sở hữu evidence hoặc action của scope horizon category filters mà không vay product semantics. | Theo sau `risk-overview` trong semantic order và giữ cùng selection context. |
| `impact-by-likelihood-matrix` | Sở hữu evidence hoặc action của impact by likelihood matrix mà không vay product semantics. | Theo sau `scope-horizon-category-filters` trong semantic order và giữ cùng selection context. |
| `prioritized-risk-register` | Sở hữu evidence hoặc action của prioritized risk register mà không vay product semantics. | Theo sau `impact-by-likelihood-matrix` trong semantic order và giữ cùng selection context. |
| `mitigation-summary` | Sở hữu evidence hoặc action của mitigation summary mà không vay product semantics. | Theo sau `prioritized-risk-register` trong semantic order và giữ cùng selection context. |
| `selected-risk-evidence` | Sở hữu evidence hoặc action của selected risk evidence mà không vay product semantics. | Theo sau `mitigation-summary` trong semantic order và giữ cùng selection context. |
...
- **Đáp ứng topology:** Giữ đồng thời các region bắt buộc từ `impact-by-likelihood-matrix` đến `selected-risk-evidence` khi mỗi region còn đủ measure cho nhiệm vụ.
...
- **Chủ sở hữu overflow:** `impact-by-likelihood-matrix` là region duy nhất có thể sở hữu bounded horizontal overflow.
...
- **Chủ sở hữu overflow:** `impact-by-likelihood-matrix` giữ overflow axis duy nhất và có hướng dẫn keyboard.
...
- **Chủ sở hữu overflow:** `impact-by-likelihood-matrix` là view tùy chọn; text hoặc list equivalent là primary.

### Reflow

- Semantic order và DOM order là `risk-overview` → `scope-horizon-category-filters` → `impact-by-likelihood-matrix` → `prioritized-risk-register` → `mitigation-summary` → `selected-risk-evidence`.
...
|---|---|---|
| Khởi tạo / loading | `scope-horizon-category-filters` | Nêu scope và region đang chờ; giữ trước semantic position. |
| Sẵn sàng | `impact-by-likelihood-matrix` | Trình bày đầy đủ dominant task và các association bắt buộc. |
| Rỗng / không áp dụng | `prioritized-risk-register` | Phân biệt sự vắng mặt có nghĩa với evidence không khả dụng. |
| Lỗi / thử lại | `mitigation-summary` | Giữ context còn hợp lệ và cung cấp retry cục bộ mà không reset selection. |
| Quyền / không khả dụng | `selected-risk-evidence` | Không suy diễn hidden evidence là absent; cung cấp safe exit hoặc alternate route. |
| Đang chờ | `selected-risk-evidence` | Chặn action lặp, giữ exact target và announce tiến độ mà không chuyển focus. |
| Thành công | `selected-risk-evidence` | Hiển thị outcome, giữ selected context và cung cấp next valid action. |
| Cũ / xung đột | `scope-horizon-category-filters` | Giữ last safe value, nêu version hoặc time conflict và yêu cầu recovery rõ. |
| Chuyển focus | `selected-risk-evidence` | Chỉ đưa focus vào modal hoặc error summary bắt buộc rồi trả về exact trigger. |
| Trình bày responsive | `risk-overview` | Giữ state, selection, query và recovery khi topology đổi. |
...
- Từ chối khi dominant task là portfolio health matrix; đây là evidence `AR-RI-90` và phải route sang adjacent archetype.
- Từ chối khi dominant task là generic issue queue; đây là evidence `AR-RI-91` và phải route sang adjacent archetype.
- Từ chối khi dominant task là scenario forecast; đây là evidence `AR-RI-92` và phải route sang adjacent archetype.
- Từ chối khi dominant task là color-only heatmap không có risk identities; đây là evidence `AR-RI-93` và phải route sang adjacent archetype.
...
|---|---|---|
| [NIST SP 800-30 Rev. 1 — Guide for Conducting Risk Assessments](https://csrc.nist.gov/pubs/sp/800/30/r1/final) | Hỗ trợ likelihood, impact, risk determination, response context và assessment uncertainty. | Không chọn archetype này, không định nghĩa product truth và không cho phép copy geometry. |
...
```json
{
  ...
  "dominantTask": "Ưu tiên risks theo likelihood, impact, exposure và mitigation state rồi kiểm tra evidence của một risk.",

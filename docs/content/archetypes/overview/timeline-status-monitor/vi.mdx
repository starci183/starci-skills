# Bộ giám sát trạng thái theo timeline
...
| Nhóm | Overview |
| Tác vụ chi phối | Theo dõi nhiều stream trạng thái trên một trục thời gian chung để phát hiện overlap, delay và transition bất thường. |
| Bí danh tìm kiếm | `state timeline`, `status swimlanes`, `live time monitor`, `transition monitor` |
| Thẩm quyền | Topology macro dùng chung, trung lập sản phẩm. |

### Bất biến

- Region graph phải giữ nguyên `status-monitor` → `time-range-and-live-controls` → `shared-time-axis` → `status-swimlanes` → `current-marker` → `anomaly-summary` → `selected-interval-detail`.
...
|---|---|---|
| `AR-TS-01` | Dominant task đúng là: Theo dõi nhiều stream trạng thái trên một trục thời gian chung để phát hiện overlap, delay và transition bất thường. | Bằng chứng ứng viên. |
| `AR-TS-02` | Toàn bộ region graph bắt buộc phải cùng hiện diện về mặt semantics. | Bằng chứng bắt buộc. |
| `AR-TS-03` | Compact phải giữ selection, action, state và recovery của wide. | Bằng chứng bắt buộc. |
| `AR-TS-04` | Mỗi region có owner riêng và giữ association với selection hiện tại. | Giữ như bất biến. |
| `AR-TS-90` | Dominant task thuộc ranh giới loại trừ: retrospective audit causality. | Từ chối. |
| `AR-TS-91` | Dominant task thuộc ranh giới loại trừ: project plan editing. | Từ chối. |
| `AR-TS-92` | Dominant task thuộc ranh giới loại trừ: calendar event browsing. | Từ chối. |
| `AR-TS-93` | Dominant task thuộc ranh giới loại trừ: raw append-only logs. | Từ chối. |

### Quy tắc chọn

Chỉ chọn `timeline-status-monitor` khi `AR-TS-01`, `AR-TS-02` và `AR-TS-03` có bằng chứng, đồng thời không mã nào từ `AR-TS-90` đến `AR-TS-93` mô tả dominant task. Trả `needs-evidence` khi thiếu một quan hệ bắt buộc. Trả `reject` khi có rejection evidence thay vì thích nghi topology theo vẻ ngoài.
...
|---|---|---|
| `status-monitor` | Sở hữu dominant task cấp trang và trạng thái của toàn graph. | Là gốc của graph. |
| `time-range-and-live-controls` | Sở hữu evidence hoặc action của time range and live controls mà không vay product semantics. | Theo sau `status-monitor` trong semantic order và giữ cùng selection context. |
| `shared-time-axis` | Sở hữu evidence hoặc action của shared time axis mà không vay product semantics. | Theo sau `time-range-and-live-controls` trong semantic order và giữ cùng selection context. |
| `status-swimlanes` | Sở hữu evidence hoặc action của status swimlanes mà không vay product semantics. | Theo sau `shared-time-axis` trong semantic order và giữ cùng selection context. |
| `current-marker` | Sở hữu evidence hoặc action của current marker mà không vay product semantics. | Theo sau `status-swimlanes` trong semantic order và giữ cùng selection context. |
| `anomaly-summary` | Sở hữu evidence hoặc action của anomaly summary mà không vay product semantics. | Theo sau `current-marker` trong semantic order và giữ cùng selection context. |
| `selected-interval-detail` | Sở hữu evidence hoặc action của selected interval detail mà không vay product semantics. | Theo sau `anomaly-summary` trong semantic order và giữ cùng selection context. |
...
- **Đáp ứng topology:** Giữ đồng thời các region bắt buộc từ `shared-time-axis` đến `selected-interval-detail` khi mỗi region còn đủ measure cho nhiệm vụ.
...
- **Chủ sở hữu overflow:** `status-swimlanes` là region duy nhất có thể sở hữu bounded horizontal overflow.
...
- **Chủ sở hữu overflow:** `status-swimlanes` giữ overflow axis duy nhất và có hướng dẫn keyboard.
...
- **Chủ sở hữu overflow:** `status-swimlanes` là view tùy chọn; text hoặc list equivalent là primary.

### Reflow

- Semantic order và DOM order là `status-monitor` → `time-range-and-live-controls` → `shared-time-axis` → `status-swimlanes` → `current-marker` → `anomaly-summary` → `selected-interval-detail`.
...
|---|---|---|
| Khởi tạo / loading | `time-range-and-live-controls` | Nêu scope và region đang chờ; giữ trước semantic position. |
| Sẵn sàng | `shared-time-axis` | Trình bày đầy đủ dominant task và các association bắt buộc. |
| Rỗng / không áp dụng | `status-swimlanes` | Phân biệt sự vắng mặt có nghĩa với evidence không khả dụng. |
| Lỗi / thử lại | `current-marker` | Giữ context còn hợp lệ và cung cấp retry cục bộ mà không reset selection. |
| Quyền / không khả dụng | `selected-interval-detail` | Không suy diễn hidden evidence là absent; cung cấp safe exit hoặc alternate route. |
| Đang chờ | `selected-interval-detail` | Chặn action lặp, giữ exact target và announce tiến độ mà không chuyển focus. |
| Thành công | `selected-interval-detail` | Hiển thị outcome, giữ selected context và cung cấp next valid action. |
| Cũ / xung đột | `time-range-and-live-controls` | Giữ last safe value, nêu version hoặc time conflict và yêu cầu recovery rõ. |
| Chuyển focus | `selected-interval-detail` | Chỉ đưa focus vào modal hoặc error summary bắt buộc rồi trả về exact trigger. |
| Trình bày responsive | `status-monitor` | Giữ state, selection, query và recovery khi topology đổi. |
...
- Từ chối khi dominant task là retrospective audit causality; đây là evidence `AR-TS-90` và phải route sang adjacent archetype.
- Từ chối khi dominant task là project plan editing; đây là evidence `AR-TS-91` và phải route sang adjacent archetype.
- Từ chối khi dominant task là calendar event browsing; đây là evidence `AR-TS-92` và phải route sang adjacent archetype.
- Từ chối khi dominant task là raw append-only logs; đây là evidence `AR-TS-93` và phải route sang adjacent archetype.
...
|---|---|---|
| [Grafana — State timeline](https://grafana.com/docs/grafana/latest/visualizations/panels-visualizations/visualizations/state-timeline/) | Hỗ trợ thay đổi trạng thái, intervals, nhiều entities và bounded time navigation. | Không chọn archetype này, không định nghĩa product truth và không cho phép copy geometry. |
...
```json
{
  ...
  "dominantTask": "Theo dõi nhiều stream trạng thái trên một trục thời gian chung để phát hiện overlap, delay và transition bất thường.",

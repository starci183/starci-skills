...
| Family | Work |
| Dominant task | Thử join vật lý không phá hủy giữa các fragment, kết hợp join tương thích thành assembly hypothesis và giữ uncertainty cùng conservation limit. |
...
- `refit-assembly` sở hữu toàn bộ dominant task, work state và recovery boundary.
- Thử join vật lý không phá hủy giữa các fragment, kết hợp join tương thích thành assembly hypothesis và giữ uncertainty cùng conservation limit.
...
|---|---|---|
| `REF-01` | Thử join vật lý không phá hủy giữa các fragment, kết hợp join tương thích thành assembly hypothesis và giữ uncertainty cùng conservation limit. | Bằng chứng dương bắt buộc. |
| `REF-02` | Mọi region và quan hệ bắt buộc trong graph đều cần để hoàn tất task. | Yêu cầu graph đầy đủ. |
| `REF-03` | Template phải hoàn thành scenario riêng của leaf qua toàn bộ graph, bộc lộ failure bắt buộc, sửa bằng điều khiển keyboard-complete và chỉ phát hành completion receipt sau khi proof pass. Chuỗi thao tác bắt buộc: Compare F-12 against two candidate mates → Record material, edge, and decoration evidence → Fit F-19 and expose overlap with F-27 → Lock transform within tolerance → Approve assembly A and retain rejected B → Archive non-destructive hypothesis RF-012. | Yêu cầu proof path đặc thù của domain. |
| `REF-04` | Work state phải tồn tại qua cả ba topology response đã đặt tên. | Yêu cầu responsive parity. |
| `REF-05` | Các state literal của domain được giữ nguyên để đồng bộ runtime: fragment stable/restricted, candidate unreviewed/likely/unlikely, surface sufficient/eroded, join aligned/misaligned, contact within/outside tolerance, transform provisional/locked, assembly compatible/overlapping, hypothesis active/mutually exclusive/superseded, conservation handling allowed/blocked and review accepted/rejected. | Yêu cầu bao phủ state và recovery. |
| `REF-90` | Từ chối khi candidate thuộc adjacent archetype `canvas-inspector-studio`, `entity-resolution-cluster-adjudicator`, `knowledge-graph-explorer`, `chain-of-custody-transfer-ledger` hoặc thiếu bất kỳ evidence owner bắt buộc nào của graph. Từ chối biến thể chỉ đổi noun, density, color, component, card count hoặc state. | Reject. |
| `REF-91` | Candidate chỉ khác product noun, density, color, component, card count hoặc state variation. | Reject as `duplicate-or-variation`. |
| `REF-92` | Một adjacent archetype sở hữu work object hoặc completion event chính xác hơn. | Reject. |

### Selection rule

Chọn `archaeological-fragment-refit-assembly-workbench` khi và chỉ khi các code `REF-01`–`05` đều có bằng chứng và không có code `REF-90`–`92`. Trả `needs-evidence` nếu dominant task, owner, overflow owner hoặc completion consequence chưa được chứng minh; trả `reject` khi có rejection code.
...
Biểu thức quan hệ binding là `refit-assembly → assemblage-provenience-material-and-conservation-context → fragment-register-and-surface-or-edge-evidence → candidate-pair-match-graph → selected-join-geometric-material-and-decoration-proof → rigid-transform-and-physical-contact-fit → transitive-assembly-hypothesis ↔ overlap-incompatibility-and-mutual-exclusion-ledger → manufacturing-sequence-and-context-redistribution-inference → reviewed-non-destructive-assembly-and-archive`; các operator giữ nghĩa directed, peer hoặc joint-axis đã khai báo trong prompt.

### Region obligations

| Vùng | Nghĩa vụ owner và quan hệ |
|---|---|
| `refit-assembly` | refit-assembly sở hữu bằng chứng và trạng thái của chính vùng này; sở hữu boundary của dominant task và truyền identity không đổi tới `assemblage-provenience-material-and-conservation-context`. Vùng này không hấp thụ owner của vùng khác. |
| `assemblage-provenience-material-and-conservation-context` | assemblage-provenience-material-and-conservation-context sở hữu bằng chứng và trạng thái của chính vùng này; giữ quan hệ → với upstream `refit-assembly` và truyền identity không đổi tới `fragment-register-and-surface-or-edge-evidence`. Vùng này không hấp thụ owner của vùng khác. |
| `fragment-register-and-surface-or-edge-evidence` | fragment-register-and-surface-or-edge-evidence sở hữu bằng chứng và trạng thái của chính vùng này; giữ quan hệ → với upstream `assemblage-provenience-material-and-conservation-context` và truyền identity không đổi tới `candidate-pair-match-graph`. Vùng này không hấp thụ owner của vùng khác. |
| `candidate-pair-match-graph` | candidate-pair-match-graph sở hữu bằng chứng và trạng thái của chính vùng này; giữ quan hệ → với upstream `fragment-register-and-surface-or-edge-evidence` và truyền identity không đổi tới `selected-join-geometric-material-and-decoration-proof`. Vùng này không hấp thụ owner của vùng khác. |
| `selected-join-geometric-material-and-decoration-proof` | selected-join-geometric-material-and-decoration-proof sở hữu bằng chứng và trạng thái của chính vùng này; giữ quan hệ → với upstream `candidate-pair-match-graph` và truyền identity không đổi tới `rigid-transform-and-physical-contact-fit`. Vùng này không hấp thụ owner của vùng khác. |
| `rigid-transform-and-physical-contact-fit` | rigid-transform-and-physical-contact-fit sở hữu bằng chứng và trạng thái của chính vùng này; giữ quan hệ → với upstream `selected-join-geometric-material-and-decoration-proof` và truyền identity không đổi tới `transitive-assembly-hypothesis`. Vùng này không hấp thụ owner của vùng khác. |
| `transitive-assembly-hypothesis` | transitive-assembly-hypothesis sở hữu bằng chứng và trạng thái của chính vùng này; giữ quan hệ → với upstream `rigid-transform-and-physical-contact-fit` và truyền identity không đổi tới `overlap-incompatibility-and-mutual-exclusion-ledger`. Vùng này không hấp thụ owner của vùng khác. |
| `overlap-incompatibility-and-mutual-exclusion-ledger` | overlap-incompatibility-and-mutual-exclusion-ledger sở hữu bằng chứng và trạng thái của chính vùng này; giữ quan hệ ↔ với upstream `transitive-assembly-hypothesis` và truyền identity không đổi tới `manufacturing-sequence-and-context-redistribution-inference`. Vùng này không hấp thụ owner của vùng khác. |
| `manufacturing-sequence-and-context-redistribution-inference` | manufacturing-sequence-and-context-redistribution-inference sở hữu bằng chứng và trạng thái của chính vùng này; giữ quan hệ → với upstream `overlap-incompatibility-and-mutual-exclusion-ledger` và truyền identity không đổi tới `reviewed-non-destructive-assembly-and-archive`. Vùng này không hấp thụ owner của vùng khác. |
| `reviewed-non-destructive-assembly-and-archive` | reviewed-non-destructive-assembly-and-archive sở hữu bằng chứng và trạng thái của chính vùng này; giữ quan hệ → với upstream `manufacturing-sequence-and-context-redistribution-inference`; phát hành completion evidence. Vùng này không hấp thụ owner của vùng khác. |
...
- **Topology response:** Tái cấu trúc dominant task thành một route primary theo semantic order `refit-assembly → assemblage-provenience-material-and-conservation-context → fragment-register-and-surface-or-edge-evidence → candidate-pair-match-graph → selected-join-geometric-material-and-decoration-proof → rigid-transform-and-physical-contact-fit → transitive-assembly-hypothesis ↔ overlap-incompatibility-and-mutual-exclusion-ledger → manufacturing-sequence-and-context-redistribution-inference → reviewed-non-destructive-assembly-and-archive`; support mở theo stage và không tạo horizontal scroll cấp trang.
...
- Các state literal của domain được giữ nguyên để đồng bộ runtime: fragment stable/restricted, candidate unreviewed/likely/unlikely, surface sufficient/eroded, join aligned/misaligned, contact within/outside tolerance, transform provisional/locked, assembly compatible/overlapping, hypothesis active/mutually exclusive/superseded, conservation handling allowed/blocked and review accepted/rejected.
...
- Từ chối khi candidate thuộc adjacent archetype `canvas-inspector-studio`, `entity-resolution-cluster-adjudicator`, `knowledge-graph-explorer`, `chain-of-custody-transfer-ledger` hoặc thiếu bất kỳ evidence owner bắt buộc nào của graph. Từ chối biến thể chỉ đổi noun, density, color, component, card count hoặc state.
- Reject khi adjacent archetype sở hữu work object hoặc completion event chính xác hơn.

### Boundary verdict

Trả `accept` chỉ khi selection rule pass. Trả `reject` cho `REF-90`–`92`. Trả `needs-evidence` khi business truth chưa chứng minh dominant task, owner relationship, overflow owner hoặc completion consequence.
...
| [Historic England guidance on lithic refitting studies](https://historicengland.org.uk/images-books/publications/managing-lithic-sites/heag318-managing-lithic-sites/) | Cung cấp thuật ngữ và ràng buộc chuyên ngành cho dominant task. | Không chứng minh product truth, geometry, breakpoint, component hoặc visual direction. |
| [Canadian Conservation Institute care of archaeological collections guidance](https://www.canada.ca/en/conservation-institute/services/learning-activities/care-archaeological-collections.html) | Cung cấp thuật ngữ và ràng buộc chuyên ngành cho dominant task. | Không chứng minh product truth, geometry, breakpoint, component hoặc visual direction. |

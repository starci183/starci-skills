...
| Family | Work |
| Dominant task | Sắp master lên các mặt đĩa vật lý bằng dự báo ngân sách groove theo bán kính và rủi ro cutting, rồi chọn order, edit, level hoặc format trước khi handoff. |
...
- `vinyl-sequencing` sở hữu toàn bộ dominant task, work state và recovery boundary.
- Sắp master lên các mặt đĩa vật lý bằng dự báo ngân sách groove theo bán kính và rủi ro cutting, rồi chọn order, edit, level hoặc format trước khi handoff.
...
|---|---|---|
| `VIN-01` | Sắp master lên các mặt đĩa vật lý bằng dự báo ngân sách groove theo bán kính và rủi ro cutting, rồi chọn order, edit, level hoặc format trước khi handoff. | Bằng chứng dương bắt buộc. |
| `VIN-02` | Mọi region và quan hệ bắt buộc trong graph đều cần để hoàn tất task. | Yêu cầu graph đầy đủ. |
| `VIN-03` | Template phải hoàn thành scenario riêng của leaf qua toàn bộ graph, bộc lộ failure bắt buộc, sửa bằng điều khiển keyboard-complete và chỉ phát hành completion receipt sau khi proof pass. Chuỗi thao tác bắt buộc: Load both side manifests → Move Track 5 using buttons or keyboard → Recompute radial position and expose inner-groove risk → Compare reorder and level counterfactuals → Record cutting engineer choice → Approve side manifests VM-005. | Yêu cầu proof path đặc thù của domain. |
| `VIN-04` | Work state phải tồn tại qua cả ba topology response đã đặt tên. | Yêu cầu responsive parity. |
| `VIN-05` | Các state literal của domain được giữ nguyên để đồng bộ runtime: master missing/current/revised, side under/near/over budget, track outer/mid/inner radius, gap or lock groove valid/invalid, bass/stereo/sibilance risk low/raised/blocking, counterfactual unsolved/viable, engineer note open/answered, test cut pending/pass/fail and manifest approved/superseded. | Yêu cầu bao phủ state và recovery. |
| `VIN-90` | Từ chối khi candidate thuộc adjacent archetype `multi-track-timeline-workbench`, `audio-mix-routing-console` hoặc thiếu bất kỳ evidence owner bắt buộc nào của graph. Từ chối biến thể chỉ đổi noun, density, color, component, card count hoặc state. | Reject. |
| `VIN-91` | Candidate chỉ khác product noun, density, color, component, card count hoặc state variation. | Reject as `duplicate-or-variation`. |
| `VIN-92` | Một adjacent archetype sở hữu work object hoặc completion event chính xác hơn. | Reject. |

### Selection rule

Chọn `vinyl-side-sequencing-groove-budget-planner` khi và chỉ khi các code `VIN-01`–`05` đều có bằng chứng và không có code `VIN-90`–`92`. Trả `needs-evidence` nếu dominant task, owner, overflow owner hoặc completion consequence chưa được chứng minh; trả `reject` khi có rejection code.
...
Biểu thức quan hệ binding là `vinyl-sequencing → release-format-disc-size-speed-and-lathe-profile → track-master-register-and-technical-analysis → side-a-side-b-membership-and-running-order → per-side-duration-gap-and-lock-groove-ledger → radial-groove-budget-and-inner-radius-projection ↔ level-low-frequency-stereo-phase-and-sibilance-risks → reorder-edit-level-or-format-counterfactuals → cutting-engineer-notes-and-side-master-manifest → test-cut-approval-and-pressing-handoff`; các operator giữ nghĩa directed, peer hoặc joint-axis đã khai báo trong prompt.

### Region obligations

| Vùng | Nghĩa vụ owner và quan hệ |
|---|---|
| `vinyl-sequencing` | vinyl-sequencing sở hữu bằng chứng và trạng thái của chính vùng này; sở hữu boundary của dominant task và truyền identity không đổi tới `release-format-disc-size-speed-and-lathe-profile`. Vùng này không hấp thụ owner của vùng khác. |
| `release-format-disc-size-speed-and-lathe-profile` | release-format-disc-size-speed-and-lathe-profile sở hữu bằng chứng và trạng thái của chính vùng này; giữ quan hệ → với upstream `vinyl-sequencing` và truyền identity không đổi tới `track-master-register-and-technical-analysis`. Vùng này không hấp thụ owner của vùng khác. |
| `track-master-register-and-technical-analysis` | track-master-register-and-technical-analysis sở hữu bằng chứng và trạng thái của chính vùng này; giữ quan hệ → với upstream `release-format-disc-size-speed-and-lathe-profile` và truyền identity không đổi tới `side-a-side-b-membership-and-running-order`. Vùng này không hấp thụ owner của vùng khác. |
| `side-a-side-b-membership-and-running-order` | side-a-side-b-membership-and-running-order sở hữu bằng chứng và trạng thái của chính vùng này; giữ quan hệ → với upstream `track-master-register-and-technical-analysis` và truyền identity không đổi tới `per-side-duration-gap-and-lock-groove-ledger`. Vùng này không hấp thụ owner của vùng khác. |
| `per-side-duration-gap-and-lock-groove-ledger` | per-side-duration-gap-and-lock-groove-ledger sở hữu bằng chứng và trạng thái của chính vùng này; giữ quan hệ → với upstream `side-a-side-b-membership-and-running-order` và truyền identity không đổi tới `radial-groove-budget-and-inner-radius-projection`. Vùng này không hấp thụ owner của vùng khác. |
| `radial-groove-budget-and-inner-radius-projection` | radial-groove-budget-and-inner-radius-projection sở hữu bằng chứng và trạng thái của chính vùng này; giữ quan hệ → với upstream `per-side-duration-gap-and-lock-groove-ledger` và truyền identity không đổi tới `level-low-frequency-stereo-phase-and-sibilance-risks`. Vùng này không hấp thụ owner của vùng khác. |
| `level-low-frequency-stereo-phase-and-sibilance-risks` | level-low-frequency-stereo-phase-and-sibilance-risks sở hữu bằng chứng và trạng thái của chính vùng này; giữ quan hệ ↔ với upstream `radial-groove-budget-and-inner-radius-projection` và truyền identity không đổi tới `reorder-edit-level-or-format-counterfactuals`. Vùng này không hấp thụ owner của vùng khác. |
| `reorder-edit-level-or-format-counterfactuals` | reorder-edit-level-or-format-counterfactuals sở hữu bằng chứng và trạng thái của chính vùng này; giữ quan hệ → với upstream `level-low-frequency-stereo-phase-and-sibilance-risks` và truyền identity không đổi tới `cutting-engineer-notes-and-side-master-manifest`. Vùng này không hấp thụ owner của vùng khác. |
| `cutting-engineer-notes-and-side-master-manifest` | cutting-engineer-notes-and-side-master-manifest sở hữu bằng chứng và trạng thái của chính vùng này; giữ quan hệ → với upstream `reorder-edit-level-or-format-counterfactuals` và truyền identity không đổi tới `test-cut-approval-and-pressing-handoff`. Vùng này không hấp thụ owner của vùng khác. |
| `test-cut-approval-and-pressing-handoff` | test-cut-approval-and-pressing-handoff sở hữu bằng chứng và trạng thái của chính vùng này; giữ quan hệ → với upstream `cutting-engineer-notes-and-side-master-manifest`; phát hành completion evidence. Vùng này không hấp thụ owner của vùng khác. |
...
- **Topology response:** Tái cấu trúc dominant task thành một route primary theo semantic order `vinyl-sequencing → release-format-disc-size-speed-and-lathe-profile → track-master-register-and-technical-analysis → side-a-side-b-membership-and-running-order → per-side-duration-gap-and-lock-groove-ledger → radial-groove-budget-and-inner-radius-projection ↔ level-low-frequency-stereo-phase-and-sibilance-risks → reorder-edit-level-or-format-counterfactuals → cutting-engineer-notes-and-side-master-manifest → test-cut-approval-and-pressing-handoff`; support mở theo stage và không tạo horizontal scroll cấp trang.
...
- Template phải hoàn thành scenario riêng của leaf qua toàn bộ graph, bộc lộ failure bắt buộc, sửa bằng điều khiển keyboard-complete và chỉ phát hành completion receipt sau khi proof pass. Chuỗi thao tác bắt buộc: Load both side manifests → Move Track 5 using buttons or keyboard → Recompute radial position and expose inner-groove risk → Compare reorder and level counterfactuals → Record cutting engineer choice → Approve side manifests VM-005.
- Chỉ accept variation khi dominant task, required regions, relationships, transformations và completion event không đổi.

### Reject

- Từ chối khi candidate thuộc adjacent archetype `multi-track-timeline-workbench`, `audio-mix-routing-console` hoặc thiếu bất kỳ evidence owner bắt buộc nào của graph. Từ chối biến thể chỉ đổi noun, density, color, component, card count hoặc state.
- Reject khi adjacent archetype sở hữu work object hoặc completion event chính xác hơn.

### Boundary verdict

Trả `accept` chỉ khi selection rule pass. Trả `reject` cho `VIN-90`–`92`. Trả `needs-evidence` khi business truth chưa chứng minh dominant task, owner relationship, overflow owner hoặc completion consequence.
...
| [Ableton's mastering-for-vinyl guidance](https://www.ableton.com/en/blog/mastering-tracks-for-vinyl-record/) | Cung cấp thuật ngữ và ràng buộc chuyên ngành cho dominant task. | Không chứng minh product truth, geometry, breakpoint, component hoặc visual direction. |
| [Mixonic Vinyl Audio Guidelines](https://www.mixonic.com/VinylAudioGuidelines.pdf) | Cung cấp thuật ngữ và ràng buộc chuyên ngành cho dominant task. | Không chứng minh product truth, geometry, breakpoint, component hoặc visual direction. |

...
- Resolution: launch configured Qwen 3.8 Flash in a terminal named `[Op] <operation> - <scope>`, wait for `tui-idle`, then attach that exact handle with `worker-start --terminal` and reapply the name after attachment. A DeepSeek or other exact Qwen-compatible model override remains a separate adapter gap until Orca can pass that model through a supervised native launch.

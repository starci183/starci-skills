...
not edited - `scripts/`, `checks/`, `.github/` and `tests/` are shared-file territory that lane
...
`videos/`, one stale `blockedBy` rev), and `scripts/check-example-derived.mjs` refuses the todo
...
| `scripts/check-example-yaml.mjs` | exit 0 - "453 yaml file(s) under examples: all accepted" |
...
| `scripts/check-example-work.mjs` | **exit 1** - 4 refused, 15 warned; every line names `examples/todo-app-backend/...`, and no output line mentions `ecommerce-app-be` (grep-verified) |
| `scripts/check-example-derived.mjs` | **exit 1** - "2 work tree(s) checked: 2 refused"; both refusals name `todo-app-backend/_derived`, from the counting in the gate itself (`problems.length`, not tree count) |
| `scripts/check-scoped-lint.mjs --profile nest` | **exit 1** for both examples - see below |
| `scripts/check-json-exceptions.mjs` | **exit 1** before visiting any example - see #7 |
...
`node scripts/check-json-exceptions.mjs` exits 1 with

...
- **The example tree's family layout does not match `schemas/work-layout.yaml`'s documented shape.** The
  ...
  it, `scripts/checks/check-example-work.mjs`'s own `FAMILIES` set (`br, ac, fr, nfr, data, journey, decision, sds,
...
- **Three `done` implementation records name source files that do not exist.** All three backend `impl`
  ...
  neither the staleness check nor `scripts/checks/check-example-work.mjs` verifies that an `owners[].path` actually
...
- **A `done` implementation proves specifications that reverted to `todo`, and nothing flags it.**
  ...
  change. `scripts/checks/check-example-work.mjs` checks that `blockedBy`/`conflictsWith`/`refs`/`appliesTo`/
...
- **A gap's `statement` names two files; its `closedBy` names one record.** `gap.share.unbuilt-module`'s
  ...
  `scripts/checks/check-example-work.mjs`) allows exactly one `closedBy`, so a gap spanning two implementation

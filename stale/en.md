...
|---|---|---|
| `@stale-source-gates` | `stale/source-gates` | declared project gates and source findings |
| `@stale-lint-machine` | `stale/lint-machine` | canon adoption and vendored-rule detection |
| `@stale-strict-fix` | `stale/strict-fix` | first-party Prettier integration |
| `@stale-why` | `stale/why` | contract index findability |
| `@stale-assurance` | `stale/assurance` | backend delivery assurance applicability and fence |
| `@stale-retired-structure` | `stale/retired-structure` | removed component tiers including empty paths |
| `@stale-remnant` | `stale/remnant` | nested legacy `.claude/` trees |

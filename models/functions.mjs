...
export const VALIDATOR_RULES=[
  ...
  'an interface.draw result is image-first: for a large frontend scope it selects a small representative set of critical screens and invokes the built-in image_gen.imagegen tool using accepted business/SDS, brand, relevant UI knowledge and actual installed Grammar anatomy/reference inputs; every generated image keeps its exact prompt and actual tool provenance, and a claimed model name the tool did not expose is a defect',
  'interface.draw also carries an explicit coverage/component/state map for every remaining required screen, state and viewport; a generated direction is proposed visual guidance, not exact Grammar component/render/API proof or a working-product screenshot, so a result that uses it to waive actual implementation captures, code checks or later browser UAT is a defect',

...
|---|---|---|---|
| `@request-schema` | `requests/request.schema.json` | file | validate một frontend design request bền vững |
| `@rejects-schema` | `requests/rejects.schema.json` | file | validate bảng rejected source attempts |
| `@validate-request` | `scripts/validate-design-request.mjs` | script | validate một request hoặc toàn bộ queue |

## Record

`requests/*.request.json` là queue bền giữa bước nhận feedback và bước resolve design. Request có thể mô tả mọi
...
`requests/rejects.json` là bảng append-only cho source attempt bị reject trong resolution. Một reject giữ request

...
| A10 | `_pending-*` | 0 files | no | closed 2026-09-02: contrast → `ui/proof/contrast.md` (`384a3ec9`); surface, boundary, icon, media, control-state, field → `grammars/starci/{surface,boundary,icon,media,control-state,field}.md` (+vi) with one `Case \| Rule \| Common owner \| Core realization` table per rule, every owner read from `packages/grammar/src` (`02f06b02`); the fourteen files deleted and the "Awaiting relocation" section removed after a recount to 118 ids (`85f3863d`) | decided as recorded; one follow-up: `grammars/starci/surface.md` and `boundary.md` keep the legacy `SURFACE-1..5` and `BOUNDARY-1..5` numbers while `ui/presentation/` publishes a different `SURFACE-1..6` and `BOUNDARY-1..6`; the files carry a prefix notice, and an invocation may bind only one series per prefix |
...
| B4 | `LearnSpine/component.spec.tsx` | never completes in a full run | the only file of 497 that never reports under either pool: `forks` kills the worker, `threads` hangs it. In isolation it finishes in 38ms and fails one ordinary assertion (group label "Your path" not rendered, same family as B5). An order-dependent hang: an earlier spec in the same worker leaks a timer or handle that this file's imports then wait on | none; bisect by running it after each preceding spec directory, then close the leak at its source |
| B5 | 33 red tests in starci, confirmed under both pools | pre-existing | stale attribute contracts (`data-variant`, `data-tone`), selection-vs-destination tests, `SourceFileTree` active state | which stale contracts Grammar should re-publish |

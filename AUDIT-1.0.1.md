...
| A3 | FE loop operators | 4 packages | no | dry-run `frontend.direction.decide` → `frontend.presentation.resolve` → `frontend.source.apply` → `frontend.surface.audit` on the dashboard; each `execute.md` followable, each output validates. 2026-09-02: `frontend.presentation.resolve` run on `dashboard/ContinueLearning` from real fingerprints; both artifacts validate; outcome `blocked · RULE_MISSING` because `gap.md` has no case for a leading mark beside a copy block nor for a copy column ending in its action, and the authorized evidence holds one instance of each, so no case was written ([record](audits/1.0.1/a3-presentation-resolve/README.md)). The other three operators wait for a resolvable block and a stable FE checkout | none; the two missing gap cases wait for a second authorized block |
...
| A7 | `ui/proof/` | 5 topics | no | closed 2026-09-02: `_pending-contrast` relocated to `ui/proof/contrast.md` (+vi) as COLOR-3 and COLOR-5 in the proof rule shape, catalog row added, `frontend.surface.audit` self-test binds and cites the `contrast` topic (`384a3ec9`); `Observe` column of the other four topics still to be read by the owner | decided: measured contrast lands in `ui/proof/contrast.md` |
...
| A11 | `resources/` | 2 profile files, 14 operators | no | decided 2026-09-02: each operator binds exactly one profile and runs on it end to end, declared inside its own `operator.json` under `resources` and stated in its `context.md` (`assignments.json` and `roles` retired; the validator rejects a missing or split binding and a process-matrix row that disagrees). sol-fresh: business, architecture, fe.direction; sonnet: workspace.bind, frontend.presentation.resolve, quality.verify, git.publish; opus: backend.source.apply, frontend.source.apply, release.deploy, platform.operate; sol-reviewer: frontend.surface.audit, uat.verify; luna: content.generate (schema-pinned; now permitted bounded web research for its own brief). The policy answers stay as set | whether `fable` should bind any operator |
...
| A13 | `data-contract` in Grammar | 41 renderers | no | every element that realizes a "Common already owns" row carries `data-contract` with the published ids, computed from resolved props where the value depends on them; starci-academy-fe `14e0c20` (tsc clean, 34 spec files / 126 tests green, lint 0). Knowledge-vs-source drift found and not papered over: `SectionHeader` default gap is `1.5rem` in `styles.css` while `gap.md` says `.75rem`; `MarkdownArticle`, `FencedCodeBlock`, `MarkdownTableFrame` never apply their `starci-core-*` classes, so those CSS rules and their knowledge rows are unrealized; `EmptyNotice` has no element realizing its MARGIN-0 row; `StaticStateRow` inline `1rem` inset has no row | which side is right for `SectionHeader` gap; whether the markdown classes are wired or the rows removed |

...
|---|---|---|---|
| `@mcp-qdrant` | `platform/mcp/qdrant/context.md` | context | provision or verify the local vector service and MCP runtime |
| `@mcp-embedding` | `platform/mcp/embedding/context.md` | context | inspect hardware, install Ollama and choose one dimension-locked embedding profile |
| `@mcp-source-context` | `platform/mcp/source-context/context.md` | context | index one or more verified workspace roles and generate client configuration |
| `@mcp-clients` | `platform/mcp/clients/context.md` | context | install the verified remote endpoint into both Codex/OpenAI and Claude Code |

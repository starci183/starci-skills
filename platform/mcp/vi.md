...
|---|---|---|---|
| `@mcp-qdrant` | `platform/mcp/qdrant/vi.md` | vi | chuẩn bị hoặc kiểm tra dịch vụ vector cục bộ và runtime MCP |
| `@mcp-embedding` | `platform/mcp/embedding/vi.md` | vi | đo phần cứng, cài Ollama và chọn một embedding profile đã khóa dimension |
| `@mcp-source-context` | `platform/mcp/source-context/vi.md` | vi | lập chỉ mục một hay nhiều role workspace đã xác minh và sinh cấu hình client |
| `@mcp-clients` | `platform/mcp/clients/vi.md` | vi | cài endpoint remote đã xác minh vào cả Codex/OpenAI và Claude Code |

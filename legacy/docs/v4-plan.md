...
| 7 Contracts & docs | `providers/orca/index|recipes|validation.yaml` reference `calls`; `docs/orca-execution.md`, `execution-agent-model.md`, `legacy/docs/orca-runtime-upgrades.md` (mark F1–F3 resolved-in-runtime); `SKILL.md` paragraph on Orca launch rewritten to point at `start-op`/`settle`/`replace-monitor`; the 4.0 pre-release version; `INDEX.yaml`, `README.yaml` | as named | `npm run build`, `build:check`, `build:knowledge:check`, `npm test`, `starci doctor --quick` in a scratch host | 2 h |
...
6. `npm test`, `build:check`, `doctor --quick` green; `legacy/docs/orca-runtime-upgrades.md` records F1–F3 as runtime-resolved with the regression test names.

...
| `judged` | `interface.generate` | The operator decides for itself, from the direction, whether a region reads empty enough to need artwork, records why, and never decorates |
...
`business.decide` is exclusive per businesses root, `runtime.serve` per runtime owner.

...
description: "How one mission runs: workspace.bind, the chain planned from the mission, request.json, the branch layout, the validators, stop codes and resume."
---

# How a mission runs

New installations first use [StarCi Lite](https://github.com/starci183/starci-skills/blob/main/skills/starci-lite/SKILL.md)
to classify the request. The sequence below applies when that classification or the selected host
profile calls for full StarCi; questions and bounded Lite work do not create these formal records.
...
Nothing is looked up. The person confirms the mission goal — what it includes, what it excludes, and
the "done when" lines that end it — and `scripts/plan-chain.mjs` derives the chain from those lines
through the operators' `## Inputs`, `## Context` and `## Next` tables: every required input produced
by an earlier step, every role bound, the audit and the walk between an applied surface and its
publish, a `<domain>.plan` before an operator that several lines name so its units fan out one blind
agent each, at most three branches a step. The plan is printed as two lines per branch, and
`scripts/validate-chain.mjs` accepts it before step 4 begins; an ambiguity in the tables is a refusal
that names it, not a guess. See [Chain](/concepts/chain).
...
- `done` advances the chain to the next step the plan names;
...
one does, the runtime is not started by the operator that needs it: `runtime.serve` owns it, and
...
It runs whenever the mission's "done when" names it, and the planner adds it on its own to every
chain that writes frontend source under `mode: apply` and reaches `git.publish`: the long-flow law.
A mission with several journeys runs `uat.plan` first, which names one flow per journey, and then
`uat.verify` once per flow, at most three at a time. A backend-only mission owes no walk, and says so
by naming no surface.

Inside such a chain, exactly two things must come from outside it:

- a person who asked, by name — `requestedBy`, `feature` and `flow` have no defaults, so the entry
  takes them from the confirmed mission and the branch refuses rather than inventing a requester;
...
- the account is provisioned by `identity.provision` against the identity the runtime registry declares

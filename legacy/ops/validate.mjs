...
// Operator validation runs on the source tree; there is no compiled `.dist` form.
// Keep these authoring identities explicit here and regression-check them against the
// published specification contracts; importing runtime readers would create a validation cycle.
// The two kernel imports below are the exception and are safe because they are pure: the profiles they
// answer over are handed in by the caller (or read from the authored YAML sources).
...
const AUTHORING_POLICIES=Object.freeze({
  'business.decide':Object.freeze({
    ...
    compatibilitySchemas:Object.freeze(['starci/specification@2','starci/srs@3']),proseLanguage:'en',preserveEnglish:PRESERVED_ENGLISH
  }),
  'architecture.decide':Object.freeze({
    ...
    compatibilitySchemas:Object.freeze(['starci/specification@3']),proseLanguage:'en',preserveEnglish:PRESERVED_ENGLISH
  })
});
...
/**
 * The kinds and records profiles, for a caller that did not pass them. The canonical records are the
 * authored `modules/models/<name>.yaml` (the retired tree survives under `legacy/model/`); a tree with
 * neither simply skips the input/output check rather than failing validation on it.
 */
function ioProfiles({kinds,records,repositoryRoot}) {
  const load=(name)=>{
    for(const candidate of [path.join(repositoryRoot??'','modules','models',name+'.yaml'),path.join(repositoryRoot??'','legacy','model',name+'.yaml'),path.join(repositoryRoot??'','legacy','model',name+'.json')]) {
...
/** Public knowledge refs stay knowledge/*.json; the authored YAML beside them satisfies them. */

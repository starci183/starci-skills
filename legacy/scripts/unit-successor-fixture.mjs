import {prepareFixtureSource,fixtureSourceCritique} from './architecture-source-fixture.mjs';
...
export async function acceptUnitArchitecture(f, { startStep = 1, sourceStep = startStep + 3, writerRef = 'src/modules/fixture/worker.mjs', operationId = 'fixture-worker', coordination = null, goal = { prerequisite: `${sourceStep}/1` } } = {}) {
  ...
  const critiqueDir = path.join(dir, 'critique'); put(path.join(critiqueDir,'request/request.json'),critiqueRequest); await prepareFixtureSource(f.root,critiqueDir,critiqueRequest); await open(f, critiqueRequest, critiqueDir);
  // A separate process receives the exact source snapshot and model; no author rationale.
  const reviewProgram = "import assert from 'node:assert/strict'; import {readFileSync} from 'node:fs'; const model=JSON.parse(readFileSync(process.argv[1])); const source=JSON.parse(readFileSync(process.argv[2])); assert.equal(JSON.parse(source.files.find(file=>file.path==='package.json').chunks.join('')).type,'module'); assert.ok(source.files.find(file=>file.path==='README.md').chunks.join('').length); assert.equal(model.operations.length,2); assert.equal(new Set(model.operations.map(op=>op.writerRef)).size,2); for (const op of model.operations) { assert.equal(op.transport,'worker'); assert.equal(op.transactionBoundary,'read-only'); assert.equal(op.idempotencyKind,'none'); assert.deepEqual(op.storeRefs,[]); assert.deepEqual(op.migrationRefs,[]); } assert.deepEqual(model.stores,[]); assert.equal(model.boundaries[0].ownsData,false); process.stdout.write(JSON.stringify({pid:process.pid,paths:['partial-failure','retry-idempotency','concurrency','stale-state','deletion','recovery','dependency-outage','rollback']}));";
  ...
  put(path.join(critiqueDir,'response/critique.md'),fixtureSourceCritique(critiqueDir,readFileSync(path.join(critiqueDir,'response/critique.md'),'utf8')));

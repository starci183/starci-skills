...
// Runtime consumers require existing service only when this chain does not own its producer.
...
export function planChain({ packages, mission, options = {} }) {
  ...
  if (effectful.length && graph.has(PREFLIGHT_OPERATOR)) {
    ...
    // environment.preflight Requirements: runtime.serve owns bootstrap after binding.
    const touchesRuntime = [...nodes.values()].some((n) => RUNTIME_OPERATORS.has(n.operator));
    const ownsRuntime = [...nodes.values()].some((n) => n.operator === 'runtime.serve');
    const pre = add(PREFLIGHT_OPERATOR, PREFLIGHT_OPERATOR, roles.length ? { roles, runtimeRoles: touchesRuntime && !ownsRuntime ? roles : [] } : {}, `opens the chain: ${effectful.join(', ')} hold ${tools.join(', ')}`);

...
| `interface.draw` | sol-fresh | yes | `fileread:context-aliases`, `git:read`, `imagegen:judged`, `print:decision-points`, `registry:read` | isolated | Vẽ PNG theo nghiệp vụ và Grammar; không ghi source |
...
| `landing.compose` | sol-reviewer | yes | `fileread:context-aliases`, `git:read`, `websearch:bounded` | isolated | Suy luận và kiểm bằng chứng trong ranh giới operator |

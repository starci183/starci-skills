...
Every seed record this run consumes carries `is_uat=true` and the namespace the seed receipt binds —
the flow's own, as the seed plan gave it — so it is separable from product data; a run that binds no
seed receipt namespaces its records by its `runId`. Verification never mutates that data. It hands the exact receipt-owned

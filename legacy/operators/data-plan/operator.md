...
On `done`, print **Operator Result** as the seed-unit table in `response/data/units.json`, including namespace, order, expected state and rollback, with `response/response.md` as the readable plan. Missing store or isolation facts are shown as plan gaps and not as executable seed instructions.

...
| --- | --- | --- | --- |
| artifact conform tồn tại | tái dùng và ghi hash không đổi | contract facet và proof pass | ghi no-change observation |
| artifact thiếu | chỉ tạo operation và integration point đã đóng băng | snapshot, conformance, proof bao phủ | commit một lần sau match |
| artifact sai | update tập owner-owned nhỏ nhất | chạy lại facet, proof, snapshot bị ảnh hưởng | repair attempt mới; contract rộng handoff `architecture.decide` |
| cùng failure fingerprint | không reapply mù | giữ cả hai attempt | `NO_PROGRESS` hoặc typed owner handoff |
...
| 3 | Kiểm artifact của từng operation, tái dùng phần conform, tạo phần thiếu và update tập owner-owned sai nhỏ nhất trên session branch | `mutableFileRefs`, `protectedRefs`, `scope` | @knowledge/patterns/be cho từng khía cạnh, @workspaces/be trong ranh giới chủ sở hữu | @workspaces/be/branch/session trong ranh giới chủ sở hữu, hoặc ngoài nó dưới dạng một lần nới có ghi nhận và không bao giờ trong một ref được bảo vệ, dưới một lease độc quyền, @tools/sourcewrite | `CONTRACT_WIDENED`, `OWNER_WIDENED`, `OWNER_CONFLICT` |
...
| 7 | Chạy từng proof, so actual output với frozen criterion, và giữ transcript mismatch cho repair hoặc transient retry attempt mới thay vì hạ contract | — | @workspaces/be, câu lệnh đã pin của mỗi kiểu proof đã khai | `response/data/proofs/<operationId>.<proofKind>.json`, @tools/shell | `PROOF_UNAVAILABLE` |
...
| Khi | Operator |
| --- | --- |
| Contract dữ liệu/hành động đã chấp nhận làm đầu vào concept PNG trước khi tạo hoặc thiết kế lại frontend | `interface.draw` |

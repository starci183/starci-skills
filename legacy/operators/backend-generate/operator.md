...
Generate one backend outcome inside a frozen mutation contract, following the observed sibling
family, in full or as a fix inside the orchestrator's fix size, and return the measured conformance
and proof receipt that shows the boundary was not widened.
...
`proof`, and the checkout is untouched; or, when the request narrows the scope to fix, the same
holds under the mode it named with the `mutations` touching no more paths than the orchestrator's fix
size allows and no widening row at all.
...
## A fix is a scope, not another operator

`scope` says how much of the contract this run fills. `full` fills every operation the contract
carries. `fix` repairs one outcome inside an already-filled contract: it touches no more paths than
the orchestrator's fix size allows — the number lives in the orchestrator's own resource as
`fixSize`, the validator reads it there when it is present, and this operator hard-codes none — and
it widens nothing, neither the contract nor the owner boundary, because a repair that needs a wider
contract is not a repair; under `fix` a path outside every boundary is `OWNER_CONFLICT` and
`OWNER_WIDENED` is never taken. Everything else holds unchanged under both scopes.
...
| `scope` | choice | full | `full` fills every contract operation; `fix` repairs one outcome inside the orchestrator's fix size with no contract widening and no owner widening |
...
| When | Operator |
| --- | --- |
| the accepted data/action contract grounds a PNG concept before frontend creation or redesign | `interface.draw` |
...
| the contract is filled and a frontend surface must consume it | `interface.generate` |
...
| a declared proof cannot be executed in this environment | `runtime.serve` |

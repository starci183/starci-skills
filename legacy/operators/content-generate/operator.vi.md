...
| --- | --- | --- | --- |
| artifact khớp frozen brief | tái dùng fingerprint | check structure, source, image hoặc executable pass | receipt ghi reused |
| artifact thiếu | tạo edition, image hoặc track đã khai | chạy check riêng artifact | chỉ tiếp tục khi match |
| artifact sai | update stage prose/image/track/harness sở hữu | chạy lại check fail và phụ thuộc | repair attempt mới giữ review cũ |
| review vượt brief | không tự đổi goal | ghi scope-changing finding | phát replan evidence |
| alternative hợp lệ tied | chọn theo fit brief, rõ cho learner, support, chi phí thấp, id ổn định | ghi score và yếu tố phân định | tiếp tục |
...
| 2 | Kiểm mọi edition, image và track, phân loại reusable, missing hoặc invalid theo curriculum, source và runtime đã bind | `unit` | @remote/minio/<contentId>/<locale> cho chương trình học và văn phong như đang phục vụ, @worktrees/sessions/central-runtime cho cấu hình runtime, @tools/objectstorage | — | — |
| 3 | Viết và đóng băng brief | `naturalLanguages`, `implementationLanguages` | @remote/minio/<contentId>/<locale> cho chương trình học và bằng chứng nguồn, @tools/websearch | `response/brief.md` | `BRIEF_UNBOUND` |
| 4 | Tái dùng edition hợp lệ, tạo edition thiếu và chỉ update edition sai từ frozen brief | `naturalLanguages` | `response/brief.md` như brief đã đóng băng | `response/artifacts/article.<language>.md` | `OUTCOME_UNCOVERED` |
| 5 | Tái dùng image hợp lệ, hoặc generate và inspect image/prompt thiếu hay sai; mismatch mở image repair attempt mới | `stageModes` | `response/brief.md` cho các tuyên bố và ý định hình ảnh đã nêu | `response/artifacts/image.<name>`, `response/artifacts/prompt.<name>.txt`, @tools/imagegen | `IMAGE_UNAVAILABLE` |
| 6 | Tái dùng track hợp lệ, tạo track thiếu và sửa track sai, rồi build từng declared track mà không đổi brief | `implementationLanguages`, `commands` | `response/brief.md`, @worktrees/sessions/central-runtime nơi mỗi câu lệnh build chạy | `response/artifacts/track.<language>.<extension>`, @tools/shell | `CODE_BUILD_FAILED` |

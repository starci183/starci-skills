...
Print **Operator Result** as the pending, applied or already-applied disposition table in `response/migration-release.md`, backed by `response/data/migration-release.json`; an already-applied set is labeled a proved no-op, and `response/artifacts/migration-<n>.log` is linked only for a row that needs execution detail. A failed or uncertain migration leads with its actual journal state and recovery owner and never suggests blindly reapplying it.

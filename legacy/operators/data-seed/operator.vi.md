...
Khi `done`, in **Operator Result** là bảng record đã apply, reuse hoặc remove cùng proof before/after theo namespace trong `response/response.md`. Apply dở, rollback fail hoặc kết quả store chưa chắc phải đưa đúng các dòng incomplete và điểm dừng lên trước; không được báo namespace đã sẵn sàng.

...
OP="runtime.serve"
INPUT_COUNT=1
SPECS=[["platform-operation-receipt","file","application/json"],["runtime-delta","file","application/json"],["runtime-checks","file","application/json"]]
OBSERVE="observe-after-runtime-effect"
...
def records(root):
    ...
    for output_id,kind,media in SPECS:
        ...
        path=f"artifacts/{request_id}/{output_id}.json"; file=root/path; file.parent.mkdir(parents=True,exist_ok=True); file.write_bytes(data)
        outputs.append({"id":output_id,"type":"file","file":{"path":path,"sha256":digest(data),"mediaType":media},"description":"Synthetic contract evidence"})

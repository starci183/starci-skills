...
Bản tiếng Anh cùng tài liệu tiếng Anh của op được chọn là thẩm quyền runtime. Đọc đầy đủ cả hai. Bản `.vi.md` là bản đối chiếu cho người đọc. Chỉ đọc tài liệu chuyên môn đúng phạm vi. Catalogue hiện tại là toàn bộ thẩm quyền operator; thực thi không cần cây điều phối bổ sung hoặc file request/response.

## Chọn việc và thẩm quyền

Một task có một mục đích; prompt có thể chọn nhiều piece. Entry ánh xạ scope vào preset/mode có tên trong [skills/catalog.json](../../skills/catalog.json) rồi đọc recipe cố định. Op lấy từ recipe đã chọn, không chain tự sáng tác. User không cần nhớ tên op hoặc duyệt lại bước đã cho phép. Không preset phù hợp thì báo gap, không bịa workflow/tự sửa thư viện. Mỗi invocation có đích hữu hạn; xong không tự thêm successor. Catalogue/recipe không phải engine hoặc quyền thao tác.
...
Coordinator chọn phần recipe cố định đúng request rồi tiến lớp đủ điều kiện. Tổng mọi skill/worker/retry tối đa 3 lớp tuần tự × 3 invocation (chín); giữ thứ tự từng recipe, phần không vừa để prompt sau. Consumer cần prerequisite verified. Tối đa ba worker thực thi tổng cộng, tính cả coordinator đang làm op, theo tool/quyền delegation. Worker không spawn đệ quy/gọi successor. Không reset bằng skill/batch/task/subchain hoặc gộp op khác nhau thành op giả. Tool step trong một op không là op mới. Đây là contract agent, không background scheduler.
...
| Metadata node lá | `blocker`, `naReason`, `suspensionReason`, `completion` | Theo shape schema; cha không lưu state/completion. Lưu `todo`, `doing`, `blocked`, `done`, `na`, `suspended`; độ mới/trạng thái cha cũng suy ra. Scope nhập chưa duyệt bắt đầu suspended với lý do cụ thể, không bịa completion. |
...
Validator đọc bytes resource files đã khai và suy `suspended` bắc cầu cho node xong bị ảnh hưởng; completion/commit/evidence cũ giữ nguyên. Resource không ref không suspend việc không liên quan. Riêng migration source có thể lưu `state:suspended` với `suspensionReason` cho scope nhập chưa duyệt, không completion; không thỏa prerequisite cần done. Diagnostic `STALE_*` có thể giải thích suspension suy ra do proof cũ không còn áp dụng. Không relabel completion cũ done/xóa lịch sử/tự rerun consumer. Báo impact rồi chỉ chạy op đủ điều kiện được chọn riêng tiếp theo. Source/runtime ngoài chưa binding vẫn cần inspect thật và cập nhật revision resource; đây không là watcher nền file/deployment.

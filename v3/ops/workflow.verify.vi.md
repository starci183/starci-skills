...
Kiểm kết quả peer/piece đã chọn so với một mục đích task, không chạy/sửa việc của peer.
...
| assignments | actual selected task/piece assignments + original node IDs | Đọc peer set được chọn rõ, goal chính xác, write scope và nơi result giao. Không suy task identity từ title hoặc coi tự gửi mình là đã gửi peer. |
...
| proofs | selected peers original .work nodes and evidence | Đọc requirement/code/evidence gốc hiện tại, không tóm tắt pass. Message task chỉ chỉ nơi proof, không thay proof. |
...
| evidence | E/manifest.yaml + E/portfolio-review.json + E/result.md | id; nodeId; inputDigest; outcome; assertions; assets; provenance; codeRefs; extensions | Giữ check identity source/evidence và coverage task–piece; không bịa biên nhận giao hoặc tự nhắn follow-up. Kết quả quan sát chuyên môn ở E/result.md: Map done-when task sang peer node IDs và repo giao thật. Ghi proof peer thiếu, ownership xung đột, gap integration; không sửa state peer. |

## Thứ tự thực hiện hữu hạn

| # | Đọc ID | Ghi ID | Thực hiện và kiểm tra |
| --- | --- | --- | --- |
| 1 | assignments, target | node | Chốt purpose/peer set đã chọn cho review, resolve node/repo chính xác. Thiếu locator peer là unknown, không tự tìm task không liên quan. |
| 2 | proofs, repo, assignments | evidence | Đọc outcome gốc và bytes/commit giao thật; so goal/assertion bắt buộc với proof. Kiểm lineage integration và version hiện tại riêng. |
| 3 | target, assignments, proofs | node, evidence | Báo coverage scope tổng, proof thiếu/fail/stale và xung đột cross-piece. Giữ state peer/source/runtime/message; xong không dispatch op khác. |
...
| portfolio | Mỗi đóng góp peer được claim có proof trực tiếp xem được và mapping purpose; chỉ tóm tắt xanh bị loại. |
...
| --- | --- |
| PEER_PROOF_MISSING | Identity peer/evidence gốc/revision giao của peer được chọn không resolve được. |

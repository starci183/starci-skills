...
test('init creates only a fresh minimal root and refuses all existing roots without mutation', t => {
  ...
  assert.deepEqual(fs.readdirSync(root).sort(), ['.gitignore', 'workspace.yaml']);
  assert.deepEqual(JSON.parse(fs.readFileSync(path.join(root, 'workspace.yaml'), 'utf8')), { schema: 'work/workspace@1', id: 'workspace:test' });
...
test('catalogue and op commands show contracts but do not execute or mutate workspace', t => {
  ...
  const relocatedCLI = path.join(root, '.claude/v3/cli/main.mjs');
...
test('installed goal.setup forward-test authors only scoped todo planning and stops without a successor', t => {
  const root = temporary(t);
  ...
  const catalog = JSON.parse(fs.readFileSync(path.join(root, '.claude/v3/ops/catalog.json'), 'utf8'));
  const selected = catalog.ops.find(op => op.id === 'goal.setup');
  assert.ok(selected, 'actual installed catalogue must resolve the selected op');
  for (const relative of ['SKILL.md', 'v3/README.md', `v3/ops/${catalog.commonDocument}`, `v3/ops/${selected.document}`, 'v3/schemas/work.schema.json']) {
  ...
  const contract = invoke('op', 'goal.setup');
  ...
  assert.deepEqual(paths.filter(item => item.endsWith('node.md')).sort(), ['draft/business/acceptance/node.md', 'draft/business/functional/node.md', 'draft/node.md', 'draft/setup/node.md']);
...
test('installed quality.verify completes real synthetic gate proof without changing frozen semantic inputs', t => {
  const root = temporary(t);
  ...
  // quality.verify may neither create these inputs nor repair a missing target/dependency.
  ...
  const contract = invoke('op', 'quality.verify');
  assert.equal(contract.status, 0, contract.stderr);
  assert.ok(contract.stdout.includes('quality.verify'));

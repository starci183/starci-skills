...
`specDigest` bao gồm metadata trừ `state`, `completion`, `blocker`, `naReason`, `timestamps`, `createdAt`, `updatedAt`, cộng body chuẩn hóa. Extension lạ được tính bảo thủ. `inputDigest` bổ sung metadata workspace, spec và input của tổ tiên, node/dependency/resource được tham chiếu bắc cầu, và node con đối với nhánh. Không tính đường dẫn: đổi tên nhưng giữ ID/cấu trúc không làm proof stale. Toàn bộ metadata resource, gồm revision/details, được tính. Liên kết tùy ý bên trong resource details không được tự diễn giải; node phải refs từng resource liên quan. Nội dung extension không tự trở thành dependency.

# starci-runtime — bản đọc tiếng Việt

Bản [SKILL.md](SKILL.md) tiếng Anh là runtime authority. Preflight rồi thực hiện một effect serve, operate hoặc identity được chọn và kiểm chứng API.
...
Ba mode loại trừ nhau. Gắn environment/process/service/account, effect và rollback/custody chính xác vào yêu cầu người dùng. Không tạo identity chỉ vì test cần. Không lưu credential vào tài liệu/evidence. API verify chỉ chứng minh postcondition thật; thiếu suite hay target là gap, không pass bịa. Yêu cầu status chỉ đọc không khởi chạy preset.

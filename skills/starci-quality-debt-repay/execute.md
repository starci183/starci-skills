# Execute starci-quality-debt-repay

1. Accept only a validated global `selection` for this skill, validate the complete input, run local `analyze-input`, then enter fixed state `debt-approval`. Treat an omitted `selection.mode` as `gated`.

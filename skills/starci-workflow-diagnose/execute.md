# Execute starci-workflow-diagnose

1. Accept only a validated global `selection` for this skill, validate the complete input, run local `analyze-input`, then enter fixed state `diagnose`. Treat an omitted `selection.mode` as `gated`.

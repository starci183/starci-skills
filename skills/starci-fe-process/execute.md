...
One frontend visual finding may return once to source apply and must be proved by a new preflight,
capture, and blind review. A second finding, insufficient packet, no-progress fingerprint, stale
source/raster binding, missing populated happy-case hero, failed readiness check, or probe/capture
mismatch after the bounded recapture is terminal counterevidence, never PASS. First-round
`insufficient-evidence` returns once to recapture preflight without source mutation. Typed business or backend gaps exit to their exact owner and
resume the exact state. Visual PASS advances one way through quality and then UAT; neither gate may
route back into analysis. UAT counterevidence may explicitly resume reapply instead of completing.

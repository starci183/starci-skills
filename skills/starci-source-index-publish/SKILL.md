...
Require the ephemeral global selection, read `input.md`, validate `input.schema.json`, then follow local `analyze-input.md`. This skill owns one flow with fixed first state `mcp-config`; local analysis only validates and normalizes scope without loading operator knowledge.

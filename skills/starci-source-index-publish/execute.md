# Execute starci-source-index-publish

1. Accept only a validated global `selection` for this skill, validate the complete input, run local `analyze-input`, then enter fixed state `mcp-config`. Treat an omitted `selection.mode` as `gated`.

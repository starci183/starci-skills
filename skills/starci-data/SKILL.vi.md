# starci-data — bản đọc tiếng Việt

Bản [SKILL.md](SKILL.md) tiếng Anh là runtime authority. Plan và thực hiện seed fixture hoặc migration dữ liệu được cho phép, rồi kiểm chứng.
...
Tách mode seed và migrate; đây không phải nhập định dạng Work. Xác định environment, namespace, record, rollback và owner cleanup theo quyền hiện có. Không seed outcome cần test; yêu cầu inspect không cấp quyền phá dữ liệu. Target production chưa rõ hoặc mutation trước chưa chắc cần reconcile chỉ đọc trước effect tiếp theo. Thiếu prerequisite thì dừng; không thêm deploy hoặc identity.

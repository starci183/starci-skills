#!/usr/bin/env python3
"""Serve FE design proposals on the first free localhost port from 8080."""
...
def main() -> None:
    ...
    if not directory.is_dir():
        raise SystemExit(f"Proposal directory does not exist: {directory}")
    if not (directory / "index.html").is_file():
        raise SystemExit(f"Proposal directory has no index.html: {directory}")

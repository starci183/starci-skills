# starci-maintain — bản đọc tiếng Việt

Bản [SKILL.md](SKILL.md) tiếng Anh là runtime authority. Thực hiện một mode library update, knowledge repair hoặc scope retirement được chọn.
...
Library chỉ cập nhật dependency được chọn và chứng minh behavior bị ảnh hưởng. Knowledge sửa tài liệu có nguồn, không bịa acceptance sản phẩm. Retire cần quyền rõ, giữ lý do, evidence lịch sử và toàn vẹn ref. Không xóa dữ liệu sản phẩm/worktree như side effect retire node. Thiếu review target thì dừng, không tạo workflow setup ẩn.

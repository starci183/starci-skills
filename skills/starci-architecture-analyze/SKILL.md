---
name: starci-architecture-analyze
description: "Analyze a difficult cross-system technical decision before implementation: prove the current state, compare viable solutions and their trade-offs, challenge failure/security/cost/scale, draw one simple explanatory flow, and hand a compact decision context to planning. Read-only; not for ordinary CRUD, exact file planning or product writes."
---
...
Give a human reader enough evidence and criticism to understand a hard technical decision before anybody
names implementation files. Use it for cross-service data placement, consistency, recovery, security boundaries,

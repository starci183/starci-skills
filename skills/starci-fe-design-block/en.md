...
description: Audit, correct and prove one block already generated by Layout inside its complete page. Produces alternatives only after an explicit block-brainstorm request.
---
...
Default `audit` mode inspects the single generated block, returns pass or exact findings and corrects them while preserving the page. Explicit `brainstorm` alone emits 3–4 targeted anatomies inside the same parent geometry.
...
1. Current routed source is the parent authority; audit is the default and alternatives are opt-in.
2. Design, approval, implementation and QA happen in one invocation.
...
- Unknown product behavior, incomplete states, non-functional preview or fewer than three real alternatives.
- Required work outside the approved source boundary.

## OUTPUT

Report candidates, recommendation, cache URL and exact source paths; after `OK`, report changed paths and full-page proof. Never report registry identities.

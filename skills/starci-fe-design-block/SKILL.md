...
description: Audit, correct and visually prove one frontend block already generated by Layout inside its exact complete page. Produces no alternatives by default; creates 3–4 anatomies only when the owner explicitly requests block brainstorming.
---
...
Create `.worktrees/<project>/cache/design/<session-id>`. In default `audit` mode, inspect the one block Layout generated inside the complete parent page, return pass or exact findings, correct those findings in the same invocation and preserve all untargeted rendering. Block never re-composes the page or invents alternatives during audit.

Only an explicit owner request for `brainstorm` produces 3–4 materially distinct block anatomies. They remain inside the exact parent geometry, start from the audited baseline and vary only the requested anatomy/interaction axis.
...
2. Audit, correction, implementation and QA occur in one invocation.
...
8. Three or four block alternatives require an explicit brainstorm request; audit emits exactly one current anatomy plus findings.
...
In audit mode, report pass or exact findings/correction against the current page/block boundary. In explicit brainstorm mode, report 3–4 targeted alternatives. After correction, report changed paths and full-page code/browser proof.

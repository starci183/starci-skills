# starci-migrate — bản đọc tiếng Việt

Bản [SKILL.md](SKILL.md) tiếng Anh là runtime authority. Nhập một nghiệp vụ pilot từ source và tài liệu cũ vào Work 3, giữ provenance và intent chưa duyệt.
...
Chỉ inventory một pilot đã chọn, không quét/migrate mù toàn repo. Hành vi suy ra từ source là quan sát, không phải intent được duyệt: leaf nhập mới dùng state:suspended, suspensionReason cụ thể, không completion giả. Business review chỉ duyệt điều owner thực sự chấp thuận; thiếu quyết định quan trọng thì dừng. Workflow review tách coverage nhập liệu khỏi acceptance sản phẩm. Chỉ dọn .worktrees khi có quyền chính xác, đã xác minh giữ được commit, untracked, ảnh, evidence và ref account kín; không còn worker/worktree hoạt động; worktree đăng ký phải gỡ đúng bằng Git. Nếu chưa đủ thì giữ nguyên và báo gap.

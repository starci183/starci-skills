...
1. Run the script from the source host (the directory holding `.claude/`):
   ...
   - The script waits up to `modules/models/runtimes.yaml` `allocation.restart.waitMs`
     for the watchdogs to relaunch or adopt the kernels (`--wait-ms <ms>` to change it). Run it in the background and
     let it finish.

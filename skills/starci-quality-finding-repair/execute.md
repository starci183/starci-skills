# Execute starci-quality-finding-repair

1. Accept only a validated global `selection` for this skill, validate the complete input, run local `analyze-input`, then enter fixed state `repair-approval`. Treat an omitted `selection.mode` as `gated`.

...
Global `@selection` has already selected this one-flow skill from prompt intent. Before any operator or Qdrant retrieval, validate the invocation and verify `selection.skillId` equals `starci-business-authority`. Then perform these local checks:

1. Resolve one feature head, lifecycle state and immutable evidence set.
2. Confirm a new approved business revision is the requested outcome.
3. Reject product-source mutation or stale evidence.

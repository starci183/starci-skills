#!/usr/bin/env node
/**
 * Tests for starci-be-cannon-apply.
 *
 ...
 *   3. the approval gate gets softened in a rewrite, and a lane that builds only what was approved
 *      starts building whatever is defensible
 *
 * Whitespace is collapsed before the invariant is matched: the sentence may wrap across lines in
 * the source, and a suite that fails on a line break would be testing the formatter.
 *
 *   node .claude/skills/starci-be-cannon-apply/test.mjs [--verbose]
 */
...
const t = harness("starci-be-cannon-apply");
...
const INVARIANT = "approval comes before the diff, never after it";
...
t.expect(
    "it still says, in words, that the approval precedes the diff",
    { code: 0, out: flat },
    { has: [INVARIANT] },
);

# starci-uat — bản đọc tiếng Việt

Bản [SKILL.md](SKILL.md) tiếng Anh là runtime authority. Plan và chạy các luồng browser hoặc acceptance API được chọn; chỉ review portfolio thực sự test.
...
Browser mode có một đến ba luồng được đặt tên rõ, ánh xạ vào slot flow cố định. Mỗi luồng cần browser context, namespace dữ liệu mutable, actor/environment và served version quan sát thật; tab riêng chưa đủ isolation. Nếu thiếu capacity/isolation thì chỉ chạy tập con được nhận, không giấu wave verify nối tiếp. API mode không chứng minh UI. Thiếu credential/browser/runtime/fixture thì chặn luồng; không tự tạo account, chạy server hoặc seed outcome. Giữ case fail/not-run, screenshot, assertion và read-back thật.

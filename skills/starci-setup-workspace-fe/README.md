# starci-setup-workspace-fe — notes
...
`starci-setup-workspace-fe` and `starci-setup-workspace-be` share one record and one pair of
scripts. They are two skills because they are asked for at different moments and carry different
knowledge: a Storybook URL and a `.storybook/` folder mean nothing to someone hunting an entity,
and a migration path means nothing to someone fixing a card. A single skill would put half its
body in front of a reader who needs none of it.

They register into the same `context/workspace.json`, so `--fe <dir> --be <dir>` still does both
in one call.
...
```bash
node .claude/skills/starci-setup-workspace-fe/test.mjs

...
| `@directions` | `brainstorms/directions/context.md` | context | evidence-select visual direction |
...
Resolve the verified FE route, explicit grammar/profile, current business head, contract vocabulary and committed source baseline. Classify `businessImpact`. Business-affecting work requires the matching feature at `in-progress`; technical-only work binds current `implemented` truth.

Create one ignored session root at `.worktrees/<project>/cache/design/<session-id>`. Read the complete current composition and preserve every source-bound existing node unless the request targets it. Inventory viewport, overlay, disclosure, async, data, permission and interaction conditions before drawing. Build production-like business-content matrices for every reachable state.

Author three or four materially distinct complete functional HTML candidates covering the entire page or flow. Each includes all owned regions and blocks; there is no later block-head completion phase. Rank them using business fit, source precedent, hierarchy, reuse, accessibility, boundary ownership and responsive behavior. Audit only the selected candidate through `@principles`, recording class-free obligations.
...
Run canonical lint/tests and browser proof against the real product at every affected viewport and state. Exercise transitions through product controls, verify one scroll owner per axis, breakpoint exclusivity, clean console and visual parity with the selected cache candidate. For business-affecting work, reconcile the feature to `implemented` after final committed source. Cache may remain ignored for debugging but has no authority after completion.

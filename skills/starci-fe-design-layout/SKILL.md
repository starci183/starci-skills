...
| `@worktrees` | `contexts/worktrees` | module | Separates durable layout decisions from disposable direction and preview work. |
...
| `@artifact-validator` | `scripts/validate-artifact.mjs` | script | Enforces schemas, batch laws, vocabulary verdicts and the final layout hash. |
...
Read `@worktrees`. Accepted layout candidates and verdicts go to the project registry. Direction
choices before the final layout and generated previews stay in the session/cache. Before a registry
write, verify that it is locked, clean and owned by this Source's Git.

### 4 — Resume or open the session

Session identity is the surface, not the prompt. Two requests for the same surface are rounds in one
session. Preserve accepted layout hashes. Open a session only when that surface has none; otherwise
resume it and name the current accepted layout.

### 5 — Generate the direction choices

Run `@inventory-visual-language` against the verified checkout and record its source state as
...
Validate the batch with `@artifact-validator` and the generated vocabulary **without `--hash`**. Render
...
Run `@artifact-validator` with the layout schema, the same visual vocabulary and `--hash`. It refuses an
...
is never edited. Close with the six tables. `OWED` names the block rounds not yet designed.

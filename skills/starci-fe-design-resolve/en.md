...
Run the frontend classifier for every selected request. Exact specified corrections use `micro`; one component uses
...
Read the shared shape, requests and orchestration records, resolve `defaultLang`, then select explicit request ids.
If the invocation says to process the queue without ids, select all `open` requests in `(createdOn, id)` order and

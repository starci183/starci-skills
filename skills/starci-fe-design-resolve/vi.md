...
| `@orchestration` | `runtime/orchestration/vi.md` | vi | coordinate authority, source và proof không split decision |
| `@requests` | `knowledge/requests/vi.md` | vi | chọn và đóng durable feedback record |
| `@workspaces` | `knowledge/contexts/workspaces/vi.md` | vi | resolve project và FE role |
| `@worktrees` | `knowledge/contexts/worktrees/vi.md` | vi | resolve durable authority/product write roots |
| `@business` | `knowledge/contexts/business/vi.md` | vi | preserve hoặc block rõ ràng trên product truth |
| `@grammar` | `knowledge/grammars` | module | sở hữu product-family outcome, meaning, owner và behavior |
| `@principles` | `knowledge/compilers/principles` | module | sở hữu reusable product-neutral visual situation |
| `@patterns-fe` | `knowledge/compilers/patterns/fe` | module | bind correction vào exact FE owner |
| `@frontend-quality` | `knowledge/brainstorms/frontend-quality/vi.md` | vi | challenge unresolved UI direction khi cần |
...
| prove-close | proof | final source, evolved authority và selected requests | rerun targeted runtime/gates/real-product proof, update chính requests rồi close | passing proof và `resolved` records | mọi request có authority/final source/proof/reject refs; zero known defect |
...
replacement trong displayed write boundary. Sau approval, append immutable row đó vào `knowledge/requests/rejects.json` trước
...
1. Chỉ process valid record dưới `.claude/knowledge/requests`; không resolve feedback từ memory.
...
6. Wrong source attempt được ghi vào `knowledge/requests/rejects.json` trước khi đè; chính request nhận reject ref.

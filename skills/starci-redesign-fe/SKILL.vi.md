# starci-redesign-fe — bản đọc tiếng Việt

Bản [SKILL.md](SKILL.md) tiếng Anh là runtime authority. Plan, triển khai và audit frontend hiện có bằng direction đã duyệt và ảnh render thực tế.
...
Giữ business behavior/API trừ thay đổi được cho phép rõ. Gắn direction/ảnh nguồn qua resource.files và refs của consumer thật. Screenshot output là evidence, không tự nâng thành direction. Audit UI render thật theo viewport/state bằng browser có sẵn và được phép; mockup, ảnh tạo và kiểm tra source không chứng minh UI giao thật.

# starci-build — bản đọc tiếng Việt

Bản [SKILL.md](SKILL.md) tiếng Anh là runtime authority. Plan, triển khai và kiểm chứng piece backend/frontend đã được cho phép.
...
Chọn backend, frontend hoặc cả hai; giữ cùng lựa chọn nhánh ở plan và generate. Thiếu kiến trúc, API contract hay target đã khai thì chặn nhánh, không thêm wave kiến trúc. Gắn commit code thật khi có quyền commit; code chưa commit không được completion implementation giả. Quality evidence không thay browser UAT.

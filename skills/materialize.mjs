...
const skills = [
  ...
  },
  {
    ...
    states:{
      ...
      implement:op('be/implementation',decided({ready:'format'})), format:op('quality/format',qualityEdges('lint')), lint:op('quality/lint',qualityEdges('typecheck')),
  ...
  },
  {
    ...
    states:{
      ...
      'block-reconcile':op('fe/block-reconcile',decided({complete:'complete',blocked:'blocked'})),
      'maintenance-apply':op('fe/maintenance-apply',decided({complete:'learning-request',blocked:'blocked'})), 'learning-request':op('fe/learning-request',decided({complete:'complete',blocked:'blocked'})),
      'learning-resolve':op('fe/learning-resolve',decided({complete:'complete',blocked:'blocked'})),
      'surface-audit':op('fe/surface-audit',decided({complete:'authority-approval',blocked:'blocked'})),
      ...
      'authority-reconcile':op('fe/authority-reconcile',decided({complete:'consumer-align',blocked:'blocked'})), 'consumer-align':op('fe/consumer-align',decided({complete:'complete',blocked:'blocked'})),
      complete:terminal('complete'), blocked:terminal('blocked')
    }
  },
  {
  ...
  {
    id:'platform-services', description:'Reconcile bounded Cloudflare tunnel, source-context MCP/Qdrant, Sonar or observability services without inheriting product ownership.',
    modes:{ tunnel:{target:'tunnel-plan',label:'one HTTP tunnel/DNS route'}, mcp:{target:'mcp-config',label:'read-only source context'}, sonar:{target:'sonar',label:'shared Sonar enforcement'}, observability:{target:'observability',label:'metrics collection/remote write'} },

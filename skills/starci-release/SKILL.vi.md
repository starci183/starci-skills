# starci-release — bản đọc tiếng Việt

Bản [SKILL.md](SKILL.md) tiếng Anh là runtime authority. Publish source đã kiểm chứng, và chỉ deploy nếu release đúng đích được cho phép rõ.
...
Full cần quyền cả publication Git và deploy đúng đích cùng prerequisite release/runtime hiện có. Publish-only dùng proof hiện hành đã inspect và không ngụ ý deploy. Thiếu proof thì dừng publish-only, không tự thêm wave quality. Phân biệt commit giao, publication remote và artifact immutable được serve. Không force-push, đổi policy bảo vệ hoặc suy quyền production.

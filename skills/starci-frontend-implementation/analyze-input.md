...
Global `@selection` has already selected this one-flow skill from prompt intent. Before any operator or Qdrant retrieval, validate the invocation and verify `selection.skillId` equals `starci-frontend-implementation`. Then perform these local checks:

1. Require approved UI-detail and contract refs.

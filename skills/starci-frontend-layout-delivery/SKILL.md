...
Use to create or substantially redesign and ship one complete customer journey. Own every required business, backend, frontend and proof layer while skipping roles proven unaffected. Do not use for isolated blocks, approved maintenance, learning resolution, or cross-surface consistency.

## INPUT ANALYSIS

Require the ephemeral global selection, read `input.md`, validate `input.schema.json`, then follow local `analyze-input.md`. This skill owns one flow with fixed first state `layout-feedback-request`; local analysis only validates and normalizes scope without loading operator knowledge.

## STATE MACHINE

Execute `machine.json` through `execute.md`. Branches and loops are machine-owned; operators never invoke one another. Before a layout approval wait, read and apply `review-widget.md`; the validated HTML preview must be rendered through `visualize` before requesting `OK LAYOUT`. Before an architecture approval wait, read and apply `../../operators/architecture/review-widget.md`; `architecture/decision-challenge` must emit a validated HTML preview and the host must render it through `visualize` before requesting `OK ARCHITECTURE`. Stop at waits for the exact displayed revision and finish only at a terminal. Purge all intermediates at every terminal while preserving approved durable mutations.

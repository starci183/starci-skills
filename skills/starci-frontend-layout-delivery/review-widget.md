...
- At `layout.review`, compare all responsive layout directions in one in-conversation interactive HTML widget rendered through `visualize`. The validated `payload.reviewPreview` object is the executable binding for this render.
...
- Ask for the exact approval command outside the widget and stop at the machine wait only after the visualization has rendered. A missing visualization is an operator stop, not permission to proceed to approval.
...
- Emit the visualization directive in the same final response as the review and place the exact approval command after it. If the host cannot display a validated artifact, report the renderer blocker and do not enter the layout-approval wait; the accessible comparison fallback is diagnostic only and cannot satisfy `layout-visual-preview-ready`.

...
const BOUNDARY_EVENTS=new Set(['question','escalation','worker_done']);
...
/** The three names of 5.0: the worktree a workflow runs in, the kernel process inside it, and each operation. */
export function formatOrcaDisplayName(kind,{workflow,operation,scope}={}){
  if(kind==='workflow-worktree')return `[Workflow] ${oneLine(workflow,'workflow name')}`;
  if(kind==='workflow-kernel')return `[Kernel] ${oneLine(workflow,'workflow name')}`;
...
export function validateSupervisionPolicy(policy){
  ...
  // 5.0 has exactly two roles. The kernel owns the loop; the operation agent owns the work. A policy that
  // lets either claim the other's side is the 4.x failure this runtime exists to remove.
  if(!uniqueStrings(ownership?.kernel)||!['goal-assessment-and-one-user-approval','operation-scheduling','runtime-allocation','machine-verified-acceptance','commit-and-ledger-write','bounded-retry','gate-execution','final-reporting'].every(item=>ownership.kernel.includes(item))||['implementation-and-repair','test-and-verification-execution','operation-output-production'].some(item=>ownership.kernel.includes(item)))errors.push('Kernel ownership crosses the operation boundary');
  ...
  if(plain(policy)&&(policy.ownership?.coordinator||policy.ownership?.workflowWrapper||policy.workerLifecycle?.coordinator||policy.workerLifecycle?.workflowWrapper||policy.waitHierarchy?.coordinator||policy.waitHierarchy?.workflowManager))errors.push('A supervisor layer between the kernel and an operation is not part of this runtime');
  const kernel=policy?.workerLifecycle?.kernel;
  if(kernel?.lifetime!=='one-workflow'||kernel?.host!=='workflow-worktree'||kernel?.launch!=='local-process-never-an-orca-worker'||kernel?.state!=='one-workflow-directory-with-an-append-only-event-log'||kernel?.authority!=='owns-the-loop-after-one-user-approval'||kernel?.release!=='after-the-final-report')errors.push('Kernel lifecycle is invalid');
  ...
  if(names?.workflowWorktree!=='[Workflow] <Workflow>'||names?.workflowKernel!=='[Kernel] <Workflow>'||names?.operationAgent!=='[Op] <operation> - <scope>'||names?.applyAgentNameWith!=='task-display-name-then-provider-attestation-then-terminal-canonicalization')errors.push('Orca display naming contract is invalid');
  if(operationAgent?.lifetime!=='operation-attempt'||operationAgent?.launch!=='supervised-native-agent'||operationAgent?.isolation!=='one-operation-one-agent'||operationAgent?.release!=='after-accepted-worker_done'||operationAgent?.reuse!=='forbidden'||operationAgent?.parallelism!=='disjoint-allowlist-under-one-worktree')errors.push('Operation agent lifecycle is invalid');
  const acceptance=operationAgent?.acceptance;
  if(acceptance?.authority!=='kernel-re-runs-every-declared-check-itself'||acceptance?.changedFiles!=='computed-from-git-never-from-the-report'||acceptance?.allowlist!=='every-changed-file-inside-the-operation-allowlist'||acceptance?.commit!=='one-operation-one-commit'||acceptance?.selfDeclaredDone!=='never-sufficient')errors.push('Machine-verified acceptance is invalid');
  const admission=operationAgent?.dispatchAdmission,sidearm=operationAgent?.architectureSidearm;
  if(!uniqueStrings(admission?.required)||!['expected-operation-from-the-approved-goal','exact-operation-contract','allocated-runtime-selection','canonical-display-name'].every(rule=>admission.required.includes(rule))||admission?.providerProof!=='worker-show-exact-effective-agent-model'||admission?.effectAcceptance!=='only-after-provider-proof'||admission?.onOperationMismatch!=='fence-reconcile-release-and-dispatch-expected-operation'||admission?.onProviderMismatch!=='fence-reconcile-release-and-retry-allocated-runtime')errors.push('Operation dispatch admission is invalid')
  ...
  const deadTerminals=operationAgent?.deadTerminals;
  if(deadTerminals?.owner!=='workflow-kernel'||deadTerminals?.command!=='sweep'||deadTerminals?.when!=='after-every-settlement-and-before-workflow-completion')errors.push('Dead-terminal sweep policy is invalid');
  ...
  if(!uniqueStrings(nativeFailure?.recognizedFailures)||!['agent_prompt_stalled','session_not_reported'].every(reason=>nativeFailure.recognizedFailures.includes(reason))||nativeFailure?.action!=='fence-and-reconcile-exact-attempt'||nativeFailure?.nextCandidate!=='only-after-verified-no-effects'||nativeFailure?.unsupervisedFallback!=='forbidden'||nativeFailure?.release!=='release-or-retain-from-worker-receipt'||nativeFailure?.duplicateSubmit!=='forbidden'||nativeFailure?.onExhaustedRuntimes!=='release-the-slot-cool-the-pool-and-allocate-another-runtime')errors.push('Native operation failure policy is invalid');
  const routing=policy?.routing;
  if(routing?.operationToKernel?.recipient!=='workflow-kernel'||routing?.operationToKernel?.transport!=='one-report-file-per-dispatch'||routing?.operationToKernel?.authority!=='report-file-then-signal'||!uniqueStrings(routing?.operationToKernel?.events)||!['question','escalation','heartbeat','worker_done'].every(event=>routing.operationToKernel.events.includes(event)))errors.push('Operation events must reach the kernel as one report file per dispatch');
  if(routing?.kernelLocal?.execution!=='operation-agent-only'||!uniqueStrings(routing?.kernelLocal?.resolveWithoutUser)||routing?.kernelLocal?.userNotification!=='none')errors.push('Kernel-local resolution is invalid');
  if(!uniqueStrings(routing?.kernelToUser?.onlyWhen)||routing?.kernelToUser?.envelope!=='final-report-need-user')errors.push('Kernel-to-user routing is invalid');
  if(routing?.kernelToOperation?.recipient!=='own-operation-terminal'||routing?.kernelToOperation?.transport!=='launcher-notify'||routing?.kernelToOperation?.statusEvent!=='forbidden-for-control-instruction'||!uniqueStrings(routing?.kernelToOperation?.when))errors.push('The kernel speaks to a live operation only through notify on its own terminal');
  if(routing?.operationToOperation?.direct!=='forbidden')errors.push('An operation must never control another operation');
  const waitHierarchy=policy?.waitHierarchy;
  if(waitHierarchy?.kernel?.waitsFor!=='operation-boundary'||!uniqueStrings(waitHierarchy?.kernel?.accepts)||!['question','escalation','heartbeat','worker_done'].every(event=>waitHierarchy.kernel.accepts.includes(event))||waitHierarchy?.kernel?.actsBy!=='verify-accept-commit-retry-or-reallocate'||waitHierarchy?.kernel?.directExecution!=='forbidden'||waitHierarchy?.kernel?.loopOwner!=='code-never-a-model'||!uniqueStrings(waitHierarchy?.kernel?.modelCalls))errors.push('Kernel wait hierarchy is invalid');
  if(policy?.wait?.mode!=='blocking-event-wait')errors.push('The kernel wait must be event-driven');
  ...
  if(!uniqueStrings(policy?.wait?.endsTurnOnlyOn)||!['workflow-goal','blocked-need-user'].every(item=>policy.wait.endsTurnOnlyOn.includes(item)))errors.push('A kernel turn ends only on the workflow goal or a blocked need-user');
  ...
  if(!plain(reports?.signals)||reports.signals.done!=='worker_done'||reports.signals.partial!=='worker_done'||reports.signals.failed!=='worker_done'||reports.signals.ask!=='question'||reports.signals.blocked!=='escalation')errors.push('Report signal mapping is invalid');
  ...
  if(!uniqueStrings(policy?.forbidden)||!['model-owned-control-loop','supervisor-layer-between-kernel-and-operation','operation-agent-acts-as-the-kernel','operation-agent-creates-nested-agent','operation-agent-controls-another-operation','kernel-implements-operation-scope','kernel-runs-operation-work-instead-of-re-running-its-checks','accept-self-declared-done-without-machine-verification','commit-outside-the-operation-allowlist','periodic-terminal-poll','periodic-log-read','periodic-diff-read','progress-prompt','duplicate-test-run','restart-healthy-operation','user-report-on-unchanged-state','dispatch-operation-not-selected-by-the-approved-goal','accept-effect-before-provider-attestation','dispatch-operation-to-existing-terminal','infer-architecture-sidearm-from-review'].every(rule=>policy.forbidden.includes(rule)))errors.push('Kernel anti-polling and role-ownership invariants are incomplete');

...
const VALIDATOR_RULES=[
  ...
  'a design candidate of an interface.draw operation is a PNG generated by the image model from the requirement material and the design grammar; a script that paints SVG/PNG boxes and text, a mockup-tool export or a screenshot of unstyled markup is a defect and the result is rejected',
...
export const DEFAULT_CRITIC_RUNTIMES=['claude-fable-5.1','gpt-6-astra'];
...
export function critiqueGoal({job,scope=[],ledger=[],decisions=[],records=[],material=[],brand=null,constraints=[],
  ...
  if(!chain.length)return {ok:false,verdict:'unavailable',reason:'no critic provider was given',attempts:[],usage:null,
    objections:[],dropped:[],required:[],alternatives:[],question:null};
  ...
  if(!result.ok)return {ok:false,verdict:'unavailable',reason:result.reason??'no provider produced a valid critique',
    attempts:result.attempts??[],usage:result.usage??null,objections:[],dropped:[],required:[],alternatives:[],question:null};
  const answer=result.value;

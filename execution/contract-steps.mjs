...
const SEQUENCE_STEPS={
  ...
  'work.intake':v=>({
    steps:[
      ...
      `Reconcile before you write. A new feature C is not appended beside the decided features A and B: read every DECIDED business (SRS) and architecture (SDS) record of the features the job names or C touches - their rules, journeys, data, contracts, authority - and write a reconciliation table into C's module record (\`extensions.work3.reconciliation\`): per touched record its id, what C shares with it, what C would change in it, and whether that is a fit, a change or a conflict. What C shares, C references by record id and never restates or redefines. What A or B must become because of C is not yours to edit: report \`blocked\` with blocker \`sds-gap\` naming the exact record and the exact change, and the kernel opens the revise operation on that record. A real conflict - two decided rules that cannot both hold once C exists - is an open decision record in C stating the question and the two ways out, and every part of C that rests on it stays a draft behind it. The result the owner reads is one consistent A', B', C', never A + B + C.`,
      ...
      `Never write \`completion\`, evidence, the kernel's own extension block or any record outside the allowlist; never change another feature's record by hand - a change it needs is an \`sds-gap\` report, never an edit; never write product code.`,
      ...
      `the module record carries a reconciliation table naming every decided record the feature touches with fit, change or conflict; what the feature shares with a decided record is referenced by id and never restated or redefined; every change a decided record needs was reported as \`sds-gap\`, never edited; every real conflict is an open decision record`,

...
const SEQUENCE_STEPS={
  ...
  'owner.ask':v=>({
    steps:[
      ...
      `Otherwise write ONE policy-decision record draft at the path the allowlist names (${v.allowlist}): schema work/node@2, kind \`business\`, \`state: todo\`, the same shape as the existing policy-decision record among the references (its \`extensions.work3.srs\` with schema \`starci/srs-policy-decision@1\`, \`decisionStatus: open\`, an id under the feature's srs.decision prefix) - never a kind or a folder of your own - whose description carries in this order: the question in one sentence; why it matters and what stays blocked until it is answered; the options, numbered, each with what it means for the architecture, the user stories, the security and authority, the business rules and the quality limits, and what it costs; the decided records each option agrees or conflicts with, by id; and exactly one recommendation with its reason. Real product-shaped content, no placeholders.`,
      ...
      `either the summary begins \`answered-from: <decided record id>\` and no file changed, or exactly one policy-decision record draft (kind business, srs-policy-decision section, decisionStatus open) exists at ${v.allowlist} with the question, why it matters, numbered options each analysed per side (architecture, user stories, security and authority, business rules, quality), the decided records each option touches, and one recommendation`,

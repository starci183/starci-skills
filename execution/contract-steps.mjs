...
const SEQUENCE_STEPS={
  ...
  'interface.draw':v=>({
    steps:[
      ...
      `Write the interface design record at the path the allowlist names (${v.allowlist}): per screen its blueprint (the regions in order), each enumerated state with what it shows, the contract slots each region fills and the exact copy, AND the design binding the build will read from the Work tree - the grammar family, the exact tokens (colours, type, spacing) each region uses, the icon set, the patterns applied and the grammar component bound to every region; no product code.`,

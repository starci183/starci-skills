 ...
 * config.json, fable then astra) unless the caller names its own chain. An unanswered critique is
...
export function critiqueGoalPhase(store,state,{critiqueGoal=llm.critiqueGoal,providers=null,cwd=state.worktree,runHeadless,
  ...
  const chain=providers??supervisorRuntimes(state.host??'');
  ...
  if(!critiqued?.ok){
    state.critique={verdict:'unavailable',objections:[],required:[],alternatives:[],question:null,provider:null,
  ...
  state.critique={verdict:critiqued.verdict,objections:(critiqued.objections??[]).map(item=>({...item})),
    ...
    question:critiqued.question??null,provider:critiqued.provider??null,at:Date.now(),
  ...
  store.appendEvent({event:'goal-critiqued',verdict:state.critique.verdict,objections:state.critique.objections.length,
    provider:state.critique.provider,...(state.critique.dropped?{dropped:state.critique.dropped}:{})});
  return state.critique;
}

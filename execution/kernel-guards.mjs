...
export const LOCK_RULES=[
  // Testcontainers isolate their own stack per run, so container suites and the API e2e suite do not clash and run
  // in parallel; only the shared local stack (fixed ports), a served UI walked by UAT, and the cluster are exclusive.

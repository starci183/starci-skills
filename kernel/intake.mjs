...
export function intakeOp(state,{workRoot,loaded,index,entry=null,repositories={},mode='author',scope=[],layers=null}){
  ...
  const continuing=reconcile||loaded.list.some(node=>scopeNames(node,name));
  const continuation=continuing
    ?' Continue from the existing records and implementation. Preserve current accepted leaves, their stable IDs, substantive content and completion evidence. Keep unfinished drafts and useful implementation evidence. A stale completion means its changed inputs and affected consumers need a scoped review; it is not an instruction to reset the feature or rewrite existing code. Name the affected owner and required review, and never refresh a completion digest without that review. Create only missing records; only genuinely new or unfinished leaves are todo. Root records aggregate and carry no state.'
    :` Mirror the shape of the existing feature ${example??'(none yet)'}: the module record, the business overview and SRS as full drafts, the architecture as a skeleton; every leaf record todo (the module, business, srs, architecture and sds roots carry no state, exactly as the example), every open question an open decision.`;
  const leafCriterion=continuing
    ?'current accepted leaves and their evidence are preserved, missing or unfinished leaves are todo, affected stale inputs have an explicit scoped review requirement, and the roots carry no state'
    :'every leaf record todo and the whole feature valid - the roots (module, business, srs, architecture and sds) carry no state, as in the example feature';
  ...
  return {id,kind:intakeKindFor(name),nodeId:null,intake:{scope:name,feature,layers:[...owned],example,mode},
    ...
    acceptance:[owned.length
      ?`${ownedText} hold the records this goal approved; ${leafCriterion}`
      :`features/${feature} has its module, business overview, SRS and architecture records; ${leafCriterion}`,

...
const SEQUENCE_STEPS={
  ...
  // becomes a decision record draft the owner answers with `workflow-answer`.
  'owner.ask':v=>({
    steps:[
      ...
      `Otherwise write ONE decision record draft at the path the allowlist names (${v.allowlist}), schema work/node@2, kind \`decision\`, \`state: todo\`, whose description carries in this order: the question in one sentence; why it matters and what stays blocked until it is answered; the options, numbered, each with what it means for the architecture, the user stories, the security and authority, the business rules and the quality limits, and what it costs; the decided records each option agrees or conflicts with, by id; and exactly one recommendation with its reason. Real product-shaped content, no placeholders.`,
      `A credential or configuration question names the exact variable or secret name, where the code reads it, what the owner has to provide and where (the environment, the stack secret store), and never asks for the value itself: a value is provided by the owner out of band and never written into a record, a report, a chat or a file the owner did not name.`,
      ...
      `Report \`done\` exactly once with the first line of the summary exactly \`decision: <record id>\` and the options numbered as the owner will answer them.`
    ],
    done:[
      `either the summary begins \`answered-from: <decided record id>\` and no file changed, or exactly one decision record draft exists at ${v.allowlist} with the question, why it matters, numbered options each analysed per side (architecture, user stories, security and authority, business rules, quality), the decided records each option touches, and one recommendation`,
      `no value of any credential or secret appears anywhere; a credential question names only the variable or secret name, where it is read and what the owner provides`,

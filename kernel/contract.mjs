...
const SEQUENCE_STEPS={
  ...
  'work.intake':v=>({
    steps:[
      ...
      `Reconcile every side against the records the product already decided: read every decided business (SRS) and architecture (SDS) record the feature touches and write a reconciliation table into the module record (\`extensions.work3.reconciliation\`): per touched record its id, what this feature shares with it, what this feature would change in it, and whether that is a fit, a change or a conflict. What is shared is referenced by record id and never restated or redefined. What another feature's record must become is not yours to edit: report \`blocked\` with blocker \`sds-gap\` naming the exact record and the exact change, and the kernel opens the revise operation on that record. A real conflict - two decided rules that cannot both hold once this feature exists - is an open decision for the owner (the kernel opens the owner's question from your \`ask\`, or you write the decision record yourself with the question, the options and a recommendation), and every part of the feature that rests on it stays a draft behind it.`,
      ...
      `Run the listed validator check verbatim: ${v.declared}; every record you wrote validates in the tree as it stands, and \`git status\` shows nothing outside the allowlist. Never write \`completion\`, evidence, the kernel's own extension block or any record outside the allowlist; never change another feature's record by hand - a change it needs is an \`sds-gap\` report, never an edit; never write product code.`,
      `Report \`done\` exactly once with the records you authored, the reconciliation table and the open decisions you left for the owner; a decided record that must change is \`blocked\` with blocker \`sds-gap\` and the exact record and change; a boundary the job does not determine is \`ask\` with the exact question.`
    ],
    done:[
      ...
      `the module record carries a reconciliation table naming every decided record the feature touches with fit, change or conflict; what the feature shares with a decided record is referenced by id and never restated or redefined; every change a decided record needs was reported as \`sds-gap\`, never edited; every real conflict is an open decision record with options and a recommendation`,
      ...
      `no other feature's record changed by hand, no product code changed, and the validator check exits 0: ${v.declared}`
    ]
  }),
  'implement.ledger':v=>({
    steps:[

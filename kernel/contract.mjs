...
const SEQUENCE_STEPS={
  ...
  'interface.draw':v=>({
    steps:[
      ...
      `Render ONE candidate per screen and viewport (desktop and mobile) - the main state only - FROM THE INSTALLED GRAMMAR ITSELF: compose the screen from the grammar package's own renderers (its contract components, its stylesheet, the brand tokens) as static markup or a server-side render in a scratch folder outside the product (never product code, never a story committed to the app), open it in headless Chrome and capture the PNG into the node's own assets/. Never an image model for a surface, never hand-painted boxes, never a mockup-tool export, never a capture of a working feature: a candidate that is not the grammar rendering the screen is a defect. Real, product-shaped content in every region - names, amounts, dates, statuses - never lorem or grey bars.`,
      ...
      `Self-check: every assertion (${v.acceptance}) maps to a named screen state described in the record, every enumerated state is described with its grammar contract, the main-state candidate of every screen exists at both viewports and was captured from the grammar render, every component visible in a candidate is a grammar contract and every colour a brand token, every artwork visible in the chosen candidate has its \`artworkSlots\` entry with a crop that locates it, and nothing in the record rests on a business rule the references do not state.`,
      ...
      `every assertion (${v.acceptance}) maps to one screen, and every screen has exactly one candidate PNG per viewport of its main state, captured by a browser from the installed grammar's own rendering - never generated by an image model, never painted by a script, never a mockup export`,

...
const SEQUENCE_STEPS={
  ...
  'decision.prepare':v=>({
    steps:[
      `Read the question under \`## Question for the owner\` in the goal - who asked it, what it blocks - and every decided business (SRS) and architecture (SDS) record among the references (${v.references}). If a decided record already settles the question, do not prepare anything: report \`done\` with the first line of the summary exactly \`answered-from: <record id>\` followed by the answer in one paragraph, and write nothing. And if, having read them, the question turns out to be something the owner must PROVIDE - a credential, an account on an outside system, a real dataset, a legal authority - and not a choice between options at all, do not draft anything. For a credential, report the exact declared requirement as \`blocked\` \`environment\` so the kernel owns its researched input form; never create a credential terminal or ask for a value; for anything else, ask in this terminal what they provide and how, and ask them to reply \`provided\` once it exists. For those non-credential provisions, wait for them, verify only that it is PRESENT and never its value, and report \`done\` with the first line exactly \`provided: <what>\`. The kernel accepts this operation by what your report says, never by the name it was opened under.`,
      ...
      `Report \`done\` exactly once with the first line of the summary exactly \`decision: <record id>\`, then a line \`recommended: <n>\`, then the options numbered as the owner will answer them. The kernel takes your recommendation provisionally so the work continues, and tells the owner it did. After the report, stay in this terminal: the kernel keeps this tab open for the owner until they answer, and it is the one place they see the question. If the owner types a number here after your report, run the answer command yourself - \`node <skill root>/bin/starci.mjs workflow-answer --host <skill root> --id <workflow> --op <this op> --choice <n>\`, the skill root and workflow id being the ones in the report command below, with \`--note "<their words>"\` when they add any - print what it returns, and never report again. If the owner happened to answer here with a number before you reported, the first line is \`answered-by-owner: <n>\` instead - that is the owner's ruling and the kernel treats it exactly as the command would.`
    ],
    done:[
      `either the summary begins \`answered-from: <decided record id>\` and no file changed, or \`answered-by-owner: <n>\` because the owner answered in this terminal, or exactly one policy-decision record draft exists at ${v.allowlist} and the summary begins \`decision: <record id>\` with \`recommended: <n>\` and the numbered options, or - the question being a provision after all - the summary begins \`credential: <VAR> present in identity:<slug>\` or \`provided: <what>\` and no file changed`,
      `the question and its numbered options were printed in this terminal with the two ways to answer them, before the report, and nothing waited for the owner - unless the question was a provision, when what the owner provides and how was printed instead and this operation waited for it`,
  ...
  //
  // For a CREDENTIAL the kernel no longer launches this operation at all: a question for the owner is a prompt
  // they answer, not a command they compose. The kernel prints one line naming the variables, the custody and
  // the exact command - `starci identity fill <slug> --name <VAR> ...`, which asks `Fill <VAR>:` with the echo
  // off and puts each answer into that custody - and settles the ask itself once every variable is present. The
  // credential half of these steps is therefore that command's own contract, so the rule stays one rule.
  'provision.ask':v=>({
    steps:[
      `Read the question under \`## Question for the owner\` in the goal - who asked it, what it blocks, which kind of provision it is - and, for a credential, the references (${v.references}) and the code that reads the value, to name the exact variable. Name the provision exactly: a credential names the variable the code reads, the outside system it belongs to, and the custody it will live in: \`_resources/identity/<slug>/\` of this Work tree, encrypted with sops; an account, a dataset or an authority names what and for which system. Never invent, stub, default or skip it, and never ask for a value. And if, reading the records, the question turns out not to be a provision at all but a choice only the owner makes - where a value lives, which of two designs holds - do what a decision does instead: write ONE policy-decision record draft at the path the allowlist names (${v.allowlist}), in the shape the policy decisions of this tree already hold - the question in one sentence, the options numbered with what each costs, exactly one recommendation with its reason - print the question and those numbered options in this terminal, and report \`done\` with the first line exactly \`decision: <record id>\` and a line \`recommended: <n>\`. Nothing waits for that: the kernel takes your recommendation provisionally and the owner overturns it later.`,
      `For a credential, return the exact declared requirement to the kernel with \`blocked\` \`environment\`; the kernel owns the researched form in Orca and the hidden \`identity fill\` prompt on headless hosts. Never open a substitute terminal, print a copy-command as the Orca input flow, ask for a secret in chat, or poll for the word set. Missing official documentation is a research failure to repair before owner input, not a credential request. For a non-credential provision, ASK IN THIS TERMINAL what the owner provides and how, then wait for the reply \`provided\` as long as the contract allows. An environment variable is not custody.`,
      `For a credential the kernel checks encrypted custody through its canonical presence verifier and settles the waiting ask itself; do not run another probe or report credential success from this terminal. Presence permits the requesting operation to verify the real integration and proves no provider validity. For a non-credential provision, when the owner replies verify only PRESENCE and never the value, then report \`done\` exactly once with the first line exactly \`provided: <what the owner provided, in a few words>\`. Never print, echo, log, copy or paste the value, not even partially, and never write it into any file.`,
      `Self-audit before you report: this terminal shows exactly what the owner provides and how, no value of anything appears in it, in a file or in your summary, and \`git status\` shows nothing changed - not under the allowlist (${v.allowlist}) and not anywhere else: a provision changes nothing in the tree but its custody. The one exception is the decision draft of the step above, when the question turned out to be a choice: that single record under the allowlist, and nothing else. Never write product code.`,
      ...
      `what the owner provides and how was printed in this terminal before any report, and the operation waited - unless the question was a choice after all, when the question and its numbered options were printed instead and nothing waited`,
      `the summary begins \`credential: <VAR> present in identity:<slug>\` or \`provided: <what>\` because the owner provided it, or - the question being a choice after all - \`decision: <record id>\` with \`recommended: <n>\` over the one policy-decision draft at ${v.allowlist}, or the report is \`blocked\` with blocker kind \`authority\` naming it`,
      `no value of any credential or secret appears anywhere - not in a record, a report, a log, this terminal or a file - and presence is all that was ever checked; no product code changed, and no record either unless the question was a choice and that one policy-decision draft is it`
    ]
  }),
  ...
  'work.intake':v=>({
    steps:[
      ...
      `Reconcile every side against the records the product already decided: read every decided business (SRS) and architecture (SDS) record the feature touches, and write one typed row per touched record into the module record under \`extensions.work3.reconciliation\`, each row \`{case, record, decision, reads, hands, detail}\`. There are exactly three cases and no fourth. \`case: reference\` - this feature repeats what that record already holds: cite it by its \`record\` id and never restate, re-word or redefine a line of it. \`case: conflict\` - this feature cannot hold together with what that record decided: never overwrite it and never average the two; write a decision record under THIS feature at \`business/srs/business-rules/policy-decisions/<slug>/index.yaml\` - \`kind: business\`, \`state: todo\`, an \`extensions.work3.srs\` section of schema \`starci/srs-policy-decision@1\` with \`decisionStatus: open\`, in the shape of the policy decisions this tree already holds - stating both sides, the consequences of each, the numbered options and exactly one recommendation; it names the records it concerns by id in its text and in the row, never in \`refs\` or \`dependsOn\` (a graph edge into another feature is a cycle); name that record in \`decision\`, and leave the decided record byte for byte as you found it - the kernel puts the question to the owner, and every part of this feature that rests on it stays a draft behind it. \`case: new\` - no decided record covers it: author it under this feature and declare in \`reads\` the decided records it rests on and in \`hands\` the records of this feature it hands on to. A decided record of another feature is never edited, and no case is left as prose beside the table.`,
  ...
  'work.migrate':v=>({
    steps:[
      ...
      `Reconcile against the other features: for every decided business (SRS) or architecture (SDS) record of ANOTHER feature this feature touches, write one typed row into the module record under \`extensions.work3.reconciliation\`, each row \`{case, record, decision, reads, hands, detail}\`, and nothing but the three cases. \`case: reference\` - this feature rests on what that record holds: cite it by its \`record\` id, restate nothing. \`case: conflict\` - this feature's decided records cannot hold together with it: write a decision record under this feature at \`business/srs/business-rules/policy-decisions/<slug>/index.yaml\` - \`kind: business\`, \`state: todo\`, \`refs: []\`, an \`extensions.work3.srs\` section of schema \`starci/srs-policy-decision@1\` with \`decisionStatus: open\`, exactly the shape of the policy decisions this tree already holds and never a layout of your own - stating both sides, the consequences, the numbered options and exactly one recommendation; it names the records it concerns by id in its text and in the row, never in \`refs\` or \`dependsOn\` (a graph edge into another feature is a cycle); name it in \`decision\`, and leave both decided records byte for byte as you found them. \`case: new\` - this feature settles what no other record covers: name the record of this feature in the row and declare in \`reads\` the decided records it rests on and in \`hands\` the records it hands on to. No case is left as prose beside the table.`,
  ...
  'integration.verify':v=>({
    steps:[
      ...
      `Run the listed check verbatim, INSIDE the custody: \`sops exec-env <work tree>/_resources/identity/<slug>/${IDENTITY_SECRETS} '<check>'\` for each of ${v.checks}. The value then exists only inside that one process: never copy it into a file, a fixture, an environment file, a log, the report or this contract, never print or echo it, and never commit it. The variable name and the custody slug are what travel; what they hold never does. Keep the whole run output and put it in the report with every secret masked. The host's age key is at \`<home>/.starci/master.identity\`: when \`SOPS_AGE_KEY_FILE\` is not set, export it to that path before any sops command, or sops cannot open the custody and the credential looks absent.`,
      ...
      `Report \`done\` exactly once and only on a green live run, with the run output and the spec paths. A scenario the provider rejected is \`failed\` naming the assertion and the response it returned; a provider outage, a sandbox that will not answer or a rate limit is \`failed\` with that exact reason; a missing credential is \`blocked\` \`environment\` with the variable name. If the declared live check explicitly identifies an invalid or expired credential, report \`blocked\` \`environment\` with \`--credential-request-file\` in the report format below so the workflow can request a replacement. Other failures keep their actual failure path. Never \`done\` on a skipped, faked or unrun scenario.`
    ],
    done:[
  ...
  uat:v=>({
    steps:[
      ...
      `Run the spec on that runtime: ${v.checks}; keep the run output. A step that needs a credential runs inside its custody - \`sops exec-env <work tree>/_resources/identity/<slug>/${IDENTITY_SECRETS} '<check>'\` - so the value lives only in that process and is never copied into a file, a log, a report or this contract. The host's age key is at \`<home>/.starci/master.identity\`: when \`SOPS_AGE_KEY_FILE\` is not set, export it to that path before any sops command, or sops cannot open the custody and the credential looks absent.`,
...
export function stepsFor(op,{node=null}={}){
  ...
  return [
    ...
    ...(op.integrationPreparation?[`This is a preparation-only research repair. Change only the owning declaration's preparation field; preserve all other product semantics. Research may finish with honestly pending machine steps in its ordered plan. The existing requester performs authorized setup before owner input is admitted; record observed setup evidence through the owning record route, never fabricate readiness.`]:[]),
    ``,
    `${STEPS_HEADING}`,

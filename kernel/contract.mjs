...
const SEQUENCE_STEPS={
  ...
  'decision.prepare':v=>({
    steps:[
      `Keep the canonical policy question and its answers separate: extensions.work3.srs.requiredDecisions is the agenda of unresolved topics. Author extensions.work3.srs.options as at least two distinct concrete, mutually exclusive policy outcomes before presenting a numbered choice. Never use the agenda, questions such as "Whether notice is required", a truncated report, or an old translated label as answer options. Explain each outcome's tradeoffs in the description. Missing or malformed options are a draft-authoring defect for this operation to repair, not a missing owner answer.`,
      ...
      `a prepared policy choice has an explicit canonical srs.options array of concrete outcomes separate from requiredDecisions; its question, order and meanings match the owner presentation, and an old topic-selection receipt is never reinterpreted as a policy answer`,
  ...
  'interface.draw':v=>({
    steps:[
      `Read ${v.references}: accepted business/SRS, SDS, brand and relevant UI knowledge, plus the actual consuming Grammar package exports, types, anatomy, state contracts and reference imagery. Map every assertion (${v.acceptance}) to a screen/state/viewport before generation; invent no behavior, component, token or permission.`,
      `Classify the frontend scope as bounded or large. For a large scope select the smallest representative set of critical screens that covers its highest-risk journeys, layout anatomy, interactions and brand moments; do not generate every screen or every loading/error variant. Write a complete coverage/component/state map for the remaining screens using actual inspected Grammar component names and explicit derivation from the representative direction.`,
      `For each representative screen, build one exact prompt using the accepted business/SDS outcomes, brand tokens and masters, applicable UI rules, actual Grammar anatomy/reference inputs, real product-shaped content, viewport and forbidden treatments. Invoke built-in \`image_gen.imagegen\`; retain the returned image and exact prompt under the owning UI node, open/review the image and record \`generation.tool: image_gen.imagegen\`, prompt path and input refs. The tool does not expose model selection, so do not infer or promise a model name.`,
      `Write the UI record inside ${v.allowlist}: surfaces, every reachable state, responsive/accessibility rules, the representative images/prompts and the complete \`ui.coverage\` map. List each illustration, mascot, decorative image, chart or media region embedded by the direction as an \`artworkSlots\` entry (${SLOT_SHAPE}) for interface.asset.`,
      `State the proof boundary in the record and result: an ImageGen direction is proposed visual guidance, not exact Grammar component/render/API proof and not a screenshot of a working product. Later \`frontend.implement\` must produce actual Grammar implementation captures and checks; \`uat.verify\` must exercise the identified running build in a browser.`,
      `Validate Work and artifact hashes, then report \`done\` exactly once with representative images, prompt artifacts, actual tool provenance, coverage map and artwork slots. Missing business/SDS/brand authority is a typed gap; unresolved actual Grammar anatomy is \`grammar-gap\`; unavailable ImageGen is host-unsupported; outside writes are \`shared-change\`.`
    ],
    done:[
      `the record at ${v.allowlist} maps every required screen/state/viewport and uses a deliberately small critical-screen representative set for a large scope`,
      `every representative image was produced by an actual \`image_gen.imagegen\` invocation from accepted business/SDS, brand, relevant UI knowledge and actual Grammar anatomy/reference inputs, with exact prompt bytes and tool provenance retained`,
      `the coverage/component/state map names actual inspected Grammar components and explicitly derives all non-generated screens and states`,
      `the result states that generated direction is not exact component/render/API proof; actual implementation captures and browser UAT remain required downstream`,
      `every embedded artwork region is declared as an \`artworkSlots\` entry and every image/prompt is byte-bound under the owning node`,

...
import {machineFileFor,openLedger} from './ledger-db.mjs';
...
export function createJobBridge({ledgerFile,machineFile=null,now=Date.now,spawnChild=spawn,workerFile=path.join(here,'job-worker.mjs'),eligibility,beforeSpawn=null}={}){
  const machinePath=machineFile??machineFileFor(),journal=openLedger({file:ledgerFile,now}),admission=createAdmission({journal,machineFile:machinePath,now});
  const request=({workflowId,opId=null,attempt=1,generation=1,jobId=null,kind,input,role=null,ttlMs=15*60*1000,resources=null})=>{
    ...
    if(existing){
      ...
      if(existing.status==='effect_unknown'&&hasReplayableStagedResult(journal.path,existing)){
        ...
        if(claimed.changes===1){let replay;try{replay=spawnChild(process.execPath,[workerFile,journal.path,identity.jobId,existing.lease_token,machinePath],{detached:true,stdio:'ignore',windowsHide:true});}catch(error){journal.db.prepare("UPDATE jobs SET status='effect_unknown',worker_id=NULL,updated_at=? WHERE job_id=? AND lease_token=?").run(now(),identity.jobId,existing.lease_token);journal.appendEvent({eventId:`${identity.jobId}:completion-replay-spawn-failed`,workflowId,entityType:'job',entityId:identity.jobId,generation,kind:'job-completion-replay-spawn-failed',payload:{reasonDigest:inputDigest(String(error?.message??error))}});return {schema:JOB_PENDING,pending:true,identity,status:'effect_unknown'};}replay.once?.('error',()=>{try{journal.db.prepare("UPDATE jobs SET status='effect_unknown',worker_id=NULL,updated_at=? WHERE job_id=? AND lease_token=?").run(now(),identity.jobId,existing.lease_token);}catch{}});replay.unref();journal.appendEvent({eventId:`${identity.jobId}:completion-replay-spawned`,workflowId,entityType:'job',entityId:identity.jobId,generation,kind:'job-completion-replay-spawned',payload:{pid:replay.pid??null}});}
    ...
    let child;
    try{child=spawnChild(process.execPath,[workerFile,journal.path,identity.jobId,reserved.leaseToken,machinePath],{detached:true,stdio:'ignore',windowsHide:true});}

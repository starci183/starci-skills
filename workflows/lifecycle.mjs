...
import {validateWorkPolicy,requestWorkPolicy,sealWorkResult,verifyRunWork,scopedWorkStatus} from './work-binding.mjs';
...
export function resolveProjectSkillPath(moduleUrl=import.meta.url){const candidate=fileURLToPath(new URL('../SKILL.md',moduleUrl));requireThat(fs.existsSync(candidate)&&fs.statSync(candidate).isFile(),'Project StarCi SKILL.md is missing beside this runtime');return fs.realpathSync(candidate);}
...
export function preflightCompletion(run,completions) {
 ...
 verifyRunWork(run);
 const predicted=previewCompletion(run.workRoot,completions),verdict=scopedWorkStatus(predicted,run.goal.workTargets,{done:true,authored:run.goal.cells.some(c=>c.workPolicy)});

...
function validateMandate(plan,m) {
 ...
 for(const job of plan.workflows) {
  ...
  const ceilingKeys=['id','environment','business','resourceEffects'];
  if(ceiling&&Object.hasOwn(ceiling,'technicalQuestions'))ceilingKeys.push('technicalQuestions');
  requireThat(allowed.has(job.workflow)&&exact(ceiling,ceilingKeys)&&['local','isolated-test'].includes(ceiling.environment)&&strings(ceiling.business)&&ceiling.business.length&&Array.isArray(ceiling.resourceEffects),'Reserved workflow or invalid technical ceiling');
  if(Object.hasOwn(ceiling,'technicalQuestions'))requireThat(strings(ceiling.technicalQuestions)&&ceiling.technicalQuestions.every(q=>job.openQuestions.includes(q)),'Review unique exact technical questions for this Plan job');
...
export function assertDelegatedGoal(run,{active=true,coordinatorThreadId,taskThreadId}={}) {
 ...
 requireThat(!plan.openQuestions.length,'Unresolved Plan user choices cannot be answered by delegation');
 const answers=run.goal.inputs?.planQuestionAnswers??[],reviewed=ceiling.technicalQuestions??[];
 requireThat(questionAnswers(answers)&&answers.length===job.openQuestions.length&&answers.every(a=>job.openQuestions.includes(a.question)&&reviewed.includes(a.question)),'Unresolved user choices: every workflow question needs exact frozen goal answers and mandate technical classification');

...
function validateContract(contract){
 ...
 for(const role of ['be','fe']){
  ...
  if(!object(spec)||!Array.isArray(spec.required)||!Array.isArray(spec.requiredAny)||!Array.isArray(spec.forbidden)||!Array.isArray(spec.content))throw Error(`Invalid ${role} source-layout contract.`);
 }
 return contract;
}
...
export function validateSourceLayout({be,fe,workRoot}={},contract=readDistJson('schemas','source-layout.json')){
 validateContract(contract);
 const errors=[],roots={be:realDirectory(be,'be',errors),fe:realDirectory(fe,'fe',errors)};

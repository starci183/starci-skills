...
export function validBackendRun(run){if(!plain(run)||!['accepted','done'].includes(run.status)||run.goal?.workflow!=='implement-backend'||typeof run.goalDigest!=='string'||!plain(run.responses))return false;const resultDigest=sha256(canonicalJSON({goalDigest:run.goalDigest,responses:run.responses}));if(run.resultDigest!==resultDigest||!Array.isArray(run.approvals)||!run.approvals.some(a=>a?.actor==='user'&&a.phase==='acceptance'&&a.approved===true&&a.digest===resultDigest&&typeof a.messageId==='string'&&a.messageId.trim()))return false;const responses=Object.values(run.responses),criteria=responses.flatMap(r=>Array.isArray(r?.criteria)?r.criteria:[]);if(!['unit-tests-pass','backend-e2e-pass'].every(id=>criteria.some(c=>c?.id===id&&c.status==='pass'&&typeof c.observation==='string'&&c.observation.trim())))return false;return responses.some(r=>typeof r?.outputs?.apiContract==='string'&&r.outputs.apiContract.trim());}

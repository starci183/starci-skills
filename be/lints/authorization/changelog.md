...
sidebar_position: 4
description: Lịch sử phiên bản của tài liệu về cơ chế thi hành luật authorization.
---
...
Mỗi thay đổi được chấp nhận phải tăng phiên bản mô-đun thêm `0.01` và cập nhật **cả năm** tài liệu:
...
Dựng mới mô-đun. Nhóm `lints/` ghi **cơ chế thi hành**, không ghi lại luật: máy nhìn thấy được gì, và —
phần không ai chịu viết ra — máy **không** nhìn thấy cái gì.

- **Phủ đúng một quy tắc.** `identity-needs-guard`, giữ `AUTHZ-2`, được phát hành trong gói
...
- **Ghi mười một cách viết không lọt và mười một cách viết còn lọt.** Trong đó có ba kẽ hở nặng nhất: danh tính đọc qua
...
- **Ghi thêm bốn hình dạng quy tắc báo nhầm trên code đúng**, đo được: cổng có không gian tên, decorator gộp,
...
- **Không dùng tên sản phẩm trong văn xuôi và ví dụ.** Ngoại lệ duy nhất là các định danh **được phát hành thật**:
  ...
  liệu là mô tả sai khả năng thực tế của máy.

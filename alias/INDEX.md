...
| `@knowledge/patterns` | — | `<Source>/.claude/knowledge/patterns/  (fe/, be/)` | fingerprint per file; rule inventory = every `## PREFIX-n` heading in the folder | — | `architecture.decide (optional)`, `backend.implement` | Code conventions counted from the two live sources; a rule cites two real paths. |
...
| Alias | Params | Resolves to | Bind | Writers | Bound by | Purpose |
| --- | --- | --- | --- | --- | --- | --- |
| `@dynamic` | `<file>` | `<Source>/.worktrees/sessions/<sessionId>/steps/<n>.<operator.id>/<file>. Writing: <n>.<operator.id> is the current step, and input.project.artifactRootRef must equal that folder. Reading: the nearest earlier step of the same session that wrote <file>; @dynamic/steps/<n>/<file> names a specific step. The session folder is created by the orchestrator and deleted when the run completes; a blocked run keeps it for resume` | fingerprint per file; every file written is registered in output.artifactRefs | `*` | `architecture.decide`, `backend.implement`, `backend.implement`, `backend.implement`, `business.decide (optional)`, `business.decide`, `content.generate`, `fe.direction.decide`, `fe.direction.decide (optional)`, `fe.direction.decide (optional)`, `fe.direction.decide`, `fe.presentation.resolve (optional)`, `fe.presentation.resolve (optional)`, `fe.presentation.resolve`, `fe.source.apply`, `fe.source.apply (optional)`, `fe.source.apply`, `fe.source.apply`, `fe.surface.audit`, `fe.surface.audit`, `fe.surface.audit`, `git.publish`, `git.publish`, `git.publish`, `platform.operate`, `quality.verify`, `quality.verify`, `quality.verify`, `release.deploy`, `release.deploy`, `uat.verify`, `workspace.bind` | Everything produced inside the session and nothing that existed before it: <receiptType>.json, changes.md, named artifacts, captures. Always dynamic. |

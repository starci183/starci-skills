...
| `@worktrees/helpers` | `<id>` | `<Source>/.worktrees/helpers/<id>/runs/<runId>/  (run.json against templates/kinds/helper-run.schema.json: the helper, its arguments, every input it read with the head it read at, every output it wrote, the instants it ran between and the profile it ran on)` | fingerprint of run.json; a run binds by its runId | `generate-banks` | — | The run record of the support layer: one folder per helper, one append-only run inside it, so every draft a helper left names the run that drafted it. No session, no chain, no receipt — a helper runs outside any workflow and this is the whole trace it leaves. |

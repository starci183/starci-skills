...
| Alias | Tham số | Trỏ tới | Bind | Ai ghi | Operator ràng | Mục đích |
| --- | --- | --- | --- | --- | --- | --- |
| `@worktrees/_templates` | — | `<Source>/.worktrees/_templates/  (businesses/, debts/, sessions/); uat/ resolves to <Source>/.claude/templates/uat/` | fingerprint per file | — | — | Authority templates for new heads, debts, sessions, and UAT flows. The UAT flow template ships with the tree at templates/uat/ (flow.md, account.json.example, run.md, seed/), so a fresh install has one without provisioning a worktree. Consumed, never modified. |

...
| Alias | Tham số | Trỏ tới | Bind | Ai ghi | Operator ràng | Mục đích |
| --- | --- | --- | --- | --- | --- | --- |
| `@dynamic` | `<kind>` | `<Source>/.worktrees/sessions/<sessionId>/step-<N>-<M>/ — the current step folder. Inside it: input.json (gate in), request.md, response.md, data/<name>.json, artifacts/<file>, output.json (gate out). Reading: a kind resolves to the nearest earlier step whose output.json fields lists it; the path is relative, ../step-N-M/<file>. Operator.md packages pass dynamic files as kinds (templates/kinds/<kind>), not as aliases; the old shape still names @dynamic/<file> until each package migrates. The session folder is created by the orchestrator and deleted when git.publish finishes; a blocked run keeps it for resume` | kind template or schema under templates/kinds; output.json fields is the registry of what a step produced | `*` | `backend.implement`, `backend.implement`, `backend.implement`, `business.decide (optional)`, `business.decide`, `content.generate`, `fe.direction.decide`, `fe.direction.decide (optional)`, `fe.direction.decide (optional)`, `fe.direction.decide`, `fe.presentation.resolve (optional)`, `fe.presentation.resolve (optional)`, `fe.presentation.resolve`, `fe.source.apply`, `fe.source.apply (optional)`, `fe.source.apply`, `fe.source.apply`, `fe.surface.audit`, `fe.surface.audit`, `fe.surface.audit`, `git.publish`, `git.publish`, `git.publish`, `platform.operate`, `quality.verify`, `quality.verify`, `quality.verify`, `release.deploy`, `release.deploy`, `uat.verify`, `workspace.bind` | Everything produced inside the session and nothing that existed before it. Always dynamic; typed by kind. |

---
name: starci-backend-solution-design
...
# starci-backend-solution-design

Design one bounded backend solution from approved business, tech-stack, architecture, ownership, and realization artifacts. Do not edit source or reopen unrelated architecture.

## INPUT ANALYSIS

Require the ephemeral global selection, read `input.md`, validate `input.schema.json`, then follow local `analyze-input.md`. This skill owns one flow with fixed first state `design`; local analysis only validates and normalizes scope without loading operator knowledge.

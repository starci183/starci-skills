# Execute starci-backend-solution-design

1. Accept only a validated global `selection` for this skill, validate the complete input, run local `analyze-input`, then enter fixed state `design`. Treat an omitted `selection.mode` as `gated`.

---
name: starci-backend-implementation
...
# starci-backend-implementation

Implement one approved backend contract in a frozen exact source boundary, then run semantic architecture and data-placement conformance before generic quality checks.

## INPUT ANALYSIS

Require the ephemeral global selection, read `input.md`, validate `input.schema.json`, then follow local `analyze-input.md`. This skill owns one flow with fixed first state `scope`; local analysis only validates and normalizes scope without loading operator knowledge.

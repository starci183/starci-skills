# Execute starci-backend-contract-critique

1. Accept only a validated global `selection` for this skill, validate the complete input, run local `analyze-input`, then enter fixed state `critique`. Treat an omitted `selection.mode` as `gated`.

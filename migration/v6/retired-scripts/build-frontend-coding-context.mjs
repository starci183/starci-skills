...
function structureSignals(sourceText) {
  const signals = new Set();
  for (const match of sourceText.matchAll(/\b([A-Z][A-Z0-9_]*(?:GRAMMAR|PRINCIPLE)[A-Z0-9_]*)\b/g)) signals.add(match[1]);
  for (const match of sourceText.matchAll(/data-(?:node|principle|state)=["'{]([^"'}]+)/g)) signals.add(match[1]);
  return [...signals].sort();
}
...
export function buildSnapshot({ project, sourceRoot, config = {} }) {
  ...
  for (const file of sourceFiles) {
    ...
    const fileSignals = structureSignals(text);

    for (const declaration of exportedDeclarations(sourceFile)) {
      const name = declaration.name?.text;
      if (!name) continue;
      if (/GRAMMAR|PRINCIPLE/.test(name.toUpperCase())) {
        grammarRegistries.push({ name, source: relative, sourceSha256, description: jsDoc(checker, declaration) });
        continue;
      }
  ...
  grammarRegistries.sort((left, right) => left.name.localeCompare(right.name));
  ...
  return {
    ...
    statistics: { files: sourceFiles.length, components: components.length, grammarRegistries: grammarRegistries.length },
    components,
    grammarRegistries
  };
}
...
export function publishSnapshot({ snapshot, outputRoot, check = false }) {
  const cacheRoot = path.join(path.resolve(outputRoot), '.worktrees', 'coding-context', 'frontend');

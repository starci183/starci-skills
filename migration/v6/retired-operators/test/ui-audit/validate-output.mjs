...
export const validateOutput = validatorFor(new URL('./output.schema.json', import.meta.url), (value) => {
  ...
  if (!out.authorityVerdicts) issues.push('UI requires independent UI principles and Grammar verdicts'); else if ((out.authorityVerdicts.uiPrinciples === 'failed' || out.authorityVerdicts.grammar === 'failed') && out.outcome !== 'failed') issues.push('authority failure requires failed UI verdict'); if (out.outcome === 'suspense' && out.suspenseQuestions.length === 0) issues.push('UI suspense requires a finite authority question and owner'); if (out.outcome !== 'suspense' && out.suspenseQuestions.length > 0) issues.push('suspenseQuestions are only valid for UI suspense');
  return issues;
});

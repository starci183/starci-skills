...
Work metadata uses YAML 1.2 in workspace.yaml, resource.yaml and evidence/manifest.yaml. A canonical node is index.md: YAML front matter contains `work/node@2` metadata and the Markdown body contains the readable specification. New Work includes YAML schemas in _schema/. Legacy index.yaml, node.yaml and node.md are read compatibility only; keep exactly one node file in each folder.

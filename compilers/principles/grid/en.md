...
**Common business situations.** Activity feed · divided invoice list · submission timeline · vertical
navigation menu · wrapping row of filter chips · breadcrumb trail · comment list · the steps of a
...
**Common business situations.** A trio of metric tiles · a pricing plan grid · a row of feature cards ·
a square photo grid · team member cards · a two-column form (first name / last name) · a badge grid ·
a set of learning-path cards.
...
**Common business situations.** Search results · course catalogue · photo library · member list ·
template grid · filter results · notification cards · a horizontally scrolled rail.
...
**Common business situations.** Filter rail + result region · navigation + content · canvas +
inspector · mailbox + conversation pane · table of contents + article · catalogue + cart pane · file
tree + editor · form + order summary panel.
...
**Common business situations.** The page shell · the frame of a signed-in layout · the container of a
landing section · the reading area of a long article (`max-w-prose`) · the content area of a dialog ·
the print content area.
...
**Common business situations.** One course card in a catalogue grid · one metric tile · one photo in a
library · one field in a two-column form · one member card · a table sitting entirely inside one
column.
...
**Common business situations.** A featured card at the head of a grid · a wide chart in a dashboard ·
an empty state filling the grid · a notice banner inside a card grid · a long notes field in a
multi-column form · a totals row at the foot of a metric grid.
...
**Common business situations.** A hero with a background image · a testimonial band on a coloured
ground · a horizontally scrolled rail on mobile · a wide table that needs horizontal scrolling · a
full-width divider · an embedded map · a notice ribbon at the top of the page.
...
**Request.** "A course catalogue: content must not run the full width of a large screen; filters on
the left and results on the right; each result is a card and a card must not get narrower than 16rem;
when nothing matches, show one message across the whole grid."

The request states the field, the region holding the two roles, its two children, the region holding
the cards, one card, and the empty-state message. It states no page header, no pagination, no hero
band and no break-out, so none of those are resolved.

```text
field: page shell, mx-auto max-w-6xl, 4 / 8 / 12 columns
container: page shell
situation: GRID-4
className: mx-auto w-full max-w-6xl px-4 sm:px-6 lg:px-8
reason: it decides where content ends rather than how many columns exist, which excludes GRID-1
```

```text
field: page shell
container: filters-and-results region
situation: GRID-3
className: grid gap-8 lg:grid-cols-[16rem_minmax(0,1fr)]
reason: the two tracks are not interchangeable — swapping filters and results makes the page read wrongly — which excludes GRID-1
```
...
child: filter rail
situation: GRID-5
className: no placement class
reason: the rail accepts the 16rem track it is handed and claims nothing more, which excludes GRID-6
```
...
situation: GRID-5
className: no placement class
reason: the result region fills the track it is given rather than claiming a second one, which excludes GRID-6
```
...
reason: the request states a minimum card width and the number of results comes from data, which excludes GRID-1
```
...
situation: GRID-5
className: no placement class
reason: the card accepts one derived track and never sets its own width, which excludes GRID-6
```
...
reason: the message must occupy every column yet stays inside the field's margin, which excludes GRID-7
```

The request never says the catalogue is a fixed three-up, so no count is declared; it never says any
block touches the screen edge, so nothing resolves to `GRID-7`; and because skeleton and empty state
share the code of the real content, the loading grid keeps `GRID-2` unchanged.

...
**Common business situations.** Settings menu · billing history · leaderboard · menu commands ·
divided notification feed · mobile navigation list · row-shaped data table.
...
**Common business situations.** Name + username · fallback avatar + name · metric + unit · price +
billing cycle · title + short subtitle · rank + rank label · date + timezone · file name + file
type.
...
**Common business situations.** Save/Preview · Previous/Next · icon + label inside one button ·
input + its adjoining action button · compact filter cluster · author + body of one review · price
cluster · ordered timeline · action cluster stacked vertically when narrow.
...
**Common business situations.** Label + input · heading + cards · toolbar + result region · tab
strip + tab content · card + caption · question + answer region · search box + result summary line ·
...
**Common business situations.** Profile cluster + progress cluster · input field + input field ·
payment summary + terms · heading cluster + content cluster · address + payment method · chart +
legend cluster · question + choice cluster · course summary + enrolment cluster.
...
profile + security settings · milestones + feedback · analytics + transactions · project brief +
deliverables.
...
**Common business situations.** Filter rail + results · navigation + content · canvas + inspector ·
mailbox + conversation pane · catalogue + cart pane · file tree + editor.
...
**Request.** "A login form with three fields, each field a label and an input."

The request states two parents: the form body holding the three fields, and each field holding its
label and its input. It states no submit button, no heading and no card, so none of those are
resolved.

```text
parent: form body
siblings: field, field, field
situation: GAP-4
className: gap-4
reason: each field already owns a label-input seam and no field governs another, which excludes GAP-3
```
...
situation: GAP-3
className: gap-3
reason: the label names an interactive block rather than qualifying a value, which excludes GAP-1
```

When the request later adds a submit action, the form body then holds a field group and an action
group — two peers with their own internal structure, still `GAP-4` — while the action cluster itself
is one compact operation, `GAP-2`. A heading placed above the form body owns it: `GAP-3`.

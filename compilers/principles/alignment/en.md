...
**Common business situations.** Two card columns of equal height · cells in a catalogue grid · a
sidebar and a content region of equal height · a row of buttons of equal height · cells in a table ·
the two panes of a split screen.
...
**Common business situations.** Icon + label inside one button · avatar + one-line name · checkbox +
short label · status pill beside a title · switch + setting name · the head of a row carrying a title
and a button · a square holding only initials (`grid place-items-center`).
...
**Common business situations.** Avatar + name + multi-line comment · icon + title + description inside
a callout · checkbox + long terms · index number + long question · two content columns that must begin
at the same top edge · a column holding a label and a description, packed to the leading edge.
...
**Common business situations.** A column of amounts packed to the trailing edge · a time cluster in
the last cell of a list row · a button group in a form column · chart bars growing from one floor · a
status cluster packed to the end of a card.
...
**Common business situations.** Metric + unit · price + billing cycle · score + scale · quantity +
counted noun · current price + struck price + discount · name + inline secondary label · page number +
total pages · heading + small count beside it.
...
**Common business situations.** A button centred in a row while the text anchors to the top edge · an
icon left unstretched in a row of equally tall columns · one grid cell stretching the full row height
while the others anchor to the top · a status pill centred within a multi-line text cluster.
...
**Common business situations.** An icon + text row · a cluster of small filter pills · a breadcrumb
trail · a row of category tags · a row of buttons in a toolbar · every ordinary content row.
...
**Common business situations.** A button group at the foot of a dialog · a button group at the foot of
a form · an action cluster at the foot of a card · pagination packed to the trailing edge · a meta
cluster at the end of a row.
...
**Common business situations.** The empty state of a list · a loading indicator in the middle of a
loading region · an error message for a whole block · a square holding a letter avatar · centred
pagination · a lone call to action in the middle of a band.
...
**Common business situations.** A section heading on one side + a "View all" action on the other · an
item name + its value in a list row · a label + a value in a summary row · a Back button + a Continue
button at the two ends · a bottom navigation bar dividing its items equally · the two extreme labels
of a slider.
...
**Common business situations.** A band of filter pills in a toolbar of fixed height · a badge grid in
a cell of fixed height · a wrapping button group in a bar with a minimum height.
...
**Request.** "A settings row with the setting name on one side and a switch on the other, and below it
a comment showing a round avatar beside the commenter's name and their comment text."

The request states three containers: the settings row, the comment row, and the text cluster inside
the comment that holds the name above the comment text. It states no card around them, no heading, no
fixed height and no wrapping, so nothing resolves to `ALIGN-10`, and no child is described as departing
from its parent, so no `ALIGN-5` is emitted.

```text
container: flex
axis:      row
children:  setting name (one line of text), switch (fixed-size control)
cross:     ALIGN-1
departure: none
main:      ALIGN-9
lines:     none
className: flex items-center justify-between gap-4
reason:    the switch is a fixed control that stretching would distort, which excludes ALIGN-0, and the name states what the setting is while the switch acts on it, two roles that cannot be swapped, which excludes ALIGN-6
```

```text
container: flex
axis:      row
children:  avatar (fixed size), text cluster (grows to several lines)
cross:     ALIGN-2
departure: none
main:      ALIGN-6
lines:     none
className: flex items-start gap-3
reason:    comment text can reach a second line and the avatar must stay level with the first line, which excludes ALIGN-1; the surplus after the comment carries no meaning, which excludes ALIGN-9
```

```text
container: flex
axis:      column
children:  commenter name (text), comment text (text, multi-line)
cross:     ALIGN-2
departure: none
main:      ALIGN-6
lines:     none
className: flex flex-col items-start gap-1
reason:    in a column the cross axis is horizontal and both children must pack to the leading edge rather than take the width of the longest, which excludes ALIGN-0
```

If the request later says the switch row is a heading with a title and a small count beside it, that
pair is all text of two sizes read as one phrase, `ALIGN-4`. If it says the comment carries an action
button that must sit in the middle of the row while the text stays anchored to the top, the parent
keeps `ALIGN-2` and the button alone declares `ALIGN-5`.

## Scope

This module states a rule true of any front end. It names no product, no component library, no registry
key and no repository. Every example is an ordinary `className` on ordinary markup.

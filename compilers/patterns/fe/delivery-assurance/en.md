...
| `ASSURANCE-FE-4` | Quality analysis runs | SonarQube consumes the same LCOV and its quality gate blocks |
...
2. Local pre-push stays at lint plus unit; typecheck, build, coverage upload and analysis stay in CI.
...
5. README carries every required badge without a token.
6. Filesystem wiring never proves required checks; authorized provider/GitHub API evidence does.
7. Existing deploys cannot outrun verification.

## Proof

Run local lint, typecheck, build and unit gates; prove the hook refuses a controlled failure; parse the CI
graph; prove one LCOV path is consumed twice; verify encrypted record names, GitHub secret/variable names,

...
|---|---|---|
| `LANDMARK-1` | `.claude/@starci/eslint-canon-fe` | `LANDMARK_BRANCHES` — the set of branch names that open a landmark, one entry per element. **Not yet anchored in the application:** the application now carries the element on the entry instead, so no landmark branch remains to point at |

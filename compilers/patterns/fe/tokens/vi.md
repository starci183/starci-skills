...
4. **Hỏi câu hỏi của từng mã.** Giá trị này là một kẻ ngoài union (`TOKEN-1`), một thành viên được
   ...
   mà biến của nó có thể không tồn tại (`TOKEN-9`)?
...
Bốn mã `enforced`, một mã `unrepresentable`, bốn mã `documented`. Type từ vựng sở hữu tầng entry, còn
...
| theme | Stylesheet định nghĩa các biến mà một tên token đang xin |

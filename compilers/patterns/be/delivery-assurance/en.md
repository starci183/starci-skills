...
| `ASSURANCE-3` | Unit tests ran in CI | The run emits LCOV, uploads it through Codecov, and exposes blocking patch/project coverage statuses |
| `ASSURANCE-4` | The same revision needs quality and security analysis | SonarQube scans with the LCOV report and returns a blocking quality gate |
...
and its patch/project statuses are intended to block. Coverage percentage belongs to the service policy,
...
gate success are different facts; the workflow must wait for or receive the gate result.
...
| coverage | one LCOV path consumed by Codecov and SonarQube |
...
This pattern governs backend repository delivery assurance. It does not decide business test cases,
provider-specific quality thresholds, deployment infrastructure or the secret values themselves.

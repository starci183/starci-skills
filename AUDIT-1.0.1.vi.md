...
| A7 | `ui/proof/` | 5 topic | chưa | đóng ngày 2026-09-02: `_pending-contrast` đã chuyển về `ui/proof/contrast.md` (+vi) thành COLOR-3 và COLOR-5 theo khuôn rule của proof, đã thêm dòng danh mục, self-test của `frontend.surface.audit` bind và trích được topic `contrast` (`384a3ec9`); cột `Observe` của bốn topic còn lại vẫn chờ thầy đọc | đã quyết: đo tương phản đặt ở `ui/proof/contrast.md` |
...
| A13 | `data-contract` trong Grammar | 41 renderer | chưa | mọi phần tử hiện thực một dòng "Common đã sở hữu" mang `data-contract` với id đã publish, tính từ prop đã resolve khi giá trị phụ thuộc prop; starci-academy-fe `14e0c20` (tsc sạch, 34 file spec / 126 test xanh, lint 0). Lệch knowledge với source được ghi chứ không che: gap mặc định của `SectionHeader` là `1.5rem` trong `styles.css` trong khi `gap.md` ghi `.75rem`; `MarkdownArticle`, `FencedCodeBlock`, `MarkdownTableFrame` không hề gắn class `starci-core-*` nên các luật CSS đó và dòng knowledge tương ứng chưa được hiện thực; `EmptyNotice` không có phần tử nào hiện thực dòng MARGIN-0; inset ngang `1rem` của `StaticStateRow` không có dòng nào | bên nào đúng cho gap của `SectionHeader`; nối class markdown hay bỏ các dòng đó |
